...
import { debugLog } from "./rudder-context.js"
...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (taskStatus === "completed") {
    ...
    return `Status: COMPLETED\nTask: ${taskTitle}\nSource: ${active.source}\nNext: Archive with \`python3 ./.rudder/scripts/task.py archive ${dirName}\` or start a new task`
  }
...
function loadRudderConfig(directory, contextKey = null) {
  const scriptPath = join(directory, ".rudder", "scripts", "get_context.py")
  ...
  try {
    const output = execFileSync(PYTHON_CMD, [scriptPath, "--mode", "packages", "--json"], {
      ...
      env: {
        ...process.env,
        ...(contextKey ? { RUDDER_CONTEXT_ID: contextKey } : {}),
      },
    })
    ...
    debugLog("session", "loadRudderConfig error:", e.message)
    return { isMonorepo: false, packages: {}, specScope: null, activeTaskPackage: null, defaultPackage: null }
  }
}
...
function checkLegacySpec(directory, config) {
  ...
  const specDir = join(directory, ".rudder", "spec")
...
export function buildSessionContext(ctx, platformInput = null) {
  ...
  const rudderDir = join(directory, ".rudder")
  const contextKey = typeof ctx.getContextKey === "function"
    ? ctx.getContextKey(platformInput)
    : null

  const config = loadRudderConfig(directory, contextKey)
  const allowedPkgs = resolveSpecScope(config)

  const parts = []

  parts.push(`<rudder-context>
You are starting a new session in a Rudder-managed project.
Read and follow all instructions below carefully.
  ...
  const contextScript = join(rudderDir, "scripts", "get_context.py")
  ...
  const workflowContent = ctx.readProjectFile(".rudder/workflow.md")
  if (workflowContent) {
  ...
  parts.push(
    ...
    "session; dispatch `rudder-implement` and `rudder-check` so JSONL " +
    ...
    try {
      ...
      for (const sub of subs) {
        ...
        if (existsSync(indexFile)) {
          paths.push(`.rudder/spec/${sub}/index.md`)
        } else {
          ...
          try {
            ...
            for (const layer of nested) {
              ...
              if (existsSync(nestedIndex)) {
                paths.push(`.rudder/spec/${sub}/${layer}/index.md`)
              }
            }
          } catch {
...
function getRudderMetadata(metadata) {
  if (!metadata || typeof metadata !== "object") {
    return {}
  }

  const rudder = metadata.rudder
  if (!rudder || typeof rudder !== "object") {
    return {}
  }
...
function hasSessionStartMarker(part) {
  ...
  return getRudderMetadata(part.metadata).sessionStart === true
}

export function hasInjectedRudderContext(messages) {
...
export async function hasPersistedInjectedContext(client, directory, sessionID) {
  try {
    ...
    return hasInjectedRudderContext(response.data || [])
  } catch (error) {
    debugLog(
      "session",
