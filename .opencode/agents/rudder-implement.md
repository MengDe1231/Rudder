...
1. Run `python3 ./.rudder/scripts/task.py current --source` → get active task directory and source (e.g., `Current task: .rudder/tasks/xxx`)
...
**If `implement.jsonl` has no curated entries (only a seed row, or the file is missing)**: read `prd.md` to understand the task domain, then decide which specs apply based on `.rudder/spec/` layout. You can list available specs with `python3 ./.rudder/scripts/get_context.py --mode packages`. Do not block on the missing jsonl — proceed with prd-only context plus your own spec judgment.
