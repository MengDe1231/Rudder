...
|------|--------|----------|
| **Internal** | Existing feature, refactor, bug area | Search project code + `.rudder/spec/` |
...
Read `.rudder/spec/` to understand what guidelines exist:
...
Write one file per topic at `.rudder/tasks/{id}/context/{topic}.md`, using
...
```jsonl
{"path": ".rudder/spec/{pkg}/{layer}/index.md", "description": "Coding guidelines overview"}
{"path": ".rudder/spec/{pkg}/{layer}/error-handling.md", "description": "Error handling conventions"}
{"path": "src/services/auth.ts", "description": "Existing pattern to follow"}
{"path": ".rudder/tasks/{id}/prd.md", "description": "Requirements"}
{"path": ".rudder/tasks/{id}/context/new-sdk-usage.md", "description": "SDK API reference"}
```

### What goes into `check.jsonl`

```jsonl
{"path": ".rudder/spec/{pkg}/{layer}/quality-guidelines.md", "description": "Quality criteria"}
{"path": ".rudder/spec/guides/cross-layer-thinking-guide.md", "description": "Cross-layer check"}
```
...
- Read and reference `.rudder/spec/` files
- Write context files under `.rudder/tasks/{id}/context/`
- Populate JSONL files with discovered paths

### Forbidden (unless explicitly asked)
- Suggest improvements or criticize implementation
- Modify source code (only write to .rudder/ task directories)
...
### Specs Reviewed
- `.rudder/spec/{pkg}/{layer}/index.md` — {what it covers}
- `.rudder/spec/{pkg}/{layer}/specific-file.md` — {key points}
...
- `.rudder/tasks/{id}/context/topic-name.md` — {what it contains}
