...
Several Rudder modules (e.g. `OpenCodeContext.getContextKey`, `RudderContext.getActiveTask`) consult `process.env.RUDDER_CONTEXT_ID` and `process.env.OPENCODE_RUN_ID` as **highest-priority overrides** — production behavior, by design.
...
```ts
// test/setup.ts
delete process.env.RUDDER_CONTEXT_ID;
...
it("safe-file-delete respects update.skip", () => {
  ...
  config.update.skip = [".claude/commands/rudder/"];
...
```typescript
/**
 * Helper: simulate a fresh clone of an existing Rudder project...
 *
