...
> Standards for Python scripts in the `.rudder/scripts/` directory.
...
Use this contract when a generated `.rudder/scripts/` module performs an
advisory check during hook/session context generation, such as checking whether
a Rudder update is available. These checks must never block context output.
...
- Marker files live under `.rudder/.runtime/` and are keyed by the current
  Rudder session identity when available.
...
- Good: `rudder --version` prints an existing CLI update hint or final version,
  project `.version` is `0.5.0`, so context prints the update hint once.
- Base: `rudder --version` returns `0.5.9`; no registry parsing is needed.
...
- Existing `rudder --version` update output is parsed and normalized.
...
`.rudder/.current-task` directly. The resolver is the single source of truth
for session/window scoped task state:

1. Derive a context key from platform input, `RUDDER_CONTEXT_ID`, a
...
2. Read `.rudder/.runtime/sessions/<session-key>.json`.
...
`RUDDER_CONTEXT_ID` is a context-key override for subprocesses. It is not a
...
`RUDDER_CONTEXT_ID`; without that identity, `task.py start` fails and explains
how to provide a session runtime. For Claude Code, SessionStart receives
`CLAUDE_ENV_FILE`; Rudder must append `export RUDDER_CONTEXT_ID=<context-key>`
...
`RUDDER_CONTEXT_ID` from plugin session identity when the command does not
already set it, because some TUI sessions do not expose `OPENCODE_RUN_ID` to
Bash. The prefix must match the host shell: use
`export RUDDER_CONTEXT_ID=<context-key>;` for POSIX shells and
`$env:RUDDER_CONTEXT_ID = '<context-key>';` for Windows PowerShell. Keep the
...
and write a short-lived `.rudder/.runtime/cursor-shell/*.json` ticket for
...
`tool_call` by prefixing `export RUDDER_CONTEXT_ID=<context-key>;`. The Python
resolver then sees the explicit `RUDDER_CONTEXT_ID` override; Pi does not need
...
- `python3 .rudder/scripts/task.py create "<title>" [--slug <slug>]`
...
- `task.py create` creates only task-owned files under
  `.rudder/tasks/<date-slug>/`. It must not create `.rudder/.runtime/` and
  must not write `.rudder/.current-task`.
...
- Session state is stored at
  `.rudder/.runtime/sessions/<session-key>.json`. The runtime directory is
  created lazily by the JSON write path.
- Context filenames are derived from the resolved context key:
  - `RUDDER_CONTEXT_ID=session-demo` -> `session-demo.json`
...
- `RUDDER_CONTEXT_ID` is already a complete context key. Do not prepend a
  platform name to it.
...
| `start` without context key | Fails; no `.runtime`; no `.current-task`; hints IDE/session identity or `RUDDER_CONTEXT_ID` |
| `start` with `RUDDER_CONTEXT_ID` | Writes `.runtime/sessions/<key>.json`; does not require `.current-task` |
...
- Bad: `task.py create` pre-creates `.runtime`, or any resolver reads/writes
  `.rudder/.current-task` as an active-task fallback.
...
- Regression tests for `start` without a context key failing without creating
  `.current-task`.
- Regression tests for `RUDDER_CONTEXT_ID` and platform-native env keys.
...
if not resolve_context_key():
    write_file(".rudder/.current-task", task_path)
```
...
Windows and `python3` on macOS/Linux. `rudder init` probes that same
...
print("Run: {{PYTHON_CMD}} ./.rudder/scripts/init_developer.py <name>")
...
path = Path(".rudder") / "scripts" / "task.py"

# Bad - Unix-only
path = ".rudder/scripts/task.py"
```
...
hooks:
  after_create:
    - "python3 .rudder/scripts/hooks/my_hook.py create"
  after_start:
    - "python3 .rudder/scripts/hooks/my_hook.py start"
  after_archive:
    - "python3 .rudder/scripts/hooks/my_hook.py archive"
```
...
1. Store config in a **gitignored** local file (e.g., `.rudder/hooks.local.json`)
2. Read config at startup, fail with clear message if missing
3. Keep the script itself committable (no hardcoded secrets)

```python
# .rudder/scripts/hooks/my_hook.py — committable, no secrets
CONFIG = _load_config()  # reads from .rudder/hooks.local.json (gitignored)
...
Scripts that auto-stage / auto-commit `.rudder/` paths must go through the
...
|---|---|---|
| `safe_rudder_paths_to_add(repo_root)` | `templates/rudder/scripts/common/safe_commit.py:safe_rudder_paths_to_add` | Path whitelist for `add_session.py` — journal files, index.md, active task dirs, archive dir |
...
### Anti-pattern: AI-invented `git add -f .rudder/`

A real user incident (pre-0.5.10): a project's `.gitignore` listed `.rudder/`
...
`git add -f .rudder/`. That fan-out included every ignored subtree
(`.rudder/.backup-*/`, `.rudder/worktrees/`, `.rudder/.template-hashes.json`,
`.rudder/.runtime/`), committing 548 files / 83474 lines of caches and
backups before anyone noticed.

The root cause is generic fallback hint text in scripts, e.g. "run
`git add .rudder && git commit`" — AI agents see "ignored by" and reinvent
...
intent — putting `.rudder/` in `.gitignore` is an explicit signal "do not
track this." A script silently bypassing that with `-f`, even on a narrow
path list, is unacceptable.

The wider-grain `git add -f .rudder/` stays forbidden, AND the narrow-grain
...
def _auto_commit_workspace(repo_root: Path) -> None:
    ...
    paths = safe_rudder_paths_to_add(repo_root)  # canonical whitelist
    if not paths:
        return
...
`.gitignore` intentionally excludes `.rudder/` and who want session data kept
local-only.

### Pattern: warning text as canonical AI-defense surface

`print_gitignore_warning` in `templates/rudder/scripts/common/safe_commit.py`
...
# .rudder/.backup-*/, .rudder/worktrees/, etc.
subprocess.run(["git", "add", "-A", ".rudder/"], cwd=repo_root)
```
...
```python
paths = safe_rudder_paths_to_add(repo_root)
...
- `safe_rudder_paths_to_add` excludes `.rudder/.backup-*`, `.rudder/worktrees`,
...
`common/session_context.py` must probe the Rudder root with
...
All keys in `.rudder/config.yaml` MUST be read through `common/config.py`
(or its hook-side mirror `common/rudder_config.py` for hooks that cannot
...
This is a load-bearing chain. Any new key added to `.rudder/config.yaml`
...
configs with the `# explanation` annotation in `templates/rudder/config.yaml`
break silently.
...
### Pattern: document every key in `templates/rudder/config.yaml`

Every accessor in `common/config.py` must have a corresponding commented-out
example in `packages/cli/src/templates/rudder/config.yaml`, with:
...
# test fixture
config_yaml = """
session_auto_commit: false  # opt out — gitignored .rudder/
...
def _read_session_auto_commit(repo_root: Path) -> bool:
    text = (repo_root / ".rudder/config.yaml").read_text(encoding="utf-8")
...
```yaml
# Auto-detected monorepo packages (written by rudder init)
packages:
  cli:
    path: packages/cli
  docs-site:
...
update:
  skip:
    - .claude/commands/rudder/my-custom.md
...
# Usage
if not repo_root:
    error("Not in a Rudder project (no .rudder directory found)")
    sys.exit(1)
```
...
**Key rules**:
- Original file path stays stable (e.g., `python3 .rudder/scripts/task.py`)
...
See `.rudder/scripts/task.py` for a comprehensive example with:
