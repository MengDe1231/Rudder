...
The breadcrumb is the **only** per-turn channel that fires while a Rudder task
...
breadcrumb body lives in `.rudder/workflow.md`; this spec covers everything
...
Each breadcrumb body lives in a managed block of `.rudder/workflow.md`:
...
2. It walks up from `cwd` to find `.rudder/`. If none, exit 0.
...
5. It opens `.rudder/workflow.md` and parses every `[workflow-state:STATUS]`
   block.
...
`.rudder/workflow.md`. No script change required.
...
For that reason, `rudder update` must update `workflow.md` as one managed
...
|---|--------|-----------|-------|---------|
| 1 | `cmd_create` | `packages/cli/src/templates/rudder/scripts/common/task_store.py:206` | `"planning"` | `task.py create "<title>"` (also auto-sets the session active-task pointer when session identity is available — see R7 in 04-30-workflow-state-commit-gap PRD) |
| 2 | `cmd_start` | `packages/cli/src/templates/rudder/scripts/task.py:109-111` | `"in_progress"` (gated on prior `"planning"`) | `task.py start <dir>` |
| 3 | `cmd_archive` | `packages/cli/src/templates/rudder/scripts/common/task_store.py:319-323` | `"completed"` (unconditional flip + archive `mv`) | `task.py archive <dir>` |
| 4 | `emptyTaskJson` factory | `packages/cli/src/utils/task-json.ts:54` | `"planning"` (default) | TS callers (init, update) |
| 5 | `getBootstrapTaskJson` | `packages/cli/src/commands/init.ts:417` | `"in_progress"` (override) | `rudder init` (creator path) |
| 6 | `getJoinerTaskJson` | `packages/cli/src/commands/init.ts:460` | `"in_progress"` (override) | `rudder init` (joiner path) |
| 7 | migration-task literal | `packages/cli/src/commands/update.ts:2215-2226` | `"planning"` | `rudder update --migrate` for breaking-change manifest |

**No other writer exists.** No hook script writes `task.json.status` — verified
by `grep -rn '"status"' .rudder/scripts/`. Linear-sync hook (`linear_sync.py`)
...
1. Add a `[workflow-state:my-status]...[/workflow-state:my-status]` block to
   `.rudder/workflow.md` (STATUS charset: `[A-Za-z0-9_-]+`).
...
3. (Optional) Add the status to `.rudder/spec/cli/backend/workflow-state-contract.md`'s
...
hooks for child turns or inherit main-session per-turn context. Rudder must not
...
`rudder-implement` / `rudder-check` readers so they implement or check
directly instead of spawning nested Rudder sub-agents.

---

## DO

- Edit `.rudder/workflow.md` `[workflow-state:STATUS]` blocks for breadcrumb
  body changes; never touch the parser scripts.
- Keep `rudder update` whole-file behavior for hash-tracked `workflow.md`.
...
- Hook script structural change (parser, output envelope, what reads
  `task.json.status`)
- `workflow.md` update semantics in `rudder update`
