...
- **Common templates**: `src/templates/common/` — single source of truth for commands (start, finish-work), single-file workflow skills (before-dev, brainstorm, check, break-loop, update-spec), and multi-file bundled skills (rudder-meta) with `{{placeholder}}` resolution per platform
...
|-----------|----------|
| `src/templates/{platform}/` | Root directory |
| `src/templates/{platform}/commands/rudder/` | Slash commands (`.md` files) |
...
> Note: OpenCode uses JS plugins instead of Python hooks, has no `index.ts` template module, and has no `collectTemplates` — so `rudder update` does not track OpenCode template files. If a new platform uses JS plugins, follow this pattern.
...
| `src/templates/{platform}/agents/` | Agent definitions (`.md` files — implement, check, research) |
| `src/templates/{platform}/extensions/rudder/index.ts.txt` | Project-local extension source written to `.pi/extensions/rudder/index.ts` |
| `src/templates/{platform}/settings.json` | Platform settings that enable extension, skills, and prompts |

> Note: Pi Agent uses project-local TypeScript extensions instead of Rudder Python hooks. Keep generated hooks under `.pi/extensions/`, write prompt templates under `.pi/prompts/rudder-*.md`, write Agent Skills under `.pi/skills/`, and do not copy `shared-hooks/*.py` into `.pi/`. Do not redirect Pi to shared `.agents/skills` until shared Agent Skill text is platform-neutral; Codex and Pi command references can differ. For the nested Pi launcher contract, see "Scenario: Pi Sub-Agent Launcher".
>
> Project-local package isolation rule: when Rudder enables Pi for a project, `.pi/settings.json` must include a project-level `packages` array entry with `"source": "npm:pi-subagents"` and empty resource lists (`extensions`, `skills`, `prompts`, `themes`) to isolate global `npm:pi-subagents` effects from the repository while keeping the user's global Pi environment intact outside the project.
...
**Codex-only files under `.agents/skills/`** (currently `rudder-continue/SKILL.md` and `rudder-finish-work/SKILL.md`, written via `resolveAllAsSkillsNeutral()`) are an explicit exception: only Codex writes them, so byte-identity across platforms is not required and they may use `{{CLI_FLAG}}` / `{{PYTHON_CMD}}`. They still go through the neutral helper to keep `{{CMD_REF}}` neutralized for consistency with the surrounding shared skills.
...
- Good: `rudder-meta` installs as `<platform-skill-root>/rudder-meta/SKILL.md` plus `references/**`, and `collectPlatformTemplates(platform)` returns the same files.
- Base: no bundled skills exist; existing `resolveSkills()` / `resolveAllAsSkills()` behavior remains unchanged.
- Bad: platform-specific configurators copy `rudder-meta` manually, creating a second installer that update hash tracking can miss.

##### 6. Tests Required

- Init integration test proving at least Claude and Codex write `rudder-meta/SKILL.md` plus one reference file.
...
- Regression test proving `.rudder/.template-hashes.json` includes bundled skill reference files after init.
...
```typescript
files.set(".claude/skills/rudder-meta/SKILL.md", metaSkillContent);
```
...
**Rule**: Do not add a parallel installer for built-in multi-file skills. If `rudder init` writes a bundled skill file, the platform's `collectTemplates()` path must return the same relative path and byte-identical content so `.rudder/.template-hashes.json` can track it. At minimum, tests must cover one reference file (for example `rudder-meta/references/core/template-pipeline.md`) and the platform-specific install root.
...
> Note: Platforms using `createTemplateReader(import.meta.url)` in their `index.ts` handle their own template reading. The `extract.ts` functions (`getRudderSourcePath()`, `readRudderFile()`, `copyRudderDir()`) are primarily for the `.rudder/` workflow files, not platform templates.
...
| File | Change |
|------|--------|
| `src/templates/rudder/scripts/common/cli_adapter.py` | Add to `Platform` literal type, `config_dir_name` property, `detect_platform()`, `get_cli_adapter()` validation |
...
| `get_rudder_command_path()` | Command file path format | `.toml` vs `.md`, subdirectory vs flat |
...
| `get_commands_path()` | Command directory structure | `commands/rudder/` or `workflows/` |
...
1. `RUDDER_CONTEXT_ID` environment override for subprocesses.
...
plugin must therefore inject a shell-aware `RUDDER_CONTEXT_ID` prefix into Bash
tool commands in `tool.execute.before` when the command does not already set
it: POSIX shells use `export RUDDER_CONTEXT_ID=<context-key>;`, while Windows
PowerShell uses `$env:RUDDER_CONTEXT_ID = '<context-key>';`. Do not infer the
...
must continue to cover POSIX, `env ... RUDDER_CONTEXT_ID=...`, and PowerShell
forms.
Cursor must use `beforeShellExecution` as the shell bridge. The hook writes a
short-lived `.rudder/.runtime/cursor-shell/*.json` ticket containing the
...
through ordinary Bash environment. The generated `.pi/extensions/rudder/index.ts`
...
`export RUDDER_CONTEXT_ID=<context-key>;` when the command does not already set
one.
Copilot should use
...
through `RUDDER_CONTEXT_ID` or a platform-native environment variable exposed
by the host. Hooks and plugins should pass `RUDDER_CONTEXT_ID` to subprocesses
they launch. Claude Code is special: SessionStart provides `CLAUDE_ENV_FILE`,
so the shared hook must persist `export RUDDER_CONTEXT_ID=<context-key>` there
...
with a shell-aware `RUDDER_CONTEXT_ID` assignment using plugin session identity
...
spawn child `pi` processes with `RUDDER_CONTEXT_ID` in `env`.
...
|------|----------|----------------|
| `src/templates/rudder/scripts/common/task_store.py` | `_SUBAGENT_CONFIG_DIRS` (tuple) | Add `.{configDir}/` if the new platform can spawn sub-agents (Class-1 hook-inject, Class-2 pull-based, or extension-backed) |
...
> - **CRITICAL**: Template copy (`src/templates/rudder/scripts/`) must be byte-identical to live copy (`.rudder/scripts/`)
...
If Rudder project itself should support the new platform:

| Directory | Contents |
|-----------|----------|
| `.{platform}/` | Project's own config directory |
| `.{platform}/commands/rudder/` | Slash commands |
...
Use this pattern when a platform provides project-local JS/TS extension events and custom tools rather than Rudder-compatible Python hooks. Pi Agent is the reference implementation.
...
```text
.pi/settings.json
.pi/prompts/rudder-<command>.md
.pi/skills/<skill>/SKILL.md
.pi/agents/rudder-<agent>.md
.pi/extensions/rudder/index.ts
```
...
Extension-backed platforms MUST NOT receive `.rudder/templates/shared-hooks/*.py` under their config directory. Their hook-equivalent behavior belongs in generated extension source.

For Pi Agent:

| Rudder concept | Pi surface |
|---|---|
...
| Per-agent-invocation context | `before_agent_start` extension event — appends `buildRudderContext()` (PRD + jsonl) **and** the same per-turn breadcrumb to `systemPrompt` so sub-agent first turns see workflow state |
| Per-Bash-tool session identity | `tool_call` extension event; mutates `event.input.command` in place via `injectRudderContextIntoBash()` to prefix `export RUDDER_CONTEXT_ID=<context-key>;` |
| Sub-agent dispatch | custom `subagent` tool with `promptSnippet`/`promptGuidelines = SUBAGENT_DISPATCH_PROTOCOL`; resolves the Pi CLI JS entrypoint when possible, runs `--mode text -p --no-session`, sends the delegated prompt through stdin, and forwards `RUDDER_CONTEXT_ID` |
...
| Platform can spawn Rudder sub-agents | Add config dir to `_SUBAGENT_CONFIG_DIRS` |
...
| Extension path contains `spec/` in a skill name such as `rudder-update-spec` | Template hash exclusion must not drop it; only `.rudder/spec/` is user-owned spec data |
...
- Runtime script copies (`src/templates/rudder/scripts/**` and live `.rudder/scripts/**`) both recognize the platform.
...
- The TS-port workflow-state regex (`WORKFLOW_STATE_TAG_RE`) matches the same status names and body content as the Python `_TAG_RE` on a shared fixture from `templates/rudder/workflow.md`.
...
```typescript
await writeFile(
  path.join(configRoot, "extensions", "rudder", "index.ts"),
...
Use this contract when `.pi/extensions/rudder/index.ts` launches a nested Pi process for the Rudder `subagent` tool.
...
spawn(invocation.command, [
  ...
  cwd: projectRoot,
  env: { ...process.env, RUDDER_CONTEXT_ID: contextKey },
...
| Field / Env | Contract |
|---|---|
| `RUDDER_PI_CLI_JS` | Optional absolute or relative path to `@mariozechner/pi-coding-agent/dist/cli.js`; if set, it is authoritative |
...
| Context | Forward `RUDDER_CONTEXT_ID` into the child env when available |
...
| Condition | Behavior |
|---|---|
| `RUDDER_PI_CLI_JS` points to an existing file | Launch with `process.execPath` and `[cliJs, "--mode", "text", "-p", "--no-session"]` |
| `RUDDER_PI_CLI_JS` points to a missing file | Reject with an error naming `RUDDER_PI_CLI_JS` and the resolved missing path |
...
- the generated extension contains `resolvePiInvocation`, `RUDDER_PI_CLI_JS`, `process.execPath`, `APPDATA`, npm prefix lookup, and PATH splitting by `delimiter`;
...
- `.pi/extensions/rudder/index.ts` stays byte-for-byte aligned with `src/templates/pi/extensions/rudder/index.ts.txt`;
...
|----------|---------------|-------------|-----------------------|
| Claude Code | `/rudder:xxx` | Markdown (`.md`) | `/rudder:finish-work` |
| Cursor | `/rudder-xxx` | Markdown (`.md`) | `/rudder-finish-work` |
| OpenCode | `/rudder:xxx` | Markdown (`.md`) | `/rudder:finish-work` |
| Gemini CLI | `/rudder:xxx` | TOML (`.toml`) | `/rudder:finish-work` |
...
| Qoder | `/rudder-<name>` (commands) + `$<skill-name>` / `/skills` (workflows) | Markdown (`.md` with frontmatter + `SKILL.md`) | `/rudder-finish-work` |
| Antigravity | `/<workflow-name>` | Markdown (`.md`) | `/finish-work` |
| CodeBuddy | `/rudder:xxx` | Markdown (`.md`) | `/rudder:finish-work` |
| Copilot | `/rudder:xxx` | Markdown (`.prompt.md`) | `/rudder:finish-work` |
| Droid | `/rudder:xxx` | Markdown (`.md`) | `/rudder:finish-work` |
| Windsurf | `/rudder-xxx` | Markdown (`.md`) + `SKILL.md` | `/rudder-finish-work` |
| Pi Agent | `/rudder-xxx` prompt templates + `/skill:<name>` skills | Markdown (`.md`) + `SKILL.md` + TypeScript extension | `/rudder-finish-work` |
...
| **Class-2** — Pull-based | Platform's hook can't reliably mutate sub-agent prompts; Rudder injects a "Required: Load Rudder Context First" prelude into each sub-agent definition file so the sub-agent reads context itself at startup | Codex, Gemini CLI, Qoder, Copilot |
| **Class-3** — Extension-backed | Platform exposes hook-equivalent events and custom tools through a project-local TypeScript extension; Rudder owns the sub-agent tool and the context injection path | Pi Agent |

### Class-1 — Hook-inject (6 platforms)

Platform's PreToolUse-equivalent hook can fire on the sub-agent spawn tool AND modify the tool's prompt input. Rudder's `inject-subagent-context.py` (or OpenCode's plugin) reads `prd.md` + the JSONL-referenced spec files and rewrites the sub-agent's initial prompt.
...
```js
import { isRudderSubagent } from "../lib/rudder-context.js"

"chat.message": async (input, output) => {
  if (isRudderSubagent(input)) {
    // input.agent matched /^rudder-(implement|check|research)$/
...
`isRudderSubagent()` lives in `lib/rudder-context.js`; the regex matches `rudder-implement` / `rudder-check` / `rudder-research` exactly.
...
3. Single-session fallback in `RudderContext._resolveSingleSessionFallback()` — only when exactly 1 file exists in `.rudder/.runtime/sessions/`; refuses to guess when 0 or ≥2 files exist (source `session-fallback:<context_key>`). Mirrors Python `_resolve_single_session_fallback` (`active_task.py:497-519`).

`buildPrompt()` for implement / check / finish / research **must** prepend `<!-- rudder-hook-injected -->` so generated agent definitions in `.opencode/agents/*.md` can detect a successful injection (Rudder-internal contract; OpenCode itself ignores the marker).

`getActiveTask()` in `lib/rudder-context.js` itself includes the single-session fallback so any caller (`workflow-state` breadcrumb, `session-start` task status) sees the same resolved task as the prompt injector. The fallback only activates when the explicit context-key lookup misses, so multi-window setups remain isolated.

### Class-2 — Pull-based (4 platforms)

Platform's hook either doesn't expose a sub-agent spawn event or can't modify the prompt. Sub-agents must Read context themselves at startup. Rudder injects a "Required: Load Rudder Context First" prelude into each sub-agent definition file.
...
1. **`Active task: <path>` line in dispatch prompt** — primary path. The main agent is required by `workflow.md`'s `[workflow-state:in_progress]` breadcrumb to prefix every sub-agent dispatch (including `rudder-research`, since 0.5.8) with `Active task: <path from task.py current>`. The breadcrumb fires on every `UserPromptSubmit` while `task.json.status == in_progress`, so the rule is reinjected per turn.
...
When changing the prelude, the dispatch protocol, or the `session-fallback` semantics, all three layers must stay aligned. `regression.test.ts > [issue-225]` and `regression.test.ts > [session-fallback]` are the contract tests; `templates/rudder.test.ts > [issue-225]` asserts the workflow.md breadcrumb still carries the protocol. Manual e2e runbook lives in the historical task `.rudder/tasks/<archive>/05-04-fix-codex-subagent-missing-active-task/manual-verify.md`.

### Class-3 — Extension-backed (1 platform)

Platform can expose hook-equivalent events and custom tools through a project-local extension. Rudder owns the sub-agent tool and the context injection path. Unlike class-1 (which only handles sub-agent context) and class-2 (which only handles sub-agent prelude), class-3 owns **three** injection points: per-user-turn context, per-agent-invocation system prompt augmentation, and per-Bash-tool-call session-identity prefixing.

| Platform | Extension surface | Context delivery |
|---|---|---|
| Pi Agent | `.pi/extensions/rudder/index.ts` events + `subagent` tool | extension builds prompt from `.pi/agents/*.md`, `prd.md`, `info.md`, and JSONL-referenced files via `buildRudderContext()`; injects per-turn `<workflow-state>` + `<session-overview>` via `buildPerTurnInjection()`; agent definitions also receive the pull-based prelude as a fallback |
...
`templates/pi/extensions/rudder/index.ts.txt` registers handlers for three platform events plus one custom tool. Each injection point has a distinct lifecycle and a distinct failure mode if dropped.
...
| `before_agent_start` | `pi.on?.("before_agent_start", …)` | every agent invocation (main + sub-agents) | full Rudder context via `buildRudderContext()` (PRD + jsonl-referenced specs + agent definition) **appended to** the existing systemPrompt, plus the same per-turn breadcrumb so a sub-agent's first turn still sees workflow state |
| `tool_call` (Bash) | `pi.on?.("tool_call", …)` | every Bash tool call | mutates `event.input.command` in place via `injectRudderContextIntoBash()` to prefix `export RUDDER_CONTEXT_ID=<context-key>;` so child Python scripts (e.g. `task.py current`) inherit session identity |
...
- **Regex parity**: `templates/pi/extensions/rudder/index.ts.txt:WORKFLOW_STATE_TAG_RE` MUST mirror `templates/shared-hooks/inject-workflow-state.py:_TAG_RE` byte-for-byte. Both use the closing-tag backreference `\1` (or its TS equivalent in `[\/workflow-state:\1\]`) so a tag block parses identically in Python and TypeScript.
- **Breadcrumb body source**: `loadWorkflowBreadcrumbs()` in the Pi extension reads `.rudder/workflow.md` directly — same source as the Python hook. There is no separate TS-side template for breadcrumb bodies. If the regex drifts, the TS port silently falls back to hardcoded defaults and Pi loses parity.
...
- **`<session-overview>` parity**: Pi shells out to `python3 .rudder/scripts/get_context.py --mode session-overview` rather than re-implementing context generation in TS, so output stays canonical. Don't replace this with an inline TS implementation — that's a parity drift waiting to happen.
...
function onInput(event, ctx) {
  const overview = spawnSync("python3", [".rudder/scripts/get_context.py", "--mode", "session-overview"]);
...
|---|---|---|
| Workflow breadcrumb | `templates/rudder/workflow.md` `[workflow-state:in_progress]` block | Python `inject-workflow-state.py` and the Pi TS port — surfaced per-turn while a task is in progress |
| Pi extension constant | `templates/pi/extensions/rudder/index.ts.txt:SUBAGENT_DISPATCH_PROTOCOL` | Pi `subagent` tool's `promptSnippet` / `promptGuidelines` — surfaced at extension load and on each tool description render |
...
- Regression test asserting the `Active task:` rule appears in `templates/rudder/workflow.md` (`templates/rudder.test.ts > [issue-225]`).
...
Every generated `rudder-implement` and `rudder-check` agent definition must
...
`rudder-implement` or `rudder-check` applies only to the main session, and
...
`.rudder/tasks/04-17-subagent-hook-reliability-audit/research/platform-hook-audit.md`
...
```json
{"_example": "Fill with {\"file\": \"<path>\", \"reason\": \"<why>\"}. Put spec/research files only — no code paths. Run `python3 .rudder/scripts/get_context.py --mode packages` to list available specs. Delete this line when done."}
```
...
`.rudder/workflow.md` contains per-phase step detail under `#### X.X` headings, with per-platform variants demarcated by `[Platform Name, ...]` … `[/Platform Name, ...]` blocks.

Load step detail on demand (both commands and hooks use this):

```bash
python3 ./.rudder/scripts/get_context.py --mode phase                                   # Phase Index (no --step)
python3 ./.rudder/scripts/get_context.py --mode phase --step 1.1                        # Step 1.1 (all platforms)
python3 ./.rudder/scripts/get_context.py --mode phase --step 1.2 --platform cursor      # Step 1.2, cursor-filtered
```
...
When a hook can resolve the Rudder project directory before printing, prefer
the shared helper from `.rudder/scripts/common/__init__.py`:
...
For standalone hooks that cannot safely import `.rudder/scripts/common`, use
...
Every Rudder-owned SessionStart implementation that injects model-visible
...
only in implementations where Rudder can actually put context into the model
conversation:
...
| `opencode/plugins/session-start.js` | ✅ | Plugin prepends Rudder context into the first user message and persists it |
| `copilot/hooks/session-start.py` | ❌ | Microsoft documents `SessionStart.hookSpecificOutput.additionalContext` (preview, VS Code 1.110+), but consumption depends on the user's VS Code/Copilot version. Rudder emits the spec-compliant payload; do not add a first-reply notice until consumption is verified end-to-end. |
...
### Size Budget (measured on Rudder dev repo)
...
| `<workflow>` | 9.5 KB | TOC + Phase Index + Phase 1/2/3 step bodies. Meta sections (Core Principles / Rudder System / Breadcrumbs) excluded — they are either short prose Readable on demand or consumed by other hooks |
...
Before: every `.rudder/spec/*/index.md` was inlined in `<guidelines>` (10 KB+ on this repo). Main agent rarely uses index content (work is delegated to sub-agents, which get their own specific specs via `{task}/implement.jsonl` / `check.jsonl`).
...
2. `workflow.md` is **reference** (principles, file structure, best practices) — `start.md` Step 1 tells AI to `cat .rudder/workflow.md`, so it's accessed on-demand
...
On the Rudder dev repo (light use), `<guidelines>` is 10.8 KB vs 5.1 KB on vanilla — it grows linearly with spec `index.md` file count. Combined with `<instructions>` (16.1 KB), a project with many spec layers can still exceed 20 KB. Monitor this and consider the same lazy-load pattern for guidelines if it becomes a problem.
...
`SessionStart` only fires once per session. In long conversations, Claude's context compression can push the SessionStart message out of recent context, and the AI forgets the active Rudder task — resulting in workflow drift (skips `check`, forgets to `update-spec`, doesn't return to `finish` after user interruptions).
...
Breadcrumb text lives in `workflow.md` as `[workflow-state:STATUS]...[/workflow-state:STATUS]` blocks (same tag style as existing `[Platform, ...]` blocks). Users who fork the Rudder workflow edit **only the markdown**; the hook script stays untouched.

```markdown
[workflow-state:in_progress]
Flow: rudder-implement → rudder-check → rudder-update-spec → finish
Next required action: inspect conversation history + git status, then execute the next uncompleted step in that sequence.
For agent-capable platforms, do NOT edit code in the main session; dispatch `rudder-implement` for implementation and dispatch `rudder-check` before reporting completion.
[/workflow-state:in_progress]
```
...
4. No session active task → emit `no_task` pseudo-status breadcrumb instead of silent-exit. Header is `Status: no_task`; body nudges AI to load `rudder-brainstorm` + `task.py create` for multi-step work (or answer directly for trivial asks).
...
```
Net effect on a "no task" session: AI sees the Next-Action only at SessionStart; after 20 turns of context compression, the guidance is gone and AI forgets to use `rudder-brainstorm` for new multi-step requests.
...
The same rule applies to every other hook that's positioned as "repeated reminder": if the hook isn't emitting, the reminder loop is broken. The only legitimate silent-exit case is when **the hook doesn't own this codebase at all** (e.g. `.rudder/` not found → definitely not a Rudder project → OK to no-op).
...
`rudder init` generates a first-session task based on checkout state. Three branches dispatch off two filesystem flags:

| `.rudder/` exists? | `.rudder/.developer` exists? | Meaning | Task generated |
|---|---|---|---|
...
**Options Considered**:
1. `.rudder/workspace/<name>/` directory existence — ❌ this dir is committed to git, so a fresh clone already has it
2. A registry file listing onboarded developers — ❌ needs migration + bookkeeping, over-engineered for single-user checkouts
3. `.rudder/.developer` file existence — ✅ **chosen**

**Decision**: Use `.rudder/.developer` (gitignored) as the per-checkout onboarding signal.

**Why**: `.rudder/.developer` is declared in `.rudder/.gitignore` (template `gitignore.txt`), so it is never committed. A fresh clone has an empty `.developer` slot by construction; the first `init` writes it. Subsequent same-machine re-inits see the file and no-op.
...
**Anti-pattern**: Do not use `.rudder/workspace/<name>/` existence as "this developer already onboarded" — that directory is the journal archive and belongs to git.

### Gotcha: Joiner Dispatch Must Be Wired in Two Places

`rudder init` has two code paths that both reach the end of initialization but through different branches of `init()`. Any new init-time trigger (joiner onboarding, future first-session tasks, etc.) must be registered in **both**:
...
- Reached when `.rudder/` already exists AND user runs default `rudder init --user <name>` (no `--force`, no `--skip-existing`)
...
**Symptom**: `rudder init` configures the platform, but `rudder update` doesn't track its template files.
...
**Symptom**: In production builds (`dist/`), `rudder init` copies compiled `index.js` (and `.js.map`, `.d.ts`) into the user's config directory (e.g., `.gemini/index.js`).
...
**Symptom**: `rudder init --{platform}` doesn't work.
...
**Symptom**: `rudder update` auto-updates platform files on every run, even when nothing changed. The update summary shows hooks/settings as "changed".
...
**Symptom**: `rudder update` always detects a "new file" to add, even on a freshly initialized project with the same version.
...
**Symptom**: Pure backend project gets empty frontend spec templates after `rudder init`. After user deletes the unwanted `spec/frontend/` dir, `rudder update` recreates it.
...
- Command invocation syntax (e.g., `/rudder:xxx` vs `$skill-name`)
...
**Symptom**: The trigger works when users pass `--force` / `--skip-existing` / run init on an empty dir, but the default `rudder init --user <name>` on an existing checkout silently does nothing. Integration tests pass.

**Cause**: `init()` at `src/commands/init.ts` early-returns into `handleReinit` when `.rudder/` already exists and neither `--force` nor `--skip-existing` is set. Main dispatch at the end of `init()` is never reached. If the new trigger is only wired into main dispatch, the most common real-user path is uncovered.
