...
```json
{
  ...
  "migrations": [
    ...
    {
      "type": "delete",
      "from": ".rudder/scripts/deprecated.py",
      "description": "删除原因"
    }
  ]
}
...
**字段语义**：
- `migrationGuide` —— 面向最终用户的 narrative，被模板化塞进 `.rudder/tasks/MM-DD-migrate-to-<version>/prd.md` 的 PRD section
...
```json
{
  ...
  "from": ".claude/commands/rudder/before-backend-dev.md",
...
**工作机制**：
- **无需 `--migrate`** — 在每次 `rudder update` 时自动执行
...
可选 manifest 字段，声明本版本在 `.rudder/config.yaml` 引入的新顶层 section。`rudder update` 走完文件写入循环后会再扫一遍 `getConfigSectionsAddedBetween(fromVersion, toVersion)`，每条 entry 命中"目标文件存在 + sentinel 不在文件里"才会从打包模板里抽对应 section 追加到末尾。仅追加，幂等（重跑时 sentinel 已在）。
...
- `file` —— 目标文件相对仓库根的路径（当前只用过 `.rudder/config.yaml`，机制本身不限定）
...
**未来加新 section**：在对应版本 manifest 加一条 entry 即可，`update.ts` 不需要改。`packages/cli/src/templates/rudder/config.yaml` 模板里加对应 `#---` 分隔块和 section 内容，`rudder init` 走默认写入路径自然会拿到新 section。

## 受保护路径

以下路径不会被任何迁移操作修改或删除（用户数据）：

- `.rudder/workspace` — 开发者工作记录
- `.rudder/tasks` — 任务追踪
- `.rudder/spec` — 开发指南（用户自定义）
- `.rudder/.developer` — 开发者身份
- `.rudder/.current-task` — 当前任务指针
...
- 存储位置：`.rudder/.template-hashes.json`
- 用途：检测用户是否修改过模板文件
- 原理：比较当前文件 SHA256 与存储的哈希值
- 初始化：`rudder init` 时自动创建
- 更新：`rudder update` 后自动更新被覆盖文件的哈希

### `workflow.md` whole-file update contract

`.rudder/workflow.md` is not only documentation. It is runtime input for
`get_context.py`, `workflow_phase.py`, SessionStart strippers, and per-turn
workflow-state hooks.

`rudder update` must therefore keep `workflow.md` on the normal whole-file
template path:
...
not only fresh-init template tests: write the older `.rudder/.version`, stage
older hash-tracked template files, run `rudder update`, then assert the
...
```bash
rudder update              # 显示迁移提示
rudder update --migrate    # 执行迁移（修改过的文件会提示确认）
rudder update --migrate -f # 强制迁移（备份后执行）
rudder update --migrate -s # 跳过修改过的文件
```
