...
strips Rudder / platform injection tags so search hits aren't dominated by
session-start preamble.

The module is one self-contained TypeScript file plus four sibling test files;
it does **not** depend on the rest of the Rudder runtime (no
...
`node-gyp` rebuild required VS2017+ build tools, and `rudder` failed to
...
Rudder / platform injection tags would otherwise dominate every match.

If you need to add a new injection tag (e.g. a new Rudder hook adds
...
- **No transitive dependency on Rudder runtime**: `mem.ts` does not import
  from `configurators/`, `migrations/`, `templates/`, or `.rudder/scripts`.
...
grep -oE '"name":"Skill","input":\{[^}]+\}' \
  ~/.claude/projects/-Users-…-Rudder/<session-id>.jsonl

# Cross-session skill usage in a project
grep -hoE '"skill":"[a-z0-9-]+"' \
  ~/.claude/projects/-Users-…-Rudder/*.jsonl | sort | uniq -c
```
...
dialogue by Rudder brainstorm windows, allowing the high-density discussion
...
cover every shape Rudder users actually write:
...
- `python ./.rudder/scripts/task.py create "title"`
- `python3 ./.rudder/scripts/task.py create my-task`
- `py -3 .rudder/scripts/task.py create ...` (Windows launcher)
- `python3 .rudder\\scripts\\task.py start ...` (JSONL-double-escaped backslash)
- `python3 .rudder\scripts\task.py start ...` (single backslash)
...
| `python3 .rudder/scripts/task.py start .rudder/tasks/05-08-foo` | task-dir has `MM-DD-` prefix from `task.py create` | `slugFromTaskDir` strips a leading `MM-DD-` so a `create --slug foo` pairs with this `start` via slug match |
...
- New surface forms (e.g., `tl task create` if Rudder ever ships a wrapper)
...
inflates hit counts on every Rudder-using session. Always run text through
...
Measured impact on a 36 MB Claude session (Rudder dogfood):
...
- A new shell-arg form picked up by `parseTaskPyCommand` /
  ...
  The dogfood case studies live under `.rudder/tasks/05-08-mem-phase-slice/`
  and `.rudder/tasks/05-09-mem-phase-multi/`.
...
- `.rudder/tasks/05-08-mem-since-cross-day-filter/` — historical context for
  the `inRangeOverlap` switch
- `.rudder/tasks/05-08-mem-phase-slice/` — historical context for the
