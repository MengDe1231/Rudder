...
> When Rudder changes **workflow**, **platforms**, **commands**, or **skills**, which docs-site pages must update in lockstep. A concrete "Audit ALL Writers" checklist for documentation.
...
Scope: any edit to `packages/cli/src/templates/rudder/workflow.md` that adds/removes a step, renames a phase, or changes required/optional/once tags.
...
```bash
cd docs-site && grep -rln "rudder-<skill-name>" --include="*.mdx" \
...
See `.rudder/spec/cli/backend/platform-integration.md` → "Agent-Curated JSONL Contract (Phase 1.3)" for the code-side contract.
...
## Trigger 7: `.rudder/config.yaml` Template or Reader Changes

Scope: any edit to `packages/cli/src/templates/rudder/config.yaml`, config readers under `packages/cli/src/templates/rudder/scripts/common/`, or `configSectionsAdded` behavior in update manifests.
...
| `advanced/appendix-a.mdx` | Update the `.rudder/config.yaml` one-line purpose if the file's responsibility changes |
