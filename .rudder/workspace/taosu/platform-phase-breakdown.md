...
| 1.1 Requirements | load `rudder-brainstorm`，写 prd.md | 同 |
| 1.2 Research | spawn `rudder-research` sub-agent，产出写到 `{task}/research/*.md` | 主线程自己 research，同样写到 `research/*.md` |
| 1.3 Configure context | curate `implement.jsonl` + `check.jsonl`：填入 spec + research 文件，禁放代码路径 | Skip this step（context 在 Phase 2 由 `rudder-before-dev` 加载） |
...
| 平台组 | 指令 | 实际机制 |
|---|---|---|
| Claude, Cursor, OpenCode, CodeBuddy, Droid (Class-1 主体) | Spawn `rudder-implement`；hook auto-handles | 主线程的 PreToolUse hook 拦截 Task 调用，把 `implement.jsonl` 引用的 spec 内容拼进 sub-agent prompt |
| Kiro (Class-1，文案单拎出) | Spawn `rudder-implement`；"platform prelude auto-handles" | 用 `agentSpawn` hook 做同样的事 |
| Gemini, Qoder, Copilot (Class-2 的 3 个) | Spawn `rudder-implement`；"plugin auto-handles" | sub-agent 定义里的 prelude 要求它第一轮自己 Read 那些文件 |
| Codex (Class-2，文案单拎出) | Spawn `rudder-implement`；"Codex sub-agent definition auto-handles：reads .current-task, prd.md, info.md, implement.jsonl" | 同 Class-2，但 prelude 显式多读几个文件 |
| Kilo, Antigravity, Windsurf (agent-less) | 5 步手工流程：1) load `rudder-before-dev` skill 2) read prd.md 3) read research 4) write code 5) run lint + typecheck | 全在主线程 |

关键：agent-less 组是**唯一**提到 `rudder-before-dev` 的地方。
Skill Routing 表也是这么切的 —— agent-capable 组的表里根本没有 `rudder-before-dev` 这一行（这次改动的 Item 7）。

### 2.2 Quality check

| 平台 | 指令 |
|---|---|
| Agent-capable (10) | Spawn `rudder-check` sub-agent → review + auto-fix + lint/typecheck |
| Agent-less (3) | load `rudder-check` skill 自己走一遍 |
...
| Step | 行为 |
|---|---|
| 3.1 Quality verification | load `rudder-check`，最终过一遍 spec/lint/type/cross-layer |
| 3.2 Debug retrospective `[on demand]` | 反复 debug 同一个 bug 才做 —— load `rudder-break-loop` |
| 3.3 Spec update `[required]` | load `rudder-update-spec`，沉淀到 `.rudder/spec/` |
...
| User intent | Route |
|---|---|
| 需求模糊 | `rudder-brainstorm` |
| 要写代码 | Dispatch `rudder-implement` sub-agent per Phase 2.1 |
| 要验证 | Dispatch `rudder-check` sub-agent per Phase 2.2 |
| 循环撞同一个 bug | `rudder-break-loop` |
| 更新 spec | `rudder-update-spec` |

没有 `rudder-before-dev` —— 因为写代码的是 sub-agent，不是主线程。
...
|---|---|
| 需求模糊 | `rudder-brainstorm` |
| 要写代码 | `rudder-before-dev`（然后主线程直接动手） |
| 要验证 | `rudder-check` |
| 循环撞同一个 bug | `rudder-break-loop` |
| 更新 spec | `rudder-update-spec` |
...
| **workflow.md Skill Routing** | 按平台分两组：agent-capable 去掉 `rudder-before-dev`，agent-less 保留 |
