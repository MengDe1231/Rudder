...
- `.rudder/scripts/common/__init__.py`
- `.rudder/scripts/common/git_context.py`
- `.rudder/scripts/task.py`
- `.rudder/scripts/add_session.py`

### Specs & Commands (3 platforms)
- `.rudder/spec/backend/script-conventions.md`
- `.rudder/spec/guides/cross-platform-thinking-guide.md`
- `.claude/commands/rudder/update-spec.md`
- `.cursor/commands/rudder-update-spec.md`
- `.opencode/commands/rudder/update-spec.md`
...
### 4. 新增 Spec 文档
- 创建 `.rudder/spec/backend/platform-integration.md` - 记录如何添加新 CLI 平台支持的完整清单
...
- `src/templates/iflow/commands/rudder/update-spec.md`
- `src/commands/update.ts`
- `src/migrations/index.ts`
- `src/templates/opencode/plugin/session-start.js`
- `.rudder/spec/backend/platform-integration.md`
...
### 修复 `rudder update` 平台选择问题

**问题**：`rudder update` 会更新所有平台模板，不管 init 时选了哪些。用户 `init --claude` 后，update 会创建 `.cursor/`、`.iflow/` 等不需要的目录。
...
更新 `.rudder/spec/backend/platform-integration.md`：
...
记录到 `.rudder/spec/backend/directory-structure.md`：
...
- `.claude/commands/rudder/update-spec.md` (源文件)
- `.cursor/commands/rudder-update-spec.md`
- `.opencode/commands/rudder/update-spec.md`
- `src/templates/*/commands/*/update-spec.md` (4个模板)
- `.rudder/spec/backend/directory-structure.md`
- `.rudder/spec/backend/index.md`
...
- `dist/templates/opencode/lib/rudder-context.js` - Runtime platform detection
- `.rudder/spec/backend/quality-guidelines.md` - CLI patterns
...
Enhanced `/rudder:brainstorm` command with major workflow improvements.
...
- `.claude/commands/rudder/brainstorm.md` - Complete rewrite
...
| Templates | 5 files | 57 | claude, cursor, iflow, rudder, extract |
...
基于测试实践经验，创建了 `.rudder/spec/unit-test/` 规范目录（4 个文件），并更新了 platform-integration 指南。
...
**Key files**:
- `.rudder/spec/unit-test/` (4 new files)
- `.rudder/spec/backend/platform-integration.md`
...
|----------------|--------|
| `/rudder:start` | Step 3 加入 `cat .rudder/spec/unit-test/index.md` |
| `/rudder:before-backend-dev` | 加入读取 unit-test/conventions.md "When to Write Tests" |
| `/rudder:check-backend` | 加入检查是否需要新增/更新测试 |
| `/rudder:finish-work` | 新增 "1.5 Test Coverage" checklist |
...
**Key files**:
- `.claude/commands/rudder/` (4 commands)
- `.rudder/tasks/02-06-*/prd.md` (3 PRDs)
...
| `test/templates/rudder.test.ts` | 删除硬编码 scripts.size=23, typeof 检查 |
...
| Workflow Docs | Added `$improve-ut` skill + `/rudder:improve-ut` command as spec-first UT guidance and aligned backend check command references. |
...
Reviewed, fixed, and merged two community PRs adding new platform support (Kilo CLI #40 and Kiro Code #43). Also synced the docs project with current Rudder state.
...
- Updated quickstart.mdx (platform flags, useful flags, rudder update)
...
- `src/templates/rudder/scripts/task.py` — removed phantom paths from init-context generators
...
**Bug context**: User reported `validate` failing because init-context injected `.rudder/spec/shared/index.md` which was never created by `rudder init`.
...
- `.claude/commands/rudder/start.md` + `brainstorm.md`
- `.agents/skills/start/SKILL.md` + `brainstorm/SKILL.md`
- `.cursor/commands/rudder-start.md`
...
Added Gemini CLI as a first-class Rudder platform with TOML command templates

### Main Changes

## What Was Done

Added Gemini CLI (Google's AI coding CLI) as the 8th supported Rudder platform, at Cursor-level (commands only, no hooks/agents/settings).

### Key Decisions
- **TOML format**: Gemini CLI is the first platform using `.toml` instead of `.md` for commands
- **Subdirectory namespacing**: `commands/rudder/start.toml` → `/rudder:start` (same as Claude)
...
### Spec Updated
- `.rudder/spec/backend/platform-integration.md` — Added TOML commands pattern, commands-only pattern, updated Command Format table, added EXCLUDE_PATTERNS gotcha
...
Added Gemini CLI as 8th Rudder platform with full tests and spec updates

### Main Changes

## What Was Done

Added Gemini CLI (Google's AI coding CLI) as the 8th supported Rudder platform, at Cursor-level (commands only, no hooks/agents/settings). Then fixed gaps found by comparing with PR #47 (Antigravity).

### Phase 1: Core Implementation (commits 1-4)
- Type definitions + AI_TOOLS registry (`ai-tools.ts`)
- 14 TOML command templates in `src/templates/gemini/commands/rudder/`
...
| Tests | 新增 10 个测试（集成 #12、迁移边界、回归防护，369 total） |
| Release Script | 新增 `release:promote` 脚本，预发布→正式版一键升级 |
| Release | 执行 `pnpm release:promote`，CI 通过，npm 发布 `@mengde1231/rudder@0.3.0` |
| Post-release Fix | `0.3.0.json` breaking flag 改为 false — rc 用户升级不再误显 breaking 警告 |
| Docs | 两个 README 加入官网链接 (docs.tryrudder.app) |
...
| `/rudder:create-manifest` | 新增 slash command，引导 AI 完成完整 manifest 创建流程（找 tag、收集变更、写 changelog、调脚本、修转义） |

**Updated Files**:
- `src/migrations/manifests/0.3.1.json` — 新版本 manifest
- `.claude/commands/rudder/create-manifest.md` — Claude slash command
- `.cursor/commands/rudder-create-manifest.md` — Cursor slash command
...
add_session.py 写完 journal/index 后自动 git add .rudder/workspace && git commit，解决 record-session 后工作目录脏的问题。同步更新 8 个平台的 record-session 命令模板。
...
record-session 执行后自动提交 workspace 改动，解决脏目录问题。新增 .rudder/config.yaml 支持配置 session_commit_message 和 max_journal_lines，替代硬编码。
...
## 概要
rudder update 不再触碰 .rudder/spec/ 下的任何文件，spec 是用户自定义内容，仅在 init 时创建。
...
## 概要
优化 rudder init/update 的网络体验：添加代理支持、超时处理、进度提示和友好错误信息。
...
**Updated Files**:
- `src/templates/rudder/scripts/common/__init__.py` — added stdin to encoding fix
- `.rudder/scripts/common/__init__.py` — local copy updated
- `src/templates/rudder/scripts/add_session.py` — removed inline encoding
- `.rudder/scripts/add_session.py` — local copy updated
- `src/templates/rudder/scripts/common/git_context.py` — removed inline encoding
- `.rudder/scripts/common/git_context.py` — local copy updated
- `.rudder/spec/backend/script-conventions.md` — documented stdin encoding
...
修复 CC v2.1.39+ 引入的嵌套会话检测导致 /rudder:parallel 报错 'Claude Code cannot be launched inside another Claude Code session'。在 start.py 和 plan.py 中清除 CLAUDECODE 环境变量。同时修复了 update 跳过 workflow.md 和 workspace/index.md 的问题。
...
Implemented parent-child subtask system for Rudder task management.
...
- Skills-based platforms (codex/qoder/kiro) reference brainstorm Step 8 instead of `/rudder:brainstorm`

## Files Modified (35 files across 4 commits)

**Core logic (template + live copy pairs):**
- `src/templates/rudder/scripts/task.py` + `.rudder/scripts/task.py`
- `src/templates/rudder/scripts/create_bootstrap.py` + `.rudder/scripts/create_bootstrap.py`
- `src/templates/rudder/scripts/common/git_context.py` + `.rudder/scripts/common/git_context.py`
- `src/templates/rudder/scripts/common/task_queue.py` + `.rudder/scripts/common/task_queue.py`
...
**Docs:**
- `.claude/skills/rudder-meta/references/core/tasks.md`
- `.rudder/tasks/` (5 new task directories)
