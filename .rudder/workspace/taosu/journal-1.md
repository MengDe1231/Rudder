...
Integrated worktree-based multi-agent pipeline support into Rudder, enabling parallel feature development with isolated git worktrees.
...
1. `rudder init` → generated multi-agent scripts ✓
...
- `.rudder/worktree.yaml` - Worktree configuration
- `.rudder/scripts/multi-agent/` - Multi-agent scripts
...
| Project Docs | .rudder/workflow.md, agent-traces/index.md | Updated references |
...
**Updated Files**:
- `.rudder/structure/backend/*.md` (5 files)
- `.rudder/scripts/multi-agent/status.sh`
- `.rudder/scripts/multi-agent/cleanup.sh`
...
Major architecture refactor: direct copy .cursor/.claude/.rudder/scripts instead of template assembly. Removed 28 obsolete template files. Updated docs for new dogfooding pattern.
...
Reverted dogfooding to dedicated templates. Added rudder update command from PR #8.
...
## Updated Files
- `.rudder/scripts/multi-agent/start.sh` - session-id generation
- `.rudder/scripts/multi-agent/status.sh` - display improvements
- `src/templates/rudder/scripts/multi-agent/start.sh` - template sync
- `src/templates/rudder/scripts/multi-agent/status.sh` - template sync
- `.rudder/.gitignore` - add .session-id
- `src/templates/rudder/gitignore.txt` - template sync
...
| Readme | Move rudder.png to assets/ folder |
| Readme | Add initialization example image (info.png) |

## Updated Files
- `.rudder/scripts/add-session.sh`
- `src/templates/rudder/scripts/add-session.sh`
- `readme.md`
- `assets/rudder.png` (new)
- `assets/info.png` (new)
...
### 1. 研究 Parallel 流程
完整分析了 Rudder 的 multi-agent parallel 流程：
- `/rudder:parallel` 命令入口
...
- `.rudder/scripts/multi-agent/create-pr.sh`
- `src/templates/rudder/scripts/multi-agent/create-pr.sh`
...
将 `.rudder/scripts/` 下的 19 个 shell 脚本完整迁移为 Python 脚本，实现跨平台兼容（Windows/macOS/Linux）。
...
**Templates:**
- `src/templates/rudder/scripts/*.py` - 新 Python 脚本
- `src/templates/claude/agents/*.md` - Agent 文档
- `src/templates/claude/commands/rudder/*.md` - 命令模板
...
Enhanced the Rudder migration system to better handle breaking changes, with user-friendly warnings and AI-assisted migration support.
...
4. **Auto-creates task** `MM-DD-migrate-to-X.X.X/` with:
   - `task.json` (creator: rudder-update, assignee: current developer)
...
- `src/templates/rudder/scripts/task.py` - Lint fix (Ruff F541)
- `.rudder/scripts/task.py` - Lint fix (Ruff F541)
...
## Session 20: Add rudder-meta skill documenting Rudder system

**Date**: 2026-01-31
**Task**: Add rudder-meta skill documenting Rudder system

### Summary

Created rudder-meta skill with comprehensive documentation organized by platform compatibility (core/, claude-code/, how-to-modify/, meta/). Added version tracking, platform support matrix, file references, and modification guides.
...
## Session 21: Add rudder-meta skill & sync hotfix

**Date**: 2026-01-31
**Task**: Add rudder-meta skill & sync hotfix
...
| Item | Description |
|------|-------------|
| rudder-meta skill | Created comprehensive documentation skill for Rudder system |
...
## rudder-meta Skill Structure

```
.claude/skills/rudder-meta/
...
| `.rudder/scripts/common/git_context.py` | 强制 UTF-8 编码 |
| `.rudder/scripts/multi_agent/status.py` | 跨平台 `tail_follow()` |
...
- [ ] 更新 `.rudder/spec/guides/` 添加 "Cross-Platform Development" 指南
...
| Context Utils | `.opencode/lib/rudder-context.js` | ✅ 完成 |
| Agents | `.opencode/agents/*.md` | ✅ 6个 agent |
| Commands | `.opencode/commands/rudder/*.md` | ✅ 15个 commands |

### Phase 1: CLI Adapter ✅
| 功能 | 方法 |
|------|------|
| 命令构建 | `CLIAdapter.build_run_command()` |
| Agent 映射 | `CLIAdapter.get_agent_name()` (plan → rudder-plan) |
...
- 有 omo → 上下文已注入，跳过
- 无 omo → agent 自己读取 `.rudder/.current-task` 和对应 JSONL
...
| Init Flow | Added --opencode flag to rudder init |
...
- `.rudder/scripts/` → `src/templates/rudder/scripts/`
- `.claude/hooks/session-start.py` → `src/templates/claude/hooks/`

**Files Modified:**
- `.opencode/agents/*.md` (6 files)
- `.opencode/commands/rudder/parallel.md`
- `.rudder/scripts/common/cli_adapter.py`
- `.rudder/scripts/common/registry.py`
- `.rudder/scripts/multi_agent/{plan,start,status}.py`
...
修复 Windows 用户使用 Rudder hooks 和 scripts 时遇到的编码错误。
...
- `src/templates/rudder/scripts/*` (同步)
...
## 修改文件
- `src/templates/rudder/workflow.md` - 反斜杠替换
- `src/templates/*/commands/rudder/*.md` - 反斜杠替换
- `.rudder/workflow.md` - 本地同步
- `.claude/commands/rudder/*.md` - 本地同步
...
| Before | After |
|--------|-------|
| `.claude/commands/rudder/...` | `{adapter.config_dir_name}/commands/rudder/...` |
...
- `.rudder/scripts/common/cli_adapter.py`
- `.rudder/scripts/task.py`
- `src/templates/rudder/scripts/common/cli_adapter.py`
- `src/templates/rudder/scripts/task.py`
...
**After**:
```
"Follow your agent instructions to execute the task workflow. Start by reading .rudder/.current-task to get the task directory, then execute each action in task.json next_action array in order."
```
...
## 修改文件
- `src/templates/rudder/scripts/multi_agent/start.py`
...
## Summary
Two improvements to the Rudder project:
...
Path handling:
- Claude: `.claude/commands/rudder/<name>.md`
- Cursor: `.cursor/commands/rudder-<name>.md`

## Files Changed
- `CONTRIBUTING.md` (new)
- `CONTRIBUTING_CN.md` (new)
- `.rudder/scripts/task.sh` (modified)
...
User reported worktrees being created in wrong locations when using `/rudder:parallel`:
- Wrong path: `.rudder/workspace/taosu/tasks/02-03-sso-unit-test/worktree`
- Correct path should be: `../rudder-worktrees/{branch-name}`

## Root Cause

Agent documentation files contained incorrect task directory path examples:
1. `dispatch.md` line 16: showed `.rudder/workspace/{dev}/tasks/` instead of `.rudder/tasks/`
2. `plan.md` line 338: showed `.rudder/workspace/xxx/tasks/` in example
...
| `.rudder/scripts/multi-agent/status.sh` | `taosu` → `john` |
...
| `src/templates/rudder/scripts/multi-agent/status.sh` | Sync template |
...
../rudder-worktrees/         # Worktree 在项目外
└── {branch-name}/
```
...
Fixed multiple bugs in the parallel workflow reported by user (WSL, Rudder 0.2.13).

## Issues Fixed

### 1. Task Directory Path Bug
- **Problem**: Agent docs showed wrong path `.rudder/workspace/{dev}/tasks/` instead of `.rudder/tasks/`
...
### 3. Migration Bug
- **Problem**: `rudder update` failed with "path argument undefined" error
...
协助配置 Rudder 仓库的 Branch Ruleset：
- 保护 main 分支
...
- `.rudder/scripts/task.py` - base_branch 自动记录 + set-base-branch
- `.rudder/scripts/common/cli_adapter.py` - Cursor 平台支持
