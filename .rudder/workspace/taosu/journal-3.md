...
From `/rudder:break-loop` analysis — root cause was implicit assumption that task.json `status` field would be up-to-date. Fix: prompt now explicitly tells AI to archive based on work completion, not status field value.
...
**Updated files**:
- `src/templates/rudder/scripts/common/worktree.py` — nested dict YAML parser
- `src/templates/rudder/scripts/common/config.py` — get_hooks()
- `src/templates/rudder/scripts/task.py` — _run_hooks() + 4 integrations
- `src/templates/rudder/config.yaml` — hooks example (commented)
- `.rudder/scripts/hooks/linear_sync.py` — Linear sync hook
- `.rudder/spec/backend/script-conventions.md` — hooks code-spec
- `.rudder/spec/backend/directory-structure.md` — hooks/ directory
...
| Direct download mode | Custom registry without `index.json` → download directory to `.rudder/spec/` |
...
- `.rudder/spec/backend/error-handling.md` — Pattern 5, Mistakes 3-4
- `.rudder/spec/backend/quality-guidelines.md` — 4 new patterns/conventions
- `.rudder/spec/guides/cross-layer-thinking-guide.md` — Mode-Detection Probe Checklist
...
- Docs task tracked in docs repo (not Rudder repo)
- Archived tmux-support task and cancelled Linear issue MIN-340

## Repos Touched

- **Rudder**: 2 commits (manifest + README)
...
发现并修复 CC v2.1.63 将 Task 工具改名为 Agent 导致 Rudder PreToolUse context injection hook 全面失效的问题。
...
**影响**：所有 CC v2.1.63+ 的 Rudder 用户，implement/check/debug/research agent 的 code-spec context 注入全部失效。
...
- 调研了 SessionStart hook vs `/rudder:start` 等效性问题
...
Restructured Rudder repo as a monorepo: moved CLI code to `packages/cli/`, added `MengDe1231/docs` as git submodule at `docs-site/`.
...
| **Submodule** | `docs-site/` → `MengDe1231/docs` |
...
- `03-09-monorepo-spec-adapt` — Reorganize `.rudder/spec/` by package name (`cli/backend/` instead of flat `backend/`)
...
| 路径替换（~55 文件） | `.claude/` 18 文件, `.cursor/` 10 文件, `.agents/` 12 文件, `.opencode/` 14 文件, `.rudder/` 3 文件 |
| init-context 适配 | `task.py` 中 `get_implement_backend/frontend()` 更新为 `spec/cli/` 路径 |
| 测试验证 | 410 测试全部通过 |

### Phase 2: PRD 设计（已记录，待实施）

完整 PRD 在 `.rudder/tasks/03-09-monorepo-spec-adapt/prd.md`，包含 7 个 Part:
...
**Updated Files** (主要):
- `.rudder/spec/cli/backend/` (9 files, moved)
- `.rudder/spec/cli/frontend/` (7 files, moved)
- `.rudder/spec/cli/unit-test/` (4 files, moved)
- `.claude/commands/rudder/*.md` (12 files)
- `.cursor/commands/*.md` (10 files)
- `.agents/skills/*/SKILL.md` (12 files)
- `.opencode/commands/rudder/*.md` + agents + plugin (14 files)
- `.rudder/workflow.md`, `scripts/task.py`, `scripts/create_bootstrap.py`
- `.rudder/tasks/03-09-monorepo-spec-adapt/prd.md` (expanded)
...
- `.rudder/scripts/common/config.py` — get_submodule_packages()
- `.rudder/scripts/multi_agent/start.py` — 选择性 submodule init
- `.rudder/scripts/multi_agent/create_pr.py` — submodule 感知 PR
...
- `.rudder/spec/cli/backend/directory-structure.md`
- `.rudder/scripts/add_session.py`
- `.gitignore`
...
## Session 79: 提取 rudder-meta skill 到 marketplace/ + skills CLI 调研

**Date**: 2026-03-09
**Task**: 提取 rudder-meta skill 到 marketplace/ + skills CLI 调研
**Package**: cli

### Summary

将 rudder-meta skill 从 docs-site 提取到主仓库 marketplace/skills/，调研 skills CLI 发现机制，设计统一 marketplace 架构

### Main Changes

| 工作项 | 说明 |
|--------|------|
| rudder-meta 提取 | 从 `docs-site/plugins/rudder-meta/skills/rudder-meta/` 提取到 `marketplace/skills/rudder-meta/`，扁平化结构，去掉 `plugin.json` 和 `.claude-plugin/` |
...
| 解决方案确认 | `npx skills add owner/repo/subpath` 子目录语法限制扫描范围，用 `MengDe1231/Rudder/marketplace` |
...
**新增文件**:
- `marketplace/skills/rudder-meta/SKILL.md` + `references/`（25 个文件）
- `.rudder/tasks/03-09-extract-repo-level-content/prd.md`
- `.rudder/tasks/03-09-update-template-source/prd.md`
...
| 任务 | 描述 |
|------|------|
| 03-09-extract-repo-level-content | 将 rudder-meta skill 和 marketplace specs 从 docs-site 迁移到主仓库 |
...
- **rudder-meta skill 迁移**: 从 `docs-site/plugins/rudder-meta/` 移到 `marketplace/skills/rudder-meta/`
- **marketplace specs 复制**: 50 个 electron-fullstack spec 文件从 docs-site 复制到 `marketplace/specs/`
- **模板源常量更新**: `MengDe1231/docs` → `MengDe1231/Rudder`（template-fetcher.ts + spec 文档中的 giget 示例）
- **全仓库 URL 清理**: 搜索并修复所有遗留的 `MengDe1231/docs` 引用（marketplace README、docs-site 模板页、quick-start 页面）
- **docs-site 清理**: 删除 `plugins/rudder-meta/`（28 文件）和 `.claude-plugin/`（2 文件），更新贡献指南
- **安装命令统一**: 所有 `npx skills add` 命令使用 `MengDe1231/Rudder/marketplace` 子路径（避免扫描 .agents 等内部 SKILL.md）

## 技术决策

- 使用 subpath 语法 `MengDe1231/Rudder/marketplace` 解决扫描污染问题（主仓库有 ~17 个内部 SKILL.md）
...
- `.rudder/spec/cli/backend/directory-structure.md` — giget URL
- `marketplace/index.json` — 新增 rudder-meta 条目
- `marketplace/README.md` — 安装命令
- `marketplace/specs/electron-fullstack/` — 50 文件
- `marketplace/skills/rudder-meta/SKILL.md` — 从 docs-site 迁移
...
| SKILL.md 修复 | 重写为双仓库贡献指南（docs vs Rudder marketplace） |
...
- contribute SKILL.md 区分两个仓库的贡献路径（文档 → docs，skills/specs → Rudder/marketplace）
...
| Task path resolution fix | Fixed bug where `.current-task` storing project-relative paths (`.rudder/tasks/xxx`) caused double-path resolution in `_get_task_status()` |
...
- `.rudder/tasks/03-10-monorepo-compat/prd.md`
...
| Dogfooding | Configured `.rudder/config.yaml` with `update.skip` for 12 type-specific commands across 3 platforms |

## Smoke Test
- Fresh `rudder init` → delete `get_context.py` → `rudder update --dry-run` → shows "Deleted by you (preserved)" ✓
...
- `.rudder/config.yaml` (skip config for monorepo)
...
| Legacy spec detection | Hooks detect old flat `.rudder/spec/backend/` layout and warn users to re-init |
...
- `.rudder/scripts/task.py` — --package support
- `.rudder/scripts/add_session.py` — --package support
- `.rudder/scripts/common/cli_adapter.py` — package argument parsing
...
**Updated Files**:
- `packages/cli/src/templates/rudder/scripts/multi_agent/start.py`
- `packages/cli/src/templates/rudder/scripts/multi_agent/create_pr.py`
- `packages/cli/src/templates/rudder/scripts/multi_agent/cleanup.py`
- `.rudder/scripts/multi_agent/start.py` (dogfood sync)
- `.rudder/scripts/multi_agent/create_pr.py` (dogfood sync)
- `.rudder/scripts/multi_agent/cleanup.py` (dogfood sync)
...
## Key Files Changed
- `packages/cli/src/templates/rudder/scripts/common/worktree.py` — `_unquote()` + parser fix
- `.rudder/scripts/common/worktree.py` — synced dogfooded copy
...
- `.rudder/spec/cli/backend/quality-guidelines.md` — string sanitization patterns
...
- `.claude/commands/rudder/create-manifest.md` — updated workflow with docs-site step
...
- `.rudder/spec/cli/unit-test/conventions.md` — 新增 Tautological Input 反模式
- `.rudder/spec/cli/backend/script-conventions.md` — 新增 Parsing Structured Command Output
- `.rudder/spec/guides/index.md` — 新增 AI cross-review 验证 checklist
...
- `.rudder/scripts/multi_agent/start.py`
- `packages/cli/src/templates/rudder/scripts/multi_agent/start.py`
- `.rudder/spec/cli/unit-test/conventions.md`
- `.rudder/spec/cli/backend/script-conventions.md`
- `.rudder/spec/guides/index.md`
...
- **误 rsync spec 到 markdown 模板目录**：`.rudder/spec/`（项目自身规范）和 `packages/cli/src/templates/markdown/spec/`（新项目空白模板）完全不相关，不能互相同步
...
Fix scoped npm package names (e.g. `@zhubao/desktop`) creating nested `@scope/` directories in `.rudder/spec/` and `config.yaml`. Released as `v0.4.0-beta.2` hotfix.
...
| **#88 record-session wording** | Updated 9 platform templates: "AI must NOT execute git commit" → clarified that scripts handling `.rudder/` metadata commits are safe to run |
...
## Session 100: fix: rudder update skips unregistered Python scripts

**Date**: 2026-03-13
**Task**: fix: rudder update skips unregistered Python scripts
**Package**: cli
...
## Bug Fix: `rudder update` 漏同步 11 个 Python 脚本

用户报告 `rudder update` 跳过了新增的 Python 模块文件（common/ 下 9 个 + multi_agent/ 下 2 个）。

### Root Cause
`commands/update.ts` 的 `collectTemplateFiles()` 手动维护了一份脚本文件列表，与 `templates/rudder/index.ts` 的 `getAllScripts()` 不同步。init 用整目录拷贝没问题，但 update 依赖这份手动列表。
...
- `packages/cli/src/commands/update.ts`
- `packages/cli/test/regression.test.ts`
- `.rudder/spec/guides/code-reuse-thinking-guide.md`
...
### 1. Bug Fix: `rudder update` 漏同步 Python 脚本
- 用户报告 `rudder update` 跳过新增 Python 模块
