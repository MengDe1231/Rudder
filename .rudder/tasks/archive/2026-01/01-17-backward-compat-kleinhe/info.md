...
| 机制 | 设计目的 | 目标用户 |
|------|----------|----------|
| `upgrade.sh` | 从本地源码升级 | Rudder 开发者/贡献者 |
| `rudder init --force` | 重新初始化 | 首次安装或重置 |
| `rudder init --skip-existing` | 补充缺失文件 | 首次安装的容错 |
| `rudder update` | 升级现有安装 | 普通用户（未实现） |
...
```
npm install -g @mengde1231/rudder   # 安装 CLI + 模板
                    ↓
rudder init                          # 复制模板到项目
                    ↓
npm update @mengde1231/rudder        # 更新 CLI + 模板
                    ↓
rudder update                        # 应用新模板（缺失！）
```
...
```
"我的项目用 Rudder 1.0，现在 Rudder 1.1 发布了，我想升级"
```

用户期望：
1. **一个命令**：`rudder update`
2. **自动化**：不需要知道文件在哪里
3. **安全**：不会丢失我填写的内容
4. **透明**：告诉我发生了什么
5. **可回滚**：出问题能恢复
...
1. **单一入口**：`rudder update` 是用户唯一需要的命令
2. **默认安全**：永不丢失用户数据
3. **透明操作**：显示将要发生的变化
...
```
rudder init                    rudder update
─────────────────────────────   ─────────────────────────────
• 创建 .rudder/ 目录            • 更新 scripts/
• 询问工具选择                    • 更新 hooks/
...
| 场景 | 命令 | 说明 |
|------|------|------|
| 用户升级项目 | `rudder update` | 从 npm 包读取最新模板 |
| 开发者测试本地分支 | `upgrade.sh` | 从本地源码路径读取 |

**为什么需要 upgrade.sh？**
- 开发者在 Rudder 仓库内开发新功能
- 想在其他项目测试当前分支的改动
- 运行 `upgrade.sh /path/to/Rudder` 直接用本地代码更新

**命令总结**：
```
rudder init               # 首次初始化
rudder update             # 从 npm 升级（普通用户）
./.rudder/scripts/upgrade.sh <path>  # 从本地升级（开发者测试用）
```

**移除**：
- `rudder init --force` → 改为 `rudder init --reset`（需要确认）
- `rudder init --skip-existing` → 不再需要（update 命令处理）
...
export async function update(options: UpdateOptions): Promise<void> {
  ...
  // 1. 验证 Rudder 已初始化
  if (!fs.existsSync(path.join(cwd, ".rudder"))) {
    throw new Error("Rudder not initialized. Run 'rudder init' first.");
  }
...
```bash
rudder update              # 交互式更新（默认）
rudder update --dry-run    # 预览变更，不执行
rudder update --force      # 所有冲突都选择覆盖
rudder update --skip-all   # 所有冲突都选择跳过
rudder update --create-new # 所有冲突都创建 .new 副本
```
...
async function checkForUpdates(cwd: string): Promise<void> {
  const versionFile = path.join(cwd, ".rudder", ".version");
  ...
  if (installed !== VERSION) {
    console.log(chalk.yellow(`\n⚠️  Rudder update available: ${installed} → ${VERSION}`));
    console.log(chalk.gray(`   Run: rudder update\n`));
  }
}
...
.rudder/scripts/feature.sh has changes.
...
--- Summary ---
  ...
  Backup: .rudder/.backup-20260118-143022/
...
1. **首次安装验证**
   - `rudder init` 后检查 `.rudder/.version` 存在

2. **更新检测验证**
   - 修改一个模板文件
   - 运行 `rudder update --dry-run`
