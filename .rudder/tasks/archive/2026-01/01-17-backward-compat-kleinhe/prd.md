...
**用户数据版本** (`.rudder/.version`)：

```
1.0.0
```

这是一个简单的文本文件，记录用户当前安装的 Rudder 版本。首次运行 `/start` 时自动创建。
...
Schema 版本独立于 Rudder 版本，只在数据格式变化时递增。
...
rename_progress_files() {
    find "$RUDDER_DIR/agent-traces" -name "progress-*.md" | while read f; do
...
main() {
    ...
    # 执行迁移
    find "$RUDDER_DIR/agent-traces" -name "feature.json" | while read f; do
...
用户运行 rudder upgrade
        ↓
检测当前版本 (.rudder/.version)
        ↓
检测目标版本 (.rudder/version.json)
        ↓
确定需要执行的迁移脚本链
例如: v1.0.0 → v1.1.0 → v1.2.0
        ↓
创建备份 (.rudder/backups/2026-01-17_v1.0.0/)
        ↓
依次执行迁移脚本
        ↓
更新用户版本文件 (.rudder/.version)
        ↓
验证迁移结果
        ↓
完成
...
# 查看会发生什么（不执行）
rudder upgrade --dry-run

# 升级到特定版本
rudder upgrade --to 1.1.0

# 强制升级（跳过确认）
rudder upgrade --force
...
Backup will be created at:
  .rudder/backups/2026-01-17_v1.0.0/
...
```bash
# 检查版本
current_version=$(cat .rudder/.version 2>/dev/null || echo "0.0.0")
target_version=$(jq -r '.version' .rudder/version.json)

if version_lt "$current_version" "$target_version"; then
    echo "⚠️  Rudder has been updated!"
    echo "   Current: $current_version"
    echo "   Available: $target_version"
    echo ""
    echo "   Run 'rudder upgrade' to update."
    echo ""
fi
```
...
1. **创建版本文件系统**
   - 更新 `.rudder/version.json` 添加 schemaVersion
   - 创建 `.rudder/.version` 用户版本文件
   - 在 `/start` 中初始化用户版本

2. **创建迁移目录结构**
   - `.rudder/migrations/` 目录
   - `index.sh` 迁移入口
   - `README.md` 开发指南
...
1. [ ] 用户可以运行 `rudder upgrade` 升级到新版本
2. [ ] 升级前自动创建备份
3. [ ] 支持 `--dry-run` 查看升级内容
4. [ ] 支持 `rudder rollback` 回滚
5. [ ] `/start` 命令会提示版本更新
