...
Remove the duplicated `.txt` template files under `src/templates/` and refactor the template system to read directly from the Rudder project's own `.rudder/` directory. This implements the "eat your own dog food" (dogfooding) principle - Rudder uses its own actual configuration files as the source of truth for templating.

## Background

Currently, the template system maintains duplicate content:
- `src/templates/markdown/*.md.txt` duplicates `.rudder/structure/**/*.md`
- `src/templates/scripts/*.sh.txt` duplicates `.rudder/scripts/*.sh`
...
1. **Refactor `src/templates/extract.ts`**
   - Create a `getRudderSourcePath()` utility that resolves to the `.rudder/` directory
   - Update `readMarkdown()` to read from `.rudder/structure/` instead of `src/templates/markdown/`
   - Update `readScript()` to read from `.rudder/scripts/` instead of `src/templates/scripts/`

2. **Update Template Index Files**
   - Update `src/templates/markdown/index.ts` to export content from `.rudder/structure/**/*.md`
   - Update `src/templates/scripts/index.ts` to export content from `.rudder/scripts/*.sh`
...
3. **Remove `.txt` Template Files**
   ...
   - Keep `src/templates/commands/` (these don't have .rudder counterparts yet)
...
4. **Update `package.json` for npm Distribution**
   - Ensure `.rudder/` directory is included in the npm package `files` array
   - Verify the paths resolve correctly when installed as an npm package

## Acceptance Criteria

- [ ] `rudder init` command works correctly and creates the same output as before
...
- [ ] When Rudder is installed as an npm package, templates are still accessible
...
4. **Build Consideration**: Since templates are read at runtime via `fs.readFileSync`, ensure the `.rudder/` files are bundled in the npm package distribution.

## Out of Scope

- Refactoring `src/templates/commands/` - these don't have `.rudder/` counterparts
- Changing how agents or hooks templates work - they already use the correct pattern
- Creating new `.rudder/` structure files - only use existing ones
