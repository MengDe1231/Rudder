# PRD: Rudder 产品形态与分发策略

## 背景

团队在讨论 Rudder 的产品定位和分发策略。核心问题是：如何让 Rudder 这套开发流程治理能力，以更低门槛的方式被用户理解、尝试和采纳。
...
**期望**：
- 用一个极简的方式**一次性初始化**系统
- 初始化后，Rudder 工作流**自动注入**到每次 AI 会话中
- **不修改用户原有的使用习惯**——用户正常和 AI 对话即可
- 用户甚至可以不知道 Rudder 在后台运行
...
之后每次：打开 Claude Code，直接开始写代码
          ↓
Rudder 自动在后台生效（读 guidelines、记录 traces、执行检查等）
```
...
1. **强制植入** — Rudder 的上下文必须在每次会话中被加载，不能依赖 AI "选择"调用
2. **项目级配置** — 每个项目的 Rudder 配置可以不同，通过 Git 同步
...
**用户体验**：
```
用户：安装 Rudder（npm install / git clone）
      ↓
Rudder 自动配置 .claude/settings.json
      ↓
之后每次：用户直接使用 Claude Code，Rudder 自动生效
```
...
| 植入确定性 | 100% 的会话中，Rudder 上下文都被加载 |
| 可理解性 | 用户能用一句话解释"Rudder 是什么" |
...
**讨论脉络**：
1. 昨天讨论了用 Skill 包装 Rudder 的可能性（入口 skill 思路）
2. 今天进一步明确：Rudder 和普通 Skill 有本质区别（强制 vs 可选）
...
**讨论背景**：
用户提出考虑用"AI coding 治理框架"作为 Rudder 的 tagline，需要分析这个定位是否合适。

**关键发现**：

#### 1. Claude Code Plugin vs Rudder 的区别

| 特性 | Claude Code Plugin（全局） | Rudder（项目级） |
|------|---------------------------|-------------------|
| 安装位置 | `~/.claude/plugins/` | 项目 `.claude/` + `.rudder/` |
| 作用范围 | 所有项目共享 | 仅当前项目 |
| 安装方式 | `npm install -g` 或 `/plugin` | `rudder init` |

**结论**：Rudder 利用的是 Claude Code 的**项目级配置能力**，而不是全局 Plugin 系统。这是设计选择——每个项目需要不同的规范和追踪。
...
#### 5. Rudder 核心价值 vs "治理"能否表达

| 问题 | Rudder 的解法 | "治理"能表达？ |
|------|----------------|----------------|
...
**方向 A: 用比喻（像 Rudder 这个名字本身）**
> *"Guide AI's wild growth along a disciplined path"*
> *"让 AI 的能力沿着正确的方向生长"*

Rudder（藤架）本身就是完美的比喻——**引导而非管束**。
...
- **推荐**继续用 Rudder 的**藤架比喻**，或更开发者友好的词：harness（驾驭）、rails（轨道）、guide（引导）
