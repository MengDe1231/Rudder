...
研究 Jesse Vincent 的 Superpowers (Claude Code Plugin) 设计理念，与 Rudder 对比，提取值得学习的模式。
...
## Rudder vs Superpowers 对比

| 维度 | Rudder | Superpowers |
|------|---------|-------------|
| **注入方式** | Hook 强制注入 specs (jsonl 配置) | Session hook + AI 主动搜索 skills |
| **规范格式** | .rudder/structure/*.md | SKILL.md (带 frontmatter) |
...
**Rudder 现状**: specs 是文档，没有 frontmatter 元数据
...
- 官方 Rudder skills 库
- 用户项目级自定义
- `rudder install skill:code-review`
...
**Rudder 现状**: 完全预配置，AI 不参与发现

**可借鉴**:
```bash
# AI 可以主动搜索相关 specs
./.rudder/scripts/find-specs.sh "error handling"
```
...
**Rudder 可借鉴**: 在 agent-traces 中增加 AI 自我反思段落
...
| 功能 | 原因 |
|------|------|
| 完全主动发现 | Rudder 的核心价值就是"强制注入" |
| Plugin 系统 | Rudder 本身就是工具，不需要嵌套 |
