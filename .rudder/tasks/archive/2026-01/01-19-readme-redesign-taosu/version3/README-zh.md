...
`-u` 是你的标识，用于创建个人工作区 `.rudder/workspace/your-name/`。
...
AI:  已添加到 .rudder/spec/frontend/state-management.md
```
...
**Q: 为什么用 Rudder 而不是 Skill？**

Skill 是可选的，用户可以跳过，质量无法保证。Rudder 通过 Hook 强制注入规范：不是"可以用"而是"必定生效"。把随机性变成确定性，确保质量不会随时间劣化。

**Q: "Wild AI ships nothing" 什么意思？**

不加约束的 AI 写不出能上线的代码。Rudder 给 AI 加上项目规范和工作流约束。
...
它们都是 all-in-one 文件，AI 每次都要读全部内容。Rudder 采用分层架构 + 上下文压缩：只加载当前任务相关的规范。工程规范应该优雅分层，而不是一锅炖。

**Q: 多人同时用会冲突吗？**

不会。每人有独立工作区 `.rudder/workspace/{name}/`，规范文件是共享的（提交到 Git）。
...
MIT 许可证 • [Mindfold](https://github.com/MengDe1231)
