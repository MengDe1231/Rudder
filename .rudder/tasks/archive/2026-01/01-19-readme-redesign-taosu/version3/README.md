...
- `u` is your identifier, creates personal workspace `.rudder/workspace/your-name/`.
...
AI:  Added to .rudder/spec/frontend/state-management.md
...
**Q: Why Rudder instead of Skills?**

Skills are optional — AI may skip them, leading to inconsistent quality. Rudder enforces specs via Hook injection: not "can use" but "always applied". This turns randomness into determinism, ensuring quality doesn't degrade over time.
...
Those are all-in-one files — AI reads everything every time. Rudder uses layered architecture with context compression: only loads relevant specs for current task. Engineering standards should be elegantly layered, not monolithic.

**Q: Will multiple people conflict?**

No. Each person has own space to record journal `.rudder/workspace/{name}/`, spec files are shared (committed to Git).
