# Rudder Context Consumption Report

> **Answer to**: "Have you tested how much context Rudder + Skill + MCP consumes?"
...
**Conclusion**: Rudder consumes ~6,500 tokens at session start, and a typical implement+check cycle adds another ~6,000-9,000 tokens. This is **3-7.5% of a 200k context window**.
...
MCP servers add tool definitions to the system prompt. This is separate from Rudder.
...
|----------|---------------|----------|-----------------|
| **Raw Claude** | 0 | 0 | 0 |
| **Rudder only** | 6,530 | 6,000-9,000 | 12,530-15,530 |
...
| Model Context | Rudder Suitability |
|---------------|---------------------|
...
1. **Rudder baseline is ~6,500 tokens** (3.3% of 200k)
...
**Bottom line**: If you have a 200k context model, Rudder overhead is minimal. You retain 93%+ of your context for actual work.
