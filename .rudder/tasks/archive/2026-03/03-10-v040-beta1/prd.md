# Rudder v0.4.0-beta.1: Monorepo Init + 单仓向前兼容
...
- Rudder 主仓库自身已是 monorepo（`packages/cli` + `docs-site` submodule）
- 项目 dotfiles（`.claude/`、`.rudder/scripts/`）已部分适配 monorepo（路径替换 + 动态发现）
- **CLI 模板源码**（`packages/cli/src/templates/`，即用户 `rudder init` 后得到的文件）**零 monorepo 逻辑**
- 已有两份详细 PRD 作为参考：
...
|--------|------|------|--------|--------|
| **S1** | `s1-infra` | Monorepo 检测 + config/paths 基础设施 + init 适配 | P1 | 用户可 `rudder init` 出 monorepo 结构 |
...
- [ ] `rudder init` 在 pnpm/npm/yarn workspace 项目中自动检测并询问 monorepo 模式
...
- [ ] 单仓项目行为完全不变
- [ ] `rudder update` 安全兼容 monorepo spec 目录
...
- 在子 package 目录运行 `rudder init`（要求 repo root）
