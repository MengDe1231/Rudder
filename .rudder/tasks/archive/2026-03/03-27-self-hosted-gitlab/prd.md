...
Make `rudder init --registry` work with self-hosted GitLab (and GitHub Enterprise) URLs, both HTTPS and SSH formats.
...
The problem is NOT that giget doesn't support self-hosted GitLab (it does via `GIGET_GITLAB_URL`). The problem is Rudder's own two parsing layers reject the URL before giget ever sees it.
...
- [ ] `rudder init --registry https://git.company.com/org/repo` works
- [ ] `rudder init --registry git@git.company.com:org/repo` works
