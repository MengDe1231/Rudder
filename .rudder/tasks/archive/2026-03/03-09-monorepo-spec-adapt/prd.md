...
将 Rudder 的整个 workflow 体系（spec、commands/skills、hooks、tasks、sessions、parallel）适配 monorepo 结构，同时保持对单仓库项目的向前兼容。

## Background

monorepo 重构后（父任务 03-09-monorepo-submodule），`src/` 移到了 `packages/cli/src/`。Rudder workflow 体系中大量路径引用、context 注入、session 记录都假定单仓库结构，需要全面适配。
...
将 `.rudder/spec/` 从扁平的 `backend/`、`frontend/` 改为按 package 组织：
...
> 以下改动发生在 `.rudder/scripts/`，后续同步到 `packages/cli/src/templates/rudder/scripts/` 时可直接复用。
...
**`cli_adapter.py`（`.rudder/scripts/common/`）**
...
> 以下改动发生在 `.rudder/scripts/task.py`，后续同步到 `packages/cli/src/templates/rudder/scripts/` 时可直接复用。
...
```bash
python3 ./.rudder/scripts/get_context.py --packages
```
...
`get_context.py` 默认模式（无 `--packages`）也在输出中追加一段 monorepo 概览，供 `/rudder:start` 消费。
...
P4 中 md 文件用 `ls -d .rudder/spec/*/` 做发现。P3 完成后替换为：
```bash
python3 ./.rudder/scripts/get_context.py --packages
```
...
```
1. 列出 spec 模块: ls -d .rudder/spec/*/
2. 读取你要工作的 package 对应的 index.md
3. 始终读取 .rudder/spec/guides/index.md（跨 package 共享）
```

**workflow.md 更新**：
- 说明 spec 目录约定：`spec/<package>/<layer>/`
- `guides/` 是跨 package 共享的特殊目录
- Quick Reference / Code Quality Checklist 引导到 `/rudder:check` 和 spec `index.md` 的 Quality Check 节
...
| 文件 | 改动 |
|------|------|
| `start.md` (×4 平台) | Step 3 从硬编码 `spec/cli/{frontend,backend}/index.md` → `ls -d .rudder/spec/*/` + `cat spec/<package>/<layer>/index.md` |
...
| 来源 | 目标 |
|------|------|
| `docs-site/.claude/commands/rudder/before-docs-dev.md` | `.claude/commands/rudder/before-docs-dev.md`（Phase 2 Part 1 后会被合并进 `before-dev.md`）|
| `docs-site/.claude/commands/rudder/check-docs.md` | 同上 → 合并进 `check.md` |
| `docs-site/.claude/commands/rudder/commit.md` | `.claude/commands/rudder/commit.md` |
| `docs-site/.claude/skills/contribute/SKILL.md` | `.claude/skills/contribute/SKILL.md` |
| `docs-site/.rudder/spec/docs/` (7 文件) | `.rudder/spec/docs-site/` |

**从 submodule 删除的冗余配置**：
- `docs-site/.claude/` 整个目录
- `docs-site/.cursor/` 整个目录
- `docs-site/.rudder/scripts/`、`workflow.md`、`.template-hashes.json`、`.version`、`.gitignore`、`worktree.yaml`、`spec/guides/`

**保留**：`docs-site/.rudder/tasks/`（历史记录）、`docs-site/.rudder/workspace/`（journal）
...
| 文件 | 问题 |
|------|------|
| `.claude/commands/rudder/parallel.md` | 硬编码 `cat .rudder/spec/cli/frontend/index.md` 和 `spec/cli/backend/index.md` |
| `.agents/skills/parallel/SKILL.md` | 同上 |
| `.opencode/commands/rudder/parallel.md` | 同上 |

**改法**：同 P3e 模式，替换为 `python3 ./.rudder/scripts/get_context.py --mode packages` + `cat .rudder/spec/<package>/<layer>/index.md`。
...
**`start.py`（`.rudder/scripts/multi_agent/`）**
...
**`config.py`（`.rudder/scripts/common/`）**
...
**`create_pr.py`（`.rudder/scripts/multi_agent/`）**
...
1. **`rudder update` 覆盖定制**：在 Rudder 仓库自身跑 `rudder update` 可能从模板覆盖已定制的 dotfiles（`spec/cli/` 路径）。需确保冲突检测机制正常工作。
...
后续 Rudder CLI 产品支持 monorepo（如 `rudder init --monorepo`）时，下面标注 **可泛用** 的改动可以直接复用到模板源（`packages/cli/src/templates/`）。

### 项目特化（仅 Rudder 仓库自身）

| 改动 | 原因 |
|------|------|
| **Phase 1 Part A**: `spec/backend/` → `spec/cli/backend/` 目录移动 | Rudder 仓库手动重组自己的 spec 目录 |
| **Phase 1 Part B**: ~55 文件路径替换 | 仅影响 Rudder 仓库的 dotfiles，模板源不改 |
| **Phase 1 Part D-partial**: `init-context` 硬编码 `spec/cli/` | 过渡方案，后续被 Part 2 的 `--package` 参数取代 |
| **Phase 2 Part 5**: docs-site submodule 迁移 | Rudder 仓库特有的 submodule 清理 |
...
- Rudder 产品级 monorepo 支持（`rudder init --monorepo`）— 独立 feature
