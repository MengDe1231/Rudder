...
允许用户配置自定义远程仓库作为 spec 模板源，而不仅限于 Mindfold 官方 marketplace。这样企业/团队可以维护自己的 spec 模板仓库，通过 `rudder init --registry` 下载使用。
...
* [ ] 自动检测两种模式：
  ...
  * **直接下载模式**：源目录下无 `index.json` → 直接下载整个目录到 `.rudder/spec/`
...
* [ ] `rudder init --registry gh:myorg/myrepo/marketplace` → 展示自定义 marketplace picker
* [ ] `rudder init --registry gh:myorg/myrepo/my-spec` → 直接下载到 `.rudder/spec/`
* [ ] `rudder init --registry gitlab:myorg/myrepo/specs` → GitLab 源也能用
* [ ] `rudder init --registry gh:myorg/myrepo/marketplace --template my-spec` → 跳过 picker
...
fetch rawBaseUrl + "/index.json"
  ...
  └── 404 → 直接下载 gigetSource → .rudder/spec/
```
