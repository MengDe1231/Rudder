...
## Overview
Add Qoder as a new skills-based platform in Rudder. Trae was originally planned but dropped after research.
...
### Trae (dropped)
Trae was dropped after research revealed it's unsuitable for Rudder:

1. **No deterministic invocation trigger**: Trae skills (`.trae/skills/{name}/SKILL.md`) are invoked by AI-automatic matching or manual mention in chat — no slash command syntax like `/rudder:start`
2. **IDE-only**: No CLI executable, cannot run headless agents for multi-agent pipeline
3. **Incompatible workflow model**: Rudder relies on deterministic command triggers to enforce workflow (e.g., `/rudder:start` before coding, `/rudder:finish-work` before commit). Without deterministic triggers, the workflow cannot be enforced
...
- `src/templates/rudder/scripts/common/cli_adapter.py` — all method branches
- `src/templates/rudder/scripts/multi_agent/plan.py` — `--platform` choices
- `src/templates/rudder/scripts/multi_agent/start.py` — `--platform` choices
...
- [x] `rudder init --qoder` creates `.qoder/skills/` with SKILL.md files
