...
|----------|------|----------|-------------------|
| Commands (md) | Claude, Cursor, iFlow, Kilo, OpenCode | `commands/rudder/*.md` | Yes (各 4 个) |
| Commands (toml) | Gemini | `commands/rudder/*.toml` | Yes (各 4 个) |
...
从硬编码 `cat .rudder/spec/frontend/index.md` 改为：
```bash
...
- [ ] CLI `rudder update` 能清理用户项目中残留的旧命令文件
