...
- 模板源 URL 从 `MengDe1231/docs` 更新为 `MengDe1231/Rudder`
- docs-site 清理（删除 `plugins/rudder-meta/`、`.claude-plugin/`，更新所有链接）
...
**前置条件**: Step 1 完成（docs-site 页面中的 `MengDe1231/Rudder/tree/main/marketplace` 链接需要主仓库 main 已有 `marketplace/`）
**产出**: Mintlify 自动部署，文档页面链接全部有效

### Step 3: 验证

- [ ] `https://github.com/MengDe1231/Rudder/tree/main/marketplace` 可访问
- [ ] `https://raw.githubusercontent.com/MengDe1231/Rudder/main/marketplace/index.json` 返回正确 JSON
- [ ] docs-site 各页面链接正常：
  - skills-market/index.mdx — install 命令
  - skills-market/rudder-meta.mdx — skill 详情
