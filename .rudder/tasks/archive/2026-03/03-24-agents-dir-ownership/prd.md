...
Decouple `.agents/skills/` from the codex platform and establish it as a shared "Agent Skills" layer. Currently Rudder binds `.agents/skills/` as codex's `configDir`, but `.agents/skills/` is an **open standard** (agentskills.io) used by 8+ Agent CLIs. This causes incorrect platform detection and blocks future multi-CLI support.
...
- `rudder init --codex` installs both `.codex/` and `.agents/skills/`
...
- Template copy (`templates/rudder/scripts/`) must stay in sync
...
- Remove workspace artifacts: `.rudder/workspace/codex-agent/`, `.rudder/tasks/03-23-add-latest-codex-support/`
- Ensure both Python copies are identical after all changes
- Replace `extraManagedPaths` → `supportsAgentSkills`

## Acceptance Criteria

- [ ] `rudder init --codex` creates `.codex/` (config + agents) AND `.agents/skills/` (shared skills)
- [ ] `rudder update` tracks files under both `.codex/` and `.agents/skills/`
...
- `.rudder/gitignore.txt` `.agents/` entry (that's for worktree multi-agent registry, not skills)
...
| `src/templates/rudder/gitignore.txt:11` | `.agents/` = worktree registry, not skills |
