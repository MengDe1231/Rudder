...
* `config.yaml` 已存在于 `.rudder/`，由 `parse_simple_yaml` 解析（零依赖手写 parser）
...
* `src/templates/rudder/scripts/common/worktree.py` — 增强 `parse_simple_yaml`
* `src/templates/rudder/scripts/common/config.py` — 新增 `get_hooks()`
* `src/templates/rudder/scripts/task.py` — 4 个 cmd_ 函数调用 hooks
* `src/templates/rudder/config.yaml` — 添加 hooks 示例
* `.rudder/` live copies — 同步更新
