...
- Lint / typecheck pass
- 模板副本同步（src/templates/rudder/scripts/ <-> .rudder/scripts/）
- rudder-meta 文档更新
...
1. `src/templates/rudder/scripts/task.py` — +2 子命令（add/remove-subtask）、create 加 --parent、list 层级展示、archive 清理引用
2. `.rudder/scripts/task.py` — live copy 同步
3. `src/templates/rudder/scripts/common/git_context.py` — 层级展示
4. `.rudder/scripts/common/git_context.py` — live copy 同步
5. `src/templates/rudder/scripts/common/task_queue.py` — children 信息
6. `.rudder/scripts/common/task_queue.py` — live copy 同步

**Schema 变更（需同步）：**
7. `src/commands/init.ts` — TaskJson 接口加 children/parent、bootstrap task 兼容
8. `src/commands/update.ts` — migration task 创建加默认值
9. `src/templates/rudder/scripts/create_bootstrap.py` + live copy — bootstrap 兼容

**Prompt 引导（10 个文件）：**
10-13. `brainstorm.md` x4 平台模板
14-17. `start.md` x4 平台模板
18-19. `.claude/commands/rudder/{start,brainstorm}.md` live copy

**文档（1 个文件）：**
20. `.claude/skills/rudder-meta/references/core/tasks.md`
