# Rudder + Docs Monorepo (Git Submodule)

## Goal

将 `MengDe1231/Rudder` 重构为 monorepo 结构，Rudder CLI 代码移入子目录，`MengDe1231/docs` 作为 git submodule 引入，实现代码与文档的同仓库协作。

## Motivation

1. **Co-development** — 同时修改 CLI 代码和文档，一个 PR 搞定
2. **Template 同步** — marketplace 模板 (在 docs 仓库) 与 CLI 代码 (template-fetcher.ts) 联调不需要跨仓库
3. **Dogfooding** — Rudder 自己就是 monorepo 工具，应该 dogfood 自己的 monorepo 支持

## What I Already Know

### 当前状态

**Rudder 仓库** (`MengDe1231/Rudder`):
- TypeScript CLI 工具，pnpm workspace (目前 `packages: []`)
- `doc/` — changelog、示例文档 zip (可删除，内容陈旧)
- `docs/` — guide.md、blog 文章 (6 个 md 文件，需迁移到 docs 仓库)
- `template-fetcher.ts` 硬编码 `gh:MengDe1231/docs` 作为模板源
- npm 发布为 `@mengde1231/rudder`，`files: ["dist", "bin", "README.md", "LICENSE"]`

**Docs 仓库** (`MengDe1231/docs`):
- Mintlify 框架，部署在 `docs.tryrudder.app`
...
**Workaround**: README 中所有文档链接使用绝对 URL 指向 `docs.tryrudder.app`（当前已经是这样做的）。

## Proposed Structure

```
Rudder/                         # 主仓库 (MengDe1231/Rudder)
├── .gitmodules                  # submodule 声明
├── .github/                     # CI/CD (保持在根)
├── .rudder/                    # Rudder 自己的任务 (保持在根)
├── .claude/                     # Claude 配置 (保持在根)
├── README.md                    # 主仓库 README (链接用绝对 URL)
├── pnpm-workspace.yaml          # packages: ["packages/*"]
│
├── packages/
│   └── cli/                     # ← Rudder CLI 代码搬到这里
│       ├── package.json
│       ├── tsconfig.json
│       ├── vitest.config.ts
│       ├── src/
...
│       └── ...
│
└── docs-site/                   # ← git submodule (MengDe1231/docs)
    ├── docs.json
    ├── marketplace/
    ├── guide/
    ├── zh/
...
包名 `@mengde1231/rudder`，当前从仓库根发布。搬到 `packages/cli/` 后：

| 项 | 变化 |
|---|---|
| `bin`, `main`, `types` 字段 | **不用改** — 相对于 package.json 的路径不变 |
| `files` 字段 | 需确认 `README.md` 和 `LICENSE` 的位置（可在 cli/ 放精简版或脚本拷贝） |
| CI publish.yml | 加 `working-directory: packages/cli` 或 `pnpm --filter @mengde1231/rudder publish` |
...
- [ ] 保留在根目录的文件：`.github/`, `.rudder/`, `.claude/`, `.cursor/`, `.opencode/`, `README.md`, `README_CN.md`, `LICENSE`, `CONTRIBUTING.md`, `pnpm-workspace.yaml`, `.gitignore`
...
- [ ] `git submodule add https://github.com/MengDe1231/docs.git docs-site`
...
- [ ] `docs-site/` submodule 指向 `MengDe1231/docs` 且可更新
- [ ] 所有 375 个测试通过
- [ ] README 链接全部有效
- [ ] `doc/` 目录已删除
