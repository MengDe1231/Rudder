...
- Rudder 本体是 pnpm workspaces + git submodules 双重叠加的 monorepo
...
**Decision**: 只返回 Rudder 实际消费的信息 — `{ name, path, type, isSubmodule }`。不保留 manager/orchestrator 类型，因为没有下游消费者（parallel 流程只需要 path + isSubmodule，运行时可从 git 获取 remote URL 等信息）。各种 workspace 格式只是提取 package 列表的不同解析器。
...
**Decision**: 检测到 monorepo 后，逐个 package 询问 spec 来源：空白 spec（Rudder 自带）或下载远程 template。模板下载到 `spec/<package>/` 而非 `spec/`。

### D4: 单仓迁移 monorepo

**Context**: 已有单仓 `.rudder/` 的项目后来改成 monorepo，re-init 时如何处理。

**Decision**: 推到后续。可作为一个独立 skill 从 Rudder 自身的实际迁移经验中总结。
...
新增：
- `get_spec_dir(package=None, repo_root=None) -> Path` — 组合 repo_root + .rudder + spec_base
...
- 对每个检测到的 package 逐个询问 spec 来源：
  - 空白 spec（Rudder 自带，基于 detectProjectType 结果）
...
- [ ] `rudder init` 在 monorepo 项目中询问并创建正确的 spec 目录结构
...
| `packages/cli/src/templates/rudder/config.yaml` | 新增 packages 注释块 |
| `packages/cli/src/templates/rudder/scripts/common/config.py` | 新增 5 个函数 |
| `packages/cli/src/templates/rudder/scripts/common/paths.py` | 新增 2 个函数 |
| `packages/cli/src/templates/rudder/scripts/common/git_context.py` | `--mode packages` |
