...
- `git add -A .rudder/tasks/` — stage 旧路径删除 + 新归档路径
- `git diff --cached --quiet -- .rudder/tasks/` — 检查有无变更
...
`_auto_commit_workspace()` 的 `git add` 和 `git diff` 命令同时覆盖 `.rudder/workspace` 和 `.rudder/tasks`。
...
| File | Change |
|------|--------|
| `src/templates/rudder/scripts/common/git_context.py` | 新增 `get_context_text_record()`，`main()` 加 `--mode` 参数 |
| `src/templates/rudder/scripts/task.py` | `cmd_archive()` 末尾加 auto-commit，argparse 加 `--no-commit` |
| `src/templates/rudder/scripts/add_session.py` | `_auto_commit_workspace()` 扩大 git add scope |
| `src/templates/claude/commands/rudder/record-session.md` | Step 1 改为 `--mode record`，移除可选 archive 段落 |
...
- `src/templates/rudder/scripts/add_session.py` ↔ `.rudder/scripts/add_session.py`
- `src/templates/claude/commands/rudder/record-session.md` ↔ `.claude/commands/rudder/record-session.md`
...
- [ ] `_auto_commit_workspace()` 的 git add 覆盖 `.rudder/workspace` 和 `.rudder/tasks`
...
- 其他平台的 record-session 命令更新（仅改 claude 版本，其他平台通过 rudder update 同步）
