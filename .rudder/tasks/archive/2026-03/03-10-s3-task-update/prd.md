...
确保 `PROTECTED_PATHS` 中的 `.rudder/spec` 在 monorepo 目录结构下仍正确保护，不误删 `spec/<package>/` 子目录。
...
`safe-file-delete` 在 `rudder update` 默认流程中自动执行（无需 `--migrate`）。rename/move/目录删除等有风险操作仍需 `--migrate` 显式触发。
...
```json
{ "type": "safe-file-delete", "from": ".claude/commands/rudder/check-backend.md", "allowed_hashes": ["a1b2c3d4", "e5f6g7h8"] }
```
...
**无版本项目策略**（Codex CR4 #2）：若 `.rudder/.version` 不存在或无法解析，`safe-file-delete` 仍然执行（基于文件存在 + hash 匹配的安全条件已足够）。其他迁移类型（rename/delete/rename-dir）跳过并输出警告 `"unknown installed version, skipping migrations — run rudder init to fix"`。
...
| 平台 | 格式 | 删除文件 |
|------|------|----------|
| Claude | md | `.claude/commands/rudder/before-backend-dev.md`, `before-frontend-dev.md`, `check-backend.md`, `check-frontend.md` |
| Cursor | md (prefix) | `.cursor/commands/rudder-before-backend-dev.md`, `rudder-before-frontend-dev.md`, `rudder-check-backend.md`, `rudder-check-frontend.md` |
| iFlow | md | `.iflow/commands/rudder/before-backend-dev.md`, `before-frontend-dev.md`, `check-backend.md`, `check-frontend.md` |
| OpenCode | md | `.opencode/commands/rudder/before-backend-dev.md`, `before-frontend-dev.md`, `check-backend.md`, `check-frontend.md` |
| Kilo | md (workflows) | `.kilocode/workflows/before-backend-dev.md`, `before-frontend-dev.md`, `check-backend.md`, `check-frontend.md` |
| Gemini | toml | `.gemini/commands/rudder/before-backend-dev.toml`, `before-frontend-dev.toml`, `check-backend.toml`, `check-frontend.toml` |
...
| 文件 | 改动 |
|------|------|
| `packages/cli/src/templates/rudder/scripts/task.py` | `--package` 参数 + `get_implement_backend/frontend()` 改为接收 package 参数，输出 `spec/<pkg>/<layer>/index.md` |
| `packages/cli/src/templates/rudder/scripts/add_session.py` | `--package` 可选参数 |
| `packages/cli/src/templates/rudder/scripts/common/config.py` | 新增 `validate_package()` + `get_spec_scope()` + `resolve_package()` |
| `packages/cli/src/templates/rudder/scripts/create_bootstrap.py` | monorepo 下 `related_files` 路径使用 `spec/<pkg>/backend/` 而非 `spec/backend/` |
...
| `packages/cli/src/templates/opencode/lib/rudder-context.js` | 可能需扩展以支持 config.yaml 读取 |
| `packages/cli/src/templates/rudder/scripts/get_context.py` | `--mode packages` 输出标注 scope 内/外 |
...
- [ ] `rudder update` 不损坏 monorepo spec 目录
- [ ] 无 `--package` 时行为完全兼容现有单仓逻辑
- [ ] `rudder update` 自动清理用户项目中残留的 S2 旧命令文件（无需 `--migrate`）
...
- [ ] 无 `.rudder/.version` 时 `safe-file-delete` 仍执行，其他迁移跳过并警告
...
| WARNING | 无 `.rudder/.version` 时 migration 行为未定义 | 新增无版本项目策略（3.4b） |
