# rudder update: Respect User Deletions + config.yaml Skip Dirs

## Goal

解决用户删掉不需要的模板文件后，`rudder update` 又把它们加回来的问题。

## Background

### 用户反馈（群聊）

用户删了不需要的 frontend/backend 相关 commands/skills 后，`rudder update` 会把它们当作 "New files" 重新添加。这让用户困惑且反复删除。
...
```
  Deleted by you (preserved):
    - .claude/commands/rudder/check-frontend.md
    - .claude/commands/rudder/before-frontend-dev.md
```
...
在 `.rudder/config.yaml` 中新增 `update.skip` 配置：

```yaml
update:
  skip:
    - .claude/commands/rudder/check-frontend.md
    - .rudder/spec/frontend/
    - .rudder/spec/backend/
```
...
- Hash 文件：`.rudder/.template-hashes.json`
- `ChangeAnalysis` 类型需新增 `userDeletedFiles: FileChange[]` 字段
- R1 优先级高于 R2，可以分两步实现
- config.yaml 读取逻辑已有：`src/templates/rudder/scripts/common/config.py`（Python 侧）
