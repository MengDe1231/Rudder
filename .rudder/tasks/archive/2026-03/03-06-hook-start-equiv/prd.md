...
让 SessionStart hook 注入上下文后，AI 的行为与用户手动执行 `/rudder:start` 等效——主动汇报上下文、询问用户要做什么、按任务分类走完整流程。
...
2. Create task: `TASK_DIR=$(python3 ./.rudder/scripts/task.py create "<title>" --slug <name>)`
...
| 文件 | 改动 | 对应需求 |
|------|------|---------|
| `src/templates/claude/commands/rudder/start.md` | 强化 Simple Task 指令 | R1 |
| `src/templates/claude/hooks/session-start.py` | 加 task 状态检查 + 改 ready 标签 | R2, R3 |
| `src/templates/iflow/commands/rudder/start.md` | 同步修改 | R1 |
