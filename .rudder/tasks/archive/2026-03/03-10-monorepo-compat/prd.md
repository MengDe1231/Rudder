...
让 `rudder init` / `rudder update` 以及生成的全部模板（命令、Hook、Python 脚本）同时支持 **单仓模式**（现状）和 **Monorepo 模式**，用户无需手动改文件即可在 monorepo 中使用 Rudder。

## Background

Rudder 主仓库自身已是 monorepo（`packages/cli` + `docs-site` submodule），`.rudder/config.yaml` 有 `packages:` 字段，spec 目录按 `spec/cli/backend/`、`spec/docs-site/docs/` 组织。但 **CLI 模板源码**（即用户 `rudder init` 后得到的文件）仍全部假设单仓结构，无任何 monorepo 逻辑。
...
**现状**: `get_repo_root()` 向上找 `.rudder/` 目录。所有路径函数返回单根路径。

**改动**:
- 新增 `get_spec_dir(package: str | None = None) -> Path`:
  - 单仓: 返回 `repo_root / .rudder / spec`
  - Monorepo: 返回 `repo_root / .rudder / spec / <package>`
- 新增 `get_package_path(package: str) -> Path`:
  - 读 config.yaml packages[name].path，返回绝对路径
- `get_repo_root()` 不变（.rudder/ 始终在 repo root）
...
**现状**: 远程模板下载到 `PATHS.SPEC`（即 `.rudder/spec/`），假设模板即为完整 spec 内容。
...
**现状**: 硬编码 `cat .rudder/spec/frontend/index.md` 等。

**改动**:
```markdown
# 动态发现 spec 索引
python3 ./.rudder/scripts/get_context.py --spec-indexes
# 输出所有可用的 spec/*/index.md 或 spec/<package>/*/index.md 路径
```

或直接在 start.md 中用 `ls .rudder/spec/*/index.md` 动态列举。
...
**现状**: `PROTECTED_PATHS` 包含 `.rudder/spec` 作为整体保护。

**改动**:
- Monorepo: 同样保护 `.rudder/spec`，无需改（spec/ 下的子目录结构变了但保护粒度不变）
...
P1-P3 完成后，monorepo 用户即可正常使用 Rudder；P4-P6 是增强。
...
### Python 模板 (`packages/cli/src/templates/rudder/scripts/`)
...
| 平台 | 文件 | 改动 |
|------|------|------|
| Claude | `commands/rudder/start.md` | 动态 spec 发现替代硬编码路径 |
| Claude | `commands/rudder/before-dev.md` | **新增** generic 命令 |
| Claude | `commands/rudder/check.md` | **新增** generic 命令 |
| Claude | `commands/rudder/before-backend-dev.md` | 标记 deprecated |
| Claude | `commands/rudder/before-frontend-dev.md` | 标记 deprecated |
| Claude | `commands/rudder/check-backend.md` | 标记 deprecated |
| Claude | `commands/rudder/check-frontend.md` | 标记 deprecated |
| Claude | `commands/rudder/parallel.md` | 动态 spec 路径 |
| Claude | `commands/rudder/finish-work.md` | 动态 spec 路径 |
| Claude | `commands/rudder/break-loop.md` | 动态 spec 路径 |
| Claude | `commands/rudder/onboard.md` | 动态 spec 路径 |
...
| Cursor | `commands/rudder-*.md` | 动态 spec 路径 |
...
| 文件 | 改动 |
|------|------|
| `templates/rudder/config.yaml` | 新增注释掉的 `packages:` 示例 |
| `templates/rudder/workflow.md` | 动态 spec 路径说明 |
...
- [ ] `rudder init` 在 pnpm workspace / npm workspace / nx 项目中自动检测并询问 monorepo 模式
...
- [ ] `rudder update` 从旧版升级时不损坏 monorepo spec 目录
...
- 跨 package 依赖管理（Rudder 不管构建系统）
- Per-package 独立 `.rudder/`（始终一个 `.rudder/` 在 repo root）
- GUI / Electron app 的 monorepo 支持
- `rudder init` 在子 package 目录运行（始终要求在 repo root 运行）
