...
更新两个 Rudder slash command（`break-loop.md` 和 `update-spec.md`），修正过时内容并增强分析能力。
...
- [ ] 引导使用 `python3 ./.rudder/scripts/get_context.py --mode packages` 发现可用 spec 位置
...
- 文件位置：`.claude/commands/rudder/break-loop.md` 和 `.claude/commands/rudder/update-spec.md`
- 这些是 Claude Code slash command，不是代码文件，不需要 lint/typecheck
- break-loop 中第 121-122 行的 template sync 提醒是 Rudder 内部细节，保留但明确标注仅限 Rudder 项目
