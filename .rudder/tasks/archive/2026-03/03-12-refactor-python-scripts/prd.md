# 重构 .rudder/scripts Python 代码

## Background

基于代码分析和 python-design skill 中的原则，当前 `.rudder/scripts/` 存在显著的设计问题：
...
- **同步模板**：`.rudder/scripts/` 改动必须同步到 `packages/cli/src/templates/rudder/scripts/`
- **入口路径稳定**：`python3 .rudder/scripts/task.py` 等路径被 .md 模板大量引用，不能变
...
- [ ] `grep -r "_read_json_file" .rudder/scripts/` 只有 `common/io.py` 一处定义
- [ ] `grep -r "class Colors" .rudder/scripts/` 只有 `common/log.py` 一处定义
- [ ] `grep -r "sys.path.insert" .rudder/scripts/` 返回 0 结果
...
- [ ] `packages/cli/src/templates/rudder/scripts/` 同步更新
- [ ] pyright/mypy 无新增类型错误
- [ ] 所有 .md 模板中的 `python3 .rudder/scripts/task.py` 等路径无需修改
