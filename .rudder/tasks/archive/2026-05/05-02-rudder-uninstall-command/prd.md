# rudder uninstall command

## Goal

提供 `rudder uninstall` 命令，把 Rudder 在项目里写入的所有内容（各平台 configDir 下的 rudder 相关文件 + `.rudder/` 目录本身）一次性清理干净，方便用户卸载或换工具对比。

Issue: #221

## Requirements

* 新增 `rudder uninstall` CLI 命令
* 删除范围：
  * `.rudder/` 目录整体（含 tasks、runtime、developer、config 等所有内容）
  * 所有 platform 写入的 rudder 管理文件，依据 `.rudder/.template-hashes.json` manifest 精确删除
  * 删除后的空目录递归清理（复用 `cleanupEmptyDirs`）
* **删除依据**：`.rudder/.template-hashes.json` manifest 是唯一的删除依据。manifest 里没有的文件（用户自己创建的）永远不动 —— 例如用户自己加在 `.cursor/hooks/my-hook.py` 的脚本。
* **用户魔改的处理**：manifest 里的文件，即使用户改过（hash 不一致），仍然删除。理由：用户既然要卸载 rudder，同名脚本/agent/skill 不再有意义。
* **按文件内容类型分两类处理**：
  * **类型 A：纯内容文件 → 整删（不论 hash）**
    * 所有 `.py` / `.md` / `.ts` / `.sh` 等 opaque 文件
    * 包括所有 hooks 脚本、agent markdown、skill markdown、command markdown、`.pi/extensions/rudder/index.ts` 等
  * **类型 B：结构化配置文件 → scrubber 剥离 rudder 字段**
    ...
    * **B2 - 字段类专用 scrubber —— 3 个文件**
      ...
      * `.pi/settings.json`：从 `extensions` / `skills` / `prompts` 数组移除 rudder 条目；移除 `enableSkillCommands` 字段
      * `.codex/config.toml`：移除 `project_doc_fallback_filenames` 字段 + rudder 引导注释段
  * 任何 scrubber 处理后文件变成空 / 仅剩空字段 → 删除文件；否则写回
* **默认 UX**：`rudder uninstall` 单命令完成
  1. 扫描 manifest，把待处理文件分两栏列出：
     * **Will be deleted**：所有纯内容文件 + scrubber 处理后会变空的结构化文件 + `.rudder/` 目录
     * **Will be modified**：结构化文件中保留用户字段、只剥离 rudder 部分的（列出每个文件 + 简短说明剥离了什么）
...
* 不保留任何 rudder 数据（含 tasks / .developer）
* **预检**：命令第一步检测当前目录下是否存在 `.rudder/` 目录
  * 不存在 → 输出 `Rudder is not installed in this project (no .rudder/ directory found).` 退出 0
  * 存在但 `.rudder/.template-hashes.json` 缺失 → 输出 `Rudder directory found but manifest is missing — cannot determine which platform files to remove. You can manually delete .rudder/ if needed.` 退出 1
  * 都存在 → 进入正常流程

## Acceptance Criteria

* [ ] `rudder uninstall` 默认输出两栏清单（Will be deleted / Will be modified）+ `Continue? [Y/n]`
* [ ] 用户回车 / 输入 `y` / `yes` → 执行删除和修改
* [ ] 用户输入 `n` / `no` → 退出 0，不修改任何文件
* [ ] `rudder uninstall --dry-run` 列出所有清单后退出，不进入提示
* [ ] `rudder uninstall --yes` / `-y` 直接执行，不交互
* [ ] 执行后：manifest 中所有路径不存在；`.rudder/` 目录不存在；产生的空 platform 目录（如空 `.claude/`）被清理
* [ ] platform 目录下的非 rudder 文件（用户自己的 hook/agent）不被误删
* [ ] `.claude/settings.json` 中用户自定义字段（如 `model`、`permissions`、非 rudder 的 hook entry）保留；rudder 的 hook entry 被剥离
* [ ] settings.json 剥离后若仅剩空 hooks 对象 → 文件被删除；否则保留
* [ ] 在没有 `.rudder/` 目录的项目运行 → 友好提示 + 退出 0
* [ ] 在有 `.rudder/` 但缺 manifest 的项目运行 → 提示无法确定删除范围 + 退出 1
* [ ] 输出末尾打印 summary（删除文件数 + 删除目录数）

## Definition of Done

* 单测覆盖 dry-run / 实际执行 / 无 manifest / `--yes` / 空目录清理 / 不误删
* 集成测试：init → uninstall → 项目状态 clean（无 `.rudder/`、无 rudder 管理文件）
...
* `scrubPiSettings(content) -> { content, fullyEmpty }`
  * 从 `extensions` / `skills` / `prompts` 数组移除 rudder 条目；空数组删字段
  * 移除 `enableSkillCommands` 字段
* `scrubCodexConfigToml(content) -> { content, fullyEmpty }`
  * 实现选择：
    * 优先：line-based 移除 `project_doc_fallback_filenames = ...` 行 + 已知的 rudder 注释段（当前模板内容稳定，可控）
...
**Context**: Issue #221 用户希望干净卸载 Rudder。
**Decision**: 用 manifest 驱动的精确删除（不是按目录整删），但不做"用户改过则保留"的判断 —— 用户明确要求全删。
**Consequences**:
- 优点：精确、不误删 platform 目录里用户自己的文件；manifest 已经存在零开发成本。
- 取舍：用户改过的 rudder 文件也会被删（按需求即如此）；`.rudder/tasks/` 里用户的工作产物会丢失（issue 场景就是要换工具，丢失可接受）。
...
* 不区分 hooks/agents/skills/commands 类文件用户改过 vs 未改过 —— 这些是 rudder 资产，统一删
* settings.json 类文件：保留用户自定义字段（见上），但**不做 hooks 字段语义合并/复杂 diff** —— 只按"command 路径匹配 manifest"这一规则剥离
* 不保留任何 rudder 数据（tasks / config / developer）
* 不卸载 rudder CLI 本身（`npm uninstall -g` 是用户操作系统层面的事）
* 不删除 git 历史中的 rudder commit

## Technical Notes

* Manifest 工具：`packages/cli/src/utils/template-hash.ts`
  * `HASHES_FILE = ".template-hashes.json"`
  * 路径：`path.join(cwd, DIR_NAMES.WORKFLOW, HASHES_FILE)` → `.rudder/.template-hashes.json`
