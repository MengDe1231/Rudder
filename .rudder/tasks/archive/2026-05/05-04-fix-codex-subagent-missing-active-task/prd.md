...
修复 issue #225：在 codex（以及 copilot / gemini / qoder 等 class-2 平台）上，主会话 dispatch `rudder-implement` / `rudder-check` / `rudder-research` 子代理后，子代理跑 `task.py current` 显示 "no active task"，导致 prelude 流程退化为"问用户"，无法自动拉到 task 上下文。
...
- 0.5.0-rc 系列的 session-scoped active-task 改造（task `04-21-session-scoped-task-state`）让 active-task 按 session_id 存进 `.rudder/.runtime/sessions/<context-key>.json`
...
claude code 有 `PreToolUse(Task)` / `PreToolUse(Agent)` hook（`packages/cli/src/templates/claude/settings.json:42`），主会话 spawn 子代理那一刻 rudder 在父进程跑 `inject-subagent-context.py` 把 task 上下文写进子代理 prompt。这条路在 codex 上不存在。
...
主会话 dispatch `rudder-implement` / `rudder-check` / `rudder-research` 时，**dispatch prompt 必须把 active task 路径作为头部一行明文写出**，格式：

```
Active task: <task path, e.g. .rudder/tasks/05-04-fix-codex-subagent-missing-active-task>

<rest of dispatch prompt...>
```

主会话靠 `python3 ./.rudder/scripts/task.py current` 拿路径（已可用）。
...
`packages/cli/src/templates/rudder/workflow.md` 的 `[workflow-state:in_progress]` 区块加一条 sub-agent dispatch protocol：

> When you spawn `rudder-implement` / `rudder-check` / `rudder-research`, your dispatch prompt **MUST** start with one line:
>
...
`.rudder/scripts/common/active_task.py:resolve_active_task()` 加 fallback：

- 当 `resolve_context_key()` 返回 None 或对应信箱不存在时
- 扫 `.rudder/.runtime/sessions/*.json`：
...
```
Current task: .rudder/tasks/05-04-foo
...
- [ ] 单测覆盖：
  ...
  - workflow.md 包含 dispatch protocol（`templates/rudder.test.ts` 或新增）
  - `resolve_active_task` 兜底分支（3 case：恰好 1 个 / ≥2 个 / 0 个）
  - `set_active_task` **不**走兜底（regression）
- [ ] 实际在 codex 跑一遍 rudder-implement / rudder-check / rudder-research，验证子代理能拿到 task
...
- `rudder update --migrate`（如需 migration）能正确升级旧项目的子代理 prelude 文案 —— 因为 prelude 是 `rudder init` / `rudder update` 写出去的静态文件，所以新版需要让用户更新
- 在 codex 上做一次 e2e 验证（dispatch rudder-implement，子代理能找到 task 并 read prd.md）
...
| 2 | `packages/cli/src/templates/rudder/workflow.md` `[workflow-state:in_progress]` | 加 sub-agent dispatch protocol 硬约束 | 模板 |
| 3 | `.rudder/scripts/common/active_task.py:resolve_active_task` | 加单 session 兜底分支 | runtime |
| 4 | `packages/cli/src/templates/rudder/scripts/common/active_task.py` | 跟 #3 同步（脚本模板镜像） | 模板 |
| 5 | `.rudder/scripts/common/active_task.py:ActiveTask` + `task.py cmd_current` | source 增 `session-fallback` 类型 | runtime |
| 6 | `packages/cli/test/configurators/shared.test.ts` | 测 prelude 新文案 | 测试 |
| 7 | 新增 / 已有 test 加 `resolve_active_task` 兜底分支测试 | runtime 测试 | 测试 |

### Rudder migration

prelude 是 `rudder init` 写到磁盘的静态文件。本次改动后，旧项目需要 `rudder update --migrate` 才能拿到新 prelude 文案。所以本 release 应该是带 migration 的（hash-verified rewrite，不强制覆盖用户自定义）。

### 不做的事

- **不**让 codex 上游加 hook（这是上游的事，rudder 控制不了）
...
- 不需要 codex 上游配合，所有改动在 rudder 内部

## Out of Scope

- codex 上游 feature request（建议另开 issue 跟踪，但不阻塞本 task 落地）
- 把 session_id 在子代理 env 里继承（依赖 codex 行为，rudder 改不了）
...
- 单测覆盖：prelude 文案生成（`shared.test.ts`）、workflow.md 内容（`templates/rudder.test.ts` 或新增）、resolver fallback 三分支（`active_task` 单测，需新建或扩展）
- 手动 e2e 文档：放在 task dir 里 `manual-verify.md`，列出在 codex 里跑一遍的具体步骤（创建 task → start → dispatch rudder-implement → 验证子代理读到 prd.md）。完成后随 task archive 一起留档作为参考。
...
- `_ENV_SESSION_KEYS` (`.rudder/scripts/common/active_task.py:48-60`) 列了 codex 用 `CODEX_SESSION_ID` / `CODEX_THREAD_ID`
...
- 04-21 的 PRD（`.rudder/tasks/04-21-session-scoped-task-state/prd.md`）解释了为什么不能把 fallback 做得太激进
