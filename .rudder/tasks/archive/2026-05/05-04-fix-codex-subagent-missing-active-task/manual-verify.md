...
- A project where Rudder is installed with `--codex` (or `--copilot` / `--gemini` / `--qoder`)
...
2. Create + start a task:
   ```bash
   python3 ./.rudder/scripts/task.py create "smoke test"
   # ... write minimal prd.md, curate jsonl ...
   python3 ./.rudder/scripts/task.py start .rudder/tasks/<MM-DD>-smoke-test
   ```
3. Verify main session sees it:
   ...
   ```
4. In the same codex session, ask the AI to dispatch `rudder-implement` **without** any explicit task path in the prompt. Watch the sub-agent's first action:
...
2. Ask the AI to do something requiring `rudder-implement` dispatch
3. Inspect the dispatch prompt the main agent sends to the sub-agent (from chat transcript). Verify:
   - **Pass**: dispatch prompt's first line is `Active task: .rudder/tasks/<MM-DD>-smoke-test`
...
## Scenario D — Older project upgrade (post `rudder update --migrate`)

1. Take a project on a previous Rudder rc/beta
2. `rudder update --migrate`
3. Inspect `~/<project>/.codex/agents/rudder-implement.toml` (or `.gemini/agents/rudder-implement.md`, `.qoder/...`, `.github/agents/...`):
...
4. Inspect `~/<project>/.rudder/workflow.md`:
