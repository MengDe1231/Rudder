...
修 Rudder 0.5.0 GA / 0.5.1 在 Python ≤3.11 + 任意带 SessionStart hook 的平台上必现的 `SessionStart hook (failed) — exited with code 1` 崩溃。准备 0.5.2 hotfix 发版。
...
- [ ] `rudder update` 后用户在 Python 3.11 + Codex 复现路径不再报 `SessionStart hook (failed)`
...
- 修法不依赖任何第三方库，一行级替换。
- Python 3.12+ 用户不受影响。
- Codex CLI 用户在 issue 群报告（Codex 0.128 + Rudder 0.5.0），Codex 自己也定位到了 line 50。
