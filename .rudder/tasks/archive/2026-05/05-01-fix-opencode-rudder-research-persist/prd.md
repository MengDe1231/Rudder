# fix: opencode rudder-research subagent must persist findings

## Goal

Make opencode 平台的 `rudder-research` subagent 与 claude/cursor/droid/gemini/codebuddy/qoder/pi/codex/kiro 行为一致：能写文件、并把每个调研话题落到 `{TASK_DIR}/research/<topic>.md`，避免 GitHub issue #211 报告的"调研结果只存活在主 agent 上下文里 / `rudder-implement` 拿不到调研结果"。

## What I already know

- opencode template `packages/cli/src/templates/opencode/agents/rudder-research.md`:
...
- 对照 `claude/cursor/droid/gemini/codebuddy/agents/rudder-research.md`：5 个平台正文几乎逐字相同，含 Core Principle (PERSIST)、Workflow (Step 1–5 含 `mkdir -p {TASK_DIR}/research/`、Persist Each Topic、Report file paths)、File Format、Scope Limits (Write ALLOWED / FORBIDDEN)、DO/DON'T。opencode 是唯一离群点。
...
1. `packages/cli/src/templates/opencode/agents/rudder-research.md` frontmatter:
   - 保留 `mode: subagent`
...
- [ ] `.opencode/agents/rudder-research.md`（新 init 一个项目后）frontmatter 有 `write: allow`、`edit: allow`
- [ ] 同文件正文出现 "PERSIST"、`{TASK_DIR}/research/`、`mkdir -p` 三个字串
- [ ] `pnpm --filter @mengde1231/rudder test` 全过，含新增的 opencode 用例
- [ ] `pnpm --filter @mengde1231/rudder lint` 干净
- [ ] `pnpm --filter @mengde1231/rudder typecheck`（如有）干净
- [ ] 手动 cross-check：用 `rudder init --platform opencode` 产出的 `.opencode/agents/rudder-research.md` 与 claude 等价（diff 仅 frontmatter 形态差异）
...
- **复制源**：`packages/cli/src/templates/cursor/agents/rudder-research.md` 的正文（与 claude 一致，少一些 claude 特定描述），但 frontmatter 用 opencode 形态。
- **frontmatter 模板**（最终）：
  ```yaml
  ---
  description: |
...
- **回归测试增量**：在现有 describe 里加一个 `it("opencode rudder-research has write permission and persist instruction", ...)`，单独处理 YAML 块解析。
...
- 参考文件：
  - `packages/cli/src/templates/claude/agents/rudder-research.md`（蓝本）
  - `packages/cli/src/templates/cursor/agents/rudder-research.md`（与 claude 几乎相同正文）
