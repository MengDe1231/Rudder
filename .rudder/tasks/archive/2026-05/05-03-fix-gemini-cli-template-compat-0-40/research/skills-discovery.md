...
- **Scope**: external (Gemini CLI source + docs) + light internal cross-reference to Rudder configurators
- **Date**: 2026-05-03
...
**Workaround that works at the cost of being noisy** — name your skills uniquely. The warning fires only when two locations share a skill `name` (parsed from `SKILL.md` frontmatter), not when they share a directory name. So a Rudder skill named `rudder-check` in `.gemini/skills/rudder-check/SKILL.md` and a Codex skill named `rudder-check` in `.agents/skills/rudder-check/SKILL.md` will collide; renaming the `.gemini/` copy to e.g. `gemini-rudder-check` would silence the warning but creates duplicate content.
...
There is one known parser bug filed at issue #25693 (2026-04-20, against 0.38.2; fix landed in PR #25728 before 0.40.0): single-line long descriptions with internal punctuation could fail to parse. Rudder's wrapper produces a single line, but the bug only triggered on specific punctuation patterns and the fix is in 0.40.0+. Worth a quick smoke test against current Rudder output but not a blocker.

Rudder's `wrapWithSkillFrontmatter` (`packages/cli/src/configurators/shared.ts:150`) emits exactly:
...
## Internal cross-reference (Rudder side, for context)
...
So the duplicate warning in issue #224 will fire on every project that runs `rudder init --gemini --codex` (or installs both at any time): Gemini sees `.agents/skills/rudder-*/SKILL.md` written by Codex AND `.gemini/skills/rudder-*/SKILL.md` written by itself, with identical `name:` frontmatter, and warns once per skill on each Gemini boot.
...
- The trust gate (`isTrusted`) is checked via Gemini's folder-trust system; Rudder projects opened in a trusted folder will always trigger discovery.

---

## Recommendation for Rudder
...
2. **Option B (init-order conditional skip) is brittle** as the PRD already notes — and would still leave duplicate warnings whenever Codex was added after Gemini, until the user re-ran `rudder update`.
...
4. **Aligns with the upstream design intent.** Gemini's docs literally tell users to prefer `.agents/skills/` for "cross-tool alignment" (issue #19167). Rudder writing to a Gemini-private path is now the off-pattern choice.

**Concrete change shape** (for the implementer, not me):

- In `gemini.ts`, swap `path.join(configRoot, "skills")` → `path.join(cwd, ".agents", "skills")` for the `writeSkills(...)` call that emits the 5 shared rudder-* skills + bundled.
...
Pick (b) for this task — it matches existing Rudder behavior, defers the placeholder problem, and resolves issue #224 in a single configurator change.
