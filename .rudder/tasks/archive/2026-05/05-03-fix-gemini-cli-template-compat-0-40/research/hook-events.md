...
So Rudder's existing semantic ("inject workflow-state breadcrumb at the start of each turn") maps to **`BeforeAgent`** in Gemini.
...
- I did not locate the exact PR that originally added `UserPromptSubmit` to Rudder's Gemini template, so cannot say with certainty whether older Gemini CLI versions silently accepted it (vs. simply not running the hook). What's certain is that 0.35+ schema validation rejects it.
- PR #16870 (the `hooksConfig` rename) is on `main` but **not** yet in any released 0.40.x build. Rudder should NOT pre-emptively migrate to `hooksConfig` until a release ships it. The 0.40.x format remains: meta fields (`enabled`, `disabled`, `notifications`) and event keys (`BeforeAgent`, `SessionStart`, …) coexist as siblings under top-level `hooks`.
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "<existing rudder SessionStart command>",
            "timeout": 10000
          }
        ]
      }
...
1. The hook command's stdout JSON must use `"hookEventName": "BeforeAgent"` (not `"UserPromptSubmit"`) inside `hookSpecificOutput`, otherwise `getAdditionalContext()` won't find the field. Example:
   ...
   ```
2. The per-turn breadcrumb is **appended** to the user's prompt (not prepended). If Rudder previously assumed prepend-semantics, double-check the breadcrumb still parses correctly when it appears after the user text. If prepend ordering matters, switch to `SessionStart` for first-turn injection or have the breadcrumb script wrap with explicit markers.
