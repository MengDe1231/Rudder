...
Make `rudder init --gemini` produce templates that load cleanly on Gemini CLI 0.40.x. Today the generated `.gemini/agents/*.md`, `.gemini/settings.json`, and `.gemini/skills/` cause schema errors and duplicate-skill warnings (issue #224).

## Reproduction (from #224)

`@mengde1231/rudder@0.5.0-rc.2` + `gemini-cli 0.40.1`:

1. Agent loading error — `tools: Expected array, received string` for every `.gemini/agents/rudder-*.md`.
...
Reporter's local workaround (verified working): drop `tools:` line, rename event to `BeforeAgent`, delete `.gemini/skills/`, prune corresponding entries from `.rudder/.template-hashes.json`.

## What I already know (from repo scan)

* Agent templates: `packages/cli/src/templates/gemini/agents/{rudder-check,rudder-implement,rudder-research}.md` — all carry the comma-separated `tools:` line.
...
* `rudder init --gemini` must not emit duplicate-skill warnings against `.agents/skills/` workspace alias.
* `.template-hashes.json` tracks exactly the file set actually written.

## Acceptance Criteria (evolving)

* [ ] In a fresh project, `rudder init --gemini --yes && gemini --help` exits clean (no agent schema errors, no duplicate-skill warnings).
* [ ] `rudder update` is a no-op on a freshly-initialized Gemini project (hashes match generated set).
...
* Research will be delegated to `rudder-research` sub-agents.

## Implementation Plan

**Commit 1 — Issue #224 fixes**

1. `packages/cli/src/templates/gemini/agents/{rudder-check,rudder-implement,rudder-research}.md` — delete the `tools:` frontmatter line.
...
6. `packages/cli/src/configurators/shared.ts` — add `resolvePlaceholdersNeutral()` and `resolveSkillsNeutral()` / `resolveAllAsSkillsNeutral()`. Neutral form rendered to TBD-but-readable string (proposal: `` `start` (Rudder command — see workflow.md) ``).
