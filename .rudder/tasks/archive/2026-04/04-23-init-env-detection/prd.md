...
梳理并改进 `rudder init` 的环境检测能力，明确哪些运行前提应该被检测、哪些应该只提示不阻塞，以及检测结果如何反馈给用户，避免当前只做零散 Python / Windows 分支判断，遗漏更关键的版本或平台前置条件。

## What I already know

- 当前 `rudder init` 已有 **Python 版本检测**，通过 `warnIfPythonVersionTooOld()` 对将要实际使用的 `python` / `python3` 做 warning，但不阻塞初始化。
- 当前 `rudder init` 已有极轻量的 **OS 分支**：
  - `getPythonCommandForPlatform()` 按 `process.platform` 决定模板里渲染 `python` 还是 `python3`
  - Windows 且选中了带 Python hooks 的平台时，会打印一条提示
- 当前 `rudder init` 会写 `.rudder/.version`，但这只是记录当前 CLI 版本，不是“环境版本比对”。
- 当前没有看到：
  - Node 版本检测
  - 更完整的 OS / arch 检测
  - `init` 阶段的 Rudder 版本迁移判断
...
- spec 也明确 Rudder workflow scripts 的目标前提是 **Python 3.9+**，并强调 Windows 与 macOS/Linux 的 `python` / `python3` 差异需要统一文案与实际行为。
...
- A3: 范围只覆盖 `rudder init`，不顺手统一 `update` / `task.py init-context` / hooks 的环境检测。
...
- [ ] 明确 `rudder init` 当前已有的检测项、缺失项、误导项
...
- [ ] 能清楚回答 `rudder init` 在哪些条件下会检测、提示、阻塞
...
- `packages/cli/src/commands/init.ts:1560` — 写 `.rudder/.version`
- `packages/cli/src/configurators/shared.ts:14` — `getPythonCommandForPlatform()`
- `packages/cli/package.json` — `engines.node = >=18.17.0`
- `.rudder/spec/cli/backend/script-conventions.md` — Python 3.9+ 与跨平台 `python`/`python3` 约束
