...
**缺口 1：已有项目里 `rudder init` 不给新开发者任何引导**
...
**问题**：新开发者加入一个已有 Rudder 项目时 `.rudder/` 已存在 → `isFirstInit = false` → **根本不创建 bootstrap task**。他只拿到一个 `init_developer.py` 写进去的 identity，然后就面对一堆陌生的目录、task、spec，没有任何引导说"从哪开始"。
...
- ❌ 已存在 `.rudder/spec/` 时怎么用（是通过 spec 来理解项目，还是补齐缺失部分）
- ❌ 项目创建者 vs 加入者两种角色的分流
...
1. **修复 init 分支**：已有 Rudder 项目 + 新开发者加入 → 生成 **joiner-onboarding** 引导任务（不是 bootstrap-guidelines，是不同内容）
2. **扩展 Bootstrap PRD**：承接被删除的 `/onboard` 命令的职责，加"了解 Rudder 工作流"步骤（注意：onboard 是带用户熟悉 Rudder 流程，不是带用户熟悉自己的业务代码）
3. **区分两种 bootstrap**：
...
`init.ts:1347` 改为三分支判定，信号源统一到 `.rudder/.developer` 文件（gitignored，天然 per-checkout）：
...
- `.rudder/.developer` 是 gitignored → 不会随 clone 带过来，每个 checkout 独立
- `.rudder/workspace/<name>/` 会进 git → 新人 clone 下来就存在，不能作为"是否新人"的信号
- 三态干净：无 `.rudder/` = creator，有 `.rudder/` 无 `.developer` = joiner，都有 = 同人 re-init

**已接受的行为**（非 bug）：
- 同一人换机器也会收到一份 joiner task — 视作"再熟悉一次当前项目约定"的机会，嫌多可直接 archive
- 共享 checkout（多人共用同一工作目录）只有第一位会收到 joiner — 这是 Rudder "一人一 checkout" 的前提

### Creator Bootstrap PRD 扩展

在现有 PRD 的开头插入 "Step 0: Learn the Rudder Workflow"（带用户熟悉 Rudder 本身，不是熟悉他自己的业务代码）：

```markdown
### Step 0: Learn the Rudder Workflow (for you + AI)

Before filling spec files, get familiar with how Rudder works:

- Read `.rudder/workflow.md` end-to-end — this is the contract between you and AI.
- Understand the task lifecycle: planning → in_progress → done → archive.
- Know the three directories: `.rudder/spec/` (conventions), `.rudder/tasks/` (active + archive), `.rudder/workspace/<you>/` (your journal).
- Know the core slash commands: `/rudder:start`, `/rudder:continue`, `/rudder:finish-work`, `/rudder:brainstorm`.

Ask AI:
- "Read .rudder/workflow.md and summarize the 3 phases and what I do in each."
- "When should I use /rudder:continue vs /rudder:finish-work?"
...
```markdown
# Joining: Onboard Yourself to This Rudder Project

Welcome! You're joining an existing Rudder project.
Here's a guided path to get productive fast.

## Step 1: Learn the Rudder Workflow

Read `.rudder/workflow.md` end-to-end — this is how you and AI collaborate here.

Ask AI:
- "Summarize .rudder/workflow.md — the 3 phases and what I do in each."
- "What does the task lifecycle look like (planning → in_progress → done → archive)?"
- "When should I use /rudder:continue vs /rudder:finish-work vs /rudder:start?"

## Step 2: Learn This Project's Conventions (via spec)

- `.rudder/spec/` — coding conventions this team has agreed on (start here)
- `.rudder/tasks/` — active work + recent archive (how people actually work)

Ask AI:
- "Summarize .rudder/spec/ — what conventions do I need to follow?"
- "Look at the last 5 archived tasks — what's the typical rhythm of work?" (skip this if `.rudder/tasks/archive/` is empty — the project may just be starting)
...
- Check `.rudder/workspace/<you>/` if it exists
...
python3 ./.rudder/scripts/task.py archive 00-join-<you>
```
```
...
2. **`handleReinit` 的 `doAddDeveloper` 分支**（init.ts:651 附近）—— 默认 `rudder init --user bob` 在 `.rudder/` 已存在时走 re-init fast path 并 early-return，**根本到不了主 dispatch**。这条路径必须在 `init_developer.py` 执行前后独立捕获 `.developer` 存在状态，然后在 "原先没有" 时调用 `createJoinerOnboardingTask`
...
- [ ] 空目录 `rudder init` → creator bootstrap
- [ ] 有 `.rudder/` 但无 `.rudder/.developer`（fresh clone）→ joiner onboarding
- [ ] 有 `.rudder/` + `.rudder/.developer` 已存在（同人 re-init）→ 不生成任何 task
...
- **不针对 CI 环境特化**（`--no-onboard` flag、`CI=true` 探测均不做）：`rudder init` 是一次性本地 setup，CI 里跑 init 非预期场景
...
🔴 **P1** —— 新开发者加入体验是 Rudder 推广的关键漏斗。目前新人只能拿到一个 identity 然后面对黑盒，是体验事故。

## 风险

- **同一人换机器也会触发 joiner**：`.rudder/.developer` gitignore，所以换机器 = 无 `.developer` = 判定为 joiner。**已接受**：视作再熟悉一次当前项目的机会，不需要则直接 archive，成本低于误判成"不是新人"后零引导的体验事故
