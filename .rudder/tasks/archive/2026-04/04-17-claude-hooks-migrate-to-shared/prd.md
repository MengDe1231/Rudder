...
| 硬编码引用 `.claude/commands/rudder/check-cross-layer.md` | 文件不存在，fallback 静默失败 |
...
- 硬编码引用 `.claude/commands/rudder/check-cross-layer.md`（文件已删）
...
- [ ] 干净目录 `rudder init --claude` → `.claude/hooks/` 下有 3 个 .py 文件
...
如果用户的 `.claude/hooks/` 已经被 Rudder 写过（旧版），`rudder update` 会用新版覆盖。hash 如果匹配（用户没改过），正常更新；如果用户自己改过 hook 代码，会跳过更新并提示。
