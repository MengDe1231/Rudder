...
触发场景：`/rudder:finish-work` skill 文档示例就是 `python3 ./.rudder/scripts/task.py archive <task-name>`，但很多调用现场（含 LLM 生成的命令）会习惯性传完整路径，导致首次失败需要重试。
...
* 测试：在 `packages/cli/test/regression.test.ts` 已有的 archive 测试附近加 1 个新 it block，覆盖相对路径 + 绝对路径两种输入形式（slug 形式已被现有测试覆盖）。沿用现有 `setupTaskRepo` + `execSync(...task.py archive ...)` + `--no-commit` 的模式。`.rudder/scripts/` 暂无 Python 测试框架，沿用项目现有 TS shell-out 模式更一致。

## Acceptance Criteria

* [ ] `python3 .rudder/scripts/task.py archive 04-XX-foo` 正常工作（slug，行为不回归）
* [ ] `python3 .rudder/scripts/task.py archive .rudder/tasks/04-XX-foo` 正常工作（相对路径，bug 修复）
* [ ] `python3 .rudder/scripts/task.py archive /abs/path/.rudder/tasks/04-XX-foo` 正常工作（绝对路径，bug 修复）
...
* 关键文件：
  - `.rudder/scripts/common/task_store.py:290-311`（cmd_archive）
  - `.rudder/scripts/task.py:425-428`（argparse 注册）
  - `.rudder/scripts/task.py:13-20, 296-323`（usage 文档）
