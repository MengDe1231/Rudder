# `rudder update --migrate` 相关三个已定位 bug

> 用户反馈来自 2026-04-22 群聊（0.5.0-beta.9 发版当天）。用户跑 migrate 流程时踩到 3 个坑，已经和 codex 一起做完 root cause 分析，本任务负责收敛 + 修复。

## 背景

用户在某个项目上跑 `rudder update --migrate`（目标可能是 beta.8 或 beta.9），出现：

1. 生成的 `04-22-migrate-to-0.5.0-beta.8` task 模板"看起来有问题，里面很多内容还是之前的"
2. `init-context` 自动生成的 `check.jsonl` 指向 `.claude/commands/rudder/check.md`，但该项目是 Codex 技能布局（无 `.claude/` 或 Codex 专属路径）
3. 就算平台正确识别为 codex，生成的路径仍是 `.agents/skills/check/SKILL.md`，但仓库实际目录是 `.agents/skills/rudder-check/SKILL.md`（0.5.0-beta.0 加的 `rudder-` 前缀没被本函数跟进）

`task.py validate` 脚本在"任务路径 vs 任务名"参数处理上也被用户说不一致，作为附带项一起核查。

---

## Bug A — `get_rudder_command_path` codex/kiro 分支漏 `rudder-` 前缀

### 定位

`packages/cli/src/templates/rudder/scripts/common/cli_adapter.py:214-251`

```python
def get_rudder_command_path(self, name: str) -> str:
    ...
    elif self.platform == "codex":
        return f".agents/skills/{name}/SKILL.md"       # ❌ 应为 rudder-{name}
    elif self.platform == "kiro":
        return f".kiro/skills/{name}/SKILL.md"         # ❌ 应为 rudder-{name}
    ...
```

### 为什么错

0.5.0-beta.0 做了一次 skill 目录改名（manifest 里有 60+ `rename` 条目把 `skills/<name>/` 搬到 `skills/rudder-<name>/`），但这个方法没被跟进。结果 codex / kiro 项目跑 `init-context` 时 check.jsonl 永远写坏路径。
...
packages/cli/src/migrations/manifests/0.5.0-beta.0.json
  ...
  "to":   ".kiro/skills/rudder-check/SKILL.md"        ← 当时加的 rudder- 前缀
  "from": ".agents/skills/check/SKILL.md"
  "to":   ".agents/skills/rudder-check/SKILL.md"      ← 共享层也加了
```
...
```python
    ...
    return f".agents/skills/rudder-{name}/SKILL.md"
elif self.platform == "kiro":
    return f".kiro/skills/rudder-{name}/SKILL.md"
```

### 测试

- 在一个只有 `.agents/skills/rudder-check/SKILL.md`、`.agents/skills/rudder-finish-work/SKILL.md` 的 fixture 下跑 `task.py init-context <dir> backend`，断言生成的 `check.jsonl` 两行 `file` 字段都带 `rudder-` 前缀。
- kiro 同款 fixture。
...
`packages/cli/src/templates/rudder/scripts/common/cli_adapter.py:detect_platform`（约 line 680-740）
...
需要复现后决定：
1. 放宽判据：只要有 `.agents/skills/rudder-*` 就算 codex？（风险：共享层规范多平台通用，误判）
...
- fixture：只跑过 `rudder init --codex` 的项目，`init-context` 应生成 codex 路径的 check.jsonl
- fixture：既 `--claude` 又 `--codex` 的项目 → 如何派发？（设计决策）

---

## Bug C — `rudder update --migrate` 生成的 task PRD 内容 stale
...
**C. 两者结合**：先给 0.5.0-beta.0 补 guide + aiInstructions（"一次性补档"），以后新 manifest 默认带这两个字段（在 `/rudder:create-manifest` 流程里增加 Step 检查 breaking=true 时必须提供 migrationGuide）

推荐 **C**。

### 测试

- fixture：`rudder update --migrate` 从 0.5.0-beta.0 到 0.5.0-beta.9 → 生成的 task PRD 应包含 0.5 系列 breaking 改动的 migration guide
...
4. **是否属于 beta.9 hotfix**：这三个 bug 都是 `rudder update` 的核心体验问题。如果用户碰上这些直接放弃 migrate 了，就需要紧急 hotfix 成 beta.10。或者延到下一个正常 beta。

---

## 优先级

🔴 **P0** —— `rudder update --migrate` 是每个老用户都要走的路径，三个 bug 叠加导致 migrate 产物不可用。

## 关联

- 用户反馈出处：2026-04-22 群聊截图（taosu image-cache 目录）
- 代码文件：
  - `packages/cli/src/templates/rudder/scripts/common/cli_adapter.py`（Bug A + B）
  ...
  - `packages/cli/src/templates/rudder/scripts/common/task_context.py`（附带项：validate）
- 相关 PR / 历史：
  - 0.5.0-beta.0 skill 改名的 60+ rename 条目（Bug A 的上下文）
  - `/rudder:create-manifest` 命令文档（Bug C 路线 C 的更新对象）

## 下一步

`/rudder:start 04-22-migrate-flow-bugs` → brainstorm：
