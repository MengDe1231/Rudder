...
`.rudder/spec/` 实际目录：
```
.rudder/spec/backend/dev/index.md    # "backend" 是 package 名
.rudder/spec/frontend/dev/index.md   # "frontend" 是 package 名
...
Path: iqs-front-human
Spec layers: dev
  - .rudder/spec/frontend/dev/index.md

### backend [git repo]
Path: iqs
Spec layers: dev
  - .rudder/spec/backend/dev/index.md
```

## 第 1 层：`Error: monorepo project requires --package`

触发路径在 `packages/cli/src/templates/rudder/scripts/common/task_context.py:128-148`:
...
def get_implement_backend(package):
    ...
    # → .rudder/spec/<package>/backend/index.md

def get_implement_frontend(package):
    spec_base = f"{DIR_SPEC}/{package}" if package else DIR_SPEC
    return [{"file": f"{DIR_WORKFLOW}/{spec_base}/frontend/index.md", ...}]
    # → .rudder/spec/<package>/frontend/index.md
```

**Rudder 代码假设的结构**：
```
.rudder/spec/<package>/backend/index.md     ← package 内部有 backend/frontend 子目录
.rudder/spec/<package>/frontend/index.md
```
...
**用户项目实际结构**：
```
.rudder/spec/backend/dev/index.md           ← package 名就是 backend / frontend
.rudder/spec/frontend/dev/index.md
```
...
- `.rudder/spec/frontend/backend/index.md` ❌ 不存在
- `.rudder/spec/frontend/frontend/index.md` ❌ 不存在
...
Rudder 自己 `init` 时生成的默认 spec 骨架是按"业务域 package"设计的，所以自用/template case 一直正常。用户**手工配了 config.yaml + 按端拆包**（Maven 多模块 + 独立前端仓，monorepo 典型姿势），立刻踩到了隐含假设。
