...
1. **`task.py create` 行为改动**
   - 检测 platform 是否支持 sub-agent（查 `.rudder/config.yaml` 的 `features` 或已装 `.{platform}/` 目录）
...
3. **Phase 1.3 重写成 "agent 自填 jsonl" 指令**（`workflow.md`）
   ...
   - **必填什么**：根据 prd 选择相关的 **spec 文件**（`.rudder/spec/**/*.md`）和 **research 文件**（`{TASK_DIR}/research/*.md`）
   ...
   - **发现 spec 的方式**：`python3 .rudder/scripts/get_context.py --mode packages` 列出本项目所有 spec layer 和路径
   
   Kilo / Antigravity / Windsurf 这一组仍然 "skip this step"（它们不消费 jsonl）。

4. **`rudder-before-dev/SKILL.md` 仍可读取 spec，不再承担 jsonl curate**
...
7. **Skill Routing + DO NOT skip skills 表的平台分支**（`workflow.md:130-149`）
   ...
   现状是全平台共享一份表，但 `rudder-before-dev` 其实只对 "主线程直接写代码" 的平台有意义 —— sub-agent 平台应该是"dispatch sub-agent，sub-agent 内部自己读 spec"。
   
   拆成两份：
   
   **[Claude Code, Cursor, OpenCode, Codex, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid]**：
   - "要写代码" → `Phase 2.1 — dispatch rudder-implement`（去掉 rudder-before-dev 行）
   - "要验证" → `Phase 2.2 — dispatch rudder-check`
   - 其他行不变
   
   **[Kilo, Antigravity, Windsurf]**：
   - "要写代码" → `rudder-before-dev`
   - "要验证" → `rudder-check`
   - 其他行不变
   
   同样给 "DO NOT skip skills" 表做分支：sub-agent 平台那行关于 "before-dev takes under a minute" 改成 sub-agent 语境下的对应劝诫（如 "dispatching rudder-implement is the cheap path; skipping it tempts you to write code in the main thread and lose spec context"）。
...
- `rudder-before-dev` 语态从"祈使句"改成"元描述" —— 单独 task 做
...
```jsonl
{"_example": "Fill with {\"file\": \".rudder/spec/<pkg>/<layer>/index.md\", \"reason\": \"why this spec applies\"}. Run `python3 .rudder/scripts/get_context.py --mode packages` to list available specs. Delete this line when done."}
```
...
3. 按端拆包的 monorepo（spec 布局 `.rudder/spec/{backend,frontend}/dev/`）不再卡在 "requires --package"（因为根本不跑 init-context 了）
...
7. Skill Routing 表按平台分支后，sub-agent 平台的主线程不再被 `rudder-before-dev` 诱导代入执行者角色
...
- `packages/cli/src/templates/rudder/scripts/task.py` — 删 `init-context` subparser
- `packages/cli/src/templates/rudder/scripts/common/task_context.py` — 删 `cmd_init_context` + 相关 helper
- `packages/cli/src/templates/rudder/scripts/common/task_store.py` — `cmd_create` 加 platform 探测 + seed jsonl 生成
...
- `packages/cli/src/templates/common/skills/rudder-before-dev/SKILL.md` — curate 职责
- `packages/cli/src/templates/rudder/workflow.md` — "Skip when jsonl exists" 描述
