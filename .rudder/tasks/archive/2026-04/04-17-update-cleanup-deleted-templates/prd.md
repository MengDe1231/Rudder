# rudder update 清理已删除模板

## 背景

`rudder update` 命令的当前逻辑：**比对模板源和项目文件，把新增/修改的同步到项目**。但不处理"**模板源已删除但项目里还在**"的情况。

### 实际影响

Rudder 老版本升级到 v0.5.0-beta 的用户，项目里会残留一堆废弃文件：
...
| `.claude/commands/rudder/check-cross-layer.md` | 合并进 check |
| `.claude/commands/rudder/record-session.md` | 合并进 finish-work |
| `.claude/commands/rudder/onboard.md` | 已删 |
| `.claude/commands/rudder/create-command.md` | 已删 |
| `.claude/commands/rudder/integrate-skill.md` | 已删 |
| `.claude/commands/rudder/parallel.md` | 已删 |
| `.codex/skills/parallel/SKILL.md` | 已删 |
| `.agents/skills/parallel/SKILL.md` | 已删 |
| `.rudder/worktree.yaml` | multi-agent 删除 |
| `.rudder/scripts/multi_agent/*` | 8 个文件，multi-agent pipeline 删除 |
| `.rudder/scripts/common/{phase,registry,worktree}.py` | multi-agent 依赖，已删 |
| `.iflow/` 整个目录 | iFlow 平台已删 |

### 残留的后果

1. **AI 被误导** — 读到 `.claude/commands/rudder/parallel.md` 里的旧指令，尝试调用不存在的命令
...
| 文件类型 | 识别方式 | 处理 |
|---------|---------|------|
| 1. Rudder 管理的、未改动 | 历史 hash 匹配 | 安全删 ✅ |
| 2. Rudder 管理的、用户改过 | 存在于历史 manifest 但 hash 不匹配 | 不删，警告 ⚠️ |
| 3. 用户自己加的文件 | 从未出现在任何 Rudder manifest | 保留 ✅ |

当前 Rudder 已有 **template-hash** 机制：`.rudder/.template-hashes.json` 追踪哪些文件由 Rudder 写入。
...
- `"delete"` 类型 = 无条件删除（仅用于 Rudder 自己维护的内部文件）
...
（或者接受"曾经是 Rudder 模板"这个信任 = 用 `"delete"` 而不是 `"safe-file-delete"`，hash 保护不严但文件归属明确——见下方讨论）
...
- `.opencode/agents/{dispatch,debug,plan,rudder-plan}.md`
...
- `.claude/commands/rudder/check-cross-layer.md`
- `.claude/commands/rudder/record-session.md`（实际文件名可能是 `record-agent-flow.md`，扫 hash 比对）
- `.claude/commands/rudder/onboard-developer.md`（注意实际名）
- `.claude/commands/rudder/create-command.md`
- `.claude/commands/rudder/integrate-skill.md`
- `.claude/commands/rudder/parallel.md`

**Skills**：
- `.codex/skills/parallel/SKILL.md`
- `.agents/skills/parallel/SKILL.md`（若存在）

**Rudder 内部**（走 `"delete"` 不需 hash）：
- `.rudder/worktree.yaml`
- `.rudder/scripts/multi_agent/`（目录删除用 `rename-dir` 到 trash 或枚举每个 .py 逐个删）
- `.rudder/scripts/common/{phase,registry,worktree}.sh`（shell 版已用 Python 替代）
- `.rudder/scripts/common/phase.py`（workflow-enforcement-v2 会重写这个，不在本 task 清理范围 —— 留给 v0.5.0 后续）
...
- [ ] 集成测试：用一个 0.4.x 的 fixture 项目跑 `rudder update`，验证清理效果
  - 干净升级：所有 stale 文件被清理
  - 用户修改过的文件：保留 + 警告
  - 用户自加的文件：保留
...
## 父 Task
`.rudder/tasks/04-16-skill-first-refactor`
