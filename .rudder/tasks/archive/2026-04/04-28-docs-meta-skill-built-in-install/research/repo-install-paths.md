# Repo Research: Built-in `rudder-meta` Install Path

## 结论

`rudder-meta` 现在是 marketplace skill，不在 `rudder init` 的内置模板链路里。要让它像 `rudder-brainstorm` 一样随平台安装，必须接入 `packages/cli/src/templates/common/` 到各平台 configurator 的模板生成路径，并同步 update hash tracking、init 测试和文档站。
...
* `packages/cli/src/configurators/shared.ts` 提供：
  * `resolveSkills(ctx)`：把 `common/skills` 渲染成 `rudder-{name}/SKILL.md`，用于大部分平台。
  * `resolveAllAsSkills(ctx)`：把 commands + skills 全部渲染成 `rudder-{name}/SKILL.md`，用于 Codex shared layer 和 Kiro。
...
* marketplace 源头在 `marketplace/skills/rudder-meta/SKILL.md`，并有 `references/` 目录。
* docs-site 的技能市场页面仍指导用户运行 `npx skills add MengDe1231/marketplace --skill rudder-meta`。
* `rudder-meta` 的现有 frontmatter name 已经是 `rudder-meta`，与内置命名目标一致。
* 现有内容包含过期平台描述和 Claude-centric 安装说明；如果变成 built-in，需要同步改成 Rudder 0.5 平台矩阵和 `rudder init` 安装语义。

## 关键约束

1. 不能简单把 `marketplace/skills/rudder-meta/SKILL.md` 原样放进 `common/skills/meta.md`：
...
4. 该变更会新增用户项目落盘文件，`rudder update` 必须能追踪 template hash；这通常不需要 migration，因为是新增文件而不是重命名/删除旧文件。

## 可行方案

### A. 增强现有 built-in skill 链路，支持多文件 skill（推荐）

在现有 `common/skills` 内置分发链路上升级模板模型：继续由共享 helper 统一产出各平台 skill，只是让模板源除了单文件 `.md` body 之外，也能包含完整 skill 目录。`rudder-meta` 作为多文件内置 skill 放进 CLI template tree，例如：

```text
packages/cli/src/templates/common/bundled-skills/rudder-meta/
  SKILL.md
  references/
```
...
让 CLI 直接读取 `marketplace/skills/rudder-meta` 作为模板源。
...
2. 把 `rudder-meta` 多文件 skill 复制进该 template tree，并更新 frontmatter/内容为 built-in 语义。
3. 在所有平台 configurator 的 init 路径写入 `rudder-meta`。
4. 在所有平台 `collectTemplates()` 路径收集 `rudder-meta` 文件，保证 `rudder update` 可更新和追踪 hash。
5. 更新 init/configurator 测试，覆盖至少：
   * 默认 Claude + Cursor 会写对应平台 `rudder-meta`。
   * Codex 写 `.agents/skills/rudder-meta`，并且 `.codex/skills` 不重复写。
...
`rudder-meta` 变成 built-in 后，是否仍保留 marketplace 安装入口作为兼容说明，还是从技能市场里移除并只作为 `rudder init` 内置能力展示？
