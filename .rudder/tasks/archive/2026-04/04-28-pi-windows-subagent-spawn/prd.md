...
Fix GitHub issue #201 so Rudder-generated Pi Agent sub-agents can launch reliably on Windows, while preserving the existing Pi extension behavior on macOS and Linux.

## What I already know

* Issue: https://github.com/MengDe1231/Rudder/issues/201
* The issue is open and has no linked pull request or timeline reference.
* Reporter environment:
  * Rudder `0.5.0-beta.15`
  * Node.js `v22.19.0`
  * Windows
...
* The dogfood extension at `.pi/extensions/rudder/index.ts` and the CLI template at `packages/cli/src/templates/pi/extensions/rudder/index.ts.txt` currently use the old launcher shape.
...
* `RUDDER_PI_CLI_JS` is acceptable as an explicit override for unusual Windows or package-manager installs.
...
* Support `RUDDER_PI_CLI_JS` as an explicit path override and fail with a clear error if it points to a missing file.
...
* Keep Rudder context propagation through `RUDDER_CONTEXT_ID`.
...
* Missing `RUDDER_PI_CLI_JS` produces a clear error message naming the missing path.
...
* Tests cover the new launcher behavior and continue covering:
  * Rudder sub-agent tool registration
  * Pi hook-equivalent events
  * runtime context resolution
  * Rudder context propagation
...
* Implementation is reviewed against `.rudder/spec/cli/backend/` and `.rudder/spec/cli/unit-test/`.
* Tests are added or updated for the launcher change.
* The dogfood `.pi/extensions/rudder/index.ts` stays in sync with the CLI template.
...
* Changing Pi agent definitions or Rudder prompt/skill installation.
...
* Relevant specs:
  ...
  * `.rudder/spec/cli/unit-test/index.md`
  * `.rudder/spec/cli/unit-test/conventions.md`
* Research artifact:
  * `.rudder/tasks/04-28-pi-windows-subagent-spawn/research/issue-201-local-analysis.md`
