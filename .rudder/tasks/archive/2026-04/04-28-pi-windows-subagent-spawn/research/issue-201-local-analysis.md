...
GitHub issue #201 reports that Rudder-generated Pi sub-agents cannot be launched on Windows because the project-local Pi extension calls `spawn("pi", ...)`, but Windows installs can expose Pi through an npm shim rather than a discoverable `pi.exe`.
...
* Add `RUDDER_PI_CLI_JS` for explicit override.
