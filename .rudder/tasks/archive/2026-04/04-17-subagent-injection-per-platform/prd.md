...
原 PRD 基于 "hook-based 注入" 思路写成。实际落地走了不同路线：**4 个 class-2 平台改为 pull-based prelude**（由 `d2c6682` commit 实现），在 sub-agent definition 开头注入「Load Rudder Context First」指令，不再依赖平台 hook。
