...
让 AI 在长对话里不会忘记 Rudder task 存在，靠 **UserPromptSubmit hook 每轮注入一行短面包屑**，不靠 AI 自觉。
...
- `packages/cli/src/templates/rudder/workflow.md` + `.rudder/workflow.md`（加 `[workflow-state:STATUS]...[/workflow-state:STATUS]` 区块作为 breadcrumb 单一事实源）
- `.rudder/scripts/common/task_store.py`（读取端宽容 legacy 字段，忽略 `current_phase` / `next_action`）
- `.rudder/scripts/common/task_store.py`（`cmd_create` 不再写 legacy 字段）
...
文件：
- `packages/cli/src/templates/rudder/workflow.md`（模板源）
- `.rudder/workflow.md`（项目自用，与模板源同步）
...
<!-- Injected per-turn by UserPromptSubmit hook (inject-workflow-state.py).
     ...
     block to customize flow reminders. Users who fork the Rudder workflow
     only need to edit this file, not the hook script. -->

[workflow-state:planning]
Complete prd.md via rudder-brainstorm skill; then run task.py start.
[/workflow-state:planning]
...
核心逻辑：
1. `find_rudder_root()` 从 CWD 向上查找 `.rudder/` 目录（顺带解决 CWD 漂移 / submodule 场景——`hook-path-robustness` 的最小修复）
2. `get_active_task(root)`：读 `.rudder/.current-task` → 读指向的 `task.json` 的 `id` + `status`
...
6. 无 active task / 无 `.rudder/` → 静默退出 0
...
- mock stdin `{"cwd": "/tmp/foo"}` + `/tmp/foo/.rudder/.current-task` 存在 → 输出正确 JSON
- 无 `.rudder/` 时静默退出 0
...
- `rudder init --codex` 执行时 stderr 输出 warning：`Codex hooks require features.codex_hooks = true in ~/.codex/config.toml. See rudder docs.`
- spec 文档章节里列为前置条件
...
- Codex：`configureCodex` 改造完成，`rudder init --codex` 写入 `inject-workflow-state.py` + config 里含 UserPromptSubmit 且有 feature flag 警告
- Kiro：不接线，spec 里明确标注
...
两处都改，内容一致：
- `.rudder/scripts/common/task_store.py`（项目自用）
- `packages/cli/src/templates/rudder/scripts/common/task_store.py`（模板源）
...
`.rudder/scripts/common/phase.py`（multi_agent 遗留，255 行）读 `current_phase` 和 `next_action`，但：
...
- `inject-workflow-state.py`：
  ...
  - CWD 漂移（子目录 / submodule）→ 向上查找 `.rudder/`
...
- 改 `.rudder/workflow.md` 里 `[workflow-state:in_progress]` block → 下轮面包屑立即变（验证热更新）
...
更新 `.rudder/spec/cli/backend/platform-integration.md`：
...
- [ ] 双 `task_store.py`（`.rudder/scripts/` + `packages/cli/.../rudder/scripts/`）同步：不写 legacy + 宽容读取
- [ ] `inject-subagent-context.py`（Python 共享）+ OpenCode JS 插件删除 `update_current_phase()` 写入
- [ ] `pnpm test` + `python -m pytest .rudder/scripts/`（若有）全绿
...
- 顺带解决：`04-17-hook-path-robustness` 的最小修复（新 hook 用 `find_rudder_root()` 向上查找，CWD 漂移不再影响）
