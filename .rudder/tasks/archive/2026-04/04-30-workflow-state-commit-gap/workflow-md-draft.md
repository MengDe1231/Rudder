...
```bash
python3 ./.rudder/scripts/init_developer.py <your-name>
```

创建 `.rudder/.developer`（gitignored）和 `.rudder/workspace/<your-name>/`。

### 规范系统 (Spec System)

`.rudder/spec/` 按 package 和 layer 组织编码规范。

- `.rudder/spec/<package>/<layer>/index.md` —— 入口，包含 **Pre-Development Checklist** 和 **Quality Check**。具体规范在它指向的 `.md` 文件里
- `.rudder/spec/guides/index.md` —— 跨 package 的思考指引

```bash
python3 ./.rudder/scripts/get_context.py --mode packages   # 列出 packages 和 layers
```
...
每个任务独立目录 `.rudder/tasks/{MM-DD-name}/`，下含 `prd.md`、`implement.jsonl`、`check.jsonl`、`task.json`，可选 `research/`、`info.md`。

```bash
# 任务生命周期
python3 ./.rudder/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python3 ./.rudder/scripts/task.py start <name>          # 设为当前任务（支持时按 session 隔离）
python3 ./.rudder/scripts/task.py current --source      # 查看当前任务及来源
python3 ./.rudder/scripts/task.py finish                # 清掉当前任务（触发 after_finish hooks）
python3 ./.rudder/scripts/task.py archive <name>        # 归档到 archive/{year-month}/
python3 ./.rudder/scripts/task.py list [--mine] [--status <s>]
...
python3 ./.rudder/scripts/task.py add-context <name> <action> <file> <reason>
python3 ./.rudder/scripts/task.py list-context <name> [action]
python3 ./.rudder/scripts/task.py validate <name>

# 任务元信息
python3 ./.rudder/scripts/task.py set-branch <name> <branch>
python3 ./.rudder/scripts/task.py set-base-branch <name> <branch>    # PR 目标分支
python3 ./.rudder/scripts/task.py set-scope <name> <scope>

# 父子关系
python3 ./.rudder/scripts/task.py add-subtask <parent> <child>
python3 ./.rudder/scripts/task.py remove-subtask <parent> <child>

# PR 创建
python3 ./.rudder/scripts/task.py create-pr [name] [--dry-run]
```

> 跑 `python3 ./.rudder/scripts/task.py --help` 看权威的最新命令清单。

**当前任务机制**：`task.py start` 把任务路径写进 `.rudder/.runtime/sessions/` 下 session/window-scoped 的 runtime state，由 active-task resolver 统一管理。如果 hook input、`RUDDER_CONTEXT_ID`、平台原生 session 环境变量三者都拿不到 context key，就没有当前任务，`task.py start` 会失败并提示 session identity。`task.py finish` 删掉当前 session 文件。`task.py archive <task>` 也会清掉所有指向被归档任务的 runtime session 文件。

### 工作区系统 (Workspace System)

记录每次 AI session，便于跨 session 跟踪，存放于 `.rudder/workspace/<developer>/`。

- `journal-N.md` —— session 日志。**每个文件最多 2000 行**；超出会自动新建 `journal-(N+1).md`
- `index.md` —— 个人索引（总 session 数、最近活跃时间）

```bash
python3 ./.rudder/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### 上下文脚本 (Context Script)

```bash
python3 ./.rudder/scripts/get_context.py                            # 完整 session runtime
python3 ./.rudder/scripts/get_context.py --mode packages            # 可用 packages + spec layers
python3 ./.rudder/scripts/get_context.py --mode phase --step <X.Y>  # 工作流某步骤的详细指引
```
...
<!--
  ...
  改任何 [workflow-state:STATUS] block 时:
    ...
    - 改完跑 rudder update 让用户项目拿到新内容
    - 完整运行时合约见 .rudder/spec/cli/backend/workflow-state-contract.md
-->
...
**B 建任务** —— 任何实施 / 改动 / 构建 / 重构 工作。入口顺序：(1) `python3 ./.rudder/scripts/task.py create "<title>"` 建任务（status=planning，切到 [workflow-state:planning] 拿 brainstorm + jsonl 阶段指引）→ (2) 激活 `rudder-brainstorm` skill 跟用户讨论需求并迭代完善 prd.md → (3) prd 完成 + jsonl 整理好后跑 `task.py start <task-dir>` 进 [workflow-state:in_progress] 拿实施骨架。调研密集型派 `rudder-research` sub-agent，主 agent 禁 3+ inline WebFetch / WebSearch / `gh api`。**看上去"小"不是降级到 A 或 C 的理由**。
**C inline 改**（仅本轮，B 的 escape hatch）—— 当前消息**必须显式包含** "跳过 rudder" / "别走流程" / "小修一下" / "直接改" / "先别建任务" / "skip rudder" / "no task" / "just do it" / "don't create a task" 之一 → 简短确认（如 "好，本轮跳过 rudder 流程"）后 inline。**没看到这些短语不得自行 inline**；禁编造用户没说过的 override。
[/workflow-state:no_task]
...
激活 `rudder-brainstorm` skill，跟用户讨论需求并迭代完善 prd.md。
...
调研产出**必须**落到 `{task_dir}/research/*.md`，由 `rudder-research` sub-agent 写。主 agent 尽量不 inline WebFetch / WebSearch —— PRD 只链接 research 文件。
[/workflow-state:planning]
...
**默认（无 override 时）**：派 `rudder-implement` / `rudder-check` sub-agent，主 agent**默认不直接写代码**。Phase 3.4 commit（required, once）：rudder-update-spec 后或实施可验证完成时，主 agent**主导 commit** —— 先陈述 commit 计划再跑 `git commit`，然后再引导用户执行 `/rudder:finish-work`。
...
代码已通过 Phase 3.4 提交；跑 `/rudder:finish-work` 收尾（归档任务 + 记录 session）。
...
**为什么 `rudder-before-dev` 不在表里**：你不是写代码的人 —— `rudder-implement` sub-agent 才是。Sub-agent 平台通过 `implement.jsonl` 注入 / prelude 拿到 spec context，主 agent不需要加载 `rudder-before-dev`。
...
|---|---|
| Wants a new feature / requirement unclear | `rudder-brainstorm` |
| About to write code / start implementing | `rudder-before-dev`（之后在主 agent直接实施） |
...
| 你脑子里在想 | 为什么不对 |
|---|---|
| "这事简单，主 agent直接写完了" | 派 `rudder-implement` 是低成本路径；偷懒不派会导致你在主 agent写代码、丢失 spec context —— sub-agent 有 `implement.jsonl` 注入，你没有 |
| "我在 plan mode 里已经想清楚了" | Plan-mode 的输出在内存里 —— sub-agent 看不到；必须落到 prd.md |
| "我已经知道 spec 了" | Spec 可能在你上次读之后被更新了；sub-agent 拿到的是最新版，你不一定 |
| "先写代码，后面再 check" | `rudder-check` 能发现你自己看不出来的问题；越早越省 |
...
| 你脑子里在想 | 为什么不对 |
|---|---|
| "这事简单，直接写" | 简单任务经常会膨胀；`rudder-before-dev` 一分钟内就能加载好你需要的 spec context |
...
```bash
python3 ./.rudder/scripts/get_context.py --mode phase --step <step>
# 例：python3 ./.rudder/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
python3 ./.rudder/scripts/task.py create "<task title>" --slug <name>
```
...
跳过条件：`python3 ./.rudder/scripts/task.py current --source` 已经指向一个任务。

#### 1.1 Requirement exploration `[required · repeatable]`

加载 `rudder-brainstorm` skill，按 skill 的指引跟用户交互式探索需求。
...
**写什么**：
- **Spec 文件** —— `.rudder/spec/<package>/<layer>/index.md` 以及任务相关的具体规范文件（`error-handling.md`、`conventions.md` 等）
...
```bash
python3 ./.rudder/scripts/task.py add-context "$TASK_DIR" implement "<path>" "<reason>"
python3 ./.rudder/scripts/task.py add-context "$TASK_DIR" check "<path>" "<reason>"
```
...
跳过此步骤。Context 在 Phase 2 由 `rudder-before-dev` skill 直接加载。
...
如果 `task.py start` 报 session identity 错误（hook input、`RUDDER_CONTEXT_ID`、平台原生 session 环境变量都拿不到 context key），按报错提示设置 session 标识后重试。
...
1. 加载 `rudder-before-dev` skill 读项目规范
...
加载 `rudder-check` skill，按它的指引复核代码：
- Spec 合规
...
如果本任务涉及反复调试（同一问题被修过多次），加载 `rudder-break-loop` skill：
- 分类根因
- 解释之前的修复为什么没成功
- 提出预防方案
...
加载 `rudder-update-spec` skill，复盘本任务有没有产生值得记录的新知识：
- 新发现的模式或约定
- 踩到的坑
- 新的技术决策

相应更新 `.rudder/spec/` 下的文档。即便结论是"没有需要更新的"，也要走一遍判断过程。
...
## Customizing Rudder (for forks)

想改 Rudder 工作流的开发者看这里。所有自定义都通过编辑本文件完成；脚本只负责解析。
...
直接编辑对应 `[workflow-state:STATUS]` block 内容。改完跑 `rudder update`（如果是模板维护者）或重启 AI session（如果是用户项目自定义）即可生效，无需碰任何脚本。
...
- `.rudder/spec/cli/backend/workflow-state-contract.md` —— 运行时合约 + writer 表 + 测试 invariant
- `.rudder/scripts/inject-workflow-state.py` —— 实际解析逻辑（只读 workflow.md，不嵌入文本）
