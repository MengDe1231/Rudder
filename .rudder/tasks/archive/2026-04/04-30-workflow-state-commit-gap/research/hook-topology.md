# Research: Hook Topology Across Rudder Platforms

- **Query**: enumerate every hook script across every supported platform and its event wiring
- **Scope**: internal
- **Date**: 2026-04-30
- **Repo**: `/Users/taosu/workspace/company/mindfold/product/share-public/Rudder`
...
| `packages/cli/src/templates/pi/extensions/rudder/index.ts.txt` | TypeScript | pi-only | Pi "extension" — not file-per-event, single TS module that calls `pi.on(...)` |
...
| `inject-workflow-state.py` | UserPromptSubmit (per-turn) | JSON: `{cwd, prompt, ...}` | `{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"<workflow-state>Task: ID (status)\n...</workflow-state>"}}` | Adds a short breadcrumb for every user turn. Body comes from `[workflow-state:STATUS]` tag in `.rudder/workflow.md`, falling back to hardcoded `_FALLBACK_BREADCRUMBS` (`no_task`, `planning`, `in_progress`, `completed`). See `inject-workflow-state.py:144-201`. |
| `inject-subagent-context.py` | PreToolUse (Task / Agent / Subagent) | JSON: `{tool_name, tool_input:{subagent_type, prompt, ...}}` | Modified `tool_input.prompt` rewritten with prepended Rudder context | Pulls `implement.jsonl` / `check.jsonl` + prd.md + info.md and prepends to the sub-agent prompt. Only reacts to `rudder-implement`, `rudder-check`, `rudder-research` (see `AGENTS_ALL` at line 63). |
| `inject-shell-session-context.py` | beforeShellExecution (Cursor only) | JSON: hook payload incl. session id | writes a short-lived runtime ticket file under `.rudder/.runtime/cursor-shell/` | Bridges Cursor's conversation identity into `task.py` shell calls (Cursor shell env doesn't inherit SessionStart data). See file header comment at line 1-8. |
...
| **Copilot** | `copilot.ts:23` | `.github/copilot/hooks/` (+ duplicated `.github/hooks/rudder.json`) | `.github/copilot/hooks.json` | copilot-specific session-start.py (NOT shared); see warning in file header — Copilot ignores SessionStart output today | inject-workflow-state.py (event name `userPromptSubmitted`, with `bash`/`powershell` keys) | not wired — issues #2392/#2540 (`copilot.ts:45-47`) | Sub-agents get markdown prelude |
...
|---|---|---|---|
| **Pi** | `pi.ts:55`, template at `templates/pi/extensions/rudder/index.ts.txt` | `.pi/extensions/rudder/index.ts` | `session_start`, `before_agent_start` (mutates `systemPrompt`), `context`, `input`, `tool_call` (injects `RUDDER_CONTEXT_ID` env into bash). See `index.ts.txt:962-996`. |
...
| `templates/pi/extensions/rudder/index.ts.txt` | pi | Pi runs a TS extension in-process. |
...
Everything else under `shared-hooks/` is platform-independent Python that reads only from `.rudder/`.
...
3. **Resolution.** Walks up from cwd to find `.rudder/`, then calls `common.active_task.resolve_active_task()` to find current task, reads `task.json` for status. See `inject-workflow-state.py:255-285`.

4. **Body.** Looks up status in `_FALLBACK_BREADCRUMBS` (line 144) merged with parsed `[workflow-state:<status>]` blocks from `.rudder/workflow.md` (`load_breadcrumbs` at line 204).
...
// load Rudder context themselves via the prelude injected here.
```
...
```ts
return `## Required: Load Rudder Context First

This platform does NOT auto-inject task context via hook. Before doing anything else, you MUST load context yourself:

1. Run \`python3 ./.rudder/scripts/task.py current --source\` to find the active task path and source ...
...
It instructs the sub-agent to read prd / info / jsonl. It does NOT mention `<workflow-state>`, `task.status`, the `[workflow-state:STATUS]` tags, or any state from the breadcrumb. The only "status-ish" data the sub-agent sees is the implicit fact that `task.py current --source` returned a task — but the breadcrumb body (e.g. the "rudder-implement → rudder-check → rudder-update-spec → finish" guidance for `in_progress`) is omitted entirely.
...
| `templates/pi/extensions/rudder/index.ts.txt` | pi only |
...
- Codex hooks require `features.codex_hooks = true` in user-level `~/.codex/config.toml` (warning at `codex.ts:80-90` and `codex/config.toml:7-15`). If unset, hooks.json is silently ignored — no breadcrumb fires. This is the most common reason a Codex user reports "Rudder context missing".
