...
- **Query**: Survey every code path that mutates `task.json.status` across the Rudder runtime + CLI; build a definitive transition map.
- **Scope**: internal
- **Date**: 2026-04-30
- **Repo root**: `/Users/taosu/workspace/company/mindfold/product/share-public/Rudder`

> Runtime sources at `.rudder/scripts/**` are byte-identical to the CLI templates at `packages/cli/src/templates/rudder/scripts/**` (verified `diff -q`). Line numbers below cite the CLI template tree (the source of truth shipped via `rudder init`).
...
|---|---|---|
| `planning` | `task.py create` *or* `rudder update --migrate` (migration task) *or* TS factory default | `task.py start <dir>` → `in_progress` |
| `in_progress` | `task.py start` *or* `rudder init` (bootstrap / joiner task) | `task.py archive <dir>` → `completed` (and dir moves to `archive/`) |
...
|---|---|---|---|---|
| 1 | `cmd_create` | `packages/cli/src/templates/rudder/scripts/common/task_store.py:206` | `"planning"` (literal) | `python3 task.py create "<title>"` |
| 2 | `cmd_start` | `packages/cli/src/templates/rudder/scripts/task.py:109-111` | `"in_progress"` (only if previous was `"planning"`) | `python3 task.py start <dir>` |
| 3 | `cmd_archive` | `packages/cli/src/templates/rudder/scripts/common/task_store.py:319-323` | `"completed"` + sets `completedAt = today` | `python3 task.py archive <dir>` (also called from `/rudder:finish-work` step 3) |
...
| 5a | `getBootstrapTaskJson` | `packages/cli/src/commands/init.ts:417` | `"in_progress"` (overrides factory default) | `rudder init` on a fresh repo (creator path) |
| 5b | `getJoinerTaskJson` | `packages/cli/src/commands/init.ts:460` | `"in_progress"` (overrides factory default) | `rudder init` joining an existing project (`.rudder/` exists, `.developer` missing) |
| 6 | migration-task block | `packages/cli/src/commands/update.ts:2215-2226` | `"planning"` | `rudder update` (when a breaking-change migration manifest exists for the version delta) |
...
`packages/cli/src/templates/rudder/scripts/common/task_store.py:201-228`
...
`packages/cli/src/templates/rudder/scripts/task.py:107-112`
...
`packages/cli/src/templates/rudder/scripts/common/task_store.py:316-345`
...
- `packages/cli/src/templates/rudder/scripts/hooks/linear_sync.py` — writes `task["meta"]["linear_issue"]` (`linear_sync.py:154-157`) but never touches `task["status"]`. Linear's own status workflow is mirrored separately via `linearis issues update -s "In Progress"|"Done"`.
...
| (no file) | `planning` | `rudder update` (when migration manifest exists) | `update.ts:2215` (TS factory call with `status: "planning"`) | One-time bootstrap of a `MM-DD-migrate-vX.Y.Z` task. |
| (no file) | `in_progress` | `rudder init` (creator) | `init.ts:412` (`getBootstrapTaskJson`, overrides factory default) | Bootstrap task is born in-progress because the user is already working it. |
| (no file) | `in_progress` | `rudder init` (joiner) | `init.ts:454` (`getJoinerTaskJson`) | Joiner onboarding task; same reasoning. |
...
3. `clear_task_from_sessions(str(task_dir), repo_root)` (line 345) — deletes any per-session JSON in `.rudder/.runtime/sessions/` that points at this task. See §6.
4. `archive_task_complete` → `archive_task_dir` → `shutil.move(task_dir, archive/YYYY-MM/<name>)` (`task_utils.py:138`).

Then optionally:

5. `_auto_commit_archive` — `git add -A .rudder/tasks` + commit `chore(task): archive <name>` (lines 369-387). Suppressed by `--no-commit`.
...
- In active tree: status was `planning | in_progress | …`, dir at `.rudder/tasks/<name>/`.
- After archive: status is `completed`, dir at `.rudder/tasks/archive/YYYY-MM/<name>/`.
...
`packages/cli/src/templates/rudder/scripts/common/task_utils.py:218-261`
...
Hooks are user-configured shell commands listed under `hooks.<event>:` in `.rudder/config.yaml`. Each command receives:
- `cwd = repo_root`
...
Documented in `packages/cli/src/templates/rudder/config.yaml:25-33` (commented-out scaffold) and `common/bundled-skills/rudder-meta/references/customize-local/change-task-lifecycle.md:18-21`.

### 5.c Subscribers

The repo ships exactly one hook script that consumes these events: `linear_sync.py` (`packages/cli/src/templates/rudder/scripts/hooks/linear_sync.py`).

| `linear_sync.py` action | Suggested wiring | What it does |
|---|---|---|
| `create` (line 117) | `hooks.after_create: ["python3 .rudder/scripts/hooks/linear_sync.py create"]` | Creates a Linear issue, stores `meta.linear_issue` on `task.json`. |
...
|---|---|---|---|
| `task.json.status` | `.rudder/tasks/<task>/task.json` | Per-task lifecycle (created → in-flight → archived) | All consumers above |
| Session active-task pointer | `.rudder/.runtime/sessions/<context_key>.json` (`current_task` field) | Per-AI-session/window pointer to which task this conversation is working on | `inject-workflow-state.py`, `session-start.py`, `task.py current` |
...
```json
{
  ...
  "session_id": "...",
  "current_task": ".rudder/tasks/04-30-foo",
  "current_run": null
}
```
...
`packages/cli/src/templates/rudder/workflow.md:504-512` (the comment block immediately above the `[workflow-state:…]` blocks):
...
### Primary writers
- `packages/cli/src/templates/rudder/scripts/task.py` (lines 70-122 `cmd_start` / `cmd_finish`)
- `packages/cli/src/templates/rudder/scripts/common/task_store.py` (lines 139-283 `cmd_create`, 290-366 `cmd_archive`, 369-387 `_auto_commit_archive`)
...
### Readers / breadcrumbs
- `packages/cli/src/templates/rudder/scripts/common/tasks.py` (lines 23-112)
- `packages/cli/src/templates/rudder/scripts/common/task_queue.py` (lines 47-114)
- `packages/cli/src/templates/rudder/scripts/common/session_context.py` (lines 164, 320, 340, 420, 425, 443, 505)
...
### Lifecycle / hooks
- `packages/cli/src/templates/rudder/scripts/common/task_utils.py` (lines 218-261 `run_task_hooks`, lines 106-167 archive helpers)
- `packages/cli/src/templates/rudder/scripts/common/active_task.py` (lines 466-583 active-task + `clear_task_from_sessions`)
- `packages/cli/src/templates/rudder/scripts/common/config.py` (lines 190-207 `get_hooks`)
- `packages/cli/src/templates/rudder/scripts/hooks/linear_sync.py` (lines 117-178)
- `packages/cli/src/templates/rudder/config.yaml` (lines 17-33 hooks scaffold)

### Workflow + spec
- `packages/cli/src/templates/rudder/workflow.md` (lines 500-541 `[workflow-state:*]` blocks)
- `.rudder/spec/cli/backend/quality-guidelines.md` (lines 264-329 Schema Deprecation guideline)
- `packages/cli/src/templates/common/bundled-skills/rudder-meta/references/customize-local/change-task-lifecycle.md` (lines 18-38 hook taxonomy)

### Skill that consumes the lifecycle
- `.claude/commands/rudder/finish-work.md` (drives `task.py archive` from the AI side)
...
- The runtime tree at `.rudder/scripts/` and the source tree at `packages/cli/src/templates/rudder/scripts/` are byte-identical (verified `diff -q`). All line numbers are valid for both trees.
- I did not exhaustively audit every backup directory under `.rudder/.backup-*`. Those are stash dumps and not loaded at runtime.
