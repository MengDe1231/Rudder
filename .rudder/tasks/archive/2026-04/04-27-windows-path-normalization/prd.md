...
1. **#198**：`.template-hashes.json` 在 Windows 上写入 `.rudder\\config.yaml` 这种反斜杠 key，跨平台不可移植，且每次 `rudder update` 在不同 OS 下都会让所有文件被判定为"新增/修改"。
...
* **测试 fixture**：`regression.test.ts` 大量用 `path.join(tmpDir, ".rudder", ...)` 写文件，这是真实 fs 调用，不需要改。但若有比对字符串的地方需要 normalize。
...
* [ ] macOS/Linux 上 `rudder init` 后 `.template-hashes.json` 全部 key 为 `/`（回归不破）。
* [ ] Windows 上 `rudder init` 后 `.template-hashes.json` 全部 key 为 `/`（关闭 #198）。
...
* 涉及 issue：
  - https://github.com/MengDe1231/Rudder/issues/198 (root cause: hash key)
  - https://github.com/MengDe1231/Rudder/issues/194 (root cause: 路径 + 行尾符)
