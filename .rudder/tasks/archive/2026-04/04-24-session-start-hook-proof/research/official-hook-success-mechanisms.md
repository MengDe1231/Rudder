...
- OpenCode: plugin API supports session events and custom JS logic; proof must be implemented through Rudder' plugin behavior, not a Claude/Codex-style `additionalContext` contract.
...
Implication for Rudder:

- Registration proof: `/hooks` should show the Rudder `SessionStart` command.
...
- Use `statusMessage` in `.codex/hooks.json` for an immediate "Loading Rudder context" status when the host supports it.
...
- Do not promise that Copilot `sessionStart` output proves Rudder context was injected.
- If Rudder keeps a Copilot session-start script, it should be treated as logging/diagnostics unless host behavior changes.
...
- OpenCode proof should be implemented in the Rudder plugin itself.
...
1. Add a stable, short marker to all Rudder-owned context-injecting session-start implementations:

   ```xml
   <rudder-injection hook="SessionStart" status="ok" platform="..." version="...">
   Context below was injected by Rudder session-start.
   </rudder-injection>
   ```
...
3. Add platform-specific verification docs:
   ...
   - OpenCode: verify through Rudder plugin-injected marker or plugin logs.
