...
Make SessionStart injection visibly obvious to users across hook-capable platforms by instructing the AI to acknowledge the injected Rudder context at the start of the first visible assistant reply in each new session.

## What I already know

* Users cannot easily tell whether Rudder `SessionStart` context was actually injected.
...
* The selected MVP is: put a strong one-shot instruction inside SessionStart injected context, so the AI begins the first response by saying Rudder SessionStart was injected and briefly lists what was loaded.

## Requirements

* Every Rudder-owned SessionStart implementation that injects context must include a concise first-reply notice instruction.
...
* Phase 2 implementation is followed by `rudder-check`.
