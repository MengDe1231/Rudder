...
Fix Python 3.9 compatibility issue (#113). Rudder Python scripts use `X | Y` union type syntax (PEP 604, Python 3.10+), causing runtime errors on Python 3.9.

## Requirements

- Add `from __future__ import annotations` to all 21 affected Python files in `.rudder/scripts/`
- Add the same to all corresponding template copies in `packages/cli/src/templates/rudder/scripts/`
- Verify both copies remain identical after changes
- Consider adding Python version check during `rudder init`
