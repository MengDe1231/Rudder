...
把 Rudder 的核心流程文档 `workflow.md` 从"混合大纲 + 详细步骤"重构为 **Phase Index + Phase Detail 两级结构**，并把 `continue / start / finish-work` 三个命令按"有 hook / 无 hook 平台"差异化输出。最终实现：
...
- `.rudder/workflow.md` — 项目自用（跟模板源同步）
...
- `.rudder/scripts/get_context.py` — 新增 `--mode phase`
...
1. Diff `workflow-draft.md` 与当前 `.rudder/workflow.md`，确认 draft 覆盖所有必需内容
...
4. 把 draft 写入两处：
   - `packages/cli/src/templates/rudder/workflow.md`（目前有 1 处 iFlow 残留在第 42 行附近，需清理）
   - `.rudder/workflow.md`（目前有 1 处 iFlow 残留在第 42 行，需清理）
...
**动作**：扩展 `.rudder/scripts/get_context.py`
...
**完成标志**：
- CLI 示例：`python3 ./.rudder/scripts/get_context.py --mode phase --step 1.1 --platform cursor` 输出正确
...
判断当前处于哪个 phase（参考 `.rudder/workflow.md` 的 Phase Index），
然后用 `--step X.X --platform {{PLATFORM}}` 加载具体步骤详情。

详细流程见 `.rudder/workflow.md`。
```
...
**动作**：更新 `.rudder/spec/cli/` 下的 platform-integration.md（如存在）
...
- [x] spec 更新（`.rudder/spec/cli/backend/platform-integration.md`）
...
- **不改 `.rudder/scripts/task.py`**：task 生命周期管理保持现状，本 task 只扩 `get_context.py`
