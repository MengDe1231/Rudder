...
Rudder 是一个 AI 辅助开发的工作流框架。它通过以下机制让 AI agent 的产出更可靠：

- **Spec 系统**：项目编码规范持久化在 `.rudder/spec/` 下，AI 通过 skill 或 hook 读取，而不是靠"记住"
...
创建：
- `.rudder/.developer` — 身份文件（gitignored）
- `.rudder/workspace/<your-name>/` — 个人工作空间
...
`.rudder/spec/` 下存放项目的编码规范。按包（package）和层（layer）组织。
...
**读取 spec**：
```bash
cat .rudder/spec/<package>/<layer>/index.md    # 入口，包含 Pre-Dev Checklist
cat .rudder/spec/guides/index.md               # 思维指南（跨包通用）
```
...
```bash
python3 ./.rudder/scripts/task.py create "<title>" [--slug <name>]  # 创建
python3 ./.rudder/scripts/task.py start <dir>                       # 设为当前 task
python3 ./.rudder/scripts/task.py finish                            # 清除当前 task
python3 ./.rudder/scripts/task.py archive <name>                    # 归档到 archive/
python3 ./.rudder/scripts/task.py list                              # 列出活跃 task
```

**Current task 机制**：`task.py start` 把 task 路径写入 `.rudder/.current-task`。有 hook 的平台会在每次会话开始时自动注入当前 task 信息，AI 无需被告知就能知道在做什么。
...
```bash
python3 ./.rudder/scripts/get_context.py              # 完整上下文
python3 ./.rudder/scripts/get_context.py --json        # JSON 格式
python3 ./.rudder/scripts/get_context.py --mode packages  # 可用 packages 和 spec layers
python3 ./.rudder/scripts/get_context.py --mode record    # 用于 session 记录的上下文
python3 ./.rudder/scripts/get_context.py --mode phase --step <X.X>  # 获取某个步骤的详细指引
```
...
| 用户意图 | 对应 skill |
|---|---|
| 想做新功能 / 需求不清晰 | rudder-brainstorm |
| 准备写代码 / 开始实现 | rudder-before-dev |
| 写完了 / 检查一下 | rudder-check |
| 卡住了 / 同一个 bug 修了好几次 | rudder-break-loop |
| 规范需要更新 | rudder-update-spec |
...
跳过条件：`.rudder/.current-task` 已指向一个 task。

#### 1.1 需求探索 `[必做·可重复]`

加载 `rudder-brainstorm` skill，按照 skill 指引和用户交互式探讨需求。
...
```bash
python3 ./.rudder/scripts/task.py init-context "$TASK_DIR" <type>
...
跳过此步。上下文由 skill（rudder-before-dev）在 Phase 2 直接加载。
...
1. 加载 `rudder-before-dev` skill 读取项目规范
...
加载 `rudder-check` skill，按照 skill 指引验证代码：
- spec 合规性
...
如果本次开发过程中经历了反复调试（同一个问题修了多次），加载 `rudder-break-loop` skill：
- 分析 bug 根因类别
- 总结为什么之前的修复失败
- 提出预防机制
...
加载 `rudder-update-spec` skill，回顾本次开发是否产生了值得记录的新经验：
- 新发现的模式或约定
- 踩过的坑
- 新的技术决策

有则更新 `.rudder/spec/` 下对应的文档。即使结论是"没有需要更新的"，也要走一遍这个判断。
