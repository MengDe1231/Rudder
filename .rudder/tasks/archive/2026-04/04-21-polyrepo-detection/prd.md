...
让 Rudder 能覆盖 polyrepo / meta-repo / 非标准多 package 布局，同时给出显式的手动配置入口。
...
```bash
rudder init --packages frontend,backend
rudder init --packages "apps/*"  # glob 支持
```
...
- [ ] config.yaml 模板注释补 `git: true` 示例（`packages/cli/src/templates/rudder/config.yaml`）
...
- ❌ `--packages` CLI flag —— 与 config.yaml 语义重叠，UX 糖。Rudder 哲学是声明式优先，不养成 flag/config 二元习惯
- ❌ `DetectedPackage.source: enum` —— 运行时已用 `isSubmodule` + `isGitRepo` 双 boolean，跨语言保持对齐
- ❌ N > 阈值时 confirm 的 default 翻转 —— 用户拍板"按 Enter 是用户责任"
- ❌ 裸 `rudder init` 时禁用 polyrepo 扫描 —— 现有 confirm prompt 已是意图门槛，不需要额外 `--monorepo` 卡口

## 非目标

- **不处理**跨 repo 的 git 操作（Rudder 不做 git 工具）
...
- `.git` sibling 扫描可能误伤：用户 `~/projects/` 下一堆独立项目，`rudder init` 会把所有项目当 package。Mitigation：`--polyrepo` 默认关闭，需显式 `--monorepo` 或 `--packages` 才触发
...
- `.rudder/scripts/common/config.py:267` `get_git_packages()` —— 已识别 `git: true` 字段
...
- **缺口**：
  ...
  2. `.rudder/config.yaml` 默认模板**没有**文档 `git: true` 字段（只有 `type: submodule` 示例）
...
- "`~/projects/` 误伤"风险被放大：如果用户在 polyrepo 父目录跑 `rudder init` 不带任何 flag，`parsePolyrepo` 会自动触发 → 把所有项目当 package。**必须**用"显式 `--monorepo` 才触发"或"打印检测结果让用户确认"门槛。
