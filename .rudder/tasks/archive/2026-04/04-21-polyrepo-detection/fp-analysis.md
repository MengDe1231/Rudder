...
1. **Rudder 在单一根目录管理工作流状态**；多 package 支持的存在原因是某些项目跨越多个需要独立 spec context 的代码单元。不可再分，因为"单一根"是 Rudder 的存在前提。
2. **init 时 Rudder 只能用文件系统作为信号** — `.git`、`pnpm-workspace.yaml` 等。没有语义信号 — 只有文件 — 因为 Rudder 是本地 CLI，无外部上下文输入。
...
- 为什么沉默惊喜糟糕？因为 Rudder 会写 config.yaml + 创建 N 个 spec 目录
- 为什么有摩擦是核心？因为 Rudder init 哲学是非侵入式
...
- 在 `~/projects/` 下裸 `rudder init` 误按 Enter 会一次性创建 N 个 spec 目录。可接受 —— 47 个候选展开屏幕已是足够视觉警告
