...
On Qoder, Rudder currently delivers **all** its workflows (start / finish-work / continue + brainstorm / before-dev / check / update-spec / break-loop) as auto-trigger skills under `.qoder/skills/{name}/SKILL.md`. Verified against current (0.5.0-beta.8) and 0.4.0 GA code:
...
This is an inconsistency with Rudder's command/skill design principle applied on every other platform:

| Primitive | Trigger | Purpose | Examples |
|---|---|---|---|
| **Command** | User (`/rudder:*`) | Session boundaries — explicit user entry | `start`, `finish-work`, `continue` |
...
On Qoder, deliver Rudder in the correct two-layer form:

- **`.qoder/commands/rudder-{name}.md`** (flat, YAML frontmatter) — session-boundary commands. Qoder is `agentCapable` so `start` is filtered out (session-start hook injects the workflow overview), leaving `finish-work` + `continue` as the 2 user-facing commands in 0.5.
- **`.qoder/skills/rudder-{name}/SKILL.md`** — auto-trigger workflows only: `brainstorm`, `before-dev`, `check`, `update-spec`, `break-loop`.

User types `/rudder-finish-work` or `/rudder-continue` explicitly when they want session boundaries. AI matches skills for workflow-level triggers.
...
2. **Configurator** (`packages/cli/src/configurators/qoder.ts`):
   - Writes `.qoder/commands/rudder-{name}.md` for each `resolveCommands(ctx)` result, wrapped via new `wrapWithCommandFrontmatter(...)` helper.
   - Writes `.qoder/skills/rudder-{name}/SKILL.md` via `resolveSkills(ctx)` (5 auto-trigger skills only).
...
4. **Update tracking**: mirrored the same layout into `PLATFORM_FUNCTIONS.qoder.collectTemplates()` (`configurators/index.ts`) so `rudder update` hash-tracks the new command files.
5. **Migration manifest** `packages/cli/src/migrations/manifests/0.5.0-beta.10.json`:
   - `safe-file-delete` for `.qoder/skills/rudder-finish-work/SKILL.md` and `.qoder/skills/rudder-continue/SKILL.md`, with `allowed_hashes` covering both Unix (`python3`) and Windows (`python`) `{{PYTHON_CMD}}` variants of the current template content.
   - New commands written by the standard configure step on `rudder update`.
...
- [x] Running `rudder init --qoder` in `/tmp/qoder-migrate-test` produces the expected layout (verified); `collectTemplates()` entries match so `rudder update` tracks them.
- [ ] Typing `/rudder-finish-work` in Qoder's Agent input invokes the command (requires a real Qoder session — done offline by the user when available).
- [x] Migration manifest handles existing 0.5.0-beta.X installs: legacy `.qoder/skills/rudder-{finish-work,continue}/SKILL.md` hash-verified and deleted, new `.qoder/commands/rudder-{finish-work,continue}.md` written fresh. Verified end-to-end in `/tmp/qoder-migrate-test`.
- [ ] `docs-site` beta + release tracks updated (deferred — separate doc task).
- [x] Dogfood fixture: `/tmp/qoder-migrate-test/.qoder/` shows correct post-migration layout after `rudder update --migrate --force`.

## Notes

- Confirmed symptom in `tmp1/.qoder/skills/` (2026-04-21): lists `rudder-start`, `rudder-finish-work`, `rudder-continue`, plus the five true auto-trigger workflows. Session-boundary ones shouldn't be there.
