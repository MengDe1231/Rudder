...
Fix Pi Agent Rudder subagent execution so the generated `subagent` tool can launch `rudder-implement` and `rudder-check` without Pi treating agent markdown frontmatter as CLI options.

## What I already know

- The generated Pi extension registers a `subagent` tool and injects active Rudder task context successfully.
- User testing shows both explicit `agent: rudder-check` and default `rudder-implement` fail inside subagent execution.
- The reported error is `Error: Unknown option: --- name: rudder-check ...`, which points to the delegated prompt or agent definition being interpreted as command-line options.
- `.pi/agents/rudder-*.md` files contain YAML frontmatter beginning with `---`.
...
- The fix should preserve the existing `subagent` tool contract and Rudder task-context injection.
- The smoke project under `smoke_test/pi-agent-rudder-support` should be updated so the user can retry immediately.

## Requirements

- `subagent` tool must successfully launch a delegated Pi run for `rudder-implement` and `rudder-check`.
...
- Primary files expected: `packages/cli/src/templates/pi/extensions/rudder/index.ts.txt`, Pi template tests, configurator tests, and `smoke_test/pi-agent-rudder-support/.pi/extensions/rudder/index.ts`.
...
- Smoke verified both explicit `rudder-check` and default `rudder-implement` subagent calls.
