...
**日期**：2026-04-17
**分支**：`feat/v0.5.0-beta`
**当前任务**：`.rudder/tasks/04-16-skill-first-refactor`（父）
...
Rudder 是一个 AI 辅助开发的工作流框架。支持 13 个 AI coding 平台（Claude Code / Cursor / OpenCode / Codex / Kilo / Kiro / Gemini / Antigravity / Windsurf / Qoder / CodeBuddy / Copilot / Droid），通过 hooks + sub-agents + skills 让 AI 遵守编码规范。

### 核心机制

- **Spec 系统**：项目编码规范持久化到 `.rudder/spec/`，AI 通过 skill 或 hook 读取
...
- ✅ `inject-subagent-context.py` 平台无关
  ...
  - 只读 `.rudder/` 路径的 JSONL，**去掉所有 hardcoded fallback**
...
- `NO ACTIVE TASK` → 用户说需求后 load `rudder-brainstorm` + `task.py create`
- `STALE POINTER` → `task.py finish` 清理
- `COMPLETED` → load `rudder-update-spec` + archive
- `PLANNING` → load `rudder-brainstorm` 产出 prd.md
- `READY` → load `rudder-before-dev` + spawn `implement` / `check`
...
1. **Codex（P1）**：改 `templates/codex/agents/{implement,check}.toml` 的 `developer_instructions`，在开头加一段"pre-work context loading"（读 `.rudder/.current-task` → 读 JSONL → 逐个读文件 → 再开始干活）。✅ 接受"best-effort"（fresh sub-agent context 开头遵守率高，不加 pre-spawn prelude）
...
1. **workflow.md 重写**：基于 draft 落盘到 `templates/rudder/workflow.md`，用**内联平台标记**实现多平台单一文件：
   ```
...
**候选方案**（需要调研后选择）：
- 选项 1：shell 向上查找 `.rudder/` 目录
- 选项 2：Python bootstrap wrapper
- 选项 3：平台 env var 优先 + fallback
- 选项 4：统一到 `.rudder/hooks/`（脱离平台 config 目录）
...
- 旧 agent：`dispatch.md` / `debug.md` / `plan.md` / `rudder-plan.md`（各平台）
...
- Rudder 内部：`.rudder/scripts/multi_agent/` / `worktree.yaml`
- iFlow：`.iflow/` 整个目录
...
各 CLI / IDE 内置 worktree 支持，不需要 Rudder 自己造。
...
- **各子 task 详细 PRD**：`.rudder/tasks/04-17-*/prd.md`
- **workflow.md draft**：`.rudder/tasks/04-16-rewrite-workflow-full/workflow-draft.md`（420 行）
- **Rudder spec**：`.rudder/spec/cli/backend/platform-integration.md`（已更新反映新架构）
