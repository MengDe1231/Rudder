...
将 Rudder 重构为 **skill-first + 共享 hooks** 架构，降低用户心智负担，提升 AI 自主遵守工作流的能力。同时砍掉已死或跨平台不可移植的特性，让核心流程更稳。
...
- [x] `statusline.py` 共享
- [x] Hook 只读 `.rudder/` 路径（JSONL），去掉所有 hardcoded command/skill fallback
...
- [x] **Codex / Copilot / Gemini / Qoder 改为 pull-based prelude** —— sub-agent definition 顶部注入 `## Required: Load Rudder Context First`，由 sub-agent 自己 Read `.current-task` + prd + jsonl（替代原计划的 Codex TOML pre-work + Copilot preToolUse 方案）
...
- [ ] 旧 agent 文件：`dispatch.md` / `debug.md` / `plan.md` / `rudder-plan.md`
...
- [ ] 实现最小可用方案（向上查找 `.rudder/` / 平台 env var / Python bootstrap）
...
| `04-17-hook-path-robustness` | 🟡 P2 → 🟢 P3 | - | 顺带解决 | 新 hook 用 `find_rudder_root()` 向上查找 `.rudder/`，CWD 漂移最小修复由 workflow-enforcement-v2 带走；完整跨平台（Windows）留 P3 |
...
**Sprint 2（下一步）** 🔴：
- `workflow-enforcement-v2` — FP 简化后 S-M 工作量，1 个 session 可完成。顺带把 `hook-path-robustness` 最小修复（向上查找 `.rudder/`）带完
...
各 CLI / IDE 现在都内置 worktree 支持（Claude Code、Cursor 都有），不需要 Rudder 自己造。
...
改为 sub-agent definition 顶部注入 "Load Rudder Context First" prelude，sub-agent 启动后自己 Read `.current-task` + prd + jsonl。
