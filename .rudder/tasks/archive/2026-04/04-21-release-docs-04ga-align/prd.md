...
Prior audit: `.rudder/tasks/archive/2026-04/04-20-docs-site-version-audit/research/release-snapshot-audit.md` has the complete findings (commands, platform counts, missing sections, orphan files).
...
`ch01`, `ch04` architecture listing, `release/index.mdx` tagline, `release/skills-market/rudder-meta.mdx` — all list 6 platforms ("Claude Code, Cursor, Codex, OpenCode, Kilo, Kiro"). Update to the 12 platforms present in 0.4.0 GA (add Gemini CLI, Qoder, CodeBuddy, GitHub Copilot, Windsurf, Factory Droid). Remove Antigravity references (it was added in 0.5).
...
- Insert a new §6.X in `release/guide/ch06-task-management.mdx` covering `--package` flag on `task.py create` and `init-context`, plus `.rudder/spec/<package>/` layout.
- Reference the 0.4.0 changelog's monorepo bullet from `ch01-what-is-rudder.mdx` features list.
...
3. Kilo (0.3.4 renamed `commands/rudder/` → `workflows/`)
4. Kiro (0.3.x)
5. Gemini CLI (0.3.x)
6. Qoder (0.3.4)
...
- [ ] No occurrences of `/before-backend-dev`, `/before-frontend-dev`, `/check-backend`, `/check-frontend`, `/rudder-check-backend`, `/rudder-before-backend-dev` in any `release/guide/` or `release/zh/guide/` file (grep returns empty).
- [ ] Platform count in `release/index.mdx` + `release/guide/ch01-what-is-rudder.mdx` + `release/guide/ch04-architecture.mdx` + `release/skills-market/rudder-meta.mdx` matches the 12-platform reality of 0.4.0 GA.
- [ ] `release/guide/ch06-task-management.mdx` has a monorepo section covering `--package` + `.rudder/spec/<package>/` layout.
- [ ] `ch01-what-is-rudder.mdx` features list mentions monorepo.
