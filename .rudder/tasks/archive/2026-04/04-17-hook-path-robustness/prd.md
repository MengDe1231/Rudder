...
用户在 monorepo 子目录里用 AI 工具：`cd apps/web && claude`。CWD 会是 `apps/web/`，`.rudder/` 在 monorepo 根，hook 找不到。

#### 场景 3: Git submodule 内部

Rudder 本身有 `docs-site/` submodule。如果 CWD 在 `docs-site/` 里：
- `git rev-parse --show-toplevel` 返回 submodule 的根（`docs-site/`）
- 但 `.rudder/` 在**父 repo** 根（一级往上）
- 任何基于 `.git` 查找的方案都会错
...
### 选项 1: 向上查找 `.rudder/` 的 shell 命令

```json
"command": "sh -c 'd=$(pwd); while [ \"$d\" != \"/\" ] && [ ! -d \"$d/.rudder\" ]; do d=$(dirname \"$d\"); done; python3 \"$d/.claude/hooks/session-start.py\"'"
```

- ✅ 跨平台逻辑（`.rudder/` 是真正的项目根标志）
...
### 选项 3: 统一把 hooks 放到 `.rudder/hooks/`

所有平台 settings.json 都指向同一位置：

```json
"command": "python3 .rudder/hooks/session-start.py"
```

- ✅ 统一路径，方便维护
- ❌ 没解决 CWD 问题（.rudder 也是相对路径）
- ✅ 但脚本内可以 `os.chdir(find_rudder_root())` 修复后续路径（因为脚本已启动了）
...
d = os.getcwd()
while d != "/" and not os.path.isdir(os.path.join(d, ".rudder")):
    d = os.path.dirname(d)
script = sys.argv[1]  # e.g. "session-start"
subprocess.run([sys.executable, os.path.join(d, ".rudder/hooks", f"{script}.py")],
               stdin=sys.stdin)
```
...
`rudder init` 时把项目绝对路径写进 settings.json：
...
- [ ] 在 `.rudder/spec/cli/backend/platform-integration.md` 加一节 "Hook Path Resolution"
