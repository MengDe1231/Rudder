...
Clean up Pi Agent Rudder subagent output so the parent Pi conversation shows the delegated agent's final answer instead of the full `--mode json` event stream.

## What I already know

- The `subagent` tool now works functionally for `rudder-check` and default `rudder-implement`.
...
- [x] Final assistant text is returned for both `rudder-check` and default `rudder-implement`.
...
- Smoke verified explicit `rudder-check` returns `CLEAN_CHECK_OK` and default `rudder-implement` returns `CLEAN_IMPLEMENT_OK` without nested JSON.
