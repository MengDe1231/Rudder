...
|---|---|---|
| Normal user task | `.rudder/scripts/common/task_store.py:147-172` (`cmd_create`) | 24 fields — the canonical shape (id / name / title / description / status / dev_type / scope / package / priority / creator / assignee / createdAt / completedAt / branch / base_branch / worktree_path / commit / pr_url / subtasks / children / parent / relatedFiles / notes / meta) |
...
5. **Remove `.rudder/scripts/common/phase.py`** at the same time — orphan code, no importers.
...
- Add `safe-file-delete` entry for `.rudder/scripts/common/phase.py` to the next migration manifest (e.g., `0.5.0-beta.9.json` or wherever this ships) — it wasn't in `0.5.0-beta.0`'s 138-entry cleanup (only `multi_agent/*` was), so existing users on early 0.5 betas still have the file.
...
- Update `.rudder/spec/cli/backend/quality-guidelines.md:288` — uses `phase.py:get_current_phase` as a reader example; swap for a still-live reader.
...
- Auto-migrating existing tasks in users' repos to add missing keys. `task.py` already treats missing fields as null, so no user breaks; a future `rudder update` migration entry can normalize if desired.
...
- [ ] `.rudder/scripts/common/phase.py` deleted.
...
- [ ] `templates/markdown/spec/backend/script-conventions.md` directory-tree + `.rudder/spec/cli/backend/quality-guidelines.md` `phase.py` reader example updated (D2).
- [ ] `rudder init` on a fresh project still produces a working bootstrap task end-to-end.
- [ ] `rudder update --migrate` across a breaking boundary still produces a working migration task end-to-end.
