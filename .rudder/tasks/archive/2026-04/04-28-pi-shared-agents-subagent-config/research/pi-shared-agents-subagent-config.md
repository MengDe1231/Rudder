...
* whether subagent behavior should be implemented by the Rudder plugin;
* whether `https://github.com/nicobailon/pi-subagents` provides a better configuration model.

## Current Rudder Pi Shape
...
```json
{
  ...
  "extensions": ["./extensions/rudder/index.ts"],
  "skills": ["./skills"],
  "prompts": ["./prompts"]
}
```
...
There is no Rudder-side support today for `model`, `thinking`, `fallbackModels`, `skills`, or `tools` per agent/subagent call.
...
This means Pi can consume `.agents/skills` without requiring Rudder to duplicate skills under `.pi/skills`.
...
* Rudder already has a shared Agent Skills layer for Codex and adjacent tools.
* It avoids duplicate generated skill trees.
* It makes `.pi/settings.json` smaller and closer to Pi conventions.

But it should not be done by blindly pointing Pi at the current Codex-rendered `.agents/skills` output. Rudder common skill rendering is currently platform-aware:

* Codex context renders some command references as `$start`, `$finish-work`, etc.
* Pi context renders them as `/rudder-start`, `/rudder-finish-work`, etc.

Only `rudder-brainstorm` and `rudder-update-spec` differed in the local comparison, but that is enough to require a design decision:
...
Recommendation: keep Rudder Pi subagent definitions in `.pi/agents/` for now. Moving them to `.agents/{name}.md` would overload `.agents` with both Agent Skills (`.agents/skills`) and Pi-specific agent markdown. It is valid, but it makes ownership less clear and may collide with other tools that use `.agents/` differently.

### Subagent Model / Thinking

Rudder should support model/thinking directly in its own Pi extension. The minimal model is:
...
* preserve the existing Rudder context prompt assembly.

This gives the requested behavior without importing the full `pi-subagents` runtime.

## Recommended MVP

1. Keep Rudder-owned `.pi/extensions/rudder/index.ts`; do not install `pi-subagents`.
2. Add `model`, `thinking`, `fallbackModels`, and optionally `skills` to Rudder Pi agent frontmatter and parser.
3. Add optional per-call `model` / `thinking` fields to the Rudder `subagent` tool schema.
4. Move common Rudder workflow skills for Pi to shared `.agents/skills` only after shared skill output is made platform-neutral or otherwise explicitly canonical.
5. Keep `.pi/agents` as the Rudder Pi agent definition root.
...
* `packages/cli/test/templates/pi.test.ts`
  ...
  * existing Rudder context injection still happens.
...
| Adopt config shape from `pi-subagents` | familiar to Pi users, supports model/thinking cleanly | Rudder must implement parsing/tests |
| Depend on `pi-subagents` | feature-rich immediately | external dependency, larger behavior surface, harder deterministic tests |

## Conclusion

The long-term implementation path is to reuse `.agents/skills` for shared Rudder skills, but not to move Rudder subagent definitions wholesale into `.agents` yet. For subagent model/thinking, Rudder should implement a small frontmatter/per-call config layer in its own Pi extension, using Pi CLI's native `--model` / `--thinking` support and borrowing the proven field names from `pi-subagents`.

Post-implementation note: the MVP intentionally keeps Rudder-owned Pi workflow skills under `.pi/skills` and keeps `.pi/settings.json -> "skills": ["./skills"]`. Shared `.agents/skills` remains deferred until the shared Agent Skill rendering is platform-neutral or explicitly canonical; pointing Pi at the current Codex-rendered `.agents/skills` would risk platform-wrong command references.
