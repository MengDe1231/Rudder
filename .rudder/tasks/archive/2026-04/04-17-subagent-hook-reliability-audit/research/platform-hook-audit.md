...
**Date**: 2026-04-17
**Scope**: cursor, gemini, qoder, codebuddy, droid（B 类：Rudder 已配置但可靠性未确认的 5 个平台）
...
| Platform | Rudder 现有配置 | 官方 subagent hook | 能改 sub-agent prompt? | 判定 |
|---|---|---|---|---|
...
**Rudder 的 hook 用的正是 `updatedInput.prompt`**（见 `.claude/hooks/inject-subagent-context.py:790-795`）。理论上受上述 bug 影响。
...
**结论**：上述两个 issue 要么已静默修复，要么只影响特定场景（Rudder 不踩）。Claude Code 归为 A 类"确认能注入"**成立**，无需调整。
...
- **Rudder 现有配置**：`preToolUse` + matcher `Task` → `inject-subagent-context.py`，返回 `updated_input` 改写 Task 的 prompt
...
- **Rudder 现有配置**：`BeforeTool` + matcher `^(check|implement|research)$`（regex 对 tool name 匹配，sub-agent 名 = tool name，能 match 上）
...
- **建议**：**降级为 Pull-based**（2026-04-17 决定）。理由：#18128 的 context 可见性限制让 hook 难以稳定推断要注入什么；改用 sub-agent definition 里的 system prompt 指示启动时 Read `.rudder/.current-task` + `prd.md` + jsonl 引用的 spec，跟 Qoder/Codex/Copilot 统一，实现更简单，可靠性更高
...
- **Rudder 现有配置**：`PreToolUse` + matcher `Task` → `inject-subagent-context.py`
...
- **Bug / 限制**：
  - Qoder 没有公开的"创建 sub-agent 的 tool"名，Rudder 的 `matcher: "Task"` 在 Qoder 上**很可能根本不 fire**
  - 官方无公开 issue 仓，无法源码验证
- **建议**：**在 Qoder 上放弃 hook-based sub-agent prompt 注入**。Rudder 当前 `PreToolUse + Task` 在 Qoder 上**无文档依据**，应在 Qoder 配置中移除或替换。三条路径评估：
  ...
  | C. **sub-agent definition 里让它启动时 Read 文件**（Pull-based dynamic）| ✅ | sub-agent 有 `Read/Grep/Glob` 工具（默认 `*`），在 `.qoder/agents/<name>.md` 的 system prompt 里写 "Before starting: read `.rudder/.current-task`, then its `prd.md` + spec files in `implement.jsonl`/`check.jsonl`" — 每次启动拉到最新内容 |

  **C 为什么算"动态"**：内容是动态的（`prd.md` / `jsonl` 随 task 阶段变），行为是静态的（pull 规则固化在 agent definition 里）。Rudder 的 `implement.jsonl` / `check.jsonl` 本来就是为这种 pull 场景设计的。
- **关键证据**（动态方案确认）：
...
- **建议**：**保持现状**。matcher `Task` 与 Claude Code 同名同语义，Rudder 脚本若兼容 Claude Code 的 `hookSpecificOutput.updatedInput` 结构即可（大概率兼容）
- **版本**：最近 changelog 到 Release 1.10

---

## Recommendations for Rudder

### 优先级 P0（破坏性，需立即处理）

| 平台 | 问题 | 动作 |
|---|---|---|
| **Qoder** | `PreToolUse + Task` 在 Qoder 上无文档依据，很可能 hook 根本不 fire；UserPromptSubmit 因 Context Isolation 也到不了 sub-agent | 改 configurator：Qoder 上不再安装 `inject-subagent-context.py` hook。改为 **Pull-based**：在 `.qoder/agents/<name>.md` 的 system prompt 里指示 sub-agent 启动时 Read `.rudder/.current-task` → `prd.md` + `implement.jsonl` 引用的 spec。CodeBuddy 若实测失效可套用同方案 |
