...
在 `04-16-rewrite-workflow-full` 的调研中发现：**"平台配了 sub-agent hook" ≠ "sub-agent hook 能真的注入上下文"**。多个平台存在官方 bug 或文档与实现不一致。Rudder 当前按"配了就算有"的字段 (`AI_TOOLS.templateContext.hasHooks`) 分类，可能跟真实能力有出入。

本 task 的目标：**实测**每个"疑似能注入 sub-agent"的平台，确认 Rudder 现有 hook 配置是否真的在 sub-agent 启动时修改了 prompt / 注入了上下文。
...
**准备**：
1. 在每个平台新建一个干净项目，`npx rudder init --platform <name>`
...
- [ ] 创建独立目录 `/tmp/rudder-hook-audit/`
- [ ] 对 5 个平台各自 `rudder init`（可能需要 `--platform` 参数每次单选）
- [ ] 写一份共享的 `TESTFILE.md` + minimal `prd.md`
- [ ] 修改每个平台的 `inject-subagent-context.py` 临时加 stderr 调试日志

**完成标志**：5 个独立项目目录准备好，每个都有 Rudder 配置。
...
根据结果，决定：
- 哪些平台的 Rudder hook 配置需要改（如 cursor 切 subagentStart、qoder 切 SubagentStart）
- 哪些平台的 `hasHooks` 需要修正为 false（真的不能注入的）
- 哪些平台继续观察（上游会修）

**完成标志**：`results.md` 产出 + 一份调整清单。

### Step 4 — 调整 Rudder 配置 [按需]
...
- [ ] 更新 `.rudder/spec/cli/platform-integration.md`（如存在），记录每个平台的 sub-agent 注入真实能力
