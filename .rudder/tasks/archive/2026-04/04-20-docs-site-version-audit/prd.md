...
- [`research/ancillary-pages-audit.md`](research/ancillary-pages-audit.md) — showcase + use-cases 最过期（含已删命令 `/rudder:record-session`），marketplace / templates / blog 基本稳定
...
- `implement / check / research / dispatch / plan / debug` 6 个 agent → 实际只剩 `rudder-implement / rudder-check / rudder-research` 3 个
...
严重过期（~25 处）：
- `showcase/terminal-demo.mdx` + ZH 镜像：分支号 `feat/v0.4.0-beta` + 旧 agent 名 + 已删 `/rudder:record-session`
- `use-cases/open-typeless.mdx` + ZH 镜像：`/rudder:record-session` + 旧 agent
- `skills-market/rudder-meta.mdx`：提到 Antigravity（0.4 不存在）
...
**Beta EN + ZH**：
- [ ] `guide/ch01-what-is-rudder.mdx` — 重写核心介绍（10 平台、skill-first、3 个 rudder-agent）
...
**Ancillary**：
- [ ] `skills-market/rudder-meta.mdx` + ZH — 删 Antigravity 表述

### P2（低触达 / 时效性弱）

- [ ] `guide/appendix-d/e/f.mdx` — 附录小改
- [ ] `guide/resources.mdx` — 外部资源链接清理
- [ ] `blog/use-k8s-to-know-rudder.mdx` — agent 命名小改
...
- Q3（Policy）：ZH 用机械翻译（lark-mcp skill / 手工）还是用 rudder-research / 专门 translate skill？
