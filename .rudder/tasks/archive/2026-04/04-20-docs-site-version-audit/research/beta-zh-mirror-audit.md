...
- **分类问题数量**：
  - 旧 sub-agent 名（implement/check/research/dispatch/plan/debug）：全站未用 `rudder-*` 前缀，约 30+ 处。
...
3. **Sub-agent 命名全部旧版**（`implement` 而非 `rudder-implement`）：ch04 Agent 表、appendix-A Agent 表都要改。
...
- **结构**：完全对齐（4 个 CardGroup、5 行 why-Rudder 表）。
...
### zh/guide/ch01-what-is-rudder.mdx
**跟 EN 同步度**：高
**EN 对照**：`guide/ch01-what-is-rudder.mdx`
...
- **L29**："Hook 自动注入，按任务精准加载"——这一句在 Skill-first 架构下略微失真。当前 skill 模型下，rudder-before-dev、rudder-check 等是 auto-trigger skill，不是 hook 注入；Hook 仍然在 claude-code 场景生效但只是多平台方案之一。
...
- **L30**：`rudder init --codex -u your-name` —— 当前 CLI 也支持 `--codex` 但新增的 `--gemini`、`--qoder`、`--droid`、`--copilot` 标志全部缺。
...
- **L78-188（首次使用三步走 Tabs）**：7 个 Tab，同样缺 gemini / qoder / copilot / droid。而且 Cursor Tab L106 里的 `/rudder-check-backend` 是老命令名，当前应为 `/rudder-check` 或通过 rudder-check skill。
- **L350-407（目录结构示意图）**：
  - L357-368：`.claude/commands/rudder/` 下列的命令全错——record-session.md、before-backend-dev.md、before-frontend-dev.md、check-backend.md、check-frontend.md、check-cross-layer.md、create-command.md 都不在新版 CLI 里。
  - L370-376：Agent 列表是 dispatch / plan / implement / check / debug / research——老 agent 名，应改为 rudder-implement / rudder-check / rudder-research，且当前只有 3 个 sub-agent。
...
- **L52**："调用 Implement Agent" → 应为 `rudder-implement` sub-agent。
- **L56**："调用 Check Agent" → 应为 `rudder-check`。
- **L43**："调用 Research Agent" → 应为 `rudder-research`。
- **L78**：`/record-session` —— 该 slash command 已移除；现在 session 记录通过 rudder:finish-work skill。
...
- **L114-117（Info 块）**："规范自动注入是 Claude Code 的专属功能" —— 过期。现在 rudder-before-dev 等是跨平台 auto-trigger skill，不再 Claude Code 专属。
- **L46（Agent 系统示意）**：`Plan → Research → Implement → Check → Debug` —— 旧流水线。当前是 Phase 1 Plan → Phase 2 Execute → Phase 3 Finish 的三阶段 workflow，sub-agent 只有 rudder-implement、rudder-check、rudder-research 三个。
...
- **L281-290（Agent 表）**：六个 agent 名（dispatch / plan / implement / check / debug / research）全部是旧名。当前只有 3 个 sub-agent：rudder-implement / rudder-check / rudder-research。dispatch / plan / debug 已不存在或合并。
...
- **L200-204（注入时机和控制）**：列出 implement / check / debug / research 4 种 agent 的注入规则——debug agent 是否还存在需核实；应替换为当前三个 rudder-* agent。
- **L195**：`check.jsonl` 文件路径 `.claude/commands/rudder/finish-work.md`——现在 finish-work 在 skill 目录下（`.claude/skills/rudder:finish-work`）。
...
**独立过期项**：
- **L7**："Rudder 提供 13 个 Slash 命令（Claude Code），分为 6 个类别。Cursor 版本有 12 个（不含 `/parallel`）" —— 全错。当前只有 2 个 slash command（start, finish-work），其他是 skill；且 `/parallel` 已移除，并行通过 `/rudder:parallel` 或 brainstorm skill 处理。
- **L11-26（跨平台命令对照表）**：
  - 整张表列的命令 90% 都不存在：
    - `/before-backend-dev` → 已合并为 `rudder-before-dev` skill（一个 skill，不分前后端）。
    - `/before-frontend-dev` → 同上。
    - `/check-backend` / `/check-frontend` → 已合并为 `rudder-check` skill。
    - `/check-cross-layer` → 已移除或合并进 rudder-check skill。
    - `/record-session` → 已移除，由 rudder:finish-work 处理。
    - `/create-command` → 已移除或重命名。
    - `/parallel` → 需核实是否仍存在。
  - Cursor 列里的命名 `/rudder-check-backend` 等是更老版本的 cursor 命名惯例（用 `-` 而非 `:`）。
...
- **L52-92（`/parallel` 章节）**：描述 "仅 Claude Code 支持" —— 需核实。当前 `.opencode/commands/rudder/` 下已经有 `continue.md` 等新命令，`parallel` 命令状态未知。
- **L95-107**：`/record-session` 描述 —— 该 slash command 已移除。
- **L116-122**：`/onboard` 描述 —— 也可能是 skill 而非 slash。
- **L135-149**：`/check-backend`、`/check-frontend` 章节 —— 均已合并为 `rudder-check` skill。章节需整体重写。
- **L151-161**：`/check-cross-layer` —— 已移除或重构。
- **L169-199**：`/break-loop` —— 当前是 `rudder-break-loop` skill 而非 slash。
...
- **L152-157**：check.jsonl 示例里路径 `.claude/commands/rudder/finish-work.md` 和 `.claude/commands/rudder/check-backend.md` —— check-backend.md 已不存在，finish-work 在 skill 目录。
- **L178-218（Plan Agent 评估）**：Plan Agent 仍存在，但 reject 机制现在可能在 `rudder-brainstorm` skill 里（与 git status 中 `.opencode/skills/rudder-brainstorm/` 一致）。
...
**独立过期项**：
- **L9-32**：Spec 目录结构仍是 frontend / backend / guides 三大类。当前项目里 `.rudder/spec/` 目录下有 `cli/` (backend, unit-test)、`docs-site/` (docs)、`guides/`——结构变了。但这只是"示例"，不一定要同步到真实项目结构。
...
- **L45**：`/rudder-check-backend` —— 旧命名，应为 `/rudder-check`。
- **L55**：`/rudder-record-session` —— 已移除。
...
- **L120-121**：`/rudder-before-backend-dev`、`/rudder-before-frontend-dev` —— 已合并为 `/rudder-before-dev`。
- **L129**：`/rudder-check-cross-layer` —— 已移除。
- **L180**：`/break-loop` —— 现在是 skill 触发（`rudder-break-loop` auto-trigger），不是 slash。
...
- **L55-63（修改示例）**：`name: check` —— 应为 `rudder-check`。
...
**独立过期项**：
- **L9-14（概念表）**："Skill 系统自动匹配" —— 这条描述符合当前 Skill-first 架构。但章节没有提到 Rudder 当前大量 first-party skill（rudder-before-dev、rudder-check、rudder-implement、rudder-break-loop、rudder-update-spec 等）都是这种形式。
- **L17-42（SKILL.md 文件格式）**：文件格式示例过于简化 —— 当前 Rudder skill 使用 YAML frontmatter 形式（`name`、`description` 字段），示例应更新。参见 `.opencode/skills/rudder-before-dev/SKILL.md`。
...
- **L7-22（13.1 Claude Code）**：
  - L21："6 个专业 Agent（dispatch、plan、implement、check、debug、research）" —— 全错。当前 3 个 sub-agent：rudder-implement、rudder-check、rudder-research。
- **L24-66（13.2 Cursor）**：
  - L27-40（命令对照表）：`/rudder:start`、`/rudder:check` 等左列全部不是当前命令集。
  - L44："Cursor 共 12 个命令（不含 /parallel）" —— 当前 skill 化后命令数量变了。
  - L49-66（推荐工作流）：`/rudder-start`、`/rudder-before-dev`、`/rudder-check`、`/rudder-check-cross-layer`、`/rudder-finish-work` —— 部分已移除或重命名，且 cursor 现在也支持 skill auto-trigger。
- **L68-93（13.3 Codex）**：
...
- **L95-160（13.4 Kilo Code）**：
  - L99：`rudder init --kilo` —— 需核实 `--kilo` flag 是否仍支持（记忆里 kilo 存在但 Kilo 可能没重点维护）。
...
- **L358-386（13.9 rudder update）**：版本号 `0.5.0-beta.8` 与当前一致——但文档里只给 `cat .rudder/.version` 例子，没写死版本号，OK。
...
**独立过期项**：
- **L15-31（命令定义表）**：13 个命令文件全部按 `.claude/commands/rudder/{name}.md` 路径列出 —— 但其中 check.md、check-cross-layer.md、break-loop.md、create-command.md、integrate-skill.md、onboard.md、record-session.md、update-spec.md 当前大多是 skill 而非 command。需要重做整张表，区分 slash command vs skill。
- **L33-42（Agent 定义表）**：6 个 agent（dispatch / plan / implement / check / debug / research）全部按 `.claude/agents/{name}.md` 列出 —— 应改为 3 个（rudder-implement、rudder-check、rudder-research）。
...
- **L9-23（Slash 命令跨平台对照表）**：13 行命令，每行 5 个平台列：
  - Claude Code/OpenCode 列仍用 `/rudder:*` 格式——正确，但所列具体命令过半已不存在（`/rudder:before-dev`、`/rudder:check`、`/rudder:check-cross-layer`、`/rudder:break-loop`、`/rudder:update-spec`、`/rudder:create-command`、`/rudder:integrate-skill`、`/rudder:onboard`、`/rudder:record-session` 多数是 skill 而非命令）。
  - Cursor 列用 `/rudder-*` —— 过时命名惯例。
...
**独立过期项**：
- **L19-29（完整示例 fullstack implement.jsonl）**：列出的 spec 文件名（backend/api-module.md、backend/quality.md、frontend/components.md）在当前 rudder 默认 spec 结构里可能已重命名。
...
- **Q2 (L16-20)**：`/check-cross-layer` —— 已合并进 `/rudder-check` skill。
- **Q3 (L22-28)**：`/break-loop` —— 现在是 skill。
- **Q4 (L30-38)**：multi-agent scripts —— 需核实。
- **Q8 (L66-73)**：迁移步骤 —— "Add .rudder/ and .claude/ to Git" 过时，当前有 `.opencode/`、`.gemini/` 等 10 个平台目录。
...
- **Q13 (L120-129)**：`rudder init -u your-name` 自动检测环境 —— 当前还要考虑新加的 `--gemini`、`--qoder`、`--droid`、`--copilot` flags。
...
3. **Sub-agent 名字全错** —— 影响：ch04、appendix-A（2/21 文件）。
   - 应为 rudder-implement / rudder-check / rudder-research 三个，而非 6 个旧 agent。
