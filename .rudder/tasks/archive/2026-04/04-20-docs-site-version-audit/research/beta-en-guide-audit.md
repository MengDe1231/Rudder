...
- **需要更新的文件数**：20 / 20（全部）
- **最严重类别**：整个 EN guide 停留在 0.3.x / 0.4.x 的心智模型——"6 平台 + Claude Code 独占 + 13 slash commands + ralph-loop + dispatch/plan/implement/check/debug/research 6 agent"。**0.5.0-beta 的三大变化（skill-first、10 平台对称、rudder-* 重命名）完全没有体现**。
- **最严重的结构性问题**：
  1. **Agent 命名全部过期**：每个文件里 `implement` / `check` / `research` / `plan` / `dispatch` / `debug` 都还是旧名字；0.5.0-beta.5 已经把保留的三个统一改成 `rudder-implement` / `rudder-check` / `rudder-research`，并**删除**了 `dispatch` / `plan` / `debug` 三个 agent（0.5.0-beta.0 Legacy cleanup 部分）。
  ...
  6. **`.current-task` 文件名**：实际代码里目录下是 `.rudder/.current-task`（带点），跟文档一致——这个没问题。但 `current_phase` / `next_action` 字段文档还写，实际 0.5.0-beta.0 已从 `task.py create` 中**移除**。
...
- 具体问题：
  ...
  - L60-68 平台配置表格，同样平台数量对不上，且 "Codex `$start`" / "Kiro `@start`" 这些 invocation 格式不对（真实 codex 走 `/rudder:start` 或 shell `rudder` CLI，Kiro 现在是 skill）。
  ...
  - L104-113 Cursor tab "you manually run /rudder-check-backend" → 实际命令格式是 `/rudder:check`（加了 skill 以后是 auto-trigger，不再需要手动喊）。同时 `check-backend` / `check-frontend` 旧名字已合并成单个 `check` skill。
  ...
  - L357-386 `.claude/` 目录树：
    ...
    - L374-380 agents 列表 `dispatch / plan / implement / check / debug / research` → 只剩 `rudder-implement / rudder-check / rudder-research`，其余 3 个 agent 已删除。
    - L383 `ralph-loop.py` → **已删除**。
  - L395-404 `.kilocode/` 块 → kilo 已不再官方支持。
  - L406-411 `.codebuddy/` 目录树里 commands 也是旧列表。
  - **完全缺失**：Gemini / Qoder / Copilot / Droid 四个平台的 init 说明；`common/` skill-first 架构；Skill 文件布局（`{platform}/skills/rudder-*/SKILL.md`）。
- **建议改动量**：大（几乎整页重写）。
...
- 具体问题：
  ...
  - L42 "Invoke Research Agent" → 应改为 `rudder-research` sub-agent。
  - L52 "Invoke Implement Agent" → 应改为 `rudder-implement`。
  - L56 "Invoke Check Agent" → 应改为 `rudder-check`。
...
- 具体问题：
  ...
  - L132 `Task(subagent_type="implement", prompt="...")` → 应为 `"rudder-implement"`。
  ...
  - L209-214 injection timing 列表 里 `debug agent` / `research agent` → debug 不存在了；research 名字应为 `rudder-research`。
...
- 具体问题：
  - L7 "Rudder provides 13 Slash commands (Claude Code)" → 实际 0.5.0-beta.0 只剩 **2-3 个 slash commands**（start + finish-work，加可选 continue.md），其余 5 个变成 skill。
  - L11-26 跨平台 command 参考表 → 整表过期：
    - L13 "before-backend-dev" / L14 "before-frontend-dev" → **已合并成 skill `rudder-before-dev`**。
    - L15-16 "check-backend" / "check-frontend" → **合并成 skill `rudder-check`**。
  ...
  - L202-211 "`/finish-work`" 保留，但应更新为 3-step（Quality Gate + Commit Reminder + Session Journal，对应 0.5.0-beta.0 changelog "Its single job ... is now Step 3 of /rudder:finish-work, which also covers Quality Gate and Commit Reminders"）。
  - L213-219 "`/create-command`" / "`/integrate-skill`" → 已删除。
  - **完全缺失**：`rudder-before-dev` / `rudder-brainstorm` / `rudder-check` / `rudder-break-loop` / `rudder-update-spec` 作为 **skill** 而非 command 的讲法；`session-start` / `inject-subagent-context` / `inject-workflow-state` 三个 shared-hooks 的角色。
...
- 具体问题：
  ...
  - L131-134 "Status transitions: planning → in_progress → review → completed / rejected" → 基本还对，但 Workflow 概念现在是 Phase 1 Plan / Phase 2 Execute / Phase 3 Finish（由 `.rudder/workflow.md` 驱动），跟 status 不完全对应。
  ...
  - L178-220 "Plan Agent and Requirements Assessment" 整节 → **Plan Agent 已删除**。需求评估的角色现在由 `rudder-brainstorm` skill 承担（见 changelog "dispatch / debug / plan agents (replaced by skill routing)"）。
...
- ❌ **需要大改**
- 具体问题：
  - L12-57 八个 Tab 里全部使用 `/record-session` / `/rudder-record-session` → 已合并进 finish-work。
  - L45-47 "`/rudder-check-backend`" → 应改为 `/rudder:check` 或 auto-trigger。
  - L60-88 整节 "8.2 Complex Parallel Development (Multi-Agent Worktree)" → **已删除**。
  - L111-112 `/check-cross-layer` → 已合并进 check。
  - L119-120 `/rudder-before-backend-dev` + `/rudder-before-frontend-dev` → 已合并成 `rudder-before-dev` skill。
  - L128 `/rudder-check-cross-layer` → 已合并。
...
- 具体问题：
  ...
  - L51-63 "Modifying Existing Agents" 用 `name: check` 做例子 → 应改为 `name: rudder-check`。
  - L65-107 "Creating a New Agent" 例子 `name: test`，`model: opus` → 同样去掉 model 字段。
- **建议改动量**：中（主要是把 `model: opus` 从所有示例里移除，统一用 `rudder-*` 前缀）。
...
- 具体问题：
  ...
  - **完全缺失**：Rudder 自带 5 个 skill（rudder-before-dev, rudder-brainstorm, rudder-break-loop, rudder-check, rudder-update-spec）的讨论——它们是 0.5.0 的核心概念。
- **建议改动量**：中（删 12.3 / 12.4，补 Rudder 5 个 auto-trigger skill 讲解）。
...
- 具体问题：
  ...
  - L23 "Agent system — 6 specialized Agents (dispatch, plan, implement, check, debug, research)" → 现在只有 3 个 rudder-*。
  ...
  - L358-386 "13.9 `rudder update` and Version Management" → 基本还对，但缺 `--migrate` 标志（0.5.0-beta.0 "--migrate is now required for breaking releases"）。
...
- 具体问题：
  ...
  - L33-42 "Agent Definitions" 6 个 → 只剩 rudder-implement / rudder-check / rudder-research。
...
- ✅ **基本还对**
- 具体问题：
  - L10-28 JSONL 例子均使用 `implement.jsonl` → 概念仍对（虽然 sub-agent 改名成 rudder-implement，但 jsonl 文件名是否跟着改还要核实代码；根据 `shared-hooks/inject-subagent-context.py`，jsonl 文件名仍是 `implement.jsonl` / `check.jsonl`，和 subagent_type 不直接耦合，所以可能还对）。
...
- **建议改动量**：大（整文件删除，或重写成 `.rudder/config.yaml` reference，那是当前还存在的配置文件）。
...
- 具体问题：
  ...
  - L82-93 "Q10: rudder update" → 基本对，缺 `--migrate`。
  ...
  - L154-158 "Q16: Kilo Skills vs Rudder Skills" → Kilo 已移除，整个 Q16 删除。
...
1. **ch01-what-is-rudder.mdx** — 第一章就把 Ralph Loop / Plan Agent / Multi-Agent / 6 平台这些**已删除概念**当核心卖点讲，严重误导新用户。
2. **ch02-quick-start.mdx** — 用户按章节二的命令跑会遇到：`/record-session` 不存在、`/rudder-check-backend` 不存在、`.kilocode/` 配不出来、Gemini / Qoder / Copilot / Droid 用户找不到自己的 init 命令。
...
17. **ch12-custom-skills.mdx** — 删 `/create-command` / `/integrate-skill` 小节，加 Rudder 自带 5 skill 介绍。
18. **appendix-d.mdx** — 基本还对，微调。

### 几乎不需要动 / 已删除

19. **appendix-e.mdx** — **整文件应删除**（worktree.yaml 不存在了）；或重写为 `.rudder/config.yaml` reference。
