...
这些命名跟 `packages/cli/src/templates/claude/agents/rudder-*.md` 的 `tools:` 字段一致，本质是 Claude Code 的 agent frontmatter schema。其他平台：
...
章节只讲 **"你可以定制 agent 的名字 / 工具权限 / 模型 / 系统提示"** 这种**能力**层次的介绍，具体语法不在 guide 里展开，链到各平台自己的官方 agent 文档或 Rudder 的 `packages/cli/src/templates/{platform}/agents/README`。
...
- 10.3 修改已有 agent → 改 `name: check` → `name: rudder-check`（已在 batch 1 修了）
