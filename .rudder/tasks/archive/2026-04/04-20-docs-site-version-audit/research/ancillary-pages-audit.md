...
|---|---|
| CLI 版本 | 0.5.0-beta.8 |
| Sub-agents | `rudder-implement` / `rudder-check` / `rudder-research`（3 个，都有 rudder- 前缀） |
| 平台数 | 10 |
| 命令（common） | 3 个 slash command：`/rudder:start`、`/rudder:continue`、`/rudder:finish-work` |
| Skills | 5 个 common skill：`rudder-before-dev`、`rudder-brainstorm`、`rudder-break-loop`、`rudder-check`、`rudder-update-spec` |
| Plugin / Agent Trace | Planning 阶段（`.rudder/tasks/04-20-plugin-system-design/`） |
| Marketplace skills | 3 个：`rudder-meta`、`cc-codex-spec-bootstrap`、`frontend-fullchain-optimization` |
| Marketplace specs | 3 个：`electron-fullstack`（50 files）、`nextjs-fullstack`（35 files）、`cf-workers-fullstack`（38 files） |
| 已删/改名命令 | `/rudder:record-session`（已删）、`/rudder:parallel`（不在 common，仅 Claude 特有）、`/rudder:onboard`（不在 common，仅部分平台） |
...
- **辅助页面普遍稳定度：中偏低**
  ...
  - `showcase/*`、`use-cases/*`：**严重过期**，大量使用已删除命令 `/rudder:record-session`、旧 agent 名 `implement`/`check`/`research`（无 `rudder-` 前缀）
  - `blog/*`：时效性弱，`use-k8s-to-know-rudder` 引用 Agent 命名也需更新；`ai-collaborative-dev-system` 是 Mosi 项目历史叙事，基本时间无关
- **最需要更新的目录**：`showcase/`、`use-cases/`（多处引用已删命令和旧 agent 名）、`index.mdx`（命令总数"13 slash commands"与现实 3 common + 若干平台特定不符）
- **发现的 broken / 过期命令引用**：约 14 处 `/rudder:record-session`，2 处旧 Rudder v0.2.12 版本号
...
- **整体稳定**。两文件都只链接到 open-typeless、rudder-cursor，命令引用无过期内容（`/contribute` 在 L29/L28 是 skill 名，符合现状）。
...
- EN L14：`Used \`/rudder:parallel\` to launch 3 worktree agents...` — `/rudder:parallel` 当前是 **Claude Code 专属**命令（`.claude/commands/rudder/parallel.md` 存在），提及时最好加平台前缀。不算 broken。
- ZH L14：同样引用 `/rudder:parallel`，同上。
...
- L50：`<span className="td-agent-name">research</span>` — **过期**。应为 `rudder-research`（有 rudder- 前缀）。
- L77：`<span className="td-agent-name">implement</span>` — **过期**，应为 `rudder-implement`。
- L94：`<span className="td-agent-name">check</span>` — **过期**，应为 `rudder-check`。
- L125：`<span className="td-cmd">/rudder:record-session</span>` — **已删命令**。`/rudder:record-session` 在当前 CLI（.claude/commands/rudder/ 和 common templates）中**已不存在**，只有 `.opencode/commands/rudder/record-session.md` 在本仓库 opencode fork 里残留。应替换为 `/rudder:finish-work` 或删除该阶段。
- L173：`Research Agent finds relevant specs and code patterns.` — 文案 "Research Agent" 尚可（人类可读名），但 agent 技术名已是 `rudder-research`。
- L181：`Check Agent reviews every changed file` — 同上。
- L215：`Implement Agent writes code across 5 layers` — 同上。
- L224：``/rudder:record-session\` archives the session to your journal.` — **同 L125 已删命令**。
...
- L50/L77/L94：`research` / `implement` / `check` 无 `rudder-` 前缀 — **过期**。
- L110 / L125 / L131 / L137 / L224 / L237：多次 `/rudder:record-session` 或 `record-session` — **已删命令**。
- L233：`| **工具调用** | 169 次（explore + research + implement + check）` — "explore" 阶段在当前工作流里未体现（Phase 1/2/3 为 implement → check → finish）。

#### showcase/rudder-cursor.mdx / zh/showcase/rudder-cursor.mdx
- EN L8 / ZH L8：`Based on Rudder v0.2.12.` / `基于 Rudder v0.2.12。` — **版本号陈旧**。虽然 fork 基于的是 v0.2.12 是历史事实，但建议加 "(as of fork date)" 或注明当前 Rudder 已在 0.5.0-beta。不算绝对错误。
...
- **历史叙事文章**，主题是 "Mosi 项目早期如何搭建 AI 协作系统"，使用的是 `.cursor/commands/` + `workflow/agent-progress/` 老结构（非 Rudder 结构）。这是**故意的历史视角**，不代表当前 Rudder。
- 作为 "thought-piece" 基本不需要改；若要加 "edited" 声明可以在开头加一行 "本文记录的是 Rudder 诞生前的 Mosi 项目实践" 以避免误导。
- 无需视作过期。

#### blog/use-k8s-to-know-rudder.mdx / zh/blog/use-k8s-to-know-rudder.mdx
...
- EN L290 / ZH L289：``/rudder:start\` or \`/rudder:parallel\` (Claude Code only) launches with one click` — `/rudder:parallel` 标注了 Claude Code only，**准确**。
...
- **核心内容（K8s 类比、Hook 注入、Ralph Loop）基本与当前 Rudder 架构对齐**。无高优先级过期。
...
- EN L82：`Skills are hosted in the [Rudder main repo]...` — 准确。
- EN L89：`templates/specs-your-template.mdx` — 路径规则与当前 `docs-site/templates/` 一致。
- ZH L76 / L78：同上。

#### contribute/rudder.mdx / zh/contribute/rudder.mdx
- **最短文件**。EN 仅引用 `CONTRIBUTING.md`，ZH 仅引用 `CONTRIBUTING_CN.md`。
- 需验证仓库根目录：
  - `/Users/taosu/workspace/company/mindfold/product/share-public/Rudder/CONTRIBUTING.md` — 存在性未验证（略微疑点，但此次审计不展开）。
...
- L41：`rudder init` — 命令存在且正确，OK。
- L97：`**Prompt:** \`/rudder:parallel\`` — `/rudder:parallel` 是 Claude Code 专属命令；若 use-case 面向通用场景，应标注平台限制。
- L119：`**Prompt:** \`/rudder:record-session\`` — **已删命令**。需替换为 `/rudder:finish-work`（当前归档/结束流程）。
- L133：`Select the next task, AI uses implement agent then check agent` — 旧 agent 名（无 `rudder-` 前缀）。"implement agent" / "check agent" 作为口语名可保留，但更严谨应写 `rudder-implement` / `rudder-check`。
- L152：表格 `/rudder:parallel` + `/rudder:record-session` — `record-session` **已删**。

#### zh/use-cases/open-typeless.mdx（高优先级！）
- L40 / L96 / L118 / L132 / L150：**完全对应 EN 的同类问题** — `/rudder:record-session` 两处、agent 名无 `rudder-` 前缀、`/rudder:parallel` 未标注平台限制。
...
- EN L17–L20 / ZH L17–L20：列出 3 个 skill（rudder-meta、cc-codex-spec-bootstrap、frontend-fullchain-optimization）— **与 marketplace/index.json + marketplace/skills/ 目录完全一致**。
- **稳定**。

#### skills-market/rudder-meta.mdx / zh/skills-market/rudder-meta.mdx
...
- 表格内容（`core/`、`claude-code/`、`how-to-modify/`、`meta/`）与 marketplace/skills/rudder-meta/ 结构需验证 — 未展开。

#### skills-market/cc-codex-spec-bootstrap.mdx / zh/skills-market/cc-codex-spec-bootstrap.mdx
- Prerequisites 表（Rudder / GitNexus / ABCoder / Codex CLI）— 基本稳定。
...
- 内容围绕 Web Vitals / Lighthouse / DevTools。**不依赖 Rudder 版本**，稳定。
- **无过期**。
...
| 文件 | 行 | 引用 | 状态 |
|---|---|---|---|
| showcase/terminal-demo.mdx | L125 | `/rudder:record-session` | **已删命令** |
| showcase/terminal-demo.mdx | L224 | `/rudder:record-session` | **已删命令** |
| zh/showcase/terminal-demo.mdx | L125 | `/rudder:record-session` | **已删命令** |
| zh/showcase/terminal-demo.mdx | L224 | `/rudder:record-session` | **已删命令** |
| use-cases/open-typeless.mdx | L119 | `/rudder:record-session` | **已删命令** |
| use-cases/open-typeless.mdx | L152 | `/rudder:record-session` | **已删命令** |
| zh/use-cases/open-typeless.mdx | L118 | `/rudder:record-session` | **已删命令** |
| zh/use-cases/open-typeless.mdx | L150 | `/rudder:record-session` | **已删命令** |
| contribute/rudder.mdx | L13 | `github.com/MengDe1231/Rudder/blob/main/CONTRIBUTING.md` | **待验证**（仓库根 CONTRIBUTING.md 未核实） |
| zh/contribute/rudder.mdx | L13 | `github.com/MengDe1231/Rudder/blob/main/CONTRIBUTING_CN.md` | **待验证** |
...
1. **`showcase/terminal-demo.mdx` + `zh/showcase/terminal-demo.mdx`** —
   ...
   - Agent 名 `research`/`implement`/`check` → `rudder-research`/`rudder-implement`/`rudder-check`
   - 删除 / 替换 `/rudder:record-session` → `/rudder:finish-work`
   - "record-session" 阶段（Phase 8）整体改写为 "Finish work / Archive" 阶段
2. **`use-cases/open-typeless.mdx` + `zh/use-cases/open-typeless.mdx`** —
   - 所有 `/rudder:record-session` → `/rudder:finish-work`
   - Agent 名规范化
   - `/rudder:parallel` 标注 "Claude Code only"
3. **`index.mdx` + `zh/index.mdx`** —
   - 描述从 "Claude Code & Cursor" 扩到 "10 platforms"
   - L28 "13 slash commands" 重新计算或改为 "all slash commands"

### 中优先级（平台数 / 版本号措辞）
4. **`skills-market/rudder-meta.mdx` + ZH** — 支持平台列表补全（加 Qoder、CodeBuddy）
5. **`showcase/rudder-cursor.mdx` + ZH** — `v0.2.12` 加 "(fork date)" 注释；"13 slash commands" 重数
...
8. **`blog/use-k8s-to-know-rudder.mdx` + ZH** — 核心类比依然准确；可选加一句当前 Plugin system 计划
9. **`contribute/docs.mdx` / `contribute/rudder.mdx`** + ZH — 外链待快速校验即可
...
| 类型 | 数量 |
|---|---|
| **小改（1–3 行，版本号 / 命令名替换）** | 4 个文件 — index.mdx (×2)、rudder-cursor.mdx (×2) |
| **中改（多段改写，一个文件内 5+ 处）** | 6 个文件 — terminal-demo.mdx (×2)、open-typeless use-case (×2)、rudder-meta skill-market (×2) |
| **大改（整段重写 / 换命令阶段）** | 0（最多 terminal-demo 算边缘情况） |
| **需新建 / 删除页面** | 0（但若 `/rudder:record-session` 彻底废弃，可考虑合并记录进 `finish-work` 叙事） |
| **外链待校验** | 3 处（contribute/rudder.mdx × 2、contribute/docs.mdx 中的 code.claude.com） |
...
**共发现 ~25 处需要修改**，集中在：
- `/rudder:record-session` 已删命令残留：**10 处**（跨 4 个文件）
- 旧 agent 名（无 `rudder-` 前缀）：**6 处**（主要在 terminal-demo）
