...
* Rudder can support Pi sub-agents by installing a Pi extension plus `.pi/agents` definitions.

## Implications for Rudder

Rudder should mark Pi as agent-capable if the generated project includes the sub-agent extension and Rudder agent definitions.

Pi hook mapping:

* Rudder `SessionStart`: Pi `session_start` can show/inject startup workflow state through an extension.
* Rudder `UserPromptSubmit`: Pi `input` can intercept, transform, block, or handle user input before skill/template expansion.
* Rudder "inject workflow state before model call": Pi `before_agent_start` can inject a persistent message and/or modify the turn system prompt.
* Rudder "modify messages before each provider request": Pi `context` can modify the message list before every LLM call.
* Rudder `PreToolUse`: Pi `tool_call` can inspect, modify, or block tool input before execution.
* Rudder tool execution telemetry: Pi `tool_execution_start`, `tool_execution_update`, and `tool_execution_end` expose lifecycle events.

Pi is therefore not a no-hook platform. It is an extension-hook platform. The key distinction is that Rudder should generate TypeScript extension assets instead of Python hook scripts and JSON hook config.

Comparison with OpenCode:

* Same broad category: both should use JS/TS extension assets rather than Rudder Python hooks.
* Same Rudder file style likely applies: generated project-local extension, shared helper module for reading `.rudder`, `.rudder/.current-task`, `prd.md`, and JSONL context files.
* OpenCode current templates use plugin files under `.opencode/plugins/` plus a helper library under `.opencode/lib/`.
* Pi should likely use `.pi/extensions/rudder/` plus a small local helper module, not `.opencode/plugins/` layout.
...
* OpenCode sub-agent context injection hooks `tool.execute.before` and mutates Task tool prompt args. Pi can either:
  ...
  * inject inside the Rudder-maintained `subagent` tool before spawning child `pi` processes.
* OpenCode has an existing Task tool model; Pi's sub-agent path is extension/tool based, so Rudder likely owns more of the subagent orchestration code on Pi.
* Consequence: reuse Rudder's OpenCode plugin architecture ideas, but do not copy the OpenCode plugin event names or assume identical input/output payload shapes.

MVP should use a Pi-specific platform shape:

* Generate `.pi/settings.json`.
* Generate `.pi/skills/*/SKILL.md` from common skills.
* Generate `.pi/prompts/rudder-*.md` or `.pi/prompts/*.md` from common commands for slash-command invocation.
* Generate `.pi/agents/rudder-implement.md`, `.pi/agents/rudder-check.md`, and `.pi/agents/rudder-research.md`.
* Generate or package-load a Pi Rudder extension that exposes a `subagent` tool and handles Rudder hook-equivalent events.
* Add `.pi` to `_SUBAGENT_CONFIG_DIRS` only when Rudder's generated Pi setup actually consumes `implement.jsonl` / `check.jsonl`.
...
* Either vendor/adapt the official subagent extension into Rudder templates, or depend on a Pi package that provides the same tool. Vendoring gives deterministic init output and testability; package dependency keeps Rudder smaller but creates an external version dependency.
* Decide whether context injection should happen at `before_agent_start` for every Pi turn, inside the Rudder `subagent` tool before spawning child `pi` processes, or both.
