...
把 4 个类 2 平台（Gemini / Qoder / Codex / Copilot）的 sub-agent 上下文注入从 hook-based 改为 pull-based：sub-agent 启动时自己 Read `.rudder/.current-task` + `prd.md` + jsonl 引用的 spec，不再依赖平台 hook 注入 prompt（因为这些平台的 hook 要么不 fire、要么收不到 prompt 字段、要么受 bug 影响）。
...
Before starting any work, you MUST load task context (Rudder hook does not auto-inject on this platform):

1. Read `.rudder/.current-task` to find current task path (e.g. `.rudder/tasks/04-17-foo/`)
...
**完成标志**：每个文件顶部都有 "Load Rudder Context First" 段，未注释、未隐藏。
...
- `test/templates/{qoder,codex,copilot,gemini}.test.ts` — 验证 sub-agent definition 顶部含 "Load Rudder Context First" 标记
...
**动作**：更新 `.rudder/spec/cli/backend/platform-integration.md`，加一节 "Subagent Context Injection: Hook-based vs Pull-based"，说明：
...
- [x] 4 平台 sub-agent definition 顶部有 "Load Rudder Context First" 段（通过 configurator transform，不污染 template 源文件）
