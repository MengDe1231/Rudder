...
为 `src/templates/rudder/scripts/` 下的 Python 脚本引入单元测试，重点覆盖 Windows 兼容性代码路径。在 macOS 开发环境上通过 mock 模拟 Windows 行为，防止历史 Windows bug 回归。
...
* Windows 相对路径 `".rudder\tasks\my-task"` — `"/" in` 不匹配
* 正常 Unix 路径仍然工作
