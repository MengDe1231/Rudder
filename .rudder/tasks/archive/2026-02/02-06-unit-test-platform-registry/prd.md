# Rudder CLI 全量单元测试 + 回归测试

## Goal

为 Rudder CLI 的所有 TypeScript 功能模块引入全面的单元测试和历史 bug 回归测试，建立质量安全网，防止功能回退。
...
**`isManagedPath()`**:
* 正向匹配：`.claude/commands/foo.md`, `.rudder/spec`, `.iflow`（精确匹配）
* MC/DC：`startsWith(d + "/")` true + `=== d` false；反之
* 边界：前缀相似 `.claude-backup`, `.rudder-old` → false
* 边界：空字符串 `""` → false
* 边界：路径穿越 `"../.claude"` → false
* **[BUG FIX beta.12]** Windows 反斜杠：`.claude\\commands` → true

**`isManagedRootDir()`**:
* 正向匹配各平台 configDir + `.rudder`
* 边界：非根目录路径、不存在的目录
...
* `DIR_NAMES`: 6 个常量存在性 + 值验证（`.rudder`, `workspace`, `tasks`, `archive`, `spec`, `scripts`）
* `FILE_NAMES`: 7 个常量存在性 + 值验证（`.developer`, `.current-task`, `task.json`, `prd.md`, `workflow.md`, `journal-`）
* `PATHS`: 8 个构造路径正确性（组合 DIR_NAMES）
* `getWorkspaceDir("john")` → `.rudder/workspace/john`
* `getTaskDir("01-21-my-task")` → `.rudder/tasks/01-21-my-task`
* `getArchiveDir()` → `.rudder/tasks/archive`
* 边界：路径不含 `\` 分隔符
...
**文件读取**: `readRudderFile`, `readScript`, `readMarkdown`, `readClaudeFile`, `readOpenCodeFile`
...
* `getAllCommands()`: **返回空 Map**（已知 quirk：命令在 `commands/rudder/` 子目录）
* `getAllAgents()`: 返回 Map
* `getAllHooks()`: 返回 Map、JSON 有效
* `getSettingsTemplate()`: 含 `{{PYTHON_CMD}}`

#### `test/templates/rudder.test.ts` — 10 tests
...
**路径问题 (5 tests)**:
* `[0.2.15]` `PATHS.TASKS` = `.rudder/tasks`（非 `.rudder/workspace/*/tasks`）
* `[0.2.14]` Claude agent 模板无硬编码 `workspace/*/tasks/` 路径
* `[beta.13]` `cli_adapter.py` 无硬编码开发者路径
* `[0.2.15]` 脚本模板无硬编码用户名（路径中）
* `[beta.12]` `task.py` `resolve_task_dir` 处理 `.rudder` 前缀
...
| `iflow getAllCommands()` 返回空 | `templates/iflow/index.ts` | 命令在 `commands/rudder/` 子目录 |
...
│   └── rudder.test.ts            # 10 tests — Rudder 脚本模板
├── types/
