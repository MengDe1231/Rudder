# rudder init 支持从远程模板初始化

## Goal

让 `rudder init` 命令支持从 docs 仓库 (https://github.com/MengDe1231/docs) 下载模板，替代默认的空白模板。设计需考虑未来扩展到多种模板类型。
...
```bash
# 用户不需要关心模板类型
rudder init --template electron-fullstack  # 自动识别为 spec
rudder init --template code-reviewer       # 未来：自动识别为 skill
```
...
| 命令 | 行为 |
|------|------|
| `rudder init` | 交互式，**默认选中 blank** |
| `rudder init -y` | 跳过交互，**直接用 blank** |
| `rudder init --template <name>` | 使用指定模板 |

## 关键决策

### 已有目录处理策略

当目标目录（如 `.rudder/spec/`）已存在时：
...
**交互式询问**：
```
? Directory .rudder/spec/ already exists. What do you want to do?
  ❯ Skip (keep existing)
...
- 模板源：`https://github.com/MengDe1231/docs/tree/main/marketplace/`
...
- [ ] `rudder init` 交互式显示模板列表，blank 为默认
- [ ] `rudder init -y` 直接使用 blank，不触发模板选择
- [ ] `rudder init --template electron-fullstack` 下载指定模板
- [ ] 模板根据 `type` 字段安装到正确位置
...
| 项目 | 改动 |
|------|------|
| **Rudder CLI** | `src/commands/init.ts` 添加模板选择逻辑 |
| **Rudder CLI** | 新增 `src/utils/template-fetcher.ts` 下载逻辑 |
| **Rudder CLI** | `src/cli/index.ts` 添加 `--template` 参数 |
| **Rudder CLI** | `package.json` 添加 giget 依赖 |
...
async function downloadWithStrategy(
  ...
  if (strategy === 'append' && exists) {
    const tempDir = path.join(os.tmpdir(), `rudder-${Date.now()}`)
    await downloadTemplate(`gh:MengDe1231/docs/${templatePath}`, {
      dir: tempDir,
      preferOffline: true
    })
