...
**Context**: init/update 是 Rudder 的两个核心命令，当前只有单元测试覆盖内部函数（cleanupEmptyDirs、sortMigrationsForExecution 等），缺乏对命令整体行为的集成测试。
...
| # | 场景 | 选项 | 验证点 |
|---|------|------|--------|
| 1 | 基本初始化（全默认） | `{ yes: true }` | .rudder/ 目录结构完整、.version 文件正确、默认平台（cursor+claude）配置生成 |
...
| 8 | 版本文件写入 | `{ yes: true }` | .rudder/.version 内容与 VERSION 常量一致 |
| 9 | Hash 追踪初始化 | `{ yes: true }` | .rudder/.template-hashes.json 存在且包含预期 key |
| 10 | Spec 模板生成 | `{ yes: true }` | .rudder/spec/backend/、.rudder/spec/frontend/、.rudder/spec/guides/ 均存在且有文件 |
...
| 8 | 版本文件更新 | `{ force: true }` | update 后 .rudder/.version 更新为新版本 |
| 9 | 备份创建 | `{ force: true }` | update 后 .rudder/.backup-* 目录存在 |
...
- `src/configurators/workflow.ts` — 创建 .rudder 目录结构
...
2. **`cross-platform-thinking-guide.md` init/update 不一致**（Low） — **已修复**
   ...
   - **修复**：从 `update.ts` 的 `collectTemplateFiles` 中移除 `cross-platform-thinking-guide.md`（该文件是 Rudder CLI 项目自身的文档，不属于用户项目模板）
