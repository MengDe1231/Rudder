...
Add Gemini CLI (Google's AI coding CLI) as a first-class Rudder platform, at the same level as Cursor — commands only, no hooks/agents/settings.

## What I Already Know

### From codebase inspection:
- Rudder has 7 platforms: claude-code, cursor, opencode, iflow, codex, kilo, kiro
- Cursor is the simplest: copyDirFiltered configurator, commands-only templates, no Python hooks
- Platform integration spec (`.rudder/spec/backend/platform-integration.md`) documents 10-step checklist
...
- **Namespacing**: `commands/rudder/foo.toml` → `/rudder:foo` (same pattern as Claude!)
...
**Decision**: 方案 A — 在 `src/templates/gemini/commands/rudder/` 直接维护独立的 `.toml` 文件。
**Consequences**:
...
- [ ] `rudder init --gemini` creates `.gemini/commands/rudder/` with all commands
- [ ] `rudder update` detects and updates Gemini templates
- [ ] Commands are valid TOML and work in Gemini CLI as `/rudder:xxx`
...
- `GEMINI.md` generation (that's user/project context, not Rudder commands)
...
- Platform integration checklist: `.rudder/spec/backend/platform-integration.md`
