# brainstorm: Rudder Autopilot run queue

## Goal

Design a Rudder Autopilot mode that can advance through a queue of already-planned Rudder tasks with minimal user prompting. The goal is not to turn a vague idea into a fully automatic project. The required premise is: the user and AI first brainstorm multiple complete Rudder tasks, then Autopilot coordinates execution across those tasks.

## What I already know

- User wants a new version of the old `autonomous-task-planner` idea, but adapted to Rudder 0.5's single `workflow.md` source of truth.
- Old `autonomous-task-planner` generated `TASK-PROMPT.md` and `PROGRESS.md`; that should not be copied directly because current Rudder already has task dirs, `task.json`, `prd.md`, `implement.jsonl`, `check.jsonl`, hooks, and `workflow.md`.
...
- Each Autopilot item should be a real Rudder task, not an inline pseudo-task inside the run file.
- The run-level file should coordinate task order and progress only; it must not redefine the per-task workflow.

## Core Design

Autopilot introduces a run queue layer above existing Rudder tasks:

```text
.rudder/runs/<run-id>/run.md     # queue source of truth
.rudder/.current-run             # active run pointer
.rudder/tasks/<task>/prd.md      # per-task requirements source of truth
.rudder/workflow.md              # per-task workflow source of truth
```
...
- Hooks and `rudder-continue`: read these files and inject the next action; they should not invent workflow rules.

## Dependency: Session-Scoped Current Task

Autopilot should not ship as a serious execution feature until Rudder solves session-scoped `current-task`.

Reason: Autopilot needs to keep the active AI conversation grounded in "the current run item", and each run item is a real Rudder task. If `.rudder/.current-task` remains a repo-global pointer, one window starting or advancing an Autopilot run can silently change the task context injected into another window. Autopilot would amplify the existing multi-window `current-task` problem because it changes task context automatically as the queue advances.

Required sequencing:

1. Finish `04-21-session-scoped-task-state` or an equivalent active-context mechanism.
2. Make hooks and `rudder-continue` resolve the active task from session scope first, with global file fallback.
...
- Validation rules for "all run items are complete Rudder tasks".
...
Autopilot should support an optional user-owned notification script hook. Rudder should not hardcode notification channels; it only invokes the configured script with structured event data.

Candidate config:

```yaml
notify:
  command: ".rudder/hooks/autopilot-notify.sh"
  events:
    - blocked
    - completed
    - failed
...
```json
{
  ...
  "run_path": ".rudder/runs/frontend-api-buildout/run.md",
  "item_id": "T2",
  "task": ".rudder/tasks/04-25-build-frontend-pages",
...
- The hook receives no secrets from Rudder; users are responsible for reading their own environment if their script needs credentials.
...
- The run has 2+ linked Rudder task directories.
- Every task in the run has a `prd.md`.
- Every task has enough context to enter the normal Rudder workflow.
...
If these prerequisites are not met, the system must route back to `rudder-brainstorm` and task planning rather than pretending the run is executable.
...
- Do not skip Rudder workflow phases inside a task.
...
- [x] T1: Audit backend API shape
  task: .rudder/tasks/04-25-audit-backend-api
  status: completed
...
- [ ] T3: Browser verification
  task: .rudder/tasks/04-25-browser-verification
  status: pending
```

## Hook / Continue Behavior

When `.rudder/.current-run` exists, per-turn hook injection should include an additional Autopilot block:

```text
You are in Autopilot Run <id>.
Current queue item: T2 of 3.
Current Rudder task: .rudder/tasks/04-25-build-frontend-pages.
...
Important boundary: hook injection should primarily read state. Mutating `run.md`, switching `.current-task`, or marking tasks done should happen through explicit scripts/skills such as `rudder-autopilot` or enhanced `rudder-continue`, not as a side effect of every user prompt.

## Requirements (Evolving)

- [ ] Depend on session-scoped current-task/current-run resolution before production Autopilot execution.
- [ ] Add an Autopilot run model under `.rudder/runs/`.
- [ ] Add `.rudder/.current-run` as the active run pointer.
- [ ] Add scripts for `create`, `start`, `status`, `next`, `mark-done`, `mark-blocked`, and `validate`.
- [ ] Add or enhance a skill so AI can create a run only from existing complete Rudder tasks.
- [ ] Enhance `rudder-continue` to detect active run and resume the current queue item.
...
- Should Autopilot be a core Rudder feature or an optional plugin once the plugin mechanism exists?
