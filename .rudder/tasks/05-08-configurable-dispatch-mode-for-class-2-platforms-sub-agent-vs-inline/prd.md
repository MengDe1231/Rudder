...
Let users opt out of Rudder's "main session must dispatch rudder-implement / rudder-check sub-agents" default and instead let the main agent edit code inline. Configured project-wide via `.rudder/config.yaml`. Removes the friction Codex users have hit with `wait_agent` self-deadlock, fork_turns inheritance, and other multi-agent_v2 footguns — for users who'd rather just let the main agent do everything.

## What I already know

- `.rudder/config.yaml` already exists (session_commit_message / max_journal_lines / hooks / packages / default_package). Read by `update.ts:465` (loadUpdateSkipPaths) — pattern to follow.
- Current dispatch rule lives in `packages/cli/src/templates/rudder/workflow.md` `[workflow-state:in_progress]` block (lines 187–191):
  > "**Main-session default (no override)**: dispatch the `rudder-implement` / `rudder-check` sub-agents — the main agent does NOT edit code by default."
...
- (Q2) `.codex/agents/rudder-{implement,check,research}.toml` files: **always write** at init / update. Inline mode is a runtime behavior change, not a file-output change. User can still manually invoke sub-agents if they want.
- (Q3) "main agent check" in inline mode: **load existing `rudder-check` skill**, run lint/typecheck/tests, fix issues directly. Reuses existing `.agents/skills/rudder-check/SKILL.md` content; only the executor changes (main session vs spawned sub-agent).
...
- `#### 2.1 Implement` — has separate blocks for `[Claude Code, Cursor, OpenCode, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi]` (dispatch), `[Codex]` (dispatch with extra prelude), `[Kiro]` (dispatch with prelude), `[Kilo, Antigravity, Windsurf]` (inline: load `rudder-before-dev` skill + read prd.md + implement directly).
- `#### 2.2 Quality check` — same pattern; the non-agent-capable block says "load `rudder-check` skill and verify".
- `#### 1.3 Configure context` — non-agent-capable block says "skip this step. Context is loaded directly by the `rudder-before-dev` skill in Phase 2".

`packages/cli/src/templates/rudder/scripts/common/workflow_phase.py` already filters these blocks by `--platform` flag (`get_context.py --mode phase --step 2.1 --platform codex` returns the Codex block).
...
- `get_context.py --mode phase --step 2.1 --platform codex` (called by `/rudder:continue` step 4). Currently returns the `[Codex]` block. In inline mode should return the `[Kilo, Antigravity, Windsurf]` block.
...
- (Q5) Config key + value: `codex.dispatch_mode: sub-agent | inline` (default `sub-agent`). nested under `codex:` in `.rudder/config.yaml`.
...
- (Q8) How does the new `codex.dispatch_mode` section reach existing users on `rudder update`?
  ...
  - Implementation:
    ...
    - **Fresh `rudder init`**: template already has the new commented section, init writes it directly. The manifest field has no effect on init.
...
... dispatch rudder-implement / check ...
[/workflow-state:in_progress]
...
`inject-workflow-state.py` reads `.rudder/config.yaml`; when `codex.dispatch_mode=inline` AND platform=codex, looks up `<status>-inline` tag instead of `<status>`. For `get_context.py --platform codex`, similarly: when inline mode is set, swap the platform name to `Kilo, Antigravity, Windsurf` (or read content from those blocks).
...
- **`packages/cli/src/templates/rudder/scripts/common/rudder_config.py`** (new). Single helper:
  ```python
  def read_rudder_config(repo_root: Path | None = None) -> dict:
      """Read .rudder/config.yaml. Returns {} on missing/malformed."""
  ```
  Reuses `parse_simple_yaml` (resurrect from deleted `worktree.py` or rewrite minimal).

### Files to modify

- **`packages/cli/src/templates/rudder/config.yaml`** — add commented Codex section explaining `codex.dispatch_mode`.
- **`packages/cli/src/templates/rudder/workflow.md`** — add `[workflow-state:planning-inline]` and `[workflow-state:in_progress-inline]` blocks. Bodies describe the inline workflow (load `rudder-before-dev`, edit code, load `rudder-check`, run lint/type-check/tests, fix, then `rudder-update-spec` + commit).
...
- **`packages/cli/src/templates/rudder/scripts/common/workflow_phase.py`** (or `get_context.py` wrapper) — add `resolve_effective_platform()` that swaps `codex` → `kilo` (or any name in the `[Kilo, Antigravity, Windsurf]` block) when inline mode is on. The existing `filter_platform()` then surfaces the inline block content unchanged.
...
# in get_step():
effective = resolve_effective_platform(args.platform, read_rudder_config())
...
2. Regression: read `workflow.md`, parse both `[workflow-state:in_progress]` and `[workflow-state:in_progress-inline]` blocks, assert sub-agent body mentions "dispatch" and inline body mentions "main session"+"rudder-before-dev".
3. End-to-end: synthesize stdin payload + temp `.rudder/config.yaml`, run `inject-workflow-state.py`, assert returned `additionalContext` contains expected text per mode.
...
Files:
  + scripts/common/rudder_config.py            new (~30 LoC)
  ...
  M templates/rudder/workflow.md                +~80 LoC (two new -inline blocks)
  M templates/rudder/config.yaml                +~20 LoC (commented codex section)
...
|---|---|---|
| 1.1 Requirement exploration | `rudder-brainstorm` skill | `rudder-brainstorm` skill (same) |
...
| 2.1 Implement | Dispatch `rudder-implement` sub-agent | Main session: load `rudder-before-dev` skill, then edit code directly |
| 2.2 Quality check | Dispatch `rudder-check` sub-agent | Main session: load `rudder-check` skill, run lint/typecheck/tests, fix issues directly |
| 3.3 Spec update | Load `rudder-update-spec` skill (same) | Same |
...
- The Rudder `continue` skill / its supporting scripts also need to honor the mode so `/rudder:continue` resumes into the right phase chain (e.g. when status=`in_progress`, inline mode should hand the AI the `before-dev` skill, not the dispatch instruction).
- `.codex/agents/*.toml` files unchanged. Sub-agent infrastructure stays installed; just the breadcrumb stops calling for it.

## Requirements (evolving)

- New config key in `.rudder/config.yaml` (exact name TBD via Q1/Q2).
...
- Docs updated: `.rudder/config.yaml` template comments + a docs-site section on the new knob
...
- Files likely touched:
  - `packages/cli/src/templates/rudder/config.yaml` — add commented-out example of the new knob
  - `packages/cli/src/templates/rudder/workflow.md` — add second `[workflow-state:in_progress]` body or a templating mechanism
  - `packages/cli/src/templates/shared-hooks/inject-workflow-state.py` — read config, pick body
  - `packages/cli/src/templates/rudder/scripts/common/` — add a simple yaml config reader if one doesn't exist (Rudder already has `parse_simple_yaml` in scripts; reuse)
  - `packages/cli/test/regression.test.ts` — assert both modes
- Existing `parse_simple_yaml` helper lives in `dist/templates/rudder/scripts/common/worktree.py` (deleted area, but pattern still applies). Or build a fresh reader.
