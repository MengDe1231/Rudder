# Fix Codex generic sub-agent Rudder workflow bleed (#240)

## Goal

When a Codex parent session calls `spawn_agent(message=...)` to launch a generic sub-agent (smoke test, exploration, arbitrary one-off task), the sub-agent currently follows Rudder SessionStart + UserPromptSubmit injections instead of the parent's `spawn_agent.message`. This causes:
...
Goal: make Rudder injections **subordinate** to an explicit `spawn_agent.message` so generic sub-agents do exactly what the parent told them and nothing else.
...
- `codex/hooks/session-start.py` already injects sub-agent self-exemption text at lines 247–254 and 370–373 — but ONLY mentions `rudder-implement` / `rudder-check` roles. Generic sub-agents have no role and skip the exemption.
- v0.5.4 added "Recursion Guard" sections to `rudder-implement` / `rudder-check` agent definition files. Generic sub-agents don't read those files.
...
3. Enforce read-only role constraints before Rudder workflow steps (defensive).
...
- LLMs reliably honor a "if you have an explicit parent task message, do that and ignore everything below" wrap when placed at the very top of the injected context. v0.5.1 already used a narrower variant of this pattern successfully for `rudder-implement` recursion in `SessionStart`.
...
- The fix applies to existing `rudder-implement` / `rudder-check` recursion case too — the existing role-based exemption (lines 247–254, 370–373 in session-start.py) becomes redundant but stays in place as a second layer.
...
- [ ] Existing `rudder-implement` / `rudder-check` self-exemption text still present (defense in depth).
- [ ] Regression test in `packages/cli/test/regression.test.ts` (or new file) asserts notice presence in both hooks' output.
- [ ] Manual repro in throwaway tmp project: spawn a generic Codex sub-agent with explicit message, confirm it executes the message and does NOT touch `.rudder/tasks/*.jsonl` or call `wait_agent`.
...
task message above this hook output, that message is your ONLY job.
  ...
  • Do NOT modify .rudder/tasks/* or any other file unless your
...
Expected post-fix: sub-agent replies with the 2 facts then exits; no `.rudder/tasks/*` writes; parent's `wait_agent` returns terminated quickly.

### Related issues / commits

- Issue: https://github.com/MengDe1231/Rudder/issues/240
