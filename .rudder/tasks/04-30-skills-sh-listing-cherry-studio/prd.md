...
调研并决策：如何让 Cherry Studio 用户能在其内置 Agent 技能市场里直接安装/管理 Rudder，进而回应 issue [#205](https://github.com/MengDe1231/Rudder/issues/205)。
...
1. **"Cherry Studio 的技能市场只对接 skills.sh"** ❌
   ...
   报错 `在技能市场中未找到名为 "Rudder" 的技能` 是 claude-plugins.dev 没收录 Rudder，不是 skills.sh。

2. **"npm 全局装完应该在技能列表里看到"** ❌（类目错配）
   Cherry Studio 的"技能"=`SKILL.md` 目录（Anthropic Claude Skill 形态），不是 CLI 不是 npm 包。Cherry Studio 完全不扫 `npm root -g` / `~/.claude/skills/` / `~/.codex/skills/`，只扫它自己 Electron `userData` 下的 `Skills/` 和当前 agent workspace。`npm i -g @mengde1231/rudder` 进入的位置 Cherry Studio **设计上看不到**。
...
- Rudder 已有 `templates/claude/skills/*` 和 `templates/codex/skills/*`（含 `rudder-meta/SKILL.md`），形态本身就对，但路径在 `templates/` 下，**两个 registry 的爬虫都不扫这个目录**（爬虫只看 repo 根、`skills/`、`.claude/skills/`、`.codex/skills/` 等约定路径）。
...
### Approach A：发布"Rudder 引导型 Skill"到 GitHub 公仓的标准位置（Recommended）

**怎么做**：
- 把 `rudder-meta/SKILL.md`（或新写一个 `rudder-init` skill）从 `templates/claude/skills/` 复制/软链到 repo 根的 `skills/rudder-init/SKILL.md`（或者写一个 `.claude-plugin/marketplace.json` 把 `templates/claude/skills/*` 暴露出去）。
- 内容定位为"教 agent 调用 Rudder CLI"——遵循 skills.sh 上同类工具（`app-builder`、`cli-developer`）的成熟范式。
- 触发一次 `npx skills add MengDe1231/Rudder` 让 skills.sh 索引；claude-plugins.dev 的爬虫会在数小时内自动收录（无需提交）。
- 在 README + `/install` 引导文档加一段"Cherry Studio 用户：在 Agent 技能市场搜 'rudder-init'"。
...
**Cons**：
- 解决的是"Cherry Studio 能搜到并装上 SKILL.md"——装的是个**指引文档**，agent 看到后还得自己去跑 `npx @mengde1231/rudder init`。这跟用户预期的"装完即用"还是有差距。
- claude-plugins.dev 爬虫覆盖度不像 skills.sh 那么明确，回收时延未知。

### Approach B：A + 主动联系 Cherry Studio 把 Rudder 加入官方 marketplace 白名单

**怎么做**：A 的全部 + 在 CherryHQ/cherry-studio 提一个 issue/PR，请求把 Rudder 收入其 bundled marketplace catalog（具体提交流程未公开，需要先开 issue 询问）。
...
**怎么做**：直接关闭 issue 或回复说明 Cherry Studio 不扫 npm 是 by design，建议用户用 Cherry Studio 内置 `skills` MCP 的 `init`+`register` 让 agent 在对话里现场 author 一个引导 Rudder 的 skill。
...
**Consequences**（A 路径）：
- 多一份 `skills/rudder-init/SKILL.md`（或通过 `.claude-plugin/marketplace.json` 把现有模板暴露），增加少量维护负担。
...
- 实际产出 `skills/rudder-init/SKILL.md`（A 路径的实施）→ 后续任务
...
- 入口：`bin/rudder.js` → `dist/cli/index.js`，主流程是把模板写到用户 repo
