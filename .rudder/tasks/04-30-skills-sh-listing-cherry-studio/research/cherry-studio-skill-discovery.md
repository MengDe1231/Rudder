# Research: Cherry Studio Skill Discovery & Why `npm i -g @mengde1231/rudder` Is Invisible

- **Query**: How does Cherry Studio (https://github.com/CherryHQ/cherry-studio) discover, install, and manage "Agent skills"? Why can't a user who installed `@mengde1231/rudder@beta` via global npm see/manage Rudder inside `Agent → 属性 → 技能`?
...
5. **Roadmap is clear: Cherry Studio team has committed to a fuller marketplace** (issue #13758: "我们下个大版本会支持" / "we will support this in the next major version"), and issue #14408 plans a unified v2 "资源库 (Library)" page combining Assistants + Agents + Skills. Today the `skills` tool's marketplace lookup only finds skills that are present in Cherry Studio's own catalog — hence "在技能市场中未找到名为 'Rudder' 的技能".

**Bottom line for Rudder:** Being on global npm gives Cherry Studio zero discovery signal. To appear in `Agent → 属性 → 技能`, Rudder must (a) ship a `SKILL.md` directory the user can drag-and-drop or unzip into Cherry Studio's skills folder, (b) get listed in Cherry Studio's marketplace, or (c) have the user invoke the `skills` MCP `init`+`register` actions pointing at a Rudder-authored skill folder. Listing on https://skills.sh alone will not solve issue #205 because Cherry Studio doesn't talk to skills.sh.
...
The user-visible error `skills:skills MCP 服务器工具 已完成 / 在技能市场中未找到名为 "Rudder" 的技能` is the agent's `skills.search`/`skills.install` returning empty against Cherry Studio's own catalog — **not** a failed lookup against skills.sh. The Chinese phrase "技能市场" maps to Cherry Studio's internal marketplace, not to skills.sh's domain.
...
**Implication:** `npm install -g @mengde1231/rudder@beta` writes to a location Cherry Studio never looks at. The user's CLI binary on `$PATH` is invisible to the Skills panel by design.
...
- **Skillhub CLI (Tencent, https://skillhub.cn/)** — issue #14660. User installed skills via Skillhub's `npm`-style CLI prompt to `Project\skills\`. They became invisible to Cherry Studio's Agent SDK. The workaround the agent itself proposed: "call them directly via file path" — i.e. abandon the Skills panel and just point the LLM at the file path manually. **This is exactly the analogous failure mode to Rudder on `npm i -g`.**
...
## Implications for Rudder

### Hard constraints (from research)

- **Listing on https://skills.sh does NOT make Rudder appear in Cherry Studio.** Cherry Studio has no built-in skills.sh client. (It has `skills` MCP, but it points at Cherry Studio's own marketplace catalog.)
- **Being a global npm CLI does NOT make Rudder appear.** Cherry Studio doesn't scan npm at all.
...
|---|---|---|---|
| **A. Ship a `SKILL.md` bundle for Rudder** alongside the npm package, document drag-and-drop install into Cherry Studio Skills | Low | Drags `rudder-skill/` folder onto Settings → Skills | Most direct. The SKILL.md just teaches the LLM how to invoke the `rudder` CLI that npm installed. CLI does the work; SKILL.md is the discovery surface. |
| **B. Submit Rudder to Cherry Studio's marketplace catalog** (Claude-plugin marketplace.json schema, source can be `npm`) | Medium | Click install in marketplace UI | Requires identifying Cherry Studio's marketplace registry endpoint and submission process — research did not surface a public submission flow; likely PR to a CherryHQ-controlled repo or coordinate with maintainers. |
| **C. Document the agent-driven install** ("ask Cherry Studio's agent: 'create a Rudder skill that runs rudder init when called'") | Low | Pastes a one-line prompt | Fragile (model behavior dependent), but works today via `skills.init`+`register`. |
| **D. Build & publish a Rudder-specific MCP server** that Cherry Studio users add via Settings → MCP | Medium | Adds MCP via URL/config | Sidesteps the Skills panel entirely; Rudder becomes an MCP tool, not a skill. Different UX surface. |
...
- **Whether Rudder would even be accepted as a marketplace skill**: skills there are SKILL.md prompt bundles, not workflow CLIs. A "Rudder skill" would essentially be a doc instructing the model to use the Rudder CLI — the team may or may not consider that on-brief.
