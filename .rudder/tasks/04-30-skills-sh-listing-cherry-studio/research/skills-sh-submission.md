# Research: skills.sh listing for Rudder (Cherry Studio integration angle)

- **Query**: Whether and how to list `@mengde1231/rudder` on skills.sh; relevance to Cherry Studio
- **Scope**: external
- **Date**: 2026-04-30
...
- **The artifact format is strict: an Anthropic-style Skill** — a directory with a `SKILL.md` that has YAML frontmatter (`name`, `description` required) plus optional `scripts/`, `references/`, `assets/`. **NPM packages, MCP servers, and plain prompts are NOT accepted.** A CLI tool like Rudder cannot be listed as such; only Skill-shaped content packaged inside a GitHub repo can.
...
- **Recommended action for Rudder**: (a) for skills.sh, simply ensure the existing `templates/claude/skills/*` directories live in a public repo that follows the discovery layout, then run `npx skills add MengDe1231/Rudder` once to seed the leaderboard; (b) for Cherry Studio specifically, target `claude-plugins.dev` instead — that's the actual registry Cherry Studio queries.
...
## 6. Examples — already-listed CLI / dev-tooling skills similar in shape to Rudder
...
| `app-builder` (davila7/claude-code-templates) | https://skills.sh/davila7/claude-code-templates/app-builder | `npx skills add https://github.com/davila7/claude-code-templates --skill app-builder` | Closest analog to Rudder's purpose: scaffolding orchestrator with 13 templates referenced from `references/`. |
...
**Pattern observation**: every listed dev-tooling skill is *advisory content* (instructions for an agent to follow), not a wrapper around an installable CLI. None of them say "install this npm package, then run it." This is a hint about positioning: Rudder's CLI itself is not what gets listed; what gets listed is a Skill that *teaches* an agent to use Rudder (e.g., "rudder-init" skill that tells the agent to run `npx @mengde1231/rudder init`, what flags exist, what the spec layout means).

The `rudder-meta` skill that already exists in this repo (`templates/claude/skills/rudder-meta/SKILL.md`) is exactly the right shape.
...
- **CLI vs Skill confusion**: Rudder the npm CLI cannot be listed. Only the Skill folders inside (or at the root of) a public repo can. The "listing" is for the agent-facing instruction artifact, not for the binary that scaffolds it.
