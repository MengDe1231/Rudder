...
6. **Rudder 的 runtime 仍在各 AI CLI 里** — Rudder 不应该默认接管 agent loop；Memory/Trace 的存储层应通过 CLI/MCP/hook 被调用，而不是要求用户迁移到新 runtime。
...
- 团队共享内容必须落到现有可 review 的 Rudder 文档面，或显式 promote 到未来 team backend，而不是自动采集的 raw transcript。
...
|---|---|---|---|
| Memory 应该放在 `.rudder/memory/` 并进 git | 这会和现有 `spec` / `tasks` / `journal` 形成第三套项目知识库。repo 只适合承载已有 review 面里的 curated 内容，不适合 raw session 或私人偏好。 | A1, A2, A3 | Discard as default |
| gitignored 的 `.rudder/.traces/` 放在项目下就够了 | 它适合项目 scoped raw capture，但不解决跨项目记忆；多个 worktree / clone 也会产生重复。 | A3, A4 | Modify |
...
6. **各 IDE/CLI 已经拥有原生 transcript source-of-truth**，Rudder 默认复制 raw session 会制造重复存储；更合理的是索引指针、内容哈希和派生摘要。

## Reasoning Chain

GT#1 + GT#3 → raw session 和 personal memory 的默认落点应是用户全局本地 store，而不是 repo。

GT#2 + GT#4 → repo 里不默认新增 `.rudder/memory/wiki/`；共享知识应 promote 到现有 `spec` / `tasks` / `journal` 中最合适的位置，保持一个事实只有一个 canonical home。
...
GT#1 + GT#6 → raw session 不应默认复制到 Rudder store；Rudder store 应优先保存 native transcript pointer、index state、content hash 和 digest。
...
**Key insight:** “当前项目搜索”不要求“数据存放在当前项目目录”。物理存储和逻辑 scope 应该分离：数据默认放 user-global，本项目只是查询 filter；真正适合进 repo 的知识应落到现有 Rudder 文档面，而不是新建一套 parallel memory wiki。
...
```text
~/.rudder/memory/                       # or platform data dir
...
<repo>/.rudder/                         # existing git-tracked knowledge surfaces
...
task_id: <rudder task id or empty>
...
2. **project_id 不稳定**：换路径、换 remote、fork、worktree 后召回错乱。
   - 缓解：project_id 需要用 remote + repo root fallback + Rudder config 显式 override，而不是只用路径。
3. **新增 repo memory 重复 spec / journal**：团队不知道该读哪一份，AI 也会召回冲突事实。
   - 缓解：不默认创建 `.rudder/memory/wiki/`；promote 必须选择写入 `spec`、`tasks` 或 `journal` 的 canonical home。
