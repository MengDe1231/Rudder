...
点名了个人化记忆的 **4 核心原则**（直接适用于 Rudder Memory plugin 设计原则）：
...
→ **PRD 没想到的形态**：不是 session 结束手动 `/ingest`，而是常驻 wiki subagent 订阅主对话流实时摄取。Rudder 可做成 Claude Code subagent（`.claude/agents/wiki.md`）+ 挂 `Stop`/`SubagentStop` hook。

**② 错误传播是最大痛点**（@LuckyCurveC 实测反馈）：
> "ingest 犯蠢之后，后续的 query 都得完蛋 ... lint 指令当你的 wiki 大了也不好用。"
> jakevin7 回："目前得配合 SOTA 模型搞，一旦犯了错误会引起更多错误。"

→ 现有 lint 机制不足以处理错误传播。**Rudder Memory MVP 必须有可撤销机制**：每次 `/ingest` 作为一个独立 git commit，`wiki-log.md` 里带 commit SHA，`rudder memory revert <log-entry>` 原子回滚。比依赖 lint 纠错更靠谱。

**③ 规模上限约 10 人团队**（@MacKFCBK 问几千个跨 7-8 年的公司 wiki）：
> jakevin7："超大规模级别不合适，最多只能维护不到十人团队的 wiki。"

→ 验证 PRD Out-of-Scope：Rudder Memory 服务**单项目单小组**，不做企业 wiki / 跨项目知识库。

**④ 检索质量 > 写作质量**（@crypto_fyy 提问）：
> "wiki 一旦变大，检索质量比写作更关键。你有做 chunk 粒度 / 标题层级 / 引用规范来稳住召回吗？"

→ 当前 PRD 只靠 `_index.md` 路由 + BM25，**页面切分粒度和引用规范没硬规则**。建议从 skill 文件抄硬约束："Keep pages focused on a single topic. If a section grows too large, split into its own page."，并把 `## Related` 固定格式写进 Rudder Memory schema。
...
**Vault 结构（Rudder Memory plugin 直接可抄）**：
```
my-wiki/
...
- **`wiki-agent.md` 是可选覆盖层**（不是 bootstrap 必出）：存在时覆盖 `CLAUDE.md`/`AGENTS.md` 里的默认 MUST / MAY / NEVER ingest 标准；缺失时 fallback 到 bootstrap 里的默认值 → **对 Rudder 的启示**：每个项目可自定义 ingest filter，不需要改 skill 本体
...
- **对 Rudder 的启示**：skill 的 activation pattern / trigger 语义可直接借鉴
...
- **目录约定范本**：Rudder Memory plugin 可采用同构布局
...
- **基于哈希的增量编译**（只处理变更源）
- `query` 命令 + `--save` 存为新页面
- 可借鉴点：增量编译思路，Rudder 如果做 compile 步骤可以参考
...
- **Rudder 决策：MVP 明确拒绝**，复杂度爆炸，是独立研究方向
...
- Rowboat 的解法：typed-entity knowledge graph
  ...
  - 运行后台 agent 做日报装配
- **Rudder 决策：out of scope**（已在 PRD out-of-scope 明示 graph 栈）
  - 但知道**边界**很重要：Rudder Memory 服务于**代码/项目知识**（相对稳定，wiki 合适）
...
- **对 Rudder 的启示**：这个架构范式和 Rudder 哲学 100% 一致（配置生成器 + 平台原生扩展点），可以当作实现参考
...
当前 PRD（第 93 行）：`.rudder/memory/*.md`
...
### 4. 两文件 bootstrap 模式可复用到 Rudder 跨平台分发

jackwener/llm-wiki 解决了一个 Rudder 本来就要解决的问题：**一套 skill 跨 Claude Code + Codex**。

其模式：
- Entry: `CLAUDE.md` / `AGENTS.md` auto-load（几十行，极小）
- Skill: `.claude/skills/llm-wiki.md` + `.agents/skills/llm-wiki.md` 按需加载（完整 playbook）

**对 Rudder 的启示**：Memory plugin 的 adapter 层**不需要平台特化的业务逻辑**，只需要把同一份 skill 分发到各平台的 skills 目录。这和 Rudder 现有 Shared Templates 机制完全契合，不新增子系统。

### 5. Knowledge compounding 机制值得纳入

`/query` 写回 valuable insights 是传统 RAG 没有的能力。
- Rudder Memory plugin 建议**默认启用** compounding
...
**对 Rudder 的启示**（重要决策）：
- Rudder 本身（`rudder` CLI）**也不调 LLM**，只做资源分发 / schema 检查 / migration
- Memory plugin 如果需要 CLI 侧能力（BM25 search、graph 分析），走**独立 sub-CLI**（`rudder memory search` / `rudder memory graph`），不混进 AI 调用
- 这和 Rudder "配置生成器，runtime 在 AI CLI 里" 的哲学一致
...
## 信号汇总：Rudder Memory plugin 踩中了共识

- Karpathy 4 原则 = Rudder 气质
- 社区 12 天内涌现 N 个方案 = 需求真实存在
- **jackwener/llm-wiki 已经是接近"成品参考实现"**，Rudder Memory plugin 不需要从零设计，只需要：
  - 复用其 vault schema（sources/wiki/log/purpose/schema）
  - 复用其 4 个 slash command 语义（ingest/query/lint/research）
  - 复用其 CLAUDE.md + AGENTS.md + skill 分发模式（和 Rudder 跨平台分发天然契合）
  - 特化：scope 收敛到代码/项目域
- wiki9 + jackwener/llm-wiki 的"生成 AGENTS.md + skill 文件让 agent 直接操作"架构范式 = 和 Rudder 哲学 100% 一致
- Rowboat 的边界（work context）提醒：Rudder Memory 的定位是**项目知识**（相对稳定 → wiki 合适），不是**通用生活 work context**（需要 graph）

## 决策速记（待 brainstorm 确认）

1. Memory plugin vault 结构**直接采用 jackwener/llm-wiki schema**（不自造）
2. Skill 内容以 `jackwener/llm-wiki/skills/llm-wiki.md` 为基础改写，加 Rudder 代码场景的 MUST/NEVER 规则
3. Rudder CLI 侧不调 LLM（和 llm-wiki 一致）；如需 search/graph 走 `rudder memory <subcmd>`
4. 和 Rudder 已有 spec 体系的关系：
   - `.rudder/spec/` = 人类 curate 的规范（authoritative）
   - `.rudder/memory/` = AI 维护的项目知识 wiki
...
7. **新增（基于错误传播痛点）**：每次 ingest 原子化 commit，`wiki-log.md` 带 commit SHA，支持 `rudder memory revert`，不指望 lint 兜底
8. **新增（基于 wiki-agent.md 模式）**：`.rudder/memory/wiki-agent.md` 作为项目级 ingest filter 覆盖层，缺失时 fallback 到 `CLAUDE.md` / `AGENTS.md` bootstrap 默认
