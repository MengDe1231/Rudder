...
**Practical implication for Rudder Trace plugin:** the MCP semconv gives us a stable naming convention (`tools/call {tool_name}`, `mcp.method.name`, `mcp.session.id`) that we can safely adopt today even though the OTel label says "Development" — the schema has shipped and major SDKs are implementing it. The main risk area is trace-context propagation format, not attribute/span names.
