...
- **Query**: 除 Serena 和 Supermemory 以外，mem0 / Letta (MemGPT) / Zep / Cognee / Claude 原生 memory / 其他值得关注的 memory 产品调研，用于 Rudder memory plugin 设计参考
- **Scope**: external
- **Date**: 2026-04-20

> Rudder 已有调研：Serena（本地 md + 5 个 MCP tools，agent 主动调用）与 Supermemory（SaaS）。本文补齐剩余主流玩家，并聚焦 **self-host + 本地文件优先** 的可借鉴点。
...
> **跟 Rudder 的区别**：Letta 是「整个 agent 生命周期都住在 Letta server 里」；Rudder 是 workflow orchestrator，agent 本体住在 Claude/Cursor/OpenCode 里。Letta 的 **MemFS「git-tracked 文件 memory」** 和 **agent self-editing** 两点最值得借鉴。
...
> 这是 2025-10-28 beta 启动、**2026-04 正式 GA** 的一等公民，Rudder 必须跟它协同而不是重复造。
...
**这是 Rudder 最该关注的一层，因为它就是「本地 markdown 文件 + agent 主动调用」。**
...
> **关键洞察**：Anthropic 官方钦定的 memory 方向 = **agent 主动 CRUD 本地文件**，而不是 vector/graph embedding。这**直接印证 Rudder 的 Serena 风格设计是正确方向**。
...
- **定位**：open-source「coding agent memory layer」— 跟 Serena / Rudder 目标重叠度最高
...
> **最大借鉴价值**：ByteRover 2.0 把 vector 换成 **「hierarchical markdown tree + file-search」** —— 这跟 Rudder 气质**非常接近**（本地 .md、人类可读、git 可 diff）。可以对照他们的 tree schema。

### 6.2 Basic Memory（basicmachines-co/basic-memory）
- **定位**：最「Rudder-ish」的 memory — **本地 Markdown + MCP**
...
- **跟 Serena/Rudder 的关系**：**几乎就是 Serena 的 Markdown-first 版本 + 加了 Obsidian 互通**
...
## 给 Rudder 的建议（按「值得借鉴 / 需警惕 / 不适合」分）

### A. 值得直接借鉴 ✅

1. **方向大对路**：Anthropic 2026-04 GA 的 **Memory Tool (`memory_20250818`)** = **本地文件 + 6 个 CRUD 命令 + agent 主动调用**，**和 Rudder/Serena 哲学完全一致**。不是我们赌错了方向，而是业界主流在往这个方向收敛。
...
3. **路由表模式（root CLAUDE.md / `_index.md`）**：Reddit 社区和 Obsidian MCP 都独立地验证了「顶层有个 `when-to-read-what` 索引」是省 token + 精准召回的最佳结构。Rudder memory plugin 应该生成 / 维护一个 `research/_index.md` 或 `memory/_index.md`。
4. **Letta 的 git-tracked MemFS**：memory 文件纳入 git 是**非常自然的工程化选择**（可 diff、可 review、可回滚、可 PR review memory 修改）。Rudder 已经以 `.rudder/` 目录形式放进 repo，天然继承这个优势，**值得在文档里明说这是我们对 mem0/Cognee 的优势**。
5. **Basic Memory / Obsidian 的 vault 互通**：Rudder 的 `.rudder/research/` 和 `.rudder/spec/` 都是 .md，用户理论上可以直接挂进 Obsidian 当 vault 看。这是**零成本的差异化卖点**，可在 README 写一行。
6. **ByteRover 2.0 的 hierarchical markdown tree**：比扁平 md 文件更结构化，但不至于上 graph。对 Rudder 来说 spec 已经是树形（`cli/backend/`、`docs-site/`…），memory 也应沿用同一树形结构，保持脑图一致性。
7. **时间感知**：Graphiti 的 fact invalidation 很强，但对 Rudder 过重。**轻量化版本**：memory 文件加 `Date`、`Status: current|superseded-by: xxx.md` 这种 frontmatter 即可（Serena 风格兼容）。

### B. 需警惕 / 不完全适配 ⚠

1. **不要自己做 vector/graph**。mem0 / Zep / Cognee 都背着向量库和图库。Rudder 用户是**轻量 CLI 党**，在 repo 里跑 Neo4j + Qdrant 是**反人类**。保持「**纯 md + SQLite 索引**」上限。
2. **不要做 SaaS 托管**。Supermemory / Zep Cloud / Cognee Cloud / mem0 平台都是云，这条路 Rudder 没预算也没必要打。核心价值主张就是 self-host + git-owned。
3. **不要做 agent 运行时**。Letta 是「Rudder 的 10x 版本」，但它把整个 agent loop 抢过去；Rudder 只管 workflow + memory，让 Claude Code / Cursor / OpenCode 做 agent 本体。这条边界**必须守住**，否则和 Letta 正面冲突打不过。
...
3. **Zep Cloud / Cognee Cloud 全托管路线**：与 Rudder 脱节。
4. **Graphiti 图模型当默认**：太重，只在后期「可选 advanced backend」位置考虑。

---

## 架构建议（对照其他产品的一张「Rudder 定位图」）
...
Rudder 的坐标：**markdown tree + agent 主动调用** — 跟 Claude 原生 Memory Tool、Basic Memory、ByteRover 2.0、Letta MemFS 是同一象限，是 2026 的主流共识象限。
