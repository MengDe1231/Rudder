...
2. **降级方案**：提供 `rudder trace import-cursor` 命令，事后扒 `state.vscdb`，转成 Rudder 统一 schema
...
**结论**：Rudder 的 Trace plugin 在 Claude Code/OpenCode 上可以做**实时 JSONL** 流，在 Cursor 上最多只能做**事后 scraper**。这是平台能力的天花板差异，不是 Rudder 的问题。
