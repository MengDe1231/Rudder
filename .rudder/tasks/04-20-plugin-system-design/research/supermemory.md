...
2. **向量 + 关键词混合检索** —— 但 Rudder MVP 可用 grep / 简单 embedding 顶替
3. **云端依赖不合适 Rudder 默认路径** —— Rudder 强调"文件即真相"，云端只作为可选后端
...
- 用户在 `.rudder/plugins/memory/config.yml` 选 `backend: local | supermemory`
