...
- **Query**: OpenCode plugin mechanism — complete API for Rudder trace + memory plugin adapter
- **Scope**: mixed (internal Rudder plugins + external OpenCode docs/source)
- **Date**: 2026-04-20
...
Note: docs sometimes write `.opencode/plugin/` (singular) but current docs + source show `.opencode/plugins/` (plural). Both the Rudder repo and the public docs use the **plural** form.
...
**Starting 1.2.x** (confirmed by Rudder commit `5fbf961` + GitHub source `anomalyco/opencode/blob/7daea69e/packages/opencode/src/plugin/index.ts`), the main loader iterates `Object.entries(mod)` and invokes each export:
...
Rudder has all three plugins on this shape. See `.opencode/plugins/inject-subagent-context.js:263`, `.opencode/plugins/inject-workflow-state.js:111`, `.opencode/plugins/session-start.js:436`.
...
- `client`: SDK client; can call `client.session.messages({ path: { id: sessionID } })` etc. — used by Rudder for dedupe.
...
Rudder's `inject-subagent-context.js:332-334` explicitly notes this:
...
### `session.compacted` payload (used by Rudder)

Rudder `.opencode/plugins/session-start.js:442-455` handles it:
...
| Inject text into user message (persisted) | ✓ | `chat.message` mutate `output.parts` | Rudder uses this for session-start + breadcrumbs |
...
Important distinction for Rudder's trace/memory plugin:

- **`chat.message`** fires once per user message; mutating `output.parts` persists the mutation to the transcript (messages are saved by `updateMessage`/`updatePart` after hooks run). Rudder uses this so its breadcrumbs show up in history.
...
This is what Rudder plugins append to via `output.parts.push({ type: "text", text: "..." })`.
...
These indicate that **OpenCode has no built-in memory concept yet**, which is exactly the gap Rudder memory plugin would fill.
...
| `UserPromptSubmit` | `chat.message` | input `{ sessionID, agent?, model?, messageID? }`; output `{ message, parts }` — mutate parts | ✓ Full (Rudder uses this) |
...
### Semantic gaps to document for Rudder adapter
...
## 8. Rudder's existing usage (concrete examples)
...
- In `chat.message`:
  ...
  3. Persisted dedupe via `client.session.messages({ path: { id: sessionID } })` checking `part.metadata.rudder.sessionStart`
  4. Builds a context block (workflow state, spec paths, task status) and prepends to the first text part in `output.parts`
  5. Marks the part with `metadata.rudder.sessionStart = true`
...
### 9.3 Debug logging pattern (Rudder idiom)

```js
// .opencode/lib/rudder-context.js:15-25
const DEBUG_LOG = "/tmp/rudder-plugin-debug.log"
...
Issues relevant to Rudder adapter design:
...
## 12. Files Found (Internal — Rudder repo)
...
| `.opencode/lib/rudder-context.js` | Shared utilities (file reading, JSONL parsing, debugLog, contextCollector) |
| `packages/cli/src/templates/opencode/plugins/*.js` | Template copies (same code, distributed via Rudder CLI) |
| `packages/cli/src/migrations/manifests/0.5.0-beta.7.json` | Migration that shipped the 1.2.x factory-function fix |

Key commit:

- `5fbf961` — `fix(opencode): plugin factory-function shape for OpenCode 1.2.x compat` (Rudder). Message spells out the `for ([_, fn] of Object.entries(mod)) await fn(input)` iteration that mandates callable exports.
