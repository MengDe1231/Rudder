...
Rudder 已有 5 种 plugin-like 机制，各自覆盖不同层级。设计新 plugin 机制应**复用**而非重建。
...
**适用**：Rudder 版本升级时的文件重构。**可复用作插件升级迁移机制**。
...
**适用**：OpenCode 平台特定的运行时行为。**是 Rudder 内部唯一的"真 plugin"**，可推广到其他平台。

## 5. Spec Marketplace（Registry Pull）

**位置**：`docs-site/marketplace/specs/`

**形态**：Git 仓库 / HTTP registry，通过 `rudder init --registry <url>` 拉取自定义 spec 模板集。

**扩展方法**：在独立仓库维护 spec 集合，URL 形式发布。

**适用**：项目特定编码规范、架构指南的复用。**可扩展为 plugin marketplace**。

## Rudder 最近改动（2026-04）观察到的方向

近期 commit 重点：
- `feat(agents)!: rudder- prefix sub-agents` — 统一 agent 命名空间
- `fix: opencode update tracking` + `windows hook cwd fix` — 跨平台稳定性
- `chore: rudder self update` — dogfooding
- `fix(update): include workflow.md in template set` — template 资源管理

**趋势**：Rudder 正在往"**更强的 template 资源管理 + 更稳的平台适配**"收敛。plugin 机制如果加入，应当复用这套已有的资源分发基础设施，**不另起炉灶**。
...
每个目录背后都是现有 Rudder 子系统，plugin 机制只是**上层的打包 + 分发约定**。

## 跟"create task 自动建 research/"的关系

当前 `cmd_create`（`common/task_store.py:85`）只建 `task.json`，**不建 research/**。

research/ 是 `rudder-research` agent 的约定（`.claude/agents/rudder-research.md:36`），由 agent 自己 `mkdir -p`。
