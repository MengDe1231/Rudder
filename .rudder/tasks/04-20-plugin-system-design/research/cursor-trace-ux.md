...
> **Cursor 把 "trace" 切成了 6 块产品维度：回滚（checkpoint）、历史（chat history）、归属（Blame）、审查（Bugbot/Review）、可观测（Agents sidebar / Debug Mode）、分享（shared transcripts）。这 6 块是独立演进的，不是一个统一的 "trace" 功能——这是 Rudder 可以差异化的关键。**

### 9.2 拆解 —— 每一块的成熟度
| 产品维度 | Cursor 成熟度 | Rudder 参考价值 |
|---|---|---|
...
| ⑤ Agents Sidebar 可观测 | 中（看得到高阶状态，看不到 reasoning） | **高** —— 这是 Cursor 自己的痛点，Rudder 有机会做更好 |
...
5. **AI/human 每行 blame**（Agent Trace RFC 已经给了 schema，Rudder 是 spec-driven 工具，天然适合）。

**跳过（P2 / 不做）**：
6. **不做 Bugbot-like PR review**（太重，需要 GitHub 深度绑定 + cloud infra）。
7. **不做 shared transcript 公共链接**（需要公有服务，和 Rudder 的 local-first 定位相悖）。
8. **不做 computer-use / browser automation**（Cursor 刚开始做，非 trace 核心）。

### 9.4 一句话差异化口径
> "Cursor 的 trace 是**给用户做 undo 和 review** 的；Rudder 的 trace 应当**给用户做 spec 沉淀和 agent 工作流回放** —— 同一份 trace 数据，Cursor 用它做 blame，Rudder 用它做 spec 提炼。这是 '为什么 Rudder 不是 Cursor 的 me-too'。"
