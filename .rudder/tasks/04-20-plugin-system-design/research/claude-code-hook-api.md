...
- **Query**: Claude Code hook 事件机制与 payload schema（为 Rudder trace plugin 设计准备）
...
6. **Plugin hooks 与用户/项目 hooks 并行执行**（不是替换），不会互相覆盖。Rudder plugin 的 hooks 可以和用户自己配置的 hooks 共存。
...
- **Transcript 文件 rotation**：Claude Code 对 JSONL 是**追加**模式。没有文档说明的 rotation / cleanup 行为；issue #20612（anthropics/claude-code）曾报告过 transcript 未写入的 bug。Rudder trace plugin 如果依赖 transcript，应对"文件可能暂时缺失"容错。
...
Rudder trace plugin 要做的是 **新增一个 hook 链**（不是替换以上），因此需要和现有 hooks 并存（Claude Code 会按配置顺序执行所有匹配的 hook）。
