...
- 仓库根 `.gitignore` 忽略 `.rudder/.developer`
- `.rudder/.gitignore` 忽略 `.developer`
...
```python
cfg = parse_ini(".rudder/.developer")
...
`rudder plugin trace enable` 可以以后再补。它本质上只是帮用户改 `.developer` 的糖，不应该成为第一版的前置条件。
...
同时在 `.rudder/.gitignore` 里加：

```gitignore
.traces/
```

我不建议放到 `.rudder/workspace/{user}/traces/`。`workspace/{user}/` 现在是会进 git 的协作材料，比如 journal 和 plan；trace 则是个人原始日志，里面可能有未清洗的对话、路径、工具输出，语义上不该混在一起。
...
第一版目标很窄：session 结束时，把原始对话导出到 `.rudder/.traces/`。
...
导出当前 session
  ↓
.rudder/.traces/session-{uuid}.jsonl
```
...
但 digest 不应该自动写进 `.rudder/memory/`。这里需要一个人审闸门。
...
```text
/rudder:digest-session <session-id>
```
...
```text
.rudder/.traces/session-xxx.jsonl
  -> .rudder/.traces/digests/xxx.md
  -> .rudder/memory/sources/YYYY-MM-DD/xxx.md
  -> .rudder/memory/wiki/*.md
```
...
source: .rudder/.traces/session-xxx.jsonl
---
```
...
`rudder plugin trace enable` 不必第一版就做。
...
2. `.rudder/.gitignore` 忽略 `.traces/`。
3. Claude Code 在 session 结束时复制 transcript 到 `.rudder/.traces/`。
...
- `.rudder/.developer`：现有 per-developer 配置文件
