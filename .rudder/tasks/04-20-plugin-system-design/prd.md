# brainstorm: Rudder plugin 机制 + memory/trace 插件设计

## Goal

为 Rudder 设计一套**可选插件机制**，让用户按需启用附加能力（记忆系统、对话/代码 trace），而不把这些能力硬编码到 Rudder 核心 init/update 流程。
...
- **项目级共享知识默认复用现有 Rudder 文档面**：稳定规范进 `.rudder/spec/`，任务设计和调研进 `.rudder/tasks/<task>/`，时间线和会话总结进 `.rudder/workspace/<dev>/journal-*.md`；不默认新增 `.rudder/memory/wiki/` 这套 parallel wiki。
...
- Claude Code / Codex / OpenCode / Cursor 本身已经有原生 transcript 存储；Rudder 默认不复制完整 raw session，避免重复存储和磁盘膨胀。
- Rudder 的价值在于跨工具、跨项目、跨时间的统一 catalog / search / digest / promote，而不是替代每个工具自己的历史记录。
...
因此 raw session capture 不是 Memory MVP 的默认核心能力。默认核心是“索引 native raw + 生成 digest + promote 到现有 Rudder 知识面”。
...
**Rudder 现状**：
- [`research/rudder-existing-extension-points.md`](research/rudder-existing-extension-points.md) — Rudder 现有 5 种 plugin-like 机制盘点
...
- [`research/trace-as-memory-feedstock.md`](research/trace-as-memory-feedstock.md) — 2026-04-23 定位切换：Trace 从 observability/attribution → Memory 底座。`.developer` ini 开关 + `.rudder/.traces/` per-developer 存储 + Case 1（纯采集）/ Case 2（采集+digest）两种实现路径。**原 OTel / exporter 路线作废**，PRD 待整章重写（结论**未合入本 PRD**）
...
- **Rudder 本质是配置生成器**（init/update 后退场），真正的 runtime 在各 AI CLI 里。所以"Rudder 插件"的抽象是：**Capability Bundle，声明需要的 skill/hook/storage/plugin.js，由 Rudder 在 init 阶段分发到各平台的原生扩展点**。
- **Rudder 已有 5 种扩展点**（Platform Registry / Shared Templates / Migration / OpenCode Plugins / Spec Marketplace），plugin 机制应**复用**而非新建子系统。

**Memory 方向（调研后确认）**：

- Anthropic **2026-04 GA 的原生 Memory Tool** (`memory_20250818`) 路线与 Serena/Rudder 哲学一致：本地文件 + 6 命令（`view/create/str_replace/insert/delete/rename`）+ agent 主动调用
- 业界主流（Basic Memory / Obsidian MCP / Letta MemFS / ByteRover 2.0）都在这个象限收敛 —— **Rudder 不是赌方向，是踩中共识**
...
- **不做**：vector / graph 栈（反 Rudder 气质）、SaaS、跨线做 agent 运行时
...
Rudder 应该用**内部形态（对齐 OTel GenAI）做 source of truth**，把 agent-trace / opentraces / git-ai 作为**可插拔 exporter**，不 1:1 绑定任何规范。

存储路径**待定**（Open Question）：trace 是否与 task 绑定尚未决策。三个候选：
- (a) 全局平铺 `.rudder/traces/session-{uuid}.jsonl` — 无绑定，按时间/session id 归档
- (b) Task-scoped `.rudder/tasks/{task}/traces/session-{uuid}.jsonl` — 需要 session↔task 绑定基建（依赖 `04-21-session-scoped-task-state`）
- (c) 先平铺后归集 — 默认写 `.rudder/traces/`，提供显式 `/rudder:attach <session> <task>` 事后归档
...
- A1: 插件是 **opt-in** 的，`rudder init` 不默认启用任何插件
- A2: 插件资源在 `rudder plugin add <name>` 或 `rudder update` 时写入目标平台
- A3: 存储默认本地（`.rudder/memory/` / `.agent-trace/`），云端后端作为可选配置
...
- Q3（待答）：Trace 存储是否与 task 绑定？
  - (a) 全局平铺 `.rudder/traces/` — 最简单，零依赖
  - (b) Task-scoped `.rudder/tasks/{task}/traces/` — 语义清晰但依赖 session↔task 绑定（见 `04-21-session-scoped-task-state`）
  - (c) 平铺 + 事后 `/rudder:attach` 归集 —— 弱绑定折中
...
- [ ] `rudder plugin add/remove/list` 命令
- [ ] `.rudder/plugins/` 目录约定 + registry 远程拉取
- [ ] 插件资源写入/清理生命周期（复用 `writeMode` / migration manifest 机制）
- [ ] 插件启用状态持久化（`.rudder/config.json` 或类似）

### Memory Plugin（Claude Memory Tool 对齐版 + llm-wiki vault 形态）
- [ ] 本地存储采用 **jackwener/llm-wiki vault schema**：`.rudder/memory/{wiki-purpose.md, wiki-schema.md, wiki-log.md, wiki/, sources/YYYY-MM-DD/, _index.md}`（raw 与 derived 分层，`sources/` immutable）
- [ ] Tool schema **对齐 Claude API Memory Tool `memory_20250818`** 的 6 命令：`view` / `create` / `str_replace` / `insert` / `delete` / `rename`
- [ ] Skill 内容以 `jackwener/llm-wiki/skills/llm-wiki.md`（315 行）为基础改写，加 Rudder 代码场景的 MUST / NEVER ingest 规则
...
- [ ] **Ingest 原子化 + 可撤销**：每次 `/ingest` 作为独立 git commit，`wiki-log.md` 记录 commit SHA，`rudder memory revert <log-entry>` 原子回滚（应对 @LuckyCurveC 反馈的 ingest 错误传播）
- [ ] **项目级 Ingest Filter**：`.rudder/memory/wiki-agent.md` 可覆盖默认 MUST / MAY / NEVER 标准，缺失时 fallback 到 `CLAUDE.md` / `AGENTS.md` bootstrap 默认
...
- [ ] **内部 source of truth**：`.rudder/traces/session-{uuid}.jsonl`（默认平铺，不与 task 绑定；task 归集作为可选能力）
...
- [ ] 用户能 `rudder plugin add memory` 启用记忆，AI 对话能写入 `.rudder/memory/`
- [ ] 用户能 `rudder plugin add trace` 启用追踪，对话过程写入 `.rudder/trace/transcripts/`，commit 时生成 `.agent-trace/traces.jsonl`
- [ ] 插件在 Claude Code / OpenCode 双平台有差异化实现，且不破坏 Rudder 核心流程
- [ ] `rudder plugin remove` 可清理，残留文件不影响平台运行
...
- Migration：如果改 `.rudder/` 结构，提供 migration manifest
- 向后兼容：现有用户 `rudder update` 不受影响
...
- **首批不做**：企业级 / 跨项目 wiki（jakevin7 实测上限约 10 人团队，Rudder Memory 定位单项目单小组）
...
- **Yours** — 数据默认在 `.rudder/memory/` 本地，不上云
...
详见 `research/rudder-existing-extension-points.md`。核心：
