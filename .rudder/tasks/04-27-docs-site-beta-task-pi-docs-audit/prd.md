...
Bring the beta docs-site into sync with the current Rudder task runtime and Pi Agent support, without touching release-track documentation.
...
- Rudder no longer uses a global `.rudder/.current-task` pointer.
- Active task state is session-scoped and stored at `.rudder/.runtime/sessions/<session-key>.json`.
...
- Pi Agent is a configured platform behind `rudder init --pi`.
- Pi writes:
  ...
  - `.pi/settings.json`
- Pi is agent-capable, but it does not use Python hooks. It uses the Pi extension to read session runtime and inject Rudder context.
- `.rudder/workflow.md` and the workflow template already mention Pi in the main workflow.

## Documentation Problem

The beta docs-site still describes old global task state and old platform counts in multiple places:

- Many pages still say `.rudder/.current-task`.
- Multiple pages still say Rudder ships on 13 configured platforms.
...
- No live beta docs page claims `.rudder/.current-task` is the active task pointer.
- Beta docs explain `.rudder/.runtime/sessions/<session-key>.json` in plain user-facing language.
- Beta docs describe that active task state is per session/window instead of global.
- `rudder init --pi` appears wherever platform flags are listed.
