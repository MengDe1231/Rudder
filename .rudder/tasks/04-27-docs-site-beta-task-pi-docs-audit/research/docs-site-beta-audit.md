...
- `packages/cli/src/configurators/pi.ts`: Pi templates are written to `.pi/prompts`, `.pi/skills`, `.pi/agents`, `.pi/extensions/rudder`, and `.pi/settings.json`.
- `packages/cli/src/templates/pi/extensions/rudder/index.ts.txt`: Pi reads `.rudder/.runtime/sessions/<session-key>.json` and injects task context through extension events.
- `.rudder/workflow.md` and `packages/cli/src/templates/rudder/workflow.md`: the main workflow already includes Pi in relevant platform guidance.
- `.rudder/spec/docs-site/docs/sync-on-change.md`: platform changes must update install docs, multi-platform docs, platform pages/navigation if used, appendices, changelog, and both English/Chinese mirrors.
...
- Add `--pi` to supported `rudder init` flags.
...
- Add `.pi/` and `.rudder/.runtime/` to the generated tree.
- Rewrite the phase walkthrough where `task.py start` is described as making `.current-task`.
- Rewrite `/rudder:continue` text so it reads session runtime plus task status, not `.current-task`.
...
- Rewrite `continue` as within-session task continuation based on `.rudder/.runtime/sessions/<session-key>.json`.
...
- Clarify that Pi does not use Python hooks; its extension performs Rudder context injection.
...
- Rewrite `/rudder:continue` so it no longer says it reads `.current-task`.
...
- Add a Pi section documenting:
  - `rudder init -u your-name --pi`
  - `.pi/prompts/rudder-{name}.md`
  - `.pi/skills/{name}/SKILL.md`
  - `.pi/agents/{name}.md`
  - `.pi/extensions/rudder/index.ts`
  - `.pi/settings.json`
  - session runtime lookup through the Pi extension
- Replace `.rudder/.current-task` in gitignored/shared-state guidance with `.rudder/.runtime/`.
...
- Document Pi command path as `.pi/prompts/rudder-{name}.md`.
...
- Explain that Pi agent files receive a pull-based Rudder prelude and extension-based runtime context.
...
- Document that `.pi/extensions/rudder/index.ts` handles session start, before-agent-start context injection, and Rudder sub-agent delegation.
...
- Replace `.rudder/.current-task` with `.rudder/.runtime/sessions/<session-key>.json`.
- Add `.pi/` paths if the appendix lists platform output paths.
- Mark `.rudder/.runtime/` as gitignored runtime state.
...
- Add runtime path: `.rudder/.runtime/sessions/<session-key>.json`.
...
- The page still describes `/rudder:parallel` and worktree agents as the showcased workflow.
- Since `/rudder:parallel` was removed in 0.5, either mark this as historical or rewrite the workflow using the current task model.
...
- The page still uses `/rudder:parallel` in the step-by-step flow and summary table.
...
- `docs-site/blog/use-k8s-to-know-rudder.mdx` already has a 0.4-era warning, so its `/rudder:parallel` references can remain unless the blog is rewritten.
