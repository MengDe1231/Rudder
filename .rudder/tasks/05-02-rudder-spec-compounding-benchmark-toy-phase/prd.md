# Rudder Spec-Injection Benchmark (Phase 0a / L1)

## Goal

Empirically test the core Rudder claim — **"injecting structured project spec into a fresh agent session improves performance"** — on a public, reproducible benchmark substrate, using the same model that real users run Rudder on.
...
1. **Substrate: SWE-bench Verified subset, NOT TerminalBench-2.** TB-2 tasks have no starting repo state ("build from scratch in a fresh container"); Rudder's spec-injection pitch needs a real codebase to extract conventions from. SWE-bench Verified gives real repos, real conventions, real grader, hidden tests, external credibility.
2. **Model: Xiaomi MiMo v2.5 Pro (same model for both spec-writer and task-solver roles).** User has 1.6B free tokens. Same-model is **closer to real Rudder usage** (one developer, one model, two sessions across time). Distillation framing was abandoned because MiMo v2.5 Pro is itself SOTA-tier (SWE-bench Pro 57.2 > Opus 4.6).
3. **Driver: OpenCode (`opencode 1.14.30`), NOT Claude Agent SDK.** OpenCode is one of Rudder's officially-supported platforms — driving via OpenCode dogfoods the actual workflow rather than building a synthetic harness. All six benchmark-driver requirements verified live (see `research/opencode-driver.md`).
...
- **Spec generation (oracle, offline, once per task)**: MiMo v2.5 Pro reads the repo snapshot at the task's base commit, writes a complete Rudder-structured `.rudder/spec/` (multi-file, with `index.md`) capturing the codified conventions a new contributor would need.
- **4 arms** (each task × each arm × 3 trials = 240 attempts):
  - **A. Bare**: MiMo, no extra context beyond task prompt.
  - **B. Wrong spec**: MiMo + a Rudder spec written for a *different* task in the corpus (control: "any extra context helps?").
  - **C. Rudder spec**: MiMo + the matching `.rudder/spec/` injected via OpenCode's standard Rudder hook path. **Main treatment.**
...
- [ ] Spec-generation script that produces `.rudder/spec/` for each task using MiMo, output reproducible across re-runs (same prompt + same temp + manual diff if regen needed).
...
- Research files retained under `.rudder/tasks/<this-task>/research/` for future replay.
- If results justify, a follow-up Phase 0b (L2 compounding) task is created with this PRD as parent.
- New conventions discovered during implementation captured into `.rudder/spec/` per Rudder discipline.

## Out of Scope (Phase 0a)

- L2 compounding test (longitudinal sequential tasks on a single repo, with `rudder-update-spec` self-accumulation).
...
|---|---|---|
| C - A ≥ 5pp, CI excludes 0, AND C > D | Rudder structured-spec injection adds robust value beyond raw context dump | Greenlight Phase 0b (L2 compounding) |
| C - A ≥ 5pp but C ≈ D | Spec content matters, structure doesn't | Pivot Rudder pitch to "auto-curated context" rather than "structured spec library" |
...
| C - A < 0 OR C - B ≈ 0 | Spec injection has no real lift on SOTA model, or any context works equally | Honest null result. Reframe Rudder value prop or stop |
...
4. Generate oracle    (bench/generate_spec.py via MiMo → .rudder/spec/ tarball per task)
...
**Context**: We need an external benchmark substrate to test Rudder's spec-injection mechanism, while staying within reproducibility norms and the user's free-MiMo budget.
...
**Consequences**:
- Pros: External credibility (SWE-bench Verified), zero dollar cost (1.6B token grant), realistic Rudder dogfood (OpenCode is a supported platform), reproducible by anyone with MiMo access.
