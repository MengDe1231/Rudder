...
- **Query**: Can OpenCode (`opencode` CLI) serve as the headless agent driver for the Rudder spec-compounding benchmark (240 SWE-bench Verified attempts)?
...
- MCP tools are referenced as `mcp__<server>__<tool>` (per the user's existing `rudder-research.md` agent: `mcp__exa__*: allow`).
...
permission:
  ...
  skill: deny        # no Rudder skills loaded inside the experiment
  todowrite: allow
...
The `.opencode/` directory inside the workspace is where per-workspace `agents/`, `plugins/`, and `skills/` are picked up. To dogfood Rudder you can place your custom benchmark agent there OR in the global config at `$XDG_CONFIG_HOME/opencode/agents/`.
...
# Arms: zero | wrong | rudder | rawdump
set -euo pipefail
...
- `Rudder/.opencode/agents/rudder-research.md` — example of YAML-frontmatter agent with `permission` block + MCP `allow` rules. Mirror this pattern for `swe-bench-runner.md`.
- `Rudder/.opencode/plugins/inject-subagent-context.js`, `inject-workflow-state.js`, `session-start.js` — plugin examples showing how OpenCode plugins hook session lifecycle. Not needed for the benchmark (we want a clean room) but useful if we ever want to inject the four-arm context via a plugin instead of prompt-templating.
