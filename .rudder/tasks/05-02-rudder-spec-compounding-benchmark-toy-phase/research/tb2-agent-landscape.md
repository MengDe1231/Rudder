...
- **Query**: Map the top agent frameworks on the TB-2 leaderboard so Rudder can position itself relative to them.
- **Scope**: External (web)
- **Date**: 2026-05-02
...
- **Spec / memory layer**: OpenHands has a **"Microagents"** mechanism — small markdown files with frontmatter that activate on triggers (similar in spirit to Rudder skills/specs), and supports an `.openhands/` directory in repos. This is the closest analog to `.rudder/spec/` among major OSS coding agents. Not as central / structured as Rudder specs, but the conceptual seed exists.
...
**Conclusion**: There is a **clear positioning gap** for an explicit, structured, user-curated *project spec / coding convention* layer that lives alongside the agent. Every TB-2 framework leaks knowledge into runtime prompts (AGENTS.md generation), runtime memory (Terminus summaries), or hand-tuned runtime rules (KIRA prompt edits). None persist user-validated coding conventions in a way that compounds across tasks and sessions. Rudder's `.rudder/spec/` is meaningfully different.

The remaining caveat: Anthropic's own Claude Code (and Codex/Cursor) recommend ad-hoc CLAUDE.md / AGENTS.md / .cursorrules. Those are unstructured one-file conventions. Rudder is a **structured, multi-file, multi-domain** spec system on top of that — which is the differentiating angle.

### Which TB-2 Haiku 4.5 score should Rudder target as the baseline to beat?
...
**Recommendation**: Use **Goose @ 35.5%** as the realistic baseline-to-beat for a *non-meta-search* harness; use **Meta-Harness @ 37.6%** as the SOTA-to-beat if Rudder claims compounding-spec gives a similar advantage to outer-loop harness search. Beating Meta-Harness on Haiku 4.5 with a pure spec-injection approach would be a strong narrative ("compounding human-curated specs ≥ automated harness search at the same scale").
