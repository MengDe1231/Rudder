...
- **Specific Cause**: `workflow.md` was treated as prose with a small managed
  ...
  scripts, while `rudder update` preserved stale marker prose outside
...
| P0 | Test Coverage | Add a versioned update scenario that writes the older `.rudder/.version`, stages older pristine template hashes, checks additive config behavior, and preserves skipped user modifications | DONE |
...
- [x] Update `.rudder/spec/cli/backend/migrations.md`
- [x] Update `.rudder/spec/cli/backend/workflow-state-contract.md`
- [x] Update `.rudder/spec/guides/cross-layer-thinking-guide.md`
