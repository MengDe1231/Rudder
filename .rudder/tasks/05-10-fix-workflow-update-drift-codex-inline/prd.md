...
Fix the upgrade path where `rudder update` upgrades Codex dispatch scripts to the `codex-inline` / `codex-sub-agent` routing model but leaves an older `.rudder/workflow.md` with `[Codex]` and `[Kilo, Antigravity, Windsurf]` platform blocks. The update command must keep runtime-parsed workflow files consistent with the shipped CLI template.
...
- Fresh init on `0.5.11` is correct: `.rudder/workflow.md` contains `codex-inline` / `codex-sub-agent`.
- Upgrade path is broken: `workflow_phase.py` maps `codex` to `codex-inline`, but `.rudder/workflow.md` still lacks that platform block.
- Runtime symptom:
  ```text
  python3 ./.rudder/scripts/get_context.py --mode phase --step 2.1 --platform codex
  ```
  returns only:
  ```text
...
- `.rudder/workflow.md` is a runtime file, not only user-facing prose.
- If `workflow.md` is unmodified from the last tracked template hash, `rudder update` should replace the whole file with the current shipped template.
- If a user modified `workflow.md`, the normal modified-file conflict path can still protect those edits.

## Requirements

- `rudder update` must update `.rudder/workflow.md` as a normal template file when the installed file is hash-matched / unmodified.
...
- [ ] Test simulates an older tracked `.rudder/workflow.md` and verifies `update({ force: true })` writes the current workflow template, including `codex-inline` / `codex-sub-agent`.
- [ ] Test verifies the upgraded Codex phase extraction includes `rudder-before-dev` and does not collapse to title-only output.
...
- Relevant implementation:
  ...
  - `packages/cli/src/templates/rudder/scripts/common/workflow_phase.py`
...
- Relevant specs:
  ...
  - `.rudder/spec/cli/unit-test/integration-patterns.md`
