...
Rewrite the Chinese architecture documentation so readers understand Rudder by following one end-to-end workflow path: a user gives an instruction, Rudder determines the current phase, persists the task, selects context, injects it through hooks or platform equivalents, delegates execution, verifies results, and records the session for next time.
...
* The preferred new narrative is workflow-first: start from the user's prompt and explain why each Rudder subsystem appears at that point.
...
* Explain execution roles:
  ...
  * `rudder-check`
  * `rudder-update-spec`
  * `rudder-break-loop` where appropriate
...
* [ ] A reader can explain how a user prompt travels through Rudder from session start to finish-work.
...
* Changing Rudder runtime behavior.
