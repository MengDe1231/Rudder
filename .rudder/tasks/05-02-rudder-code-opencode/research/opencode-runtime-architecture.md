# Research: OpenCode Runtime Architecture for Rudder Code

- Query: Research OpenCode runtime architecture for Rudder Code, focusing on CLI run modes, interactive vs headless execution, JSON/event streams, session/state storage, process lifecycle, and what a future GUI app could consume.
- Scope: mixed
- Date: 2026-05-02
...
For Rudder Code, the practical split is:

1. Interactive mode: `rudder code` should spawn `opencode` with Rudder-managed project config, plugins, env, and current-task/session identity.
2. Headless mode: `rudder code run` should spawn `opencode run --format json` and capture stdout as newline-delimited event data, with Rudder adding its own run manifest and normalized event schema.
...
Existing benchmark research verified the run-mode details on `opencode 1.14.30`: `opencode run --format json --dangerously-skip-permissions <prompt>` returns clean JSONL on stdout, creates a new session unless `--session`/`--continue` is used, and starts an embedded local server per invocation unless attached to another server. See `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:14`, `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:25`, and `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:49`.

### Interactive vs Headless Execution

Interactive execution is plugin-friendly. The existing Rudder OpenCode integration depends on OpenCode plugin hooks:

- `session-start.js` injects Rudder session context on the first `chat.message` event and persists it into conversation history. It skips when `OPENCODE_NON_INTERACTIVE=1`: `packages/cli/src/templates/opencode/plugins/session-start.js:38`, `packages/cli/src/templates/opencode/plugins/session-start.js:46`, and `packages/cli/src/templates/opencode/plugins/session-start.js:62`.
...
Headless execution should not rely on chat-message injection. The current Rudder plugins deliberately suppress session-start/workflow-state injection under `OPENCODE_NON_INTERACTIVE=1`, so a deterministic Rudder runner should inject task context through explicit prompt construction, a generated runner agent, or a Rudder-owned context file rather than assuming the interactive plugin path fires.

The one piece that is already robust across modes is session identity propagation into Bash. `inject-subagent-context.js` prefixes Bash commands with `RUDDER_CONTEXT_ID` from the OpenCode session input, using POSIX export syntax or PowerShell syntax on Windows: `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:267`, `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:293`, and `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:297`.
...
Useful fields include `sessionID`, event timestamp, assistant text, tool call input/output/status/metadata, token counters, cache counters, cost, and finish reason. See `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:54`, `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:155`, and `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:159`.
...
Rudder Code should treat OpenCode event objects as raw upstream events and wrap them in a stable Rudder event envelope. A minimal GUI-consumable stream should include:

- `run.started`: Rudder run id, mode, cwd, task id, model, agent, OpenCode version, server/attach mode, env keys used but not secret values.
...
The wrapper should preserve raw OpenCode JSONL as a forensic artifact and derive normalized Rudder events as the product contract.

### Session and State Storage

OpenCode has user-visible storage commands and internal storage layouts. Rudder Code should prefer the user-visible interfaces.
...
Existing benchmark research verified that OpenCode state can be isolated by setting `XDG_DATA_HOME`, `XDG_CONFIG_HOME`, `XDG_CACHE_HOME`, and `XDG_STATE_HOME`, and that `--dir` controls the workspace independently. See `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:18`, `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:185`, and `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:200`.

Upstream source confirms storage internals are implementation detail. Current `storage.ts` writes JSON files under `Global.Path.data/storage`, runs migrations, and exposes `read`, `write`, `update`, `remove`, and `list` over key arrays: https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/storage/storage.ts. Current `session.ts` also uses a SQL-backed session table and sync events for session/message state: https://raw.githubusercontent.com/anomalyco/opencode/dev/packages/opencode/src/session/session.ts. This is a strong reason not to make Rudder GUI read OpenCode disk state directly.

For Rudder Code:

- Use XDG isolation for benchmark/headless attempts.
- Use OpenCode server/SDK or `opencode export` for GUI/session replay.
- Archive raw run JSONL, stderr logs, OpenCode export output, and a Rudder run manifest.
- Avoid coupling GUI code to `~/.local/share/opencode/storage` or any current SQLite/JSON layout.

### Process Lifecycle

A Rudder-owned runtime should model three lifecycle classes:
...
1. Single-shot headless process.
   ...
   - Set `RUDDER_CONTEXT_ID` explicitly so Rudder scripts resolve session-scoped task state.
...
2. Interactive child process.
   ...
   - Do not set `OPENCODE_NON_INTERACTIVE=1`, because existing Rudder session-start/workflow plugins skip in that mode.
...
`opencode run --attach <server>` is important because it lets Rudder reuse a warm server while still presenting a headless run command. That is likely the best GUI bridge: the GUI owns the long-lived server, while background tasks are executed through attached headless runs that still produce JSONL and session ids.

### What a Future GUI Can Consume

The future GUI should consume a Rudder runtime boundary, not OpenCode internals directly. Recommended MVP contract:
...
```json
{
  ...
  "taskDir": ".rudder/tasks/05-02-rudder-code-opencode",
  "contextId": "rudder-code-...",
...
```json
{
  ...
  "artifacts": {
    "events": "events.jsonl",
    "normalizedEvents": "rudder-events.jsonl",
    "stderr": "stderr.log",
...
- Render live progress from normalized Rudder events.
...
- `.rudder/tasks/05-02-rudder-code-opencode/prd.md` — Defines the product goal: a Rudder-owned OpenCode-backed runtime for tasks, telemetry, and a future GUI app. Relevant lines: `.rudder/tasks/05-02-rudder-code-opencode/prd.md:5`, `.rudder/tasks/05-02-rudder-code-opencode/prd.md:25`, `.rudder/tasks/05-02-rudder-code-opencode/prd.md:30`, `.rudder/tasks/05-02-rudder-code-opencode/prd.md:62`, and `.rudder/tasks/05-02-rudder-code-opencode/prd.md:83`.
- `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md` — Prior local foundation analysis recommending an OpenCode-backed wrapper/distribution rather than a fork. Relevant lines: `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md:13`, `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md:20`, `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md:74`, and `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md:83`.
- `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md` — Verified `opencode run --format json`, JSONL telemetry, XDG isolation, and permission controls on a local OpenCode install. Relevant lines: `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:14`, `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:17`, `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:175`, and `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:185`.
- `packages/cli/src/configurators/opencode.ts` — Rudder installer/update logic for `.opencode/` templates, commands, skills, and plugin assets. Relevant lines: `packages/cli/src/configurators/opencode.ts:40`, `packages/cli/src/configurators/opencode.ts:86`, and `packages/cli/src/configurators/opencode.ts:106`.
- `packages/cli/src/templates/opencode/lib/rudder-context.js` — Resolves Rudder context identity from env or OpenCode hook input, reads `.rudder/.runtime/sessions/*.json`, and loads JSONL-referenced context files. Relevant lines: `packages/cli/src/templates/opencode/lib/rudder-context.js:83`, `packages/cli/src/templates/opencode/lib/rudder-context.js:107`, `packages/cli/src/templates/opencode/lib/rudder-context.js:120`, and `packages/cli/src/templates/opencode/lib/rudder-context.js:257`.
- `packages/cli/src/templates/opencode/lib/session-utils.js` — Builds the injected Rudder session-start payload and uses OpenCode client session history for dedupe. Relevant lines: `packages/cli/src/templates/opencode/lib/session-utils.js:204`, `packages/cli/src/templates/opencode/lib/session-utils.js:227`, `packages/cli/src/templates/opencode/lib/session-utils.js:352`, and `packages/cli/src/templates/opencode/lib/session-utils.js:412`.
...
- `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js` — OpenCode `tool.execute.before` plugin for Task sub-agent context and Bash `RUDDER_CONTEXT_ID` propagation. Relevant lines: `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:23`, `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:48`, `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:267`, `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:323`, and `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:395`.
- `packages/cli/src/templates/opencode/package.json` — Declares the OpenCode plugin dependency currently installed by Rudder. Relevant line: `packages/cli/src/templates/opencode/package.json:3`.
...
- Rudder already treats OpenCode as a JS-plugin platform rather than a Python-hook platform. `.rudder/spec/cli/backend/platform-integration.md` defines the JS plugin pattern and notes OpenCode uses plugins, lib files, and package dependencies: `.rudder/spec/cli/backend/platform-integration.md:82`.
- The OpenCode configurator makes `configureOpenCode()` and `collectOpenCodeTemplates()` use the same file map, which is important for future `rudder code doctor` compatibility checks: `packages/cli/src/configurators/opencode.ts:86` and `packages/cli/src/configurators/opencode.ts:106`.
- Rudder session identity resolution is layered: `RUDDER_CONTEXT_ID`, then `OPENCODE_RUN_ID`, then hook input `sessionID`, then conversation/transcript fallbacks: `packages/cli/src/templates/opencode/lib/rudder-context.js:83`.
- Active task state is read from `.rudder/.runtime/sessions/<contextKey>.json`, not from a global mutable pointer: `packages/cli/src/templates/opencode/lib/rudder-context.js:107` and `packages/cli/src/templates/opencode/lib/rudder-context.js:120`.
...
- Existing interactive context plugins explicitly skip non-interactive mode, which should shape headless Rudder Code design: `packages/cli/src/templates/opencode/plugins/session-start.js:46` and `packages/cli/src/templates/opencode/plugins/inject-workflow-state.js:114`.
- Bash context propagation is already cross-platform-aware and should be reused by Rudder Code rather than reimplemented: `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:267`.
...
- Prior Rudder OpenCode driver research with local CLI probe: `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md`

## Related Specs

- `.rudder/workflow.md` — The Rudder task system and persistence rule require research and decisions to land in task files; current-task state is session-scoped under `.rudder/.runtime/sessions/`.
- `.rudder/spec/cli/backend/index.md` — Backend spec index; points to platform-integration and workflow-state guidance.
- `.rudder/spec/cli/backend/platform-integration.md` — Relevant because Rudder Code extends the OpenCode platform integration into a first-party runtime.
- `.rudder/spec/cli/backend/workflow-state-contract.md` — Relevant if Rudder Code changes session identity, breadcrumb injection, or runtime state mechanics.
- `.rudder/spec/guides/cross-platform-thinking-guide.md` — Relevant for process spawning, environment variables, path normalization, and Windows shell differences.
...
- `opencode run --format json` is verified locally through prior research, but the exact event schema is still OpenCode-version-sensitive. Rudder Code should record `opencode --version` in every manifest and keep a compatibility parser with raw-event passthrough.
- Existing Rudder OpenCode plugins are interactive-first. `session-start.js` and `inject-workflow-state.js` skip non-interactive mode, so headless Rudder Code must inject context explicitly instead of relying on chat-message hooks.
- Server security details need a separate implementation-phase check. Official server docs mention password/CORS configuration; Rudder GUI should bind to loopback by default and avoid exposing the local server without explicit user configuration.
