# Research: OpenCode extension and customization surfaces for Rudder Code

- Query: Research OpenCode extension and customization surfaces for Rudder Code, focusing on plugins/hooks, agents, skills, MCP config, permissions, provider config, auth handling, project-local vs global config, and version compatibility risks.
- Scope: mixed
- Date: 2026-05-02
...
|---|---|
| `packages/cli/src/configurators/opencode.ts` | Rudder OpenCode configurator; walks `packages/cli/src/templates/opencode/`, adds common commands and skills, and writes them under `.opencode/`. |
| `packages/cli/src/templates/opencode/plugins/session-start.js` | Project-local OpenCode plugin that injects Rudder SessionStart context through `chat.message`. |
| `packages/cli/src/templates/opencode/plugins/inject-workflow-state.js` | Project-local OpenCode plugin that injects Rudder workflow-state breadcrumbs on each user message. |
...
| `packages/cli/src/templates/opencode/lib/rudder-context.js` | Shared OpenCode plugin helper for Rudder project detection, session identity, active-task lookup, JSONL context loading, and script execution. |
| `packages/cli/src/templates/opencode/lib/session-utils.js` | Builds OpenCode SessionStart context from Rudder workflow, specs, active task, and current task status. |
| `packages/cli/src/templates/opencode/agents/*.md` | Rudder subagent definitions using OpenCode Markdown frontmatter (`description`, `mode`, `permission`). |
...
| `.rudder/spec/cli/backend/platform-integration.md` | Local spec describing OpenCode as the JS plugin pattern platform. |
| `.rudder/spec/cli/backend/workflow-state-contract.md` | Local spec describing OpenCode as a class-1 push-hook platform for workflow-state and subagent-context injection. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md` | Prior Rudder Code foundation research; recommends wrapper/distribution over fork. |
| `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md` | Prior benchmark-driver research; verified `opencode run`, JSONL telemetry, XDG isolation, provider config, permissions, and MCP whitelist behavior on `opencode 1.14.30`. |

### Code Patterns

#### Rudder already uses OpenCode as a project-local extension host

- `packages/cli/src/types/ai-tools.ts:167-181` registers OpenCode as `configDir: ".opencode"`, `cliFlag: "opencode"`, agent-capable, and without Python hooks. This makes OpenCode a first-class Rudder platform but with JS plugins instead of the shared Python hook family.
- `packages/cli/src/configurators/opencode.ts:47-77` walks the OpenCode template directory and maps files to `.opencode/<relative-path>` with POSIX keys. `packages/cli/src/configurators/opencode.ts:86-99` adds common Rudder commands and skills, and `packages/cli/src/configurators/opencode.ts:106-111` writes the collected files at init time.
...
- `packages/cli/src/commands/uninstall.ts:91-95` treats `.opencode/package.json` as a structured file and removes only the Rudder-owned `@opencode-ai/plugin` dependency while preserving other user dependencies.

#### Plugins and hooks

OpenCode's plugin surface is the strongest fit for Rudder Code runtime behavior:
...
- Rudder uses `chat.message` for session-start injection in `packages/cli/src/templates/opencode/plugins/session-start.js:38-86`, workflow-state injection in `packages/cli/src/templates/opencode/plugins/inject-workflow-state.js:104-151`, and event handling for compaction cleanup in `packages/cli/src/templates/opencode/plugins/session-start.js:21-36`.
- Rudder uses `tool.execute.before` to inject `RUDDER_CONTEXT_ID` into Bash commands and to rewrite Task prompts for Rudder subagents in `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:313-408`.
- Rudder relies on in-place mutation of hook output objects: `packages/cli/src/templates/opencode/plugins/inject-subagent-context.js:397-399` notes that whole-object replacement does not work for Task args because the runtime keeps a local args reference.

Rudder Code implication: keep runtime injection in project-local plugins first. Use npm plugin packaging only after the wrapper semantics stabilize, because local plugins are reviewable and versioned with the project.
...
- Rudder ships Markdown subagents with `mode: subagent` and explicit permission blocks. Example: `packages/cli/src/templates/opencode/agents/rudder-research.md:1-14`, `rudder-implement.md:1-13`, and `rudder-check.md:1-13`.
...
Rudder Code implication: define a Rudder primary agent with bounded `steps`, explicit `permission`, explicit `model`, and a narrow `permission.task` map. Keep Rudder implement/check/research as project-local Markdown subagents unless the product needs a bundled npm distribution.
...
- Rudder installs common workflow skills into `.opencode/skills` through `collectSkillTemplates(".opencode/skills", ...)` in `packages/cli/src/configurators/opencode.ts:92-98`.

Rudder Code implication: skills are the right surface for reusable instructions, not runtime control. Runtime control belongs in plugins, while skill availability should be managed with `permission.skill` per agent.
...
- Prior local benchmark research verified MCP permission names through agent permissions as `mcp__<server>__<tool>` patterns, e.g. `mcp__abcoder__*: allow` and explicit deny for other servers in `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:108-153`.

Rudder Code implication: MCP belongs in the generated/runtime `opencode.json`, with per-agent permission maps used as the enforceable allowlist. Avoid embedding MCP secrets in project-local checked-in config.
...
- Prior local benchmark research verified `--dangerously-skip-permissions` auto-approves ask cases but still honors explicit deny rules on OpenCode 1.14.30. See `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:208-217`.

Rudder Code implication: Rudder Code should not assume OpenCode is safe by default. A Rudder-owned agent should start from explicit deny/ask floors and then allow only required tools, skills, subagents, MCP tools, and external directories.
...
- Prior local benchmark research found API-key auth persisted in OpenCode's auth store and warned not to inline secrets in research or generated project files. See `.rudder/tasks/05-02-rudder-spec-compounding-benchmark-toy-phase/research/opencode-driver.md:64-107` and `443-452`.

Rudder Code implication: provider IDs, model IDs, and allowlisted providers can be generated, but credentials should stay in OpenCode auth storage or environment/file substitutions controlled by the user. Rudder Code must never read or print auth files.
...
Rudder Code implication: slash commands are useful for interactive Rudder entry points, but deterministic headless Rudder Code runs should pass explicit prompts/config rather than relying on command expansion side effects.
...
| <https://github.com/sst/opencode/issues/20725> | Prior research noted a custom-provider sub-session fallback issue; relevant if Rudder Code enables Task subagents with custom OpenAI-compatible providers. |
...
4. Rudder OpenCode plugins depend on hook names and mutable-output contracts that are less stable than config files. Local code uses `chat.message`, `event`, `tool.execute.before`, `client.session.messages`, and in-place mutation of `output.parts` / `output.args`.
5. Permission config changed in `v1.1.1`: legacy `tools` remains supported but is deprecated. New Rudder Code config should use `permission`, and doctor checks should flag legacy-only configs when they matter.
...
8. Config precedence can override Rudder assumptions: inline config and managed config load after project `.opencode` directories. A Rudder Code doctor needs to report effective config, not only generated files.
...
- `.rudder/spec/cli/backend/workflow-state-contract.md:187-206` classifies OpenCode as a class-1 push-hook platform where main-session workflow-state and subagent context use separate injection paths.
...
- I did not find an official compatibility matrix that says which `@opencode-ai/plugin` versions are supported by which `opencode-ai` CLI versions. npm latest currently matches at `1.14.31`, but local Rudder-generated files show drift.
- I did not read any auth secret files. Auth handling findings come from official docs and prior non-secret research notes.
- Official plugin docs list many hook families but do not provide a semver stability guarantee for hook payload shapes. Rudder's existing reliance on `chat.message` and mutable output objects should be treated as version-sensitive and covered by smoke tests.
