# Research: OpenCode Forkability for Rudder Code

- Query: Research whether OpenCode is easy to fork/heavily modify for a product like Rudder Code, focusing on upstream source architecture, module boundaries, build/release packaging, binary distribution, extension points vs core internals, likely high-friction areas, and a 1-5 forkability rating.
- Scope: mixed
- Date: 2026-05-02
...
- 3/5 for a narrow Rudder Code fork: rebrand the CLI/package, add Rudder runtime manifests/events, move current Rudder context injection into core, and keep upstream UI/provider/session machinery mostly intact.
...
The practical answer is: **yes, feasible for Rudder Code if the fork delta is kept narrow and explicitly owned; no, not cheap if Rudder wants to deeply reshape OpenCode internals from day one.**
...
| Path | Description |
|---|---|
| `.rudder/tasks/05-02-rudder-code-opencode/prd.md` | Rudder Code PRD; now explicitly prefers a direct OpenCode fork and defines Rudder-owned runtime/event expectations. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-runtime-architecture.md` | Prior research on OpenCode run/server/session/event surfaces and why Rudder GUI should consume a Rudder-owned runtime contract. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-extension-surfaces.md` | Prior research on plugins, hooks, agents, skills, MCP, permissions, provider/auth config, and version skew. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-distribution-feasibility.md` | Prior distribution research; wrapper-first recommendation is superseded by the user's fork-first direction, but source/package facts remain useful. |
| `.rudder/spec/cli/backend/platform-integration.md` | Rudder spec for AI platform integrations; includes OpenCode as the JS plugin pattern, with one stale note about `collectTemplates`. |
| `.rudder/spec/cli/backend/workflow-state-contract.md` | Rudder spec for workflow-state breadcrumbs and OpenCode class-1 push-hook behavior. |
| `packages/cli/src/templates/opencode/` | Current Rudder OpenCode integration: agents, skills, commands, JS plugins, and shared JS helpers. |
| `packages/cli/src/configurators/opencode.ts` | Rudder configurator that writes `.opencode/` assets and collects OpenCode templates. |
...
Implication for Rudder Code: the source has recognizable modules, but heavy core work requires understanding Effect service composition and instance lifecycle. This is maintainable if Rudder makes small, well-named changes, but expensive if Rudder tries to flatten or bypass the runtime.

### Module Boundaries That Matter

#### CLI/headless boundary

`opencode run` is the cleanest upstream source pattern for a Rudder headless runner.
...
For Rudder Code, this is high leverage: keep upstream run mechanics, but add Rudder-owned run IDs, context assembly, manifests, and normalized event output.
...
For Rudder Code GUI, this argues for adding a Rudder runtime/event route beside upstream routes, not parsing OpenCode storage or forking the whole UI immediately.
...
For Rudder Code, SDK usage still requires owning binary resolution/PATH/package install. The SDK is a client/control helper, not an alternative to runtime packaging.
...
For Rudder Code, embedding Rudder defaults in the fork is easier than fighting config precedence from the outside, but doctor/reporting must show effective config and never print secrets.

#### Agent/tool/LLM boundary

This is where heavy Rudder modifications would cost most.
...
For Rudder Code, the first core changes should be additive around these boundaries: a Rudder context builder before LLM invocation, a Rudder event normalizer after session events/CLI JSON, and a Rudder default agent. Avoid large changes to generic tool execution and provider transforms until there is a measured blocker.
...
Product implication: Rudder Code should use extension points as prototypes and compatibility shims, but move product-critical behavior into small fork-owned modules with a stable Rudder contract. Current Rudder plugin behavior is a good test fixture for what the fork must make first-class.
...
Fork implication: Rudder can reuse this machinery, but rebranding from `opencode` to `rudder-code` is not just package.json text. It touches compiled binary names, wrapper binary names, optional dependency names, Docker image names, release artifact names, Homebrew/AUR formula metadata, install docs, environment variables, server output parsing, and possibly SDK launcher assumptions.
...
3. **Core runtime coupling.** The most valuable Rudder changes live around prompt/context assembly, event stream normalization, permissions, and session state. Those touch `agent`, `llm`, `processor`, `tool`, `session`, `config`, and `server` modules.
...
5. **Config precedence.** Global, project, env, inline, managed, `.opencode`, remote, and macOS managed preference config sources can override each other. Rudder Code should own effective defaults and diagnostics rather than assuming project-local config wins.
6. **Plugin API stability.** The public plugin package is useful, but Rudder already has local evidence of version-sensitive plugin shapes. The hook surface includes experimental hooks and mutable output objects.
7. **Storage/session schema.** OpenCode now has SQLite/session tables, sync events, JSON migrations, snapshots, and message parts. A GUI should not depend on these internal layouts. A fork should expose normalized Rudder events/manifests.
...
9. **Provider/auth handling.** The provider stack carries credentials, OAuth/API auth, custom providers, and AI SDK quirks. Rudder Code must not read, print, or migrate secrets, and should avoid deep provider rewrites initially.
10. **Platform compatibility.** musl/glibc, Windows, AVX2/baseline, arm64/x64, package-manager optional dependency behavior, and postinstall/link behavior all need verification if Rudder redistributes binaries.
11. **Generated SDK/API contract.** OpenAPI/SDK generation is useful, but adding Rudder GUI-facing APIs means deciding whether Rudder extends upstream generated clients, ships a separate Rudder SDK, or wraps upstream events.
...
- `opencode run --format json` gives Rudder a proven source pattern for first headless execution.
- `opencode serve` + SDK gives Rudder a source pattern for GUI/server embedding.
- `@opencode-ai/plugin` exposes enough hooks to prototype Rudder behavior before moving it into core.
- Existing Rudder `.opencode/` templates already define the behavior Rudder Code should make first-class.
...
2. Start from a fixed upstream commit/release pair, recording both Rudder Code version and upstream base SHA in runtime diagnostics.
3. Rebrand in layers:
   - user-facing binary/package/docs first (`rudder-code`),
   - internal `OPENCODE_*` env/config/storage names only when needed,
   - avoid rewriting every internal namespace immediately.
4. Add a Rudder runtime module rather than scattering Rudder logic:
   - context builder for task/workflow/spec/session identity,
   - run manifest writer,
   - normalized event schema,
   - doctor/version report,
   - optional Rudder server routes.
...
6. Treat raw OpenCode JSON/events/storage as forensic artifacts; expose Rudder-owned events to benchmark/GUI consumers.
7. Maintain a small upstream sync checklist:
   - re-run build for current platform,
   - smoke-test `rudder-code --version`, interactive launch, `run --format json`, `serve`, SDK attach, plugin hooks, Task subagents, provider auth listing without secrets,
   - compare event schema changes,
   - inspect plugin/config/schema changes,
   - update Rudder runtime compatibility metadata.
...
**Why not 2/5:** The source is not opaque. OpenCode already has the product surfaces Rudder needs, and there are clear first-class modules for CLI, server, session, config, plugin, agent, tool, and SDK. A narrow fork can produce value without rewriting the runtime.

**Final rating: 3/5** because a narrow product fork is realistic and likely worth it for Rudder Code runtime ownership, but heavy modification will quickly become a maintenance program rather than a feature task.

### Code Patterns

- Rudder PRD now makes runtime ownership explicit: Rudder Code should be a fork, not only a plugin/wrapper (`.rudder/tasks/05-02-rudder-code-opencode/prd.md:5`, `:32`, `:34`, `:41`, `:73-79`).
- Rudder PRD already defines the stable-runtime direction: normalized events and manifests instead of GUI coupling to raw OpenCode state (`.rudder/tasks/05-02-rudder-code-opencode/prd.md:38-40`, `:83-88`, `:90-143`).
- Existing Rudder research confirms the GUI should consume a Rudder runtime boundary, not raw OpenCode internals (`.rudder/tasks/05-02-rudder-code-opencode/research/opencode-runtime-architecture.md:75-86`, `:88-108`, `:134-180`).
- Existing Rudder extension research shows current plugin behavior is version-sensitive and should become fork-owned where product-critical (`.rudder/tasks/05-02-rudder-code-opencode/research/opencode-extension-surfaces.md:38-48`, `:136-147`).
- Existing distribution research remains valid on source/package facts but not on product direction; its wrapper-first recommendation is superseded by the current fork-first PRD (`.rudder/tasks/05-02-rudder-code-opencode/research/opencode-distribution-feasibility.md:9-20`, `:219-238`).
...
| <https://opencode.ai/docs/permissions> | Official permission model; relevant to Rudder default-deny/ask policy. |

### Related Specs

- `.rudder/spec/cli/backend/platform-integration.md` - relevant to OpenCode as Rudder's JS plugin platform, session identity propagation, and sub-agent context injection. Note: prior research found one stale line claiming OpenCode has no `collectTemplates`; current code has `collectOpenCodeTemplates()`.
- `.rudder/spec/cli/backend/workflow-state-contract.md` - relevant because OpenCode is class-1 push-hook today, while Rudder Code should make workflow-state/task context first-class inside the fork.
- `.rudder/spec/cli/backend/directory-structure.md` - relevant if Rudder adds a future `rudder code` wrapper or doctor in this repo before/alongside the fork.
- `.rudder/spec/cli/backend/error-handling.md` and `.rudder/spec/cli/backend/quality-guidelines.md` - relevant for future doctor/runtime diagnostics.
- `.rudder/spec/guides/cross-platform-thinking-guide.md` - relevant to binary resolution, shell/env behavior, Windows/musl/AVX, and path handling.

## Caveats / Not Found

- Active Rudder task pointer was empty (`task.py current --source` returned none). The user gave an exact target research path, so this artifact used `.rudder/tasks/05-02-rudder-code-opencode/` as the task boundary and wrote only under that task's `research/` directory.
- No secret files were read. Auth/provider findings are based on upstream source, public docs, npm metadata, and prior non-secret Rudder research.
- This pass inspected architecture and source boundaries, not every UI route or provider implementation.
- The rating assumes Rudder wants a product fork with a narrow initial delta. If Rudder wants to replace the TUI/web UI, provider stack, and session persistence immediately, the practical forkability drops to 2/5.
