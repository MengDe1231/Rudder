...
|---|---|
| `.rudder/tasks/05-02-rudder-code-opencode/prd.md` | Current Rudder Code PRD; already assumes a direct OpenCode fork for Rudder runtime ownership. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-foundation.md` | Earlier wrapper-first analysis; useful as contrast because this research found some fork examples but still limited direct-fork product maturity. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-distribution-feasibility.md` | Prior license/package/release-cadence research; relevant to npm/package comparison and upstream fast-move risk. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-runtime-architecture.md` | Prior runtime research on `opencode run`, `serve`, SDK/SSE, storage, and GUI-consumable contract. |
| `.rudder/tasks/05-02-rudder-code-opencode/research/opencode-extension-surfaces.md` | Prior plugin/agent/MCP/permission research; relevant because the most notable non-fork derivatives use these surfaces. |
| `.rudder/spec/cli/backend/index.md` | Relevant local spec index for Rudder OpenCode platform work. |

### Code patterns

- The Rudder Code PRD has already moved from wrapper-first to fork-first: it says "Fork OpenCode and rebrand/package it as Rudder Code" and prioritizes Rudder-owned workflow/runtime behavior inside the runtime.
...
- OpenCode local integration in this repo is currently plugin/template based, not a fork. Relevant Rudder assets are installed under `.opencode/` by `packages/cli/src/configurators/opencode.ts`, with plugins under `packages/cli/src/templates/opencode/plugins/`.
...
| [`winmin/evil-opencode`](https://github.com/winmin/evil-opencode) | Confirmed GitHub fork | GitHub `isFork: true`, parent `anomalyco/opencode` | Created 2026-01-06; pushed 2026-05-01 | 219 / 20 | Security/safety-guardrail-removal positioning. Not a positive product model for Rudder, but it is a notable fork by stars. |
...
- **VS Code -> Cursor/Void/Melty-style editor forks**: The broader AI coding market accepts editor/runtime forks when they own a differentiated interface or workflow. This is stronger precedent for a Rudder-owned runtime than the direct OpenCode fork list alone.

The lesson for Rudder Code: a fork can be justified, but the product must quickly create a new owned surface. Directly renaming OpenCode is not enough; the successful adjacent examples add platform integration, orchestration, GUI/server workflow, model routing, or domain-specific distribution.

### Related specs

- `.rudder/spec/cli/backend/index.md` — entry point for CLI/backend changes.
- `.rudder/spec/cli/backend/platform-integration.md` — relevant for existing OpenCode JS plugin/platform integration.
- `.rudder/spec/cli/backend/workflow-state-contract.md` — relevant because Rudder Code would need first-class workflow-state behavior rather than project-local plugin injection.
- `.rudder/spec/guides/cross-platform-thinking-guide.md` — relevant to binary/package distribution and config path behavior.
