...
2. **窗口 B 切到 task Y** —— 写了 `.rudder/.current-task`，窗口 A 下一次 hook 注入就变成 Y，它的上下文被污染

### 根因

`.rudder/.current-task` 是**文件级全局状态**。所有窗口共享：

- `get_current_task(repo_root)` (`.rudder/scripts/common/paths.py`) 读这一个文件
...
原始方案把核心规则写成 `RUDDER_CURRENT_TASK` env var 优先。这能解决**启动 AI 前绑定 task**，但不能完整解决当前问题，也会引入多套用户心智。

根因：AI 会话里运行的 `task.py start <task>` 是子进程。子进程无法修改父进程 / AI runtime 的环境变量，所以它不能靠打印 `export RUDDER_CURRENT_TASK=...` 来改变当前会话后续 hook 看到的 task。这个限制会直接影响两个场景：
...
```text
active task = 当前 AI 会话 / 窗口正在处理的 Rudder task
```
...
```bash
python3 ./.rudder/scripts/task.py start <task>
```

它在能识别当前会话时写 session-local 状态；识别不到时失败并提示 session identity / `RUDDER_CONTEXT_ID`。不再提供 `--global` 逃生阀。
...
```json
{
  "current_task": ".rudder/tasks/04-21-session-scoped-task-state",
  "current_run": null,
  "platform": "claude",
  "session_id": "abc123",
...
- CLI commands: if no platform input exists, optionally read `RUDDER_CONTEXT_ID`; otherwise return `None`.

Important: `RUDDER_CONTEXT_ID` is a context-key override, not a separate `current-task` mechanism.
...
- In a scoped AI session: write `current_task` into `.rudder/.runtime/sessions/<session-key>.json`.
- Outside a scoped AI session: fail clearly and tell the user to provide IDE/session identity or `RUDDER_CONTEXT_ID`.
...
`RUDDER_CONTEXT_ID` or via a platform-native session environment variable
exported by the host (for example `CODEX_SESSION_ID`). Hooks can pass
`RUDDER_CONTEXT_ID` to subprocesses they launch, but cannot mutate the parent
AI runtime environment after startup. If no such signal exists, the correct
behavior is a clear failure; Rudder must not write `.rudder/.current-task`.

Example:

```text
Current task: .rudder/tasks/04-21-session-scoped-task-state
...
- OpenCode `RudderContext`
- **Claude Code `statusline.py`**

No hook or statusline script should directly read `.rudder/.current-task` after this refactor. Direct reads are the bug source.
...
- It must not independently parse `.rudder/.current-task`.

### Verification: Platform Session Identity

Verified on 2026-04-25. Full notes live in:

- `.rudder/tasks/04-21-session-scoped-task-state/research/session-identity-platform-survey.md`

Summary:

| Tier | Platforms | Reason |
|---|---|---|
| Tier 1 MVP | Claude Code, Codex, Cursor, OpenCode, Gemini, Droid, Qoder, CodeBuddy | Docs or local logs expose stable session identity or plugin session ID; current Rudder code ignores it |
| Tier 2 | Kiro, Pi, Copilot with verified `sessionId` payload | Session identity appears possible, but Rudder integration path differs or needs installed-CLI verification |
...
- OpenCode `session-start.js` received two different `sessionID`s but still injected the same global task because `RudderContext.getCurrentTask()` reads `.rudder/.current-task`.
...
- [x] `.rudder/.gitignore` 和模板 gitignore 增加 `.runtime/`

### 2. `task.py start` 改造

- [x] `task.py start <task>` 调用统一 resolver 写入当前 active context
- [x] 无 context key 时失败，不写 `.rudder/.current-task`
- [x] 移除 `--global` flag
...
- [x] OpenCode `RudderContext.getCurrentTask()` 改用同一逻辑
...
- [x] CLI scripts：支持 `RUDDER_CONTEXT_ID` 作为 context-key override，不作为 current-task 选择机制

### 5. 向后兼容

- [x] 现有 `.rudder/.current-task` 文件不再作为 active-task fallback 使用
...
| CLI shell 直接 `task.py start foo`（无 context） | 失败，提示 session identity / `RUDDER_CONTEXT_ID` |
...
- **不再使用** `.rudder/.current-task` 作为 active-task source
...
- **stale session state**：未显式 `finish` 的 `.rudder/.runtime/sessions/*.json` 可能长期残留。Mitigation：`finish` 删除当前 session runtime，`archive` 删除所有指向被归档 task 的 session file；后续可按 `last_seen_at` 加 prune。
