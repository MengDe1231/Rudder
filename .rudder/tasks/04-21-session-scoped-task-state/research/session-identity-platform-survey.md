...
Can Rudder resolve `current-task` at AI session/window scope across supported platforms, instead of using only the repo-global `.rudder/.current-task` file?
...
| Platform | Session identity in docs | Current Rudder integration | Local verification | Verdict |
|---|---|---|---|---|
...
| Kiro CLI | Yes: CLI hook events include `session_id` | Rudder currently uses Kiro agent `agentSpawn` hook for subagent context only | CLI not installed locally | Tier 2: can scope subagent hook; main-session current-task support depends on Kiro setup |
| GitHub Copilot CLI | Public hooks config examples show `timestamp`, `cwd`, tool fields, but not a guaranteed `session_id`; some newer docs/search snippets mention `sessionId` | Copilot template has hooks, but Rudder comments say sessionStart output is not reliable | CLI not installed locally | Tier 2/3: support `sessionId/session_id` if present; otherwise global fallback |
...
| Pi | Extension API has session lifecycle concepts and session manager access; Rudder Pi extension currently reads `.current-task` directly | Pi extension under `.pi/extensions/rudder` template | Local `pi --help` verified; `.pi` is not installed in this repo | Tier 2: possible through extension context, but current template needs adapter work |
| Kilo / Antigravity / Windsurf | Not verified as hook/session transports in this pass | Rudder treats these as agentless/manual workflow platforms | CLIs not installed locally | Tier 3: global fallback |

## Actual Rudder Script Tests

The repo-global pointer currently contains:

```text
.rudder/tasks/04-21-session-scoped-task-state
```
...
Finding: `statusline.py` reads `.rudder/.current-task` and ignores `session_id` / `transcript_path`.
...
Finding: Codex hook receives session identity by current docs, but Rudder does not consume it.
...
```text
rudder-test-session-a=Status: NOT READY | Task: Session 级 current-task 指针：多窗口并发不互相污染
rudder-test-session-b=Status: NOT READY | Task: Session 级 current-task 指针：多窗口并发不互相污染
```

Finding: OpenCode plugin receives `sessionID`, but `RudderContext.getCurrentTask()` still reads `.rudder/.current-task`.
...
```json
{
  ...
  "workspace_roots": ["/Users/taosu/workspace/company/mindfold/product/share-public/Rudder"],
...
6. `.rudder/.runtime/` must be gitignored. Current `.rudder/.gitignore` does not ignore it.
