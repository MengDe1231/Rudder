...
- **Platform behavior** (verified via official docs, see Research References):
  ...
  - Workaround on the user side: override in `.claude/settings.local.json` (gitignored, per-user). But Rudder's default still silently overrides on first init.
...
- **Where it's written**: `packages/cli/src/configurators/opencode.ts:77-82`
  ...
  for (const cmd of resolveCommands(ctx)) {
    files.set(`.opencode/commands/rudder/${cmd.name}.md`, cmd.content);
  }
...
- **Rudder's original intent** (per CC convention): keep `continue` + `finish-work` as commands so they don't auto-trigger; ship the rest as skills. That intent makes sense for Claude Code, but opencode's command/skill semantics differ.
...
- **Symptom**: in a fresh Claude Code session, the AI creates a task directory, then `python3 ./.rudder/scripts/task.py start <task>` fails with:
  ```text
  Error: Cannot set active task without a session identity.
  Hint: run inside an AI IDE/session that exposes session identity, or set RUDDER_CONTEXT_ID before running task.py start.
  ```
- **Root cause**: Claude Code hooks and statusLine receive `session_id` / `transcript_path` through stdin, but a later AI-run `Bash(...)` command has no hook stdin. Rudder passed `RUDDER_CONTEXT_ID` only to subprocesses launched by `session-start.py`; it did not persist that value into Claude Code's Bash environment.
...
- **Required fix**: shared `session-start.py` must append `export RUDDER_CONTEXT_ID=<context-key>` to `CLAUDE_ENV_FILE` when available. Then `task.py start/current/finish` can remain session-scoped without requiring manual user setup.
...
- **Symptom**: in OpenCode TUI 1.14.22, a freshly created task fails to start with the same session identity error:
  ...
  Hint: run inside an AI IDE/session that exposes session identity, or set RUDDER_CONTEXT_ID before running task.py start.
  ```
- **Bad workaround observed**: the model can run `RUDDER_CONTEXT_ID="test-session-$(date +%s)" python3 ... task.py start ...`, but that creates an arbitrary runtime session unrelated to the real OpenCode conversation.
...
- **Required fix**: OpenCode `tool.execute.before` must inject `RUDDER_CONTEXT_ID` before Bash commands execute, using the same JS resolver as session-start and subagent prompt injection. The command prefix must be shell-aware: POSIX shells use `export RUDDER_CONTEXT_ID=<context-key>;`, while Windows PowerShell uses `$env:RUDDER_CONTEXT_ID = '<context-key>';`.

### Issue 5 — Cursor Bash cannot run `task.py start` after task creation

- **Symptom**: Cursor Agent can create `.rudder/tasks/<task>/`, but `python3 ./.rudder/scripts/task.py start <task>` fails with the session identity error.
- **Bad workaround observed**: the model can run `RUDDER_CONTEXT_ID=manual-test python3 ... task.py start ...`, but that creates a fake session runtime file unrelated to the Cursor conversation.
- **Root cause**: Cursor hook input has `conversation_id` / `session_id`, but Cursor's ordinary shell command does not inherit SessionStart environment output. Rudder only used that identity inside hook subprocesses.
...
- **Symptom**: Pi Agent can create a task directory, but a plain `python3 ./.rudder/scripts/task.py start <task>` has no session identity. The model can only make it work by inventing `RUDDER_CONTEXT_ID=pi-test-session`, which creates a fake runtime file unrelated to the real Pi session.
- **Root cause**: Pi exposes session identity to extensions through `ctx.sessionManager.getSessionId()`, not as an ordinary Bash environment variable. Rudder's Pi extension only injected prompt context and sub-agent tools; it did not mutate Bash tool calls.
- **Required fix**: the generated `.pi/extensions/rudder/index.ts` must derive `pi_<session-id>` from `ctx.sessionManager.getSessionId()`, prefix Bash tool calls in `tool_call` with `export RUDDER_CONTEXT_ID=<context-key>;`, and pass the same env var when spawning Pi subagents.

### Issue 7 — Cursor custom sub-agent `preToolUse.updated_input` not reached

- **Symptom**: Cursor can create/start the task in the main conversation, but a spawned `rudder-implement` sub-agent does not receive the `implement.jsonl` content when the user asks it to verify a marker without using tools. The sub-agent reports `INJECTION_MISSING`.
...
- **Root cause**: Cursor 3.2.11 can emit sub-agent spawns as `tool_name: "Subagent"` while Rudder's Cursor hook config matched only `Task`; the project hook did not run. Also, native Cursor custom-subagent args can encode the agent name as protobuf-shaped objects such as `subagent_type.custom.name`, so the parser must handle more than the string form.
- **Required fix**: keep Cursor in hook-inject mode, match `Task|Subagent`, and teach `inject-subagent-context.py` to extract Rudder agent names from both string and native Cursor custom-subagent payloads.
...
1. We will **not** ask Claude Code / opencode upstream for new platform features — Rudder must adapt within current platform contracts.
2. Existing installs must not be silently broken on `rudder update`. Migration manifests should be additive (delete is OK only behind a confirm prompt).
3. The fix scope is `packages/cli/src/` only — no changes to `.rudder/scripts/` runtime, except for Issue 3 which by definition lives there.

## Open Questions (Blocking / Preference only)

### Q1 — statusLine default

For new `rudder init` runs, should `statusLine` be:
...
When users run `rudder update` after we change the default:
...
2. **Prompt during update** ("Rudder statusLine is now opt-in; remove from this project?"). *Best UX, more code.*
...
- ✅ **Approach 3**: write SKILL.md to `.agents/skills/<name>/` (codex + opencode both read it natively), keep `.opencode/commands/rudder/{continue,finish-work}.md` as opencode's manual-only slash commands, drop the redundant `.opencode/skills/` and `.codex/skills/` writes.
...
- Keep writing `.opencode/commands/rudder/{continue,finish-work}.md` (manual-only slash + supports `$ARGUMENTS`/`@file`/`!bash` placeholders).
...
- [ ] New `rudder init` no longer silently overrides user-global Claude Code statusLine.
...
- [ ] Claude Code AI-run Bash commands inherit `RUDDER_CONTEXT_ID` after SessionStart, so `task.py start` works without manual env setup.
- [ ] OpenCode AI-run Bash commands inherit `RUDDER_CONTEXT_ID` from plugin session identity on POSIX shells and Windows PowerShell, so `task.py start` works without manual env setup.
...
- [ ] Pi Agent AI-run Bash commands inherit `RUDDER_CONTEXT_ID` from extension session identity, so `task.py start` works without manual env setup.
...
- [ ] `rudder init --claude` (no other flags) does **not** write a `statusLine` field into `.claude/settings.json`.
- [ ] `rudder init --claude --with-statusline` (or equivalent post-Q1 decision) writes the existing `statusLine` block.
- [ ] `rudder init --opencode --codex` produces no duplicated `/continue` + `continue` skill in opencode.
- [ ] `rudder update` on an existing project respects the Q2 decision (no surprise removals).
...
- Manual smoke test: fresh `rudder init` on a scratch repo for both default and `--with-statusline`; both `--opencode` alone and `--opencode --codex` combinations.
...
4. Print a one-liner at end of init: `💡 To enable Rudder statusLine later, re-init with --with-statusline; or override per-machine in .claude/settings.local.json.`
