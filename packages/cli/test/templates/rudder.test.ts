...
describe("rudder template constants", () => {
  const allTemplates = {
    scriptsInit,
    commonInit,
    commonPaths,
  ...
  it("[issue-237] workflow.md in_progress breadcrumb self-exempts implement/check sub-agents", () => {
    ...
    expect(block).toContain("already running as `rudder-implement`");
    expect(block).toContain("do NOT spawn another `rudder-implement`");
    expect(block).toContain("already running as `rudder-check`");
    expect(block).toContain("do NOT spawn another `rudder-check`");
  ...
  it("[issue-237] workflow.md Phase 2 dispatch steps require prompt recursion guards", () => {
    ...
    expect(workflowMdTemplate).toContain(
      "already the `rudder-implement` sub-agent",
    );
    expect(workflowMdTemplate).toContain(
      "not spawn another `rudder-implement` / `rudder-check`",
    );
    expect(workflowMdTemplate).toContain(
      "already the `rudder-check` sub-agent",
    );
    expect(workflowMdTemplate).toContain(
      "not spawn another `rudder-check` / `rudder-implement`",
    );
  });
