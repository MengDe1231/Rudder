...
describe("pi templates", () => {
  it("provides the three Rudder sub-agent definitions", () => {
  ...
  it("settings keep Pi-owned skills until shared Agent Skills are platform-neutral", () => {
    ...
    expect(settings.extensions).toEqual(["./extensions/rudder/index.ts"]);
  ...
  it("extension injects Rudder context into Pi bash tool calls", () => {
    const extension = getExtensionTemplate();

    expect(extension).toContain("function injectRudderContextIntoBash");
    expect(extension).toContain('toolCall.toolName !== "bash"');
    expect(extension).toContain(
      "toolCall.input.command = `export RUDDER_CONTEXT_ID=",
    );
    expect(extension).toContain("function commandStartsWithRudderContext");
    expect(extension).toContain("function shellQuote");
    expect(extension).toContain(
      "injectRudderContextIntoBash(event, contextKey)",
    );
  });
  ...
  it("extension resolves Windows npm-shim Pi installs through the CLI JS entrypoint", () => {
    ...
    expect(extension).toContain("RUDDER_PI_CLI_JS points to a missing file");
  ...
  it("extension forwards Rudder context into spawned Pi subagents", () => {
    const extension = getExtensionTemplate();
    ...
    expect(extension).toContain(
      "{ ...process.env, RUDDER_CONTEXT_ID: contextKey }",
    );
  ...
  it("extension builds subagent prompts from Rudder agent context", () => {
    const extension = getExtensionTemplate();
...
`);
  ...
  it("input and before_agent_start hooks both surface workflow-state breadcrumb", () => {
    ...
    expect(extension).toContain('buildRudderContext(\n      projectRoot,\n      "rudder-implement"');
  });
