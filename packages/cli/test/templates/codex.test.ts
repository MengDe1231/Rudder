...
describe("codex shared skills (from common source)", () => {
  ...
  it("does not include platform-specific syntax in resolved output", () => {
    const skills = resolveAllAsSkills(AI_TOOLS.codex.templateContext);
    for (const skill of skills) {
      // Codex uses $ prefix, not /rudder:
      expect(skill.content).not.toContain("/rudder:");
...
// =============================================================================
//
// rudder-implement / rudder-check agent toml MUST contain a hard recursion
// guard that tells the sub-agent it is already the dispatched agent and must
// not spawn another rudder-implement / rudder-check sub-agent. Without this,
// SessionStart's "dispatch rudder-implement" guidance leaks into sub-agent
// sessions and causes infinite recursion (see PRD).
describe("codex sub-agent recursion guard (issue #234)", () => {
  for (const name of ["rudder-implement", "rudder-check"] as const) {
    it(`${name}.toml developer_instructions forbids spawning rudder-implement / rudder-check`, () => {
      ...
      expect(content).toContain("rudder-implement");
      expect(content).toContain("rudder-check");
...
describe("codex session-start.py sub-agent self-exemption (A-soft)", () => {
  ...
  it("READY-state dispatch guidance includes a sub-agent self-exemption clause", () => {
    ...
    expect(content).toMatch(/rudder-implement.*rudder-check|rudder-check.*rudder-implement/s);
