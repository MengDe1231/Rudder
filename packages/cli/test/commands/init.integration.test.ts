...
describe("init() integration", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-init-int-"));
  ...
  it("#1 creates expected directory structure with defaults", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".claude", "skills", "rudder-meta", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
  ...
  it("#3b codex platform creates skills plus .codex assets", async () => {
    ...
    // <rudder-bootstrap> notice in inject-workflow-state.py invokes
    // `$rudder-start` to load workflow context, so the skill is emitted.
    expect(
      fs.existsSync(
        path.join(tmpDir, ".agents", "skills", "rudder-start", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".agents", "skills", "rudder-continue", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".agents", "skills", "rudder-meta", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".codex", "skills", "rudder-meta", "SKILL.md"),
      ),
    ).toBe(false);
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".codex", "agents", "rudder-check.toml"),
      ),
    ).toBe(true);
    ...
    expect(trackedPaths).toContain(".agents/skills/rudder-meta/SKILL.md");
    expect(trackedPaths).toContain(
      ".agents/skills/rudder-meta/references/local-architecture/overview.md",
    );
  });
  ...
  it("#3c kiro platform creates .kiro/skills", async () => {
    ...
    // Kiro is agent-capable → rudder-start skill not emitted.
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "rudder-start", "SKILL.md"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "rudder-finish-work", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "rudder-continue", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".kiro", "skills", "rudder-check", "SKILL.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3f windsurf platform creates .windsurf/workflows", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".windsurf", "workflows", "rudder-start.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3g qoder platform creates .qoder/commands + .qoder/skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".qoder", "commands", "rudder-finish-work.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".qoder", "skills", "rudder-brainstorm", "SKILL.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3h codebuddy platform creates .codebuddy/commands/rudder", async () => {
    await init({ yes: true, codebuddy: true });

    expect(
      fs.existsSync(path.join(tmpDir, ".codebuddy", "commands", "rudder")),
    ).toBe(true);
    // CodeBuddy is agent-capable → start.md not emitted.
    expect(
      fs.existsSync(
        path.join(tmpDir, ".codebuddy", "commands", "rudder", "start.md"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".codebuddy", "commands", "rudder", "continue.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3i copilot platform creates .github/copilot hooks and discovery config", async () => {
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".github", "hooks", "rudder.json")),
    ).toBe(true);
    ...
    expect(trackedPaths).toContain(".github/hooks/rudder.json");

    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  });

  it("#3e gemini platform creates .gemini/commands/rudder", async () => {
    await init({ yes: true, gemini: true });
    expect(
      fs.existsSync(path.join(tmpDir, ".gemini", "commands", "rudder")),
    ).toBe(true);
    // Gemini is agent-capable → start.toml not emitted.
    expect(
      fs.existsSync(
        path.join(tmpDir, ".gemini", "commands", "rudder", "start.toml"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".gemini", "commands", "rudder", "finish-work.toml"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".gemini", "commands", "rudder", "continue.toml"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#3j droid platform creates commands + skills", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".factory", "commands", "rudder", "start.md"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".factory", "commands", "rudder", "finish-work.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".factory", "commands", "rudder", "continue.md"),
      ),
    ).toBe(true);
    // Skills (rudder- prefix)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".factory", "skills", "rudder-check", "SKILL.md"),
      ),
    ).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, ".claude"))).toBe(false);
    expect(fs.existsSync(path.join(tmpDir, ".cursor"))).toBe(false);
  ...
  it("#7d renders the platform Python command into generated config and logs the adaptation", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining(
        `Rudder rendered Python commands as "${expectedPythonCmd}" in generated hooks, settings, and help text`,
      ),
    );
  });
  ...
  it("#14 monorepo: writes packages section to config.yaml", async () => {
    setupPnpmWorkspace(tmpDir, [
      { rel: "packages/cli", name: "@rudder/cli" },
      { rel: "packages/docs", name: "@rudder/docs" },
    ]);
  ...
  it("#15 monorepo: bootstrap task references per-package spec paths", async () => {
    ...
    expect(taskJson.relatedFiles).toContain(".rudder/spec/core/");
    expect(taskJson.relatedFiles).toContain(".rudder/spec/ui/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.rudder/scripts/task.py finish`,
    );
    expect(prd).toContain(
      `${expectedPythonCmd} ./.rudder/scripts/task.py archive 00-bootstrap-guidelines`,
    );
  });
  ...
  it("#17 --monorepo without workspace config exits with error", async () => {
    ...
    // Should NOT create .rudder/ (early return)
    expect(fs.existsSync(path.join(tmpDir, DIR_NAMES.WORKFLOW))).toBe(false);
  });
