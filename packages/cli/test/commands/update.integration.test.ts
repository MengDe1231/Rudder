...
import { workflowMdTemplate } from "../../src/templates/rudder/index.js";
...
describe("update() integration", () => {
  ...
  /**
   * Stage a project as if an older Rudder version installed pristine template
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-update-int-"));
  ...
  it("#4d preserves user AGENTS.md without RUDDER markers by appending the managed block", async () => {
    await setupProject();
    ...
    // User has a hand-written AGENTS.md with no RUDDER:START/END markers at
  ...
  it("#18 safe-file-delete preserves user-modified deprecated file", async () => {
    ...
    const deprecatedDir = path.join(tmpDir, ".claude", "commands", "rudder");
  ...
  it("#19 safe-file-delete handles missing deprecated files without crash", async () => {
    ...
    // The manifest has safe-file-delete entries for .claude/commands/rudder/before-backend-dev.md etc.
  ...
  function stageLegacy040Project(): void {
    ...
    const legacyDir = path.join(tmpDir, ".claude", "commands", "rudder");
  ...
  function clearMigrationTarget(): void {
    fs.rmSync(path.join(tmpDir, ".claude/skills/rudder-before-dev"), {
      recursive: true,
      force: true,
    });
  }
  ...
  // diff/merge their customizations without digging through .rudder/.backup-*/.
  ...
  it("#25 backup-rename leaves inline <new-path>.backup with original content", async () => {
    ...
    const legacyPath = path.join(
      tmpDir,
      ".claude/commands/rudder/before-dev.md",
    );
    ...
    const newPath = path.join(
      tmpDir,
      ".claude/skills/rudder-before-dev/SKILL.md",
    );
  ...
  it("#26 rename-anyway does NOT leave an inline .backup (relies on project snapshot)", async () => {
    ...
    expect(fs.existsSync(newPath)).toBe(true);
    // No inline .backup — the full-project snapshot under .rudder/.backup-*
