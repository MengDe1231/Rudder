 ...
 *   no .rudder/                        → creator bootstrap task
 *   .rudder/ exists, .developer missing → joiner onboarding task
...
describe("init() joiner onboarding", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-joiner-int-"));
  ...
  /**
   * Helper: simulate a fresh clone of an existing Rudder project — `.rudder/`
  ...
  /** Helper: simulate same-dev re-init — both `.rudder/` and `.developer` exist */
  ...
  it("#2 existing .rudder/ + no .developer → joiner onboarding task created", async () => {
    simulateExistingCheckout();
    ...
    expect(prd).toContain(".rudder/spec/");
    ...
    expect(prd).toContain(
      `${expectedPythonCmd} ./.rudder/scripts/task.py list --assignee bob`,
    );
    expect(prd).toContain(`${expectedPythonCmd} ./.rudder/scripts/task.py finish`);
    expect(prd).toContain(
      `${expectedPythonCmd} ./.rudder/scripts/task.py archive 00-join-bob`,
    );
  ...
  it("#2b issue #204: existing .rudder/ but tasks/ empty → bootstrap fallback (--yes alone, no --force)", async () => {
    // Mirrors the exact reproduction in issue #204: first run aborted partway
    // after writing the .rudder/ skeleton but before creating bootstrap;
  ...
  it("#3 existing .rudder/ + .developer → no task created", async () => {
  ...
  it("#6 joiner creation failure surfaces as warning, init does not crash", async () => {
    ...
    // thwarted by making .rudder/tasks read-only right before dispatch,
  ...
  // Tests #7/#8 cover the handleReinit path — the default flow when .rudder/
  ...
  it("#7 handleReinit path: existing .rudder/ + no .developer → joiner task created", async () => {
    simulateExistingCheckout();
  ...
  it("#8 handleReinit path: existing .rudder/ + .developer → no task created", async () => {
