...
describe("cleanupEmptyDirs", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-cleanup-"));
  });
  ...
  it("[CR#1] does not delete .rudder root even if empty", () => {
    fs.mkdirSync(path.join(tmpDir, ".rudder"), { recursive: true });
    cleanupEmptyDirs(tmpDir, ".rudder");
    expect(fs.existsSync(path.join(tmpDir, ".rudder"))).toBe(true);
  });

  it("recursively cleans parent directories but stops at root", () => {
    // Create .rudder/scripts/multi_agent/ (all empty)
    fs.mkdirSync(path.join(tmpDir, ".rudder", "scripts", "multi_agent"), {
      recursive: true,
    });
    cleanupEmptyDirs(tmpDir, ".rudder/scripts/multi_agent");

    // multi_agent and scripts should be removed (both empty)
    expect(
      fs.existsSync(
        path.join(tmpDir, ".rudder", "scripts", "multi_agent"),
      ),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".rudder", "scripts")),
    ).toBe(false);
    // .rudder root must survive
    expect(fs.existsSync(path.join(tmpDir, ".rudder"))).toBe(true);
  });
...
describe("loadUpdateSkipPaths", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-skip-"));
    fs.mkdirSync(path.join(tmpDir, ".rudder"), { recursive: true });
  });
...
describe("sortMigrationsForExecution", () => {
  ...
  it("puts rename-dir before rename and delete", () => {
    const items = [
      ...
      { type: "rename-dir" as const, from: ".rudder/old", to: ".rudder/new" },
  ...
  it("sorts rename-dir by path depth (deeper first)", () => {
    const items = [
      { type: "rename-dir" as const, from: ".rudder/a", to: ".rudder/x" },
      {
        type: "rename-dir" as const,
        from: ".rudder/a/b/c",
        to: ".rudder/x/y/z",
      },
      { type: "rename-dir" as const, from: ".rudder/a/b", to: ".rudder/x/y" },
    ];
    const sorted = sortMigrationsForExecution(items);
    expect(sorted[0].from).toBe(".rudder/a/b/c"); // depth 4
    expect(sorted[1].from).toBe(".rudder/a/b"); // depth 3
    expect(sorted[2].from).toBe(".rudder/a"); // depth 2
  });
...
describe("shouldExcludeFromBackup", () => {
  ...
  it.each([
    ...
    ".rudder/.backup-2026-04-22T10-24-27/.opencode/node_modules/zod/index.js",
  ])("excludes dependency tree %s", (p) => {
    expect(shouldExcludeFromBackup(p)).toBe(true);
  });

  it.each([
    ".rudder/workspace/developer/journal-1.md",
    ".rudder/tasks/04-17-foo/prd.md",
    ".rudder/spec/cli/backend/index.md",
    ".rudder/backlog/idea.md",
    ".rudder/agent-traces/trace.jsonl",
  ...
  it("excludes previous backups", () => {
    expect(
      shouldExcludeFromBackup(".rudder/.backup-2026-04-20T01-00-00/x"),
    ).toBe(true);
  });

  it.each([
    ".claude/commands/rudder/continue.md",
    ".claude/skills/rudder-check/SKILL.md",
    ".rudder/workflow.md",
    ".rudder/scripts/get_context.py",
    ".agents/skills/rudder-check/SKILL.md",
  ...
  // after normalization, otherwise Rudder's native worktree protection
  ...
  // `rudder update --migrate`, late April 2026).
  it.each([
    ".claude\\worktrees\\feat-x\\src\\main.ts",
    ".rudder\\tasks\\04-17-foo\\prd.md",
    ".rudder\\workspace\\dev\\journal-1.md",
