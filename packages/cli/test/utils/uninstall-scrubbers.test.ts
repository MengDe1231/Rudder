...
describe("scrubHooksJson — nested schema", () => {
  it("strips rudder hook entries from a Claude-style file", () => {
    const input = {
  ...
  it("reports fullyEmpty when only rudder hooks existed", () => {
    const input = {
      hooks: {
        SessionStart: [
          {
...
describe("scrubHooksJson — flat schema", () => {
  it("strips rudder hook entries from a Cursor-style file", () => {
    const input = {
      version: 1,
      hooks: {
        preToolUse: [
...
describe("scrubPiSettings", () => {
  it("strips rudder entries and reports fullyEmpty", () => {
    const input = {
  ...
  it("preserves user-added array entries", () => {
    const input = {
      enableSkillCommands: true,
      extensions: ["./extensions/rudder/index.ts", "./extensions/my-ext"],
...
describe("scrubCodexConfigToml", () => {
  const TEMPLATE = `# Project-scoped Codex defaults for Rudder workflows.
...
`;
  ...
  it("removes the entire rudder-shipped file and reports fullyEmpty", () => {
...
`;
    ...
    expect(content).not.toContain("Rudder's SessionStart");
  });
...
`;
  ...
  it("strips the new `hooks = true` marker line (Codex 0.129+) alongside the legacy `codex_hooks` line", () => {
    const newTemplate = `# Project-scoped Codex defaults for Rudder workflows.
# Codex loads this after ~/.codex/config.toml when you work in this project.
