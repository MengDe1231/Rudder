...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-test-"));
    // Create .rudder directory for hashes file
    fs.mkdirSync(path.join(tmpDir, ".rudder"), { recursive: true });
  });
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, ".rudder", ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("returns 0 when no template directories exist", () => {
    // Create .rudder dir for saving hashes but no template files
    fs.mkdirSync(path.join(tmpDir, ".rudder"), { recursive: true });
  ...
  it("hashes files in managed directories", () => {
    // Create .rudder with a script and .claude with a command
    fs.mkdirSync(path.join(tmpDir, ".rudder", "scripts"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".rudder", "scripts", "task.py"), "print('hello')");
    ...
    const hashes = loadHashes(tmpDir);
    expect(hashes).toHaveProperty(".rudder/scripts/task.py");
    expect(hashes).toHaveProperty(".claude/commands/start.md");
  });

  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, ".rudder", "workspace"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".rudder", "workspace", "data.md"), "user data");
    fs.mkdirSync(path.join(tmpDir, ".rudder", "tasks"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".rudder", "tasks", "task.json"), "{}");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // These should be excluded
    expect(hashes).not.toHaveProperty(".rudder/workspace/data.md");
    expect(hashes).not.toHaveProperty(".rudder/tasks/task.json");
    expect(count).toBe(0);
  });

  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(path.join(tmpDir, ".rudder", "spec", "guides"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".rudder", "spec", "frontend"), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, ".rudder", "spec", "backend"), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, ".rudder", "spec", "guides", "index.md"), "# Guides");
    fs.writeFileSync(path.join(tmpDir, ".rudder", "spec", "frontend", "index.md"), "# Frontend");
    fs.writeFileSync(path.join(tmpDir, ".rudder", "spec", "backend", "index.md"), "# Backend");

    const count = initializeHashes(tmpDir);
    const hashes = loadHashes(tmpDir);

    // All spec/ files should be excluded
    expect(hashes).not.toHaveProperty(".rudder/spec/guides/index.md");
    expect(hashes).not.toHaveProperty(".rudder/spec/frontend/index.md");
    expect(hashes).not.toHaveProperty(".rudder/spec/backend/index.md");
    expect(count).toBe(0);
  });
  ...
  it("collectFiles returns POSIX-normalized paths (no backslashes)", () => {
    ...
    fs.mkdirSync(path.join(tmpDir, ".rudder", "scripts", "common"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", "scripts", "common", "task.py"),
      "print('x')",
    );
    ...
    expect(hashes).toHaveProperty(".rudder/scripts/common/task.py");
  });
  ...
  it("does not exclude generated update-spec skills from hashing", () => {
    ...
    expect(hashes).toHaveProperty(
      ".pi/skills/rudder-update-spec/SKILL.md",
    );
    expect(count).toBe(1);
  });
});
...
describe("cross-platform hash storage (POSIX keys + v2 schema)", () => {
  let tmpDir: string;
  const HASHES_REL = path.join(".rudder", ".template-hashes.json");
  ...
  it("legacy file is discarded then initializeHashes regenerates v2", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, HASHES_REL),
      JSON.stringify({ ".rudder\\config.yaml": "deadbeef" }),
    );

    // Stage some real files to pick up.
    fs.mkdirSync(path.join(tmpDir, ".rudder", "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", "scripts", "task.py"),
      "print('hello')",
    );
    ...
    expect(parsed.hashes).not.toHaveProperty(".rudder\\config.yaml");
    expect(parsed.hashes).not.toHaveProperty(".rudder/config.yaml");
    // Newly hashed file is present with POSIX key
    expect(parsed.hashes).toHaveProperty(".rudder/scripts/task.py");
  });
