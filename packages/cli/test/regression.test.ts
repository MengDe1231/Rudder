...
import {
  ...
  getAllScripts,
} from "../src/templates/rudder/index.js";
import {
...
import { RudderContext } from "../src/templates/opencode/lib/rudder-context.js";
...
describe("regression: add_session.py runtime branch context (issue-106)", () => {
  ...
  function writeRudderScripts(): void {
    const scriptsDir = path.join(tmpDir, ".rudder", "scripts");
...
`;
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", "workspace", "test-dev", "index.md"),
      indexContent,
      "utf-8",
    );
  }
  ...
  function setupSessionRepo(options?: {
    ...
    fs.mkdirSync(path.join(tmpDir, ".rudder", "workspace", "test-dev"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-22T00:00:00\n",
      "utf-8",
    );
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", "workspace", "test-dev", "journal-1.md"),
    ...
    if (options?.taskBranch || options?.taskBaseBranch) {
      const taskDir = path.join(tmpDir, ".rudder", "tasks", "issue-106");
      fs.mkdirSync(taskDir, { recursive: true });
      fs.mkdirSync(
        path.join(tmpDir, ".rudder", ".runtime", "sessions"),
        { recursive: true },
      );
      fs.writeFileSync(
        path.join(
        ...
        JSON.stringify(
          {
            current_task: ".rudder/tasks/issue-106",
            platform: "test",
          },
          null,
          2,
  ...
  function runAddSession(title: string, options?: { branch?: string }): void {
    ...
    execSync(command.join(" "), {
      cwd: tmpDir,
      encoding: "utf-8",
      env: { ...process.env, RUDDER_CONTEXT_ID: "session-a" },
    });
  }
...
describe("regression: Windows path separator (beta.12)", () => {
  it("[beta.12] isManagedPath handles Windows backslash paths", () => {
    ...
    expect(isManagedPath(".rudder\\spec\\backend")).toBe(true);
    ...
    expect(isManagedPath(".github\\hooks\\rudder.json")).toBe(true);
  });
...
describe("regression: task directory paths (0.2.14, 0.2.15, beta.13)", () => {
  it("[0.2.15] PATHS.TASKS is .rudder/tasks (not .rudder/workspace/*/tasks)", () => {
    expect(PATHS.TASKS).toBe(".rudder/tasks");
    expect(PATHS.TASKS).not.toContain("workspace");
  });

  it("[0.2.14] Claude agent templates do not contain hardcoded .rudder/workspace/*/tasks/ paths", () => {
    const agents = getClaudeAgents();
    for (const agent of agents) {
      expect(agent.content).not.toMatch(/\.rudder\/workspace\/[^/]+\/tasks\//);
    }
  });
...
describe("regression: resolve_task_dir path handling", () => {
  it("[beta.12] resolve_task_dir handles .rudder prefix", () => {
    // The function should recognize .rudder-prefixed paths as relative paths
    expect(commonTaskUtils).toContain('.startswith(".rudder")');
  });
...
describe("regression: update only configured platforms (beta.16)", () => {
  ...
  // `rudder update` silently ignored .opencode/, so CLI-side changes to
  ...
  it("[beta.8] collectPlatformTemplates returns Map for opencode (plugins + agents + lib + package.json + commands + skills)", () => {
    ...
    // fix was a plugin-shape change that couldn't be delivered via `rudder update`.
    ...
    expect(result.has(".opencode/agents/rudder-implement.md")).toBe(true);
    expect(result.has(".opencode/lib/rudder-context.js")).toBe(true);
...
describe("regression: shell to Python migration (beta.0)", () => {
  it("[beta.0] no .sh scripts remain in rudder templates", () => {
    const scripts = getAllScripts();
  ...
  it("[beta.3] getAllScripts covers every .py file in templates/rudder/scripts/", () => {
...
describe("regression: agent-session Rudder update hint", () => {
  let tmpDir: string;
  const pythonCmd = process.platform === "win32" ? "python" : "python3";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-update-hint-"));
    const scriptsDir = path.join(tmpDir, ".rudder", "scripts");
    for (const [relativePath, content] of getAllScripts()) {
      const absPath = path.join(scriptsDir, relativePath);
      fs.mkdirSync(path.dirname(absPath), { recursive: true });
    ...
    fs.mkdirSync(path.join(tmpDir, ".rudder", "tasks"), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", ".developer"),
  ...
  function runContextWithRudderOutput(
    currentVersion: string,
    rudderVersionOutput: string | null,
  ): string {
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "sys.path.insert(0, str(Path.cwd() / '.rudder' / 'scripts'))",
        "from common import session_context",
        "output = os.environ.get('RUDDER_VERSION_OUTPUT')",
        "session_context._fetch_rudder_version_output = lambda: None if output == '__NONE__' else output",
    ...
    return execSync(`${pythonCmd} ${JSON.stringify(runnerPath)}`, {
      ...
      env: {
        ...process.env,
        RUDDER_VERSION_OUTPUT: rudderVersionOutput ?? "__NONE__",
        RUDDER_CONTEXT_ID: "test-update-session",
      },
    });
  }
  ...
  it("shows a concise update hint when rudder --version reports a newer version", () => {
    const output = runContextWithRudderOutput(
      "0.5.0",
      "Rudder update available: 0.5.0 → 0.5.9\nRun: rudder update\n0.5.9",
    );

    expect(output).toContain("Rudder update available: 0.5.0 -> 0.5.9");
  ...
  it("does not show a hint when installed version is equal or newer", () => {
    expect(runContextWithRudderOutput("0.5.9", "0.5.9")).not.toContain(
      "Rudder update available",
    );
    fs.rmSync(path.join(tmpDir, ".rudder", ".runtime"), {
      recursive: true,
      force: true,
    });
    expect(runContextWithRudderOutput("0.6.0", "0.5.9")).not.toContain(
      "Rudder update available",
    );
  });

  it("silently skips the hint when rudder --version fails or version parsing fails", () => {
    expect(runContextWithRudderOutput("0.5.0", null)).not.toContain(
      "Rudder update available",
    );
    fs.rmSync(path.join(tmpDir, ".rudder", ".runtime"), {
      recursive: true,
    ...
    expect(runContextWithRudderOutput("not-a-version", "0.5.9")).not.toContain(
      "Rudder update available",
    );
  });
  ...
  it("does not burn the once-per-session marker when version lookup fails", () => {
    ...
    const output = runContextWithRudderOutput("0.5.0", "0.5.9");

    expect(output).toContain("Rudder update available: 0.5.0 -> 0.5.9");
  });

  it("uses the final rudder --version token when no update line is present", () => {
    const output = runContextWithRudderOutput("0.5.0", "0.5.9");
  ...
  it("only attempts the default text update hint once per session", () => {
    const first = runContextWithRudderOutput("0.5.0", "0.5.9");
    const second = runContextWithRudderOutput("0.5.0", "0.5.9");

    expect(first).toContain("Rudder update available: 0.5.0 -> 0.5.9");
    expect(second).not.toContain("Rudder update available");
    expect(
      fs.existsSync(
        path.join(
          tmpDir,
  ...
  it("keeps the update hint out of JSON, record, packages, and phase paths", () => {
    ...
    for (const functionName of [
      ...
      expect(
        ...
        `${functionName} should not check Rudder updates`,
...
describe("regression: issue #252 polyrepo Git context", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-polyrepo-git-"));
    const scriptsDir = path.join(tmpDir, ".rudder", "scripts");
    for (const [relativePath, content] of getAllScripts()) {
      const absPath = path.join(scriptsDir, relativePath);
      fs.mkdirSync(path.dirname(absPath), { recursive: true });
  ...
  function writeConfigYaml(content: string): void {
    fs.writeFileSync(
      path.join(tmpDir, ".rudder", "config.yaml"),
      content,
      "utf-8",
    );
  }
...
describe("regression: current-task path normalization", () => {
  ...
  const firstReplyNoticeSentence =
    "Rudder SessionStart 已注入：workflow、当前任务状态、开发者身份、git 状态、active tasks、spec 索引已加载。";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-current-task-"));
  });
  ...
  function writeLegacyCurrentTask(taskRef: string): void {
    writeProjectFile(path.join(".rudder", ".current-task"), `${taskRef}\n`);
  }

  function writeSessionContext(contextKey: string, taskRef: string): void {
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", `${contextKey}.json`),
      JSON.stringify(
        {
  ...
  function setupTaskRepo(): void {
    writeRudderScripts();
    writeProjectFile(
      path.join(".rudder", ".developer"),
      "name=test-dev\ninitialized_at=2026-03-27T00:00:00\n",
    );
    writeProjectFile(path.join(".rudder", "workflow.md"), "# Workflow\n");
    writeProjectFile(
      path.join(".rudder", "spec", "guides", "index.md"),
      "# Guides\n",
    );
    writeProjectFile(
      path.join(".rudder", "tasks", "issue-106", "task.json"),
      JSON.stringify(
        {
          title: "Issue 106 task",
          status: "in_progress",
    ...
    writeProjectFile(
      path.join(".rudder", "tasks", "issue-106", "prd.md"),
      "# PRD\n",
    );
    writeProjectFile(
  ...
  it("[session-current-task] task.py start without context key enters degraded mode (returns 0, no pointer)", () => {
    ...
    expect(output).toContain("RUDDER_CONTEXT_ID");

    // No active-task pointer written
    expect(
      fs.existsSync(path.join(tmpDir, ".rudder", ".current-task")),
    ).toBe(false);
    expect(
      fs.existsSync(path.join(tmpDir, ".rudder", ".runtime")),
    ).toBe(false);
  ...
  it("[session-current-task] task.py start writes session runtime state when RUDDER_CONTEXT_ID is set", () => {
    setupTaskRepo();
    const taskScriptPath = path.join(tmpDir, ".rudder", "scripts", "task.py");
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".rudder\\\\tasks\\\\issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ RUDDER_CONTEXT_ID: "session-a" }),
      },
    );
    ...
    expect(context.current_task).toBe(".rudder/tasks/issue-106");
    expect(
      fs.existsSync(path.join(tmpDir, ".rudder", ".current-task")),
    ).toBe(false);
  });
  ...
  it("[session-current-task] task.py finish deletes the session runtime context", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} start ${JSON.stringify(".rudder/tasks/issue-106")}`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ RUDDER_CONTEXT_ID: "session-finish" }),
      },
    );
  ...
  it("[workflow-state-r7] task.py create auto-sets session pointer when RUDDER_CONTEXT_ID is set (planning breadcrumb reachable)", () => {
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} create "r7-auto-active" --slug r7-auto --assignee test-dev`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ RUDDER_CONTEXT_ID: "r7-session" }),
      },
    );

    // Resolve the new task directory (MM-DD-r7-auto)
    const taskDir = fs
      .readdirSync(path.join(tmpDir, ".rudder", "tasks"))
    ...
    expect(context.current_task).toBe(`.rudder/tasks/${taskDir}`);
  });
  ...
  it("[workflow-state-r7] task.py create degrades silently without session identity (no .runtime side effect)", () => {
    ...
    const sessionsDir = path.join(tmpDir, ".rudder", ".runtime", "sessions");
  ...
  it("[workflow-state-r7] task.py create then task.py start is idempotent (pointer + status flip)", () => {
    ...
    expect(taskDir).toBeDefined();
    const relTaskDir = path.posix.join(".rudder", "tasks", taskDir as string);
  ...
  it("[session-current-task] task.py archive deletes runtime sessions pointing at the archived task", () => {
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "session-a.json"),
      JSON.stringify({ current_task: ".rudder/tasks/issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "session-b.json"),
      JSON.stringify({ current_task: "issue-106" }, null, 2),
    );
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "session-other.json"),
      JSON.stringify({ current_task: ".rudder/tasks/other-task" }, null, 2),
    );
  ...
  it("[task-input-contract] task.py archive accepts task name, relative path, and absolute path", () => {
    ...
    for (const name of taskNames) {
      writeProjectFile(
        path.join(".rudder", "tasks", name, "task.json"),
        JSON.stringify(
          {
            title: `Task ${name}`,
            status: "in_progress",
    ...
    execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} archive ${JSON.stringify(`.rudder/tasks/${taskNames[1]}`)} --no-commit`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv(),
    ...
    const absPath = path.join(tmpDir, ".rudder", "tasks", taskNames[2]);
    execSync(
    ...
    for (const name of taskNames) {
      expect(
        fs.existsSync(path.join(tmpDir, ".rudder", "tasks", name)),
        `task ${name} should no longer exist in active tasks/`,
      ).toBe(false);
    }

    // All three should appear under archive/<YYYY-MM>/.
    const archiveRoot = path.join(tmpDir, ".rudder", "tasks", "archive");
  ...
  it("[session-current-task] task.py finish ignores legacy .current-task when no session task is set", () => {
    setupTaskRepo();
    writeLegacyCurrentTask(".rudder/tasks/issue-106");
    const taskScriptPath = path.join(tmpDir, ".rudder", "scripts", "task.py");
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} finish`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ RUDDER_CONTEXT_ID: "session-fallback" }),
      },
    );
  ...
  it("[session-current-task] stale session task does not fall back to legacy .current-task", () => {
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "session-b.json"),
      JSON.stringify(
        { current_task: ".rudder/tasks/missing-task", platform: "test" },
        null,
        2,
      ),
    );
    ...
    const output = execSync(
      `${pythonCmd} ${JSON.stringify(taskScriptPath)} current --source`,
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: sessionEnv({ RUDDER_CONTEXT_ID: "session-b" }),
      },
    );

    expect(output).toContain("Current task: .rudder/tasks/missing-task");
  ...
  it("[session-current-task] Python session-start hooks resolve session backslash refs without stale pointer", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".rudder\\tasks\\issue-106");
    writeSessionContext("codex_session-a", ".rudder\\tasks\\issue-106");
  ...
  it("[session-current-task] Claude SessionStart persists RUDDER_CONTEXT_ID for Bash commands", () => {
    setupTaskRepo();
    ...
    expect(fs.readFileSync(envFile, "utf-8")).toContain(
      "export RUDDER_CONTEXT_ID=claude_bash-start-a",
    );
  });
  ...
  it("[session-current-task] Cursor beforeShellExecution bridges conversation_id into task.py shell commands", () => {
    ...
    const hookOutput = runPython(
      ...
      JSON.stringify({
        ...
        generation_id: "gen-a",
        cwd: tmpDir,
        command: `${pythonCmd} ./.rudder/scripts/task.py start .rudder/tasks/issue-106 && ${pythonCmd} ./.rudder/scripts/task.py current --source`,
    ...
    expect(currentOutput).toContain("Current task: .rudder/tasks/issue-106");
  ...
  it("[session-current-task] Cursor preToolUse injects context for custom Task subagents", () => {
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "cursor_parent-a.json"),
      JSON.stringify(
        {
          current_task: ".rudder/tasks/issue-106",
          current_run: null,
    ...
    expect(prompt).toContain(
      "=== .rudder/tasks/issue-106/prd.md (Requirements) ===",
    );
    expect(prompt).toContain("TOKEN_CURSOR_HOOK_TEST");
    expect(parsed.hookSpecificOutput?.updatedInput?.prompt).toBe(prompt);
  });

  it("[session-current-task] Cursor generic subagents do not receive Rudder jsonl injection", () => {
    setupTaskRepo();
    writeProjectFile(path.join(".git", "HEAD"), "ref: refs/heads/main\n");
    const injectSubagentContextScript = getSharedHookScripts().find(
      (hook) => hook.name === "inject-subagent-context.py",
  ...
  it("[session-current-task] Cursor hook uses conversation_id when transcript_path is null", () => {
    ...
    writeProjectFile(
      path.join(".rudder", "tasks", "cursor-task", "task.json"),
      JSON.stringify(
        {
          id: "cursor-task",
          title: "Cursor task",
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "cursor_cursor-a.json"),
      JSON.stringify(
        {
          current_task: ".rudder/tasks/cursor-task",
          platform: "cursor",
        },
        null,
        2,
  ...
  it("[session-current-task] OpenCode resolver ignores legacy .current-task and uses plugin sessionID", () => {
    ...
    writeLegacyCurrentTask(".rudder/tasks/issue-106");
    writeProjectFile(
      path.join(".rudder", "tasks", "opencode-task", "task.json"),
      JSON.stringify(
        {
          title: "OpenCode task",
          status: "in_progress",
    ...
    writeProjectFile(
      ...
      JSON.stringify(
        {
          current_task: ".rudder/tasks/opencode-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    const ctx = new RudderContext(tmpDir);
    ...
    expect(none.taskPath).toBe(".rudder/tasks/opencode-task");
    ...
    expect(active.taskPath).toBe(".rudder/tasks/opencode-task");
  ...
  it("[session-current-task] OpenCode resolver prefers OPENCODE_RUN_ID over plugin sessionID", () => {
    setupTaskRepo();
    writeProjectFile(
      path.join(".rudder", "tasks", "opencode-run-task", "task.json"),
      JSON.stringify(
        {
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "opencode_run-a.json"),
      JSON.stringify(
        {
          current_task: ".rudder/tasks/opencode-run-task",
          platform: "opencode",
        },
        null,
        2,
    ...
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "opencode_oc-a.json"),
      JSON.stringify(
        {
          current_task: ".rudder/tasks/issue-106",
          platform: "opencode",
    ...
    try {
      const active = new RudderContext(tmpDir).getActiveTask({
        sessionID: "oc-a",
      });

      expect(active.source).toBe("session:opencode_run-a");
      expect(active.taskPath).toBe(".rudder/tasks/opencode-run-task");
      expect(active.stale).toBe(false);
    } finally {
  ...
  it("[#248] Copilot template does not assert Copilot ignores SessionStart hook output", () => {
    ...
    // injection mechanism. The previous Rudder hook hardcoded a misleading
    // "currently ignores" claim in both the docstring and the runtime
    // systemMessage. Both must stay removed; Rudder should not re-introduce
    ...
    expect(content).not.toContain("Rudder context injected");
  ...
  it("[workflow-v2] shared session-start READY guidance requires implement/check sub-agents", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".rudder/tasks/issue-106");
  ...
  it("[rudder-hooks-env] runtime: shared hooks emit no additionalContext when RUDDER_HOOKS=0", () => {
    setupTaskRepo();
    writeSessionContext("claude_session-a", ".rudder/tasks/issue-106");
    ...
    // With RUDDER_HOOKS=0: shared hooks short-circuit with empty stdout
    ...
    // RUDDER_DISABLE_HOOKS=1 has the same effect
    const gatedAlt = runPython(
      path.join(".claude", "hooks", "session-start.py"),
      stdinPayload,
      { RUDDER_DISABLE_HOOKS: "1" },
    );
  ...
  it("[session-current-task] OpenCode context layer normalizes backslash refs for downstream plugins", () => {
    setupTaskRepo();
    writeSessionContext("opencode_oc-a", ".rudder\\tasks\\issue-106");

    const ctx = new RudderContext(tmpDir) as RudderContext & {
    ...
    expect(ctx.getCurrentTask({ sessionID: "oc-a" })).toBe(
      ".rudder/tasks/issue-106",
    );
    expect(ctx.resolveTaskDir(".rudder\\tasks\\issue-106")).toBe(
      path.join(tmpDir, ".rudder", "tasks", "issue-106"),
    );
  });
  ...
  it("[session-fallback] single session file — fallback returns its task with session-fallback source", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_parent", ".rudder/tasks/issue-106");

    const { output, status } = runTaskCurrent();
    expect(status).toBe(0);
    expect(output).toContain("Current task: .rudder/tasks/issue-106");
  ...
  it("[session-fallback] multiple session files — refuses to guess, returns none", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_a", ".rudder/tasks/issue-106");
    writeProjectFile(
      path.join(".rudder", "tasks", "other-task", "task.json"),
      JSON.stringify({ title: "other", status: "in_progress" }, null, 2),
    );
    writeSessionContext("codex_session_b", ".rudder/tasks/other-task");
  ...
  it("[session-fallback] explicit context-key match takes precedence over fallback", () => {
    setupTaskRepo();
    writeSessionContext("codex_session_explicit", ".rudder/tasks/issue-106");

    const { output, status } = runTaskCurrent({
      RUDDER_CONTEXT_ID: "codex_session_explicit",
    });
    expect(status).toBe(0);
    expect(output).toContain("Current task: .rudder/tasks/issue-106");
  ...
  function writeWorkflowStateHook(): void {
    writeProjectFile(
      path.join(".rudder", "hooks", "inject-workflow-state.py"),
  ...
  function writeWorkflowMd(body: string): void {
    writeProjectFile(path.join(".rudder", "workflow.md"), body);
  }
  ...
  it("[workflow-state] missing/empty workflow.md degrades to generic line (post-R5: no fallback dict)", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".rudder/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] CWD drift: hook finds .rudder/ when invoked from subdirectory", () => {
    setupTaskRepo();
    writeSessionContext("session_workflow-a", ".rudder/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] no_task breadcrumb emitted when no session active task exists", () => {
    writeRudderScripts();
    writeProjectFile(path.join(".rudder", ".developer"), "name=test\n");
    ...
    // routing to rudder-brainstorm content surfaces.
    writeProjectFile(
      path.join(".rudder", "workflow.md"),
      "[workflow-state:no_task]\n" +
        "No active task. Load `rudder-brainstorm` skill to start.\n" +
        "[/workflow-state:no_task]\n",
    );
    writeLegacyCurrentTask(".rudder/tasks/issue-106");
    writeWorkflowStateHook();
  ...
  it("[workflow-state] silent exit 0 when not a Rudder project (no .rudder/ dir)", () => {
    // No .rudder/ at all — hook should silently exit
    writeWorkflowStateHook();
    fs.rmSync(path.join(tmpDir, ".rudder"), { recursive: true, force: true });
    fs.mkdirSync(path.join(tmpDir, ".rudder", "hooks"), { recursive: true });
    fs.copyFileSync(
      path.join(
        __dirname,
        "..",
      ...
      path.join(tmpDir, ".rudder", "hooks", "inject-workflow-state.py"),
    );
    // Now .rudder/ exists only as a parent for the hook script — need to move
    // the hook out of .rudder/ so root-finding fails. Use a fully separate dir.
    const nonRudderDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "non-rudder-"),
    );
    try {
      const hookPath = path.join(nonRudderDir, "hook.py");
      fs.copyFileSync(
        path.join(
          __dirname,
          "..",
      ...
      const result = execSync(`${pythonCmd} ${JSON.stringify(hookPath)}`, {
        cwd: nonRudderDir,
        input: JSON.stringify({ cwd: nonRudderDir }),
        encoding: "utf-8",
      });
      expect(result.trim()).toBe("");
    } finally {
      fs.rmSync(nonRudderDir, { recursive: true, force: true });
    }
  });
  ...
  it("[workflow-v2] task.py create does NOT write legacy current_phase / next_action fields", () => {
    ...
    const tasksDir = path.join(tmpDir, ".rudder", "tasks");
    const newDirs = fs
  ...
  it("[init-context-removal] task.py init-context is deprecated with clear pointer to Phase 1.3", () => {
    ...
    try {
      execSync(
        `${pythonCmd} ${JSON.stringify(taskScriptPath)} init-context .rudder/tasks/issue-106 fullstack`,
        { cwd: tmpDir, encoding: "utf-8" },
      );
    } catch (err) {
      threw = true;
...
`;
  ...
  it("[issue-237] all implement/check agent templates contain recursion guards", () => {
    ...
    const agentFiles = [
      "claude/agents/rudder-implement.md",
      "claude/agents/rudder-check.md",
      "codebuddy/agents/rudder-implement.md",
      "codebuddy/agents/rudder-check.md",
      "codex/agents/rudder-implement.toml",
      "codex/agents/rudder-check.toml",
      "cursor/agents/rudder-implement.md",
      "cursor/agents/rudder-check.md",
      "gemini/agents/rudder-implement.md",
      "gemini/agents/rudder-check.md",
      "kiro/agents/rudder-implement.json",
      "kiro/agents/rudder-check.json",
      "opencode/agents/rudder-implement.md",
      "opencode/agents/rudder-check.md",
      "pi/agents/rudder-implement.md",
      "pi/agents/rudder-check.md",
      "qoder/agents/rudder-implement.md",
      "qoder/agents/rudder-check.md",
    ];
    ...
    for (const relativePath of agentFiles) {
      ...
      if (relativePath.includes("implement")) {
        expect(content, `${relativePath} should forbid nested implement`).toContain(
          "spawn another `rudder-implement`",
        );
  ...
  it("[issue-241-followup] codex sub-agent toml files disable collab tools at protocol level", () => {
    ...
    const codexAgentFiles = [
      "codex/agents/rudder-implement.toml",
      "codex/agents/rudder-check.toml",
      "codex/agents/rudder-research.toml",
    ];
  ...
  it("[workflow-v2] --mode phase --platform codex (sub-agent mode) filters out generic before-dev routing", () => {
    ...
    // explicitly so the legacy spawn-rudder-implement block surfaces.
    ...
    expect(output).toContain("rudder-implement");
    expect(output).not.toContain(
      "| About to write code / start implementing | rudder-before-dev |",
    );
  ...
  it("[workflow-v2] step 2.1 for codex (sub-agent mode) describes self-loaded agent context, not hook injection", () => {
    ...
    expect(output).not.toContain("Load the `rudder-before-dev` skill");
  });
  ...
  it("[workflow-v2] --mode phase --platform kilo keeps rudder-before-dev routing (agent-less path)", () => {
    // Symmetric to the codex filter test: agent-less platforms MUST still
    // see `rudder-before-dev` because they write code in the main session.
    writeRudderScripts();
    writeProjectFile(path.join(".rudder", ".developer"), "name=test\n");
    writeProjectFile(
      path.join(".rudder", "workflow.md"),
    ...
    expect(output).toContain("`rudder-before-dev`");
    expect(output).not.toContain("Dispatch the `rudder-implement` sub-agent");
  });
  ...
  it("[workflow-v2] session-start.py <guidelines> block lists spec paths, not inlined content", () => {
    ...
    writeProjectFile(
      path.join(".rudder", "spec", "cli", "backend", "index.md"),
    ...
    expect(guidelinesBlock).toContain(".rudder/spec/cli/backend/index.md");
  ...
  it("[workflow-v2] inject-subagent-context.py does NOT write current_phase when implement spawns", () => {
    ...
    writeRudderScripts();
    writeProjectFile(path.join(".rudder", ".developer"), "name=test\n");
    writeProjectFile(path.join(".rudder", "workflow.md"), "# Minimal\n");
    // Session active task WITHOUT current_phase field (post-migration state)
    writeProjectFile(
      path.join(".rudder", ".runtime", "sessions", "claude_phase-a.json"),
      JSON.stringify(
        {
          current_task: ".rudder/tasks/issue-106",
          platform: "claude",
    ...
    const input = JSON.stringify({
      tool_name: "Task",
      tool_input: {
        subagent_type: "rudder-implement",
        prompt: "do work",
      },
      cwd: tmpDir,
      session_id: "phase-a",
    ...
    const taskJson = JSON.parse(
      fs.readFileSync(
        path.join(tmpDir, ".rudder", "tasks", "issue-106", "task.json"),
        "utf-8",
      ),
  ...
  function writeConfigYaml(content: string): void {
    writeProjectFile(path.join(".rudder", "config.yaml"), content);
  }
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb defaults to inline dispatch when config absent", () => {
    ...
    writeProjectFile(
      path.join(".rudder", "workflow.md"),
      "[workflow-state:in_progress]\n" +
        "DISPATCH the rudder-implement / rudder-check sub-agents.\n" +
        "[/workflow-state:in_progress]\n" +
        "[workflow-state:in_progress-inline]\n" +
        "MAIN SESSION edits code via rudder-before-dev directly.\n" +
        "[/workflow-state:in_progress-inline]\n",
    );
    ...
    expect(ctx).not.toContain("DISPATCH the rudder-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to plain status when codex.dispatch_mode=sub-agent", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("DISPATCH the rudder-implement");
  ...
  it("[issue-codex-dispatch-mode] codex breadcrumb routes to inline tag when codex.dispatch_mode=inline", () => {
    ...
    const ctx = parsed.hookSpecificOutput.additionalContext;
    expect(ctx).toContain("MAIN SESSION edits code");
    expect(ctx).toContain("rudder-before-dev");
    expect(ctx).not.toContain("DISPATCH the rudder-implement");
  });
  ...
  it("[issue-codex-dispatch-mode] get_context.py --platform codex swaps to inline block content", () => {
    ...
    // it tells the main session to load rudder-before-dev directly.
    expect(output).toContain("rudder-before-dev");
  ...
  it("[issue-codex-dispatch-mode] inline `#` comment after value is stripped (config.yaml uncomment leaves trailing hint)", () => {
    ...
    writeRudderScripts();
    writeProjectFile(
      path.join(".rudder", "hooks", "rudder_config.py"),
      expectTemplateContent(
        getAllScripts().get("common/rudder_config.py") ?? "",
        "rudder_config",
      ),
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "from pathlib import Path",
        `hook_path = Path(${JSON.stringify(
          path.join(tmpDir, ".rudder", "hooks", "rudder_config.py"),
        )})`,
  ...
  it("[issue-codex-dispatch-mode] resolve_effective_platform namespaces codex into codex-sub-agent / codex-inline", () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import sys, json",
        `sys.path.insert(0, ${JSON.stringify(path.join(tmpDir, ".rudder", "scripts"))})`,
...
describe("regression: cli_adapter platform support (beta.9, beta.13, beta.16)", () => {
  ...
  it("[pi] cli_adapter.py supports pi platform", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".pi/prompts/rudder-{name}.md"',
    );
  });
  ...
  it("[droid] cli_adapter.py treats droid as commands-only (no CLI run/resume yet)", () => {
    ...
    expect(commonCliAdapter).toContain(
      'return f".factory/commands/rudder/{name}.md"',
    );
  });

  it("[droid] cli_adapter.py has explicit droid branches in all key methods", () => {
    expect(commonCliAdapter).toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "droid":[\s\S]*?\.factory\/commands\/rudder\//,
    );
    expect(commonCliAdapter).toMatch(
  ...
  // get_rudder_command_path were missing the `rudder-` prefix that
  // 0.5.0-beta.0 introduced via 60+ rename manifest entries. Without the
  // prefix, any caller that built skill paths via get_rudder_command_path
  // (add-context, check agent prelude, etc.) would produce paths that don't
  // resolve to any real skill file.
  it("[migrate-flow-bugs] get_rudder_command_path codex branch uses rudder- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "codex":[\s\S]*?return f"\.agents\/skills\/rudder-\{name\}\/SKILL\.md"/,
    );
    expect(commonCliAdapter).not.toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "codex":[\s\S]*?return f"\.agents\/skills\/\{name\}\/SKILL\.md"/,
    );
  });

  it("[migrate-flow-bugs] get_rudder_command_path kiro branch uses rudder- prefix", () => {
    expect(commonCliAdapter).toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "kiro":[\s\S]*?return f"\.kiro\/skills\/rudder-\{name\}\/SKILL\.md"/,
    );
    expect(commonCliAdapter).not.toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "kiro":[\s\S]*?return f"\.kiro\/skills\/\{name\}\/SKILL\.md"/,
    );
  });
  ...
  it("[migrate-flow-bugs] detect_platform has codex shared-skills fallback guarded by no-other-platform-dir check", () => {
    // Fallback fires when .agents/skills/rudder-* exists AND no other
    ...
    expect(commonCliAdapter).toMatch(/entry\.name\.startswith\("rudder-"\)/);
  });
  ...
  it("[migrate-flow-bugs] 0.5.0-beta.0 manifest has migrationGuide + aiInstructions (back-filled)", () => {
    ...
    expect(manifest.migrationGuide).toMatch(/rudder-/); // skill renames
  ...
  it("[copilot] cli_adapter.py has explicit copilot branches in all key methods", () => {
    ...
    expect(commonCliAdapter).toMatch(
      /def get_rudder_command_path[\s\S]*?elif self\.platform == "copilot":[\s\S]*?\.github\/prompts\//,
    );
    expect(commonCliAdapter).toMatch(
...
describe("regression: migration manifest consistency", () => {
  ...
  it("[#57] shell archive migrations use rename type with correct from/to paths", () => {
    ...
    for (const m of shellArchives) {
      ...
      expect(m.from).toMatch(/\.rudder\/scripts\/.*\.sh$/);
      expect(m.to).toMatch(/\.rudder\/scripts-shell-archive\/.*\.sh$/);
  ...
  it("[0.2.14] command namespace migration renames exist", () => {
    ...
    // Should include commands moved to rudder/ subdirectory
  ...
  describe("[0.5.0-beta.0] command→skill rename coverage", () => {
    ...
    const PLATFORMS: { id: string; from: PathFn; to: PathFn }[] = [
      {
        id: "claude",
        from: (n) => `.claude/commands/rudder/${n}.md`,
        to: (n) => `.claude/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "cursor",
        from: (n) => `.cursor/commands/rudder-${n}.md`,
        to: (n) => `.cursor/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "opencode",
        from: (n) => `.opencode/commands/rudder/${n}.md`,
        to: (n) => `.opencode/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "codebuddy",
        from: (n) => `.codebuddy/commands/rudder/${n}.md`,
        to: (n) => `.codebuddy/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "droid",
        from: (n) => `.factory/commands/rudder/${n}.md`,
        to: (n) => `.factory/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "gemini",
        from: (n) => `.gemini/commands/rudder/${n}.toml`,
        to: (n) => `.gemini/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "copilot",
        from: (n) => `.github/prompts/${n}.prompt.md`,
        to: (n) => `.github/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "kilo",
        from: (n) => `.kilocode/workflows/${n}.md`,
        to: (n) => `.kilocode/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "antigravity",
        from: (n) => `.agent/workflows/${n}.md`,
        to: (n) => `.agent/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "windsurf",
        from: (n) => `.windsurf/workflows/rudder-${n}.md`,
        to: (n) => `.windsurf/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "kiro",
        from: (n) => `.kiro/skills/${n}/SKILL.md`,
        to: (n) => `.kiro/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "qoder",
        from: (n) => `.qoder/skills/${n}/SKILL.md`,
        to: (n) => `.qoder/skills/rudder-${n}/SKILL.md`,
      },
      {
        id: "shared",
        from: (n) => `.agents/skills/${n}/SKILL.md`,
        to: (n) => `.agents/skills/rudder-${n}/SKILL.md`,
      },
    ];
    ...
    it("has at least 65 rename entries for command→skill (13 platforms × 5 commands)", () => {
      ...
      // relocation under rudder- namespace on skill-only platforms).
    ...
    it("breaking + recommendMigrate flags are both set (drives gate)", () => {
      ...
      // by running `rudder update` without `--migrate`.
...
describe("regression: collectTemplates paths match init directory structure (0.3.1)", () => {
  it("[0.3.1] all platforms with commands use consistent rudder/ subdirectory", () => {
    ...
    for (const id of platformsWithCommands) {
      ...
      for (const key of commandKeys) {
        expect(
          key,
          `${id} command path should include rudder/ subdirectory: ${key}`,
        ).toContain("/commands/rudder/");
      }
    }
  });
  ...
  it("[windsurf] windsurf uses workflows/ instead of commands/rudder/", () => {
  ...
  it("[copilot] collectTemplates tracks hooks and VS Code discovery config", () => {
    ...
    expect(keys).toContain(".github/hooks/rudder.json");
  });
});
...
describe("regression: parse_simple_yaml uses _unquote not greedy strip (0.3.8)", () => {
  ...
  it("config.py uses _unquote for key-value, not .strip('\"')", () => {
    ...
    // mirrors rudder_config.py so YAML `key: false  # comment` parses
...
describe("regression: parse_simple_yaml Python execution (0.3.8)", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-yaml-py-"));
...
describe("regression: class-2 platforms use pull-based sub-agent context", () => {
  ...
  const class2 = [
    {
      ...
      nonPreludeAgents: [".qoder/agents/rudder-research.md"],
    },
    {
      id: "gemini" as const,
      ...
      nonPreludeAgents: [".gemini/agents/rudder-research.md"],
    },
    {
      id: "codex" as const,
      ...
      nonPreludeAgents: [".codex/agents/rudder-research.toml"],
    },
    {
      id: "copilot" as const,
      hooksDir: ".github/copilot/hooks",
      preludeAgents: [
        ".github/agents/rudder-implement.agent.md",
        ".github/agents/rudder-check.agent.md",
      ],
      nonPreludeAgents: [".github/agents/rudder-research.agent.md"],
    },
  ];
  ...
  for (const { id, hooksDir, preludeAgents, nonPreludeAgents } of class2) {
    describe(`[${id}]`, () => {
      ...
      beforeEach(async () => {
        tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `rudder-c2-${id}-`));
        setWriteMode("force");
      ...
      it("implement/check definitions contain pull-based prelude", () => {
        for (const file of preludeAgents) {
          ...
          expect(content).toContain("Required: Load Rudder Context First");
      ...
      it("research definition does NOT contain pull-based prelude", () => {
        // research is orthogonal: it searches .rudder/spec/ and doesn't
        ...
        for (const file of nonPreludeAgents) {
          const content = fs.readFileSync(path.join(tmpDir, file), "utf-8");
          expect(content).not.toContain("Required: Load Rudder Context First");
        }
      });
...
describe("regression: copilot agents use YAML tools frontmatter", () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-copilot-tools-"));
    setWriteMode("force");
  ...
  it("writes Copilot agent tools as YAML lists", () => {
    const content = fs.readFileSync(
      path.join(tmpDir, ".github/agents/rudder-implement.agent.md"),
      "utf-8",
    );
  ...
  it("maps research agent MCP tools to Copilot tool names", () => {
    const content = fs.readFileSync(
      path.join(tmpDir, ".github/agents/rudder-research.agent.md"),
      "utf-8",
    );
    const frontmatter = content.split("---\n")[1] ?? "";
  ...
  it("collectPlatformTemplates matches written Copilot agent output", () => {
    ...
    const generated = fs.readFileSync(
      path.join(tmpDir, ".github/agents/rudder-check.agent.md"),
      "utf-8",
    );
    expect(templates?.get(".github/agents/rudder-check.agent.md")).toBe(
      generated,
    );
  });
});
...
describe("regression: pi uses TypeScript extension assets instead of Python hooks", () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-pi-"));
    setWriteMode("force");
  ...
  it("installs a subagent-capable extension and pull-based agent context", () => {
    const extension = fs.readFileSync(
      path.join(tmpDir, ".pi", "extensions", "rudder", "index.ts"),
      "utf-8",
    );
    ...
    for (const agent of ["rudder-implement.md", "rudder-check.md"]) {
      const content = fs.readFileSync(
...
describe("regression: research agent persists findings to task dir", () => {
  ...
  const markdownPlatforms = [
    "packages/cli/src/templates/claude/agents/rudder-research.md",
    "packages/cli/src/templates/cursor/agents/rudder-research.md",
    "packages/cli/src/templates/qoder/agents/rudder-research.md",
    "packages/cli/src/templates/codebuddy/agents/rudder-research.md",
    "packages/cli/src/templates/droid/droids/rudder-research.md",
  ];
  ...
  // platforms accept (Zod expects an array or omission). Rudder omits the
  ...
  it("[packages/cli/src/templates/gemini/agents/rudder-research.md] omits tools line + has persist instruction", () => {
    const rel = "packages/cli/src/templates/gemini/agents/rudder-research.md";
    const content = fs.readFileSync(path.join(repoRoot, rel), "utf-8");
    const fm = content.split("---\n")[1] ?? "";
  ...
  it("codex research.toml uses workspace-write sandbox and persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/codex/agents/rudder-research.toml",
      ),
      "utf-8",
    );
  ...
  it("kiro research.json includes write tool and persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/kiro/agents/rudder-research.json",
      ),
      "utf-8",
    );
  ...
  it("opencode research.md grants write/edit permission and has persist instruction", () => {
    const content = fs.readFileSync(
      path.join(
        repoRoot,
        "packages/cli/src/templates/opencode/agents/rudder-research.md",
      ),
      "utf-8",
    );
    const fm = content.split("---\n")[1] ?? "";
...
describe("regression: templates/markdown/spec contains only .md.txt files (0.5.0-beta.9)", () => {
  ...
  // .rudder/spec/cli/backend/directory-structure.md "Don't: Leak dogfood
...
describe("regression: Gemini CLI 0.40.x template compatibility (#224)", () => {
  ...
  it("[#224] gemini agent .md files do NOT carry a comma-separated tools line", () => {
    ...
    // "tools: Expected array, received string". Rudder omits the line so
  ...
  it("[#224] configurePlatform('gemini') writes shared skills to .agents/skills, NOT .gemini/skills", async () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "rudder-gemini-issue224-"),
    );
    try {
      setWriteMode("force");
  ...
  it("[#224] codex + gemini render byte-identical content for shared `.agents/skills/` files", () => {
    ...
    // At least the 5 shared workflow skills + bundled rudder-meta files must
    // overlap. If this drops to 0 the assertion above is silently passing.
    expect(overlapCount).toBeGreaterThan(0);
  });

  it("[rudder-hooks-env] all hook templates honor RUDDER_HOOKS=0 / RUDDER_DISABLE_HOOKS=1", () => {
    // All shipped hook scripts must early-return when the operator sets
    // RUDDER_HOOKS=0 (or RUDDER_DISABLE_HOOKS=1), so subprocess wrappers
    // and casual-chat scenarios can disable Rudder injection without
    ...
    for (const name of sharedHookTargets) {
      ...
      expect(script).toContain('os.environ.get("RUDDER_HOOKS") == "0"');
      expect(script).toContain('os.environ.get("RUDDER_DISABLE_HOOKS") == "1"');
    }
    ...
    for (const [label, hooks] of [
      ...
      expect(sessionStart).toContain('os.environ.get("RUDDER_HOOKS") == "0"');
      expect(sessionStart).toContain('os.environ.get("RUDDER_DISABLE_HOOKS") == "1"');
    }
    ...
    for (const name of jsPlugins) {
      ...
      expect(content).toContain('process.env.RUDDER_HOOKS === "0"');
      expect(content).toContain('process.env.RUDDER_DISABLE_HOOKS === "1"');
    }
  });
  ...
  it("[#224] needsCodexUpgrade looks for Codex-only command-as-skill markers, not bare `.agents/skills/` prefix", () => {
    ...
    // (`rudder-continue/SKILL.md`, `rudder-finish-work/SKILL.md`) which
    // Gemini does NOT write (it puts continue/finish-work under
    // `.gemini/commands/rudder/*.toml`).
    ...
    expect(updateSrc).toMatch(
      /\.agents\/skills\/rudder-continue\/SKILL\.md/,
    );
    expect(updateSrc).toMatch(
      /\.agents\/skills\/rudder-finish-work\/SKILL\.md/,
    );
...
describe("regression: session-start.py f-string Python <=3.11 compat (0.5.2)", () => {
  ...
  // of an f-string. Rudder 0.5.0/0.5.1 shipped session-start hooks with
...
describe("regression: sub-agent context injection fallback (0.5.3)", () => {
  ...
  //
  // The fix: hook output now begins with a `<!-- rudder-hook-injected -->`
  // marker, and every class-1 rudder-implement / rudder-check definition
  // file carries a Rudder Context Loading Protocol section telling the
  // sub-agent to load context itself when the marker is absent.
  const HOOK_INJECTED_MARKER = "<!-- rudder-hook-injected -->";
  ...
  it("inject-subagent-context.py emits the marker for implement / check / finish", () => {
    ...
    const matches = src.match(/<!--\s*rudder-hook-injected\s*-->/g) ?? [];
  ...
  const CLASS1_MD_AGENT_FILES: { platform: string; rel: string; agent: "implement" | "check" }[] = [
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/rudder-implement.md", agent: "implement" },
    { platform: "claude", rel: "packages/cli/src/templates/claude/agents/rudder-check.md", agent: "check" },
    { platform: "cursor", rel: "packages/cli/src/templates/cursor/agents/rudder-implement.md", agent: "implement" },
    { platform: "cursor", rel: "packages/cli/src/templates/cursor/agents/rudder-check.md", agent: "check" },
    { platform: "codebuddy", rel: "packages/cli/src/templates/codebuddy/agents/rudder-implement.md", agent: "implement" },
    { platform: "codebuddy", rel: "packages/cli/src/templates/codebuddy/agents/rudder-check.md", agent: "check" },
    { platform: "opencode", rel: "packages/cli/src/templates/opencode/agents/rudder-implement.md", agent: "implement" },
    { platform: "opencode", rel: "packages/cli/src/templates/opencode/agents/rudder-check.md", agent: "check" },
    { platform: "droid", rel: "packages/cli/src/templates/droid/droids/rudder-implement.md", agent: "implement" },
    { platform: "droid", rel: "packages/cli/src/templates/droid/droids/rudder-check.md", agent: "check" },
  ];
  ...
  for (const { platform, rel, agent } of CLASS1_MD_AGENT_FILES) {
    it(`${platform}/${agent} markdown agent file carries marker + fallback protocol`, () => {
      ...
      expect(content).toContain("Rudder Context Loading Protocol");
  ...
  for (const agent of ["implement", "check"] as const) {
    it(`kiro/${agent} JSON agent carries marker + fallback protocol in prompt`, () => {
      ...
      const filePath = path.join(
        repoRootFb,
        `packages/cli/src/templates/kiro/agents/rudder-${agent}.json`,
      );
      const json = JSON.parse(fs.readFileSync(filePath, "utf-8"));
      const prompt: string = json.prompt ?? "";
      expect(prompt).toContain(HOOK_INJECTED_MARKER);
      expect(prompt).toContain("Rudder Context Loading Protocol");
  ...
  it("[issue-247] kiro agent JSON files use Kiro CLI's current schema (prompt / hooks-object)", () => {
    // Kiro CLI rejected Rudder's pre-0.5.7 agent JSON with "invalid agent"
  ...
  it("workflow.md dispatch protocol covers all platforms (not class-2 only)", () => {
    ...
    // rudder-research carve-out as of 0.5.8 — research sub-agents need the
    ...
    expect(wf).not.toContain("EXCEPT rudder-research");
    expect(wf).toContain("rudder-research");
...
describe("regression: configSectionsAdded (issue-codex-dispatch-mode)", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-config-section-"));
  });
  ...
  it("[config-sections] applyConfigSectionsAdded appends section when sentinel missing, idempotent on rerun", async () => {
    ...
    const rudderDir = path.join(tmpDir, ".rudder");
    fs.mkdirSync(rudderDir, { recursive: true });
    const userConfigPath = path.join(rudderDir, "config.yaml");
    const userConfig = [
  ...
  it("[config-sections] applyConfigSectionsAdded skips when target file does not exist", async () => {
    ...
    const result = applyConfigSectionsAdded(
      ...
      tmpDir,
      new Map<string, string>([[".rudder/config.yaml", "# fake template"]]),
    );
  ...
  it("[config-sections] manifest 0.5.7 declares the codex dispatch_mode section", () => {
    ...
    expect(entry?.file).toBe(".rudder/config.yaml");
...
// =============================================================================
// safe-commit: gitignored .rudder/ recovery (0.5.10 → 0.5.11)
// =============================================================================
//
// Real user incident: project .gitignore listed `.rudder/`. add_session.py's
// auto-commit ran `git add .rudder/workspace .rudder/tasks`, got `ignored
// by .gitignore`, fell back to a hint suggesting `git add .rudder &&
// commit`. The AI agent driving the workflow extrapolated that to
// `git add -f .rudder/`, which forced in `.rudder/.backup-*/`,
// `.rudder/worktrees/`, `.rudder/.template-hashes.json`, etc. — 548 files
...
// That auto-`-f` was an over-fix — when a user gitignores `.rudder/` they
// mean "keep .rudder/ local-only", and forcing the commit through (even on
// narrow paths) violates user intent. Group-chat report: a finish-work auto
// committed `.rudder/workspace/` straight into a repo whose .gitignore
// excluded `.rudder/`.
//
// 0.5.11 fix (current):
...
//   - The warning explicitly says ``Do NOT use `git add -f .rudder/```` so
//     AI re-reading the log doesn't reinvent the bug, and points at the new
//     `session_auto_commit: false` knob.
//
// These tests synthesize a tmp git repo with `.rudder/` gitignored and
...
describe("regression: safe auto-commit when .rudder/ is gitignored (0.5.10 → 0.5.11)", () => {
  let tmpDir: string;
  const pyCmd = process.platform === "win32" ? "python" : "python3";

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-safe-commit-"));
    execSync("git init -q -b main", { cwd: tmpDir });
    // Configure user so git commit succeeds in CI sandboxes.
    execSync('git config user.email "test@rudder.local"', { cwd: tmpDir });
    execSync('git config user.name "Rudder Test"', { cwd: tmpDir });
  });
  ...
  function writeWorkspaceIndex(): void {
    writeFile(
      ".rudder/workspace/test-dev/index.md",
      [
  ...
  function setupRepo(options?: { gitignoreRudder?: boolean }): void {
    writeRudderScripts();
    writeFile(
      ".rudder/.developer",
      "name=test-dev\ninitialized_at=2026-05-09T00:00:00\n",
    );
    writeFile(".rudder/workspace/test-dev/journal-1.md",
      "# Journal - test-dev (Part 1)\n\n---\n",
    );
    writeWorkspaceIndex();
    // Ignored caches/backups must exist on disk to prove they don't get
    // staged when -f is forced on specific paths.
    writeFile(".rudder/.backup-2026-05-09/should-not-be-committed.txt",
      "secret-backup\n",
    );
    writeFile(".rudder/worktrees/wt-a/should-not-be-committed.txt",
      "secret-worktree\n",
    );
    writeFile(".rudder/.template-hashes.json", '{"_": "should-not-be-committed"}\n');
    writeFile(".rudder/.runtime/sessions/should-not-be-committed.json", "{}\n");

    if (options?.gitignoreRudder) {
      writeFile(".gitignore", ".rudder/\n");
    }
  ...
  it("[gitignore-rudder] add_session warns and skips when .rudder/ is ignored (default mode)", () => {
    ...
    expect(stderr).toContain("Do NOT use `git add -f .rudder/`");
    expect(stderr).toContain("session_auto_commit: false");

    // Nothing under .rudder/ should be tracked: the user's .gitignore
    // intent is preserved.
    const tracked = listCommittedFiles();
    for (const tracked_path of tracked) {
      expect(
        tracked_path.startsWith(".rudder/"),
        `should not commit anything under .rudder/ (got: ${tracked_path})`,
      ).toBe(false);
    }
  ...
  it("[gitignore-rudder] add_session works normally when .rudder/ is NOT ignored", () => {
    ...
    const tracked = listCommittedFiles();
    expect(tracked).toContain(".rudder/workspace/test-dev/journal-1.md");
  });

  it("[gitignore-rudder] safe_commit module ships and contains the negative warning + new config hint", () => {
    ...
    expect(safeCommit).toContain("Do NOT use `git add -f .rudder/`");
    expect(safeCommit).toContain("safe_rudder_paths_to_add");
  ...
  it("[gitignore-rudder] task.py archive warns and skips when .rudder/ is ignored (default mode)", () => {
    ...
    writeFile(".rudder/tasks/issue-500/prd.md", "# PRD\n");
    ...
    const result = spawnSync(
      ...
      [taskScriptPath, "archive", "issue-500"],
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: { ...process.env, RUDDER_CONTEXT_ID: "session-arch" },
      },
    );
    ...
    const tracked = listCommittedFiles();
    // Nothing under .rudder/ should be tracked.
    for (const t of tracked) {
      expect(
        t.startsWith(".rudder/"),
        `should not commit anything under .rudder/ (got: ${t})`,
      ).toBe(false);
    }
    ...
    const archiveExists = fs
      .readdirSync(path.join(tmpDir, ".rudder/tasks/archive"))
      .some((monthDir) => {
        const monthPath = path.join(
          tmpDir,
          ".rudder/tasks/archive",
          monthDir,
        );
        return (
  ...
  function writeConfigYaml(content: string): void {
    writeFile(".rudder/config.yaml", content);
  }
  ...
  it("[session_auto_commit=false] task.py archive skips git entirely", () => {
    ...
    writeFile(
      ".rudder/tasks/issue-600/task.json",
      JSON.stringify(
        { title: "Test archive", status: "in_progress", package: null },
        null,
        2,
    ...
    writeFile(".rudder/tasks/issue-600/prd.md", "# PRD\n");
    ...
    const result = spawnSync(
      ...
      [taskScriptPath, "archive", "issue-600"],
      {
        cwd: tmpDir,
        encoding: "utf-8",
        env: { ...process.env, RUDDER_CONTEXT_ID: "session-arch-2" },
      },
    );
    const stderr = result.stderr ?? "";
    expect(stderr).not.toContain("Auto-committed");
  ...
  it("[session_auto_commit] inline comment is stripped before parsing", () => {
    ...
    // verifies the helper is shared with rudder_config.py's parser.
    setupRepo({ gitignoreRudder: false });
    writeConfigYaml(
