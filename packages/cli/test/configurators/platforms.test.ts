...
describe("getConfiguredPlatforms", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-platforms-"));
  });
...
describe("configurePlatform", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "rudder-configure-"));
  ...
  it("configurePlatform writes collected templates byte-for-byte for every platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `rudder-parity-${id}-`),
      );
      try {
  ...
  it("configurePlatform('codex') writes shared skill templates from common source", async () => {
    ...
    // Plus a Codex-specific `rudder-start` skill referenced by the
    // <rudder-bootstrap> notice in inject-workflow-state.py (the SessionStart
    ...
    expect(
      fs.existsSync(path.join(skillsRoot, "rudder-start", "SKILL.md")),
    ).toBe(true);
  });
  ...
  it("configurePlatform('codex') writes custom agents and config", async () => {
    ...
    for (const agent of expectedAgents) {
      ...
      const needsPrelude = ["rudder-implement", "rudder-check"].includes(
        agent.name,
      );
      if (needsPrelude) {
        expect(written).toContain("Required: Load Rudder Context First");
  ...
  it("configurePlatform('gemini') writes TOML commands + SKILL.md skills", async () => {
    ...
    // Commands as TOML
    const commandsDir = path.join(tmpDir, ".gemini", "commands", "rudder");
    ...
    for (const dir of skillDirs) {
      expect(dir.name.startsWith("rudder-")).toBe(true);
  ...
  it("configurePlatform('qoder') writes commands + skills with the correct split", async () => {
    ...
    expect(actualCommandFiles).toEqual(
      expectedCommands.map((c) => `rudder-${c.name}.md`).sort(),
    );
    for (const cmd of expectedCommands) {
      const name = `rudder-${cmd.name}`;
    ...
    expect(actualSkillDirs).not.toContain("rudder-finish-work");
    expect(actualSkillDirs).not.toContain("rudder-continue");
    expect(actualSkillDirs).not.toContain("rudder-start");
  });
  ...
  it("configurePlatform('codebuddy') writes all command templates from common source", async () => {
    ...
    const commandsDir = path.join(tmpDir, ".codebuddy", "commands", "rudder");
    expect(fs.existsSync(commandsDir)).toBe(true);
  ...
  it("configurePlatform('copilot') writes both tracked and discovery hooks config", async () => {
    ...
    const discovery = path.join(tmpDir, ".github", "hooks", "rudder.json");
  ...
  it("configurePlatform('droid') creates .factory/commands/rudder directory", async () => {
    await configurePlatform("droid", tmpDir);
    expect(
      fs.existsSync(path.join(tmpDir, ".factory", "commands", "rudder")),
    ).toBe(true);
  });
  ...
  it("droid configuration writes commands + skills", async () => {
    ...
    expect(content).toContain("name: rudder-check");
  });
  ...
  it("configurePlatform('pi') creates extension-backed Pi assets", async () => {
    ...
    expect(
      fs.existsSync(
        path.join(tmpDir, ".pi", "prompts", "rudder-finish-work.md"),
      ),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "prompts", "rudder-continue.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "prompts", "rudder-start.md")),
    ).toBe(false);
    expect(
      fs.existsSync(
        path.join(tmpDir, ".pi", "skills", "rudder-check", "SKILL.md"),
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "rudder-implement.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "rudder-check.md")),
    ).toBe(true);
    expect(
      fs.existsSync(path.join(tmpDir, ".pi", "agents", "rudder-research.md")),
    ).toBe(true);
    expect(
      fs.existsSync(
    ...
    expect(extension).toContain("function injectRudderContextIntoBash");
    expect(extension).toContain("ctx?.sessionManager?.getSessionId");
    expect(extension).toContain("RUDDER_CONTEXT_ID: contextKey");
    ...
    expect(extension).toContain("RUDDER_PI_CLI_JS");
    ...
    expect(extension).toContain('"## Rudder Agent Definition"');
    ...
    expect(extension).not.toContain("message: buildRudderContext");
    expect(extension).not.toContain('message:\n      "Rudder project context');
    ...
    // get_context.py is allowed: it lives in .rudder/scripts/ and is the
  ...
  it("configurePlatform('pi') writes tracked templates exactly", async () => {
    ...
    for (const agent of getPiAgents()) {
      ...
      if (["rudder-implement", "rudder-check"].includes(agent.name)) {
  ...
  it("collectPlatformTemplates('pi') maps prompts, skills, agents, extension, and settings", () => {
    ...
    expect(templates?.get(".pi/prompts/rudder-start.md")).toBeUndefined();
    expect(templates?.get(".pi/prompts/rudder-finish-work.md")).toBeDefined();
    expect(templates?.get(".pi/prompts/rudder-continue.md")).toBeDefined();
    expect(templates?.get(".pi/skills/rudder-check/SKILL.md")).toBeDefined();
    expect(
      templates?.get(
        ".pi/skills/rudder-meta/references/local-architecture/overview.md",
      ),
    ).toBeDefined();
    expect(templates?.get(".pi/agents/rudder-implement.md")).toContain(
      "Required: Load Rudder Context First",
    );
    expect(templates?.get(".pi/extensions/rudder/index.ts")).toBe(
  ...
  it("collectPlatformTemplates('droid') maps commands under .factory/commands/rudder/", () => {
    const templates = collectPlatformTemplates("droid");
    expect(templates).toBeInstanceOf(Map);
    // Droid is agent-capable → start.md not emitted.
    expect(
      templates?.get(".factory/commands/rudder/start.md"),
    ).toBeUndefined();
    expect(
      templates?.get(".factory/commands/rudder/finish-work.md"),
    ).toBeDefined();
    expect(
      templates?.get(".factory/commands/rudder/continue.md"),
    ).toBeDefined();
  });
  ...
  it("does not throw for any platform", async () => {
    for (const id of PLATFORM_IDS) {
      const platformDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `rudder-cfg-${id}-`),
      );
      try {
        setWriteMode("force");
  ...
  it("collectPlatformTemplates('copilot') includes tracked + discovery hooks config", () => {
    ...
    expect(templates?.get(".github/hooks/rudder.json")).toBe(
      resolvePlaceholders(getCopilotHooksConfig()),
    );
  });
