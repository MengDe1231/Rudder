...
describe("ALL_MANAGED_DIRS", () => {
  it("starts with .rudder", () => {
    expect(ALL_MANAGED_DIRS[0]).toBe(".rudder");
  });

  it("contains .rudder plus all managed dirs", () => {
...
describe("isManagedPath", () => {
  ...
  it("matches platform config sub-paths", () => {
    ...
    expect(isManagedPath(".windsurf/workflows/rudder-start.md")).toBe(true);
    ...
    expect(isManagedPath(".pi/extensions/rudder/index.ts")).toBe(true);
    expect(isManagedPath(".pi/prompts/rudder-continue.md")).toBe(true);
  });
  ...
  it("matches exact managed directory names", () => {
    ...
    expect(isManagedPath(".rudder")).toBe(true);
  });

  // Positive: .rudder hardcoded paths
  it("matches .rudder sub-paths", () => {
    expect(isManagedPath(".rudder/spec")).toBe(true);
    expect(isManagedPath(".rudder/tasks/some-task")).toBe(true);
  });
  ...
  it("rejects prefix-similar non-sub-paths", () => {
    ...
    expect(isManagedPath(".rudder-old")).toBe(false);
  ...
  it("rejects path traversal", () => {
    ...
    expect(isManagedPath("../.rudder/spec")).toBe(false);
  });
...
describe("isManagedRootDir", () => {
  ...
  it("matches .rudder", () => {
    expect(isManagedRootDir(".rudder")).toBe(true);
  });
  ...
  it("rejects sub-paths (not a root dir)", () => {
    ...
    expect(isManagedRootDir(".rudder/spec")).toBe(false);
  });
...
describe("collectPlatformTemplates", () => {
  const SKILL_ROOTS: Record<AITool, string> = {
    ...
    // Rudder writes there (shared with Codex) so a single skill set serves
  ...
  it("tracks bundled rudder-meta files for every skill-writing platform", () => {
    for (const [id, skillRoot] of Object.entries(SKILL_ROOTS)) {
      ...
      expect(result?.has(`${skillRoot}/rudder-meta/SKILL.md`)).toBe(true);
      expect(
        result?.has(
          `${skillRoot}/rudder-meta/references/local-architecture/overview.md`,
        ),
      ).toBe(true);
    }
  });
  ...
  it("copilot collectTemplates includes both tracked and discovery config files", () => {
    ...
    expect(result?.has(".github/hooks/rudder.json")).toBe(true);
  });

  it("pi collectTemplates includes prompts, agents, extension, and settings", () => {
    const result = collectPlatformTemplates("pi");
    expect(result).toBeInstanceOf(Map);
    expect(result?.has(".pi/prompts/rudder-start.md")).toBe(false);
    expect(result?.has(".pi/prompts/rudder-finish-work.md")).toBe(true);
    expect(result?.has(".pi/agents/rudder-implement.md")).toBe(true);
    expect(result?.has(".pi/extensions/rudder/index.ts")).toBe(true);
