...
describe("replacePythonCommandLiterals", () => {
  ...
  it("handles multiline content with mixed shebangs and commands", () => {
    ...
    const input = [
      ...
      "exec python3 \"$0\" \"$@\"",
      "python3 ./.rudder/scripts/task.py",
    ].join("\n");
    const expected = [
      "#!/usr/bin/env python3",
      "# comment about python",
      "exec python \"$0\" \"$@\"",
      "python ./.rudder/scripts/task.py",
    ].join("\n");
...
describe("resolvePlaceholders", () => {
  ...
  describe("{{CMD_REF:name}}", () => {
    it("resolves with /rudder: prefix (Claude)", () => {
      const result = resolvePlaceholders(
        "See {{CMD_REF:brainstorm}} for details",
        claudeCtx,
      );
      expect(result).toBe("See /rudder:brainstorm for details");
    });
    ...
    it("resolves with /rudder- prefix (Cursor)", () => {
      const result = resolvePlaceholders(
        "Use {{CMD_REF:finish-work}} when done",
        cursorCtx,
      );
      expect(result).toBe("Use /rudder-finish-work when done");
    });
    ...
    it("handles multiple CMD_REF in one template", () => {
      ...
      expect(resolvePlaceholders(input, claudeCtx)).toBe(
        "/rudder:start then /rudder:brainstorm then /rudder:check",
      );
    });
...
describe("resolvePlaceholdersNeutral", () => {
  it("renders {{CMD_REF:name}} as `name` (Rudder command) — platform-neutral", () => {
    expect(
      resolvePlaceholdersNeutral("See {{CMD_REF:brainstorm}}", claudeCtx),
    ).toBe("See `brainstorm` (Rudder command)");
    expect(
...
describe("resolveSkillsNeutral / resolveAllAsSkillsNeutral", () => {
  ...
  it("resolveSkillsNeutral renders CMD_REF without platform-specific prefix", () => {
    ...
    // Claude `/rudder:name`, Cursor `/rudder-name`).
