...
# - .rudder/workspace/{developer}/features/{name}/ -> .rudder/tasks/{MM}-{DD}-{name}-{developer}/
...
# Check if .rudder exists
if [[ ! -d "$RUDDER_DIR" ]]; then
    echo "Error: .rudder directory not found in $PROJECT_DIR"
    exit 1
fi

# Determine the workspace directory (could be workspace or agent-traces if not yet updated)
if [[ -d "$RUDDER_DIR/workspace" ]]; then
    WORKSPACE_DIR="$RUDDER_DIR/workspace"
elif [[ -d "$RUDDER_DIR/agent-traces" ]]; then
    WORKSPACE_DIR="$RUDDER_DIR/agent-traces"
    echo "Warning: Found agent-traces instead of workspace. Run 'rudder update' first."
