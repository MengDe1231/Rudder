 ...
 * public update contract. `rudder update` applies manifests where
 ...
 *   1. Query npm for all published versions of @mengde1231/rudder
 ...
 *   to npm) motivated this gate — see .rudder/spec/cli/backend/migrations.md.
 */
...
const PACKAGE_NAME = "@mengde1231/rudder";
...
function main() {
  ...
  if (newGaps.length > 0) {
    ...
    console.error(
      `\n` +
      `A version on npm without its local manifest breaks \`rudder update\`\n` +
      `for users on adjacent versions. See .rudder/spec/cli/backend/migrations.md.\n` +
      `\n` +
      `Fix options:\n` +
