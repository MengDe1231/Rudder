...
export async function configureCodex(cwd: string): Promise<void> {
  ...
  // Additionally write `rudder-start` to .agents/skills/ — Codex-specific.
  // The SessionStart hook was removed in 0.5.5 (de-recursion); inject-workflow-state.py
  // injects a `<rudder-bootstrap>` block on no_task turns instructing the AI to
  // invoke `$rudder-start` to load workflow context. Without this skill, that
  ...
  // (configurators/index.ts) — both share `resolveCodexRudderStartSkill`.
  const rudderStart = resolveCodexRudderStartSkill(
    AI_TOOLS.codex.templateContext,
  );
  if (rudderStart) {
    const rudderStartDir = path.join(sharedSkillsRoot, rudderStart.name);
    ensureDir(rudderStartDir);
    await writeFile(
      path.join(rudderStartDir, "SKILL.md"),
      rudderStart.content,
    );
  }
  ...
  // it the workflow breadcrumb won't auto-inject (the rudder-bootstrap
  // fallback in inject-workflow-state.py covers this case). Documented in
  // spec/cli/backend/platform-integration.md.
  if (!process.env.VITEST && !process.env.RUDDER_QUIET) {
    process.stderr.write(
      "⚠️  Codex hooks require `features.hooks = true` in your " +
        "~/.codex/config.toml (Codex 0.129+; older versions: `codex_hooks = true`). " +
        "On Codex 0.129+ also run `/hooks` once to approve the Rudder " +
        "UserPromptSubmit hook. Without these the Rudder workflow breadcrumb " +
        "won't auto-inject. See Rudder docs for details.\n",
    );
  }
