 ...
 * - skills/rudder-{name}/SKILL.md — other 5 as auto-triggered skills
...
export async function configureCopilot(cwd: string): Promise<void> {
  ...
  // prepend the pull-based prelude so sub-agents Read Rudder context themselves.
  ...
  await writeFile(path.join(githubHooksDir, "rudder.json"), resolvedConfig);
}
