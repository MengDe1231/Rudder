...
import { copyRudderDir } from "../templates/extract.js";

// Import rudder templates (generic, not project-specific)
import {
  workflowMdTemplate,
  configYamlTemplate,
 ...
 *
 * This function creates the .rudder/ directory structure by:
...
export async function createWorkflowStructure(
  ...
  // Create base .rudder directory
  ensureDir(path.join(cwd, DIR_NAMES.WORKFLOW));

  // Copy scripts/ directory from templates
  await copyRudderDir("scripts", path.join(cwd, PATHS.SCRIPTS), {
    executable: true,
  });
