...
const PLATFORM_FUNCTIONS: Record<AITool, PlatformFunctions> = {
  ...
  codex: {
    ...
    collectTemplates: () => {
      ...
      // Mirror configureCodex's extra rudder-start write so `rudder update`
      // picks up the file (was missing pre-0.5.7 — upgrade path silently
      // dropped the skill).
      const rudderStart = resolveCodexRudderStartSkill(ctx);
      if (rudderStart) {
        files.set(
          `.agents/skills/${rudderStart.name}/SKILL.md`,
          rudderStart.content,
        );
      }
  ...
  gemini: {
    ...
    collectTemplates: () => {
      ...
      const files = new Map<string, string>();
      for (const cmd of resolveCommands(ctx)) {
        const toml = `description = "Rudder: ${cmd.name}"\n\nprompt = """\n${cmd.content}\n"""\n`;
        files.set(`.gemini/commands/rudder/${cmd.name}.toml`, toml);
      }
  ...
  windsurf: {
    ...
    collectTemplates: () =>
      collectBothTemplates(
        ...
        (n) => `.windsurf/workflows/rudder-${n}.md`,
        ".windsurf/skills",
      ),
  },
  qoder: {
  ...
  codebuddy: {
    ...
    collectTemplates: () => {
      const files = collectBothTemplates(
        ...
        (n) => `.codebuddy/commands/rudder/${n}.md`,
        ".codebuddy/skills",
      );
  ...
  copilot: {
    ...
    collectTemplates: () => {
      ...
      files.set(".github/hooks/rudder.json", hooksConfig);
      return files;
    },
  },
  droid: {
    ...
    collectTemplates: () => {
      const files = collectBothTemplates(
        ...
        (n) => `.factory/commands/rudder/${n}.md`,
        ".factory/skills",
      );
...
/** All directories managed by Rudder (including .rudder itself) */
export const ALL_MANAGED_DIRS = [".rudder", ...new Set(PLATFORM_MANAGED_DIRS)];
