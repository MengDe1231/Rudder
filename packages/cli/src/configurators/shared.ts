 ...
 * rendered in a platform-neutral form (`` `name` (Rudder command) ``)
 ...
 * of which Rudder configurator wrote them — eliminating the
 ...
 * neutral. Codex-only skill files (e.g. `rudder-continue/SKILL.md`,
 * `rudder-finish-work/SKILL.md` written via `resolveAllAsSkillsNeutral`) DO
...
export function resolvePlaceholdersNeutral(
  ...
  result = result.replace(
    RE_CMD_REF,
    (_match, name: string) => `\`${name}\` (Rudder command)`,
  );
  result = result.replace(RE_EXECUTOR_AI, context.executorAI);
  result = result.replace(RE_USER_ACTION_LABEL, context.userActionLabel);
  result = result.replace(RE_CLI_FLAG, context.cliFlag);
...
const SKILL_DESCRIPTIONS: Record<string, string> = {
  ...
  "before-dev":
    "Discovers and injects project-specific coding guidelines from .rudder/spec/ before implementation begins. Reads spec indexes, pre-development checklists, and shared thinking guides for the target package. Use when starting a new coding task, before writing any code, switching to a different package, or needing to refresh project conventions and standards.",
  brainstorm:
  ...
  "update-spec":
    "Captures executable contracts and coding conventions into .rudder/spec/ documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions.",
};
...
export function wrapWithSkillFrontmatter(
  ...
  // Look up description by base name (without rudder- prefix)
  const baseName = name.replace(/^rudder-/, "");
...
const COMMAND_DESCRIPTIONS: Record<string, string> = {
  start: "Initialize a Rudder development session.",
...
export interface ResolvedSkillFile {
  /** POSIX path relative to the skills root, e.g. "rudder-meta/SKILL.md" */
  relativePath: string;
  content: string;
}
...
/**
 * Resolve ALL templates as skills with rudder- prefix.
...
/**
 * Resolve only the 5 skill templates with rudder- prefix + SKILL.md frontmatter.
...
/**
 * Codex needs a `rudder-start` skill in `.agents/skills/` so the
 * `<rudder-bootstrap>` notice from `inject-workflow-state.py` resolves
 * to an actual skill file (the bootstrap notice tells the AI to invoke
 * `$rudder-start` once on the first `no_task` turn — added in 0.5.5
 ...
 * "no .agents/skills/rudder-start/SKILL.md" because update only ran
 * `collectTemplates` and never wrote the file).
 */
export function resolveCodexRudderStartSkill(
  ctx: TemplateContext,
...
/** Build the standard "load Rudder context first" prelude block. */
export function buildPullBasedPrelude(agentType: SubAgentType): string {
  ...
  return replacePythonCommandLiterals(`## Required: Load Rudder Context First
...
1. **Look at the dispatch prompt** you received from the main agent. If its first line is \`Active task: <path>\` (e.g. \`Active task: .rudder/tasks/04-17-foo\`), use that path. The main agent is required to include this line on class-2 platforms.
2. **Run** \`python3 ./.rudder/scripts/task.py current --source\` and read the \`Current task:\` line.
...
/** Best-effort detect agent type from filename ("rudder-implement.md" → "implement").
...
export function detectSubAgentType(name: string): SubAgentType | null {
  ...
  if (base === "rudder-implement" || base === "rudder-check") {
    return base === "rudder-implement" ? "implement" : "check";
  }
  return null;
}
