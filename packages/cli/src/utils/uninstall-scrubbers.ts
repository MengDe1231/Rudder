/**
 * Scrubbers for structured config files during `rudder uninstall`.
 *
 ...
 * - `fullyEmpty` is true when, after stripping every rudder-managed value,
 ...
 *
 * Rudder-emitted hook commands always have the shape
...
/**
 * Rudder-specific values written by the Pi configurator.
 *
 * The `extensions`/`skills`/`prompts` arrays are paths relative to `.pi/`. We
 * remove the exact entries that the Pi configurator emits.
 */
const PI_RUDDER_EXTENSION = "./extensions/rudder/index.ts";
const PI_RUDDER_SKILLS = "./skills";
const PI_RUDDER_PROMPTS = "./prompts";
const PI_SUBAGENTS_PACKAGE = "npm:pi-subagents";

function isRudderPiEntry(value: unknown, target: string): boolean {
  return typeof value === "string" && value === target;
}

/**
 * Scrub `.pi/settings.json`:
 * - drop `enableSkillCommands` (rudder-flagged)
 * - remove rudder entries from `extensions`/`skills`/`prompts` arrays
 * - remove rudder-managed `packages["npm:pi-subagents"]` isolation override
...
export function scrubPiSettings(content: string): ScrubResult {
  ...
  const arrayCleanups: [string, string][] = [
    ["extensions", PI_RUDDER_EXTENSION],
    ["skills", PI_RUDDER_SKILLS],
    ["prompts", PI_RUDDER_PROMPTS],
  ];
  for (const [key, target] of arrayCleanups) {
    const arr = root[key];
    if (!Array.isArray(arr)) continue;
    const filtered = arr.filter((v) => !isRudderPiEntry(v, target));
 ...
 *
 * The current rudder-emitted file has two distinct chunks:
 * 1. The line `project_doc_fallback_filenames = ["AGENTS.md"]`
 * 2. A multi-line comment block that begins with the marker
 *    `# NOTE: Rudder's SessionStart + UserPromptSubmit hooks require opt-in.`
 ...
 *  - any line that is *only* a comment introduced by rudder (the entire file
 *    as shipped is comments + that one assignment)
 *  - blank lines that surrounded those removals
 *
 * If the user added their own non-rudder lines, they are preserved as-is.
...
export function scrubCodexConfigToml(content: string): ScrubResult {
  const rudderCommentMarkers = [
    "Project-scoped Codex defaults for Rudder workflows.",
    ...
    "hooks = true",
    "codex_hooks = true",
    "Without this flag, hooks.json is ignored and Rudder context won't",
    "be injected into Codex sessions.",
  ];

  // A comment line is "rudder-known" if its content (after `#` and spaces)
  // matches one of the known marker strings exactly OR is an empty `#` line.
  function isRudderCommentLine(line: string): boolean {
    const trimmed = line.trim();
    if (!trimmed.startsWith("#")) return false;
    const inner = trimmed.replace(/^#+\s?/, "").trim();
    if (inner.length === 0) return true; // bare `#` line inside rudder block
    return rudderCommentMarkers.some((m) => inner === m);
  }

  function isRudderAssignment(line: string): boolean {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    if (isRudderAssignment(rawLine) || isRudderCommentLine(rawLine)) {
      continue; // drop
    }
