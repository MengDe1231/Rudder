...
print("Run: {{PYTHON_CMD}} ./.rudder/scripts/task.py <args>")
```
...
command = `export RUDDER_CONTEXT_ID=${shellQuote(contextKey)}; ${command}`;

// GOOD - shell-dialect-aware command prefix
const prefix = process.platform === "win32" && !isWindowsPosixShell(process.env)
  ? `$env:RUDDER_CONTEXT_ID = ${powershellQuote(contextKey)}; `
  : `export RUDDER_CONTEXT_ID=${shellQuote(contextKey)}; `;
...
timeout and captured output. For example, `rudder --version` already runs the
...
- [ ] `.rudder/scripts/` - Project's own scripts (if self-hosting)
...
.rudder/scripts/script.py  ← Forgot to sync!
```
...
# Rudder docs/config should say the rule, not guess one alias everywhere
