---
name: rudder-spec-bootstarp
description: "Bootstrap project-specific Rudder coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .rudder/spec/ guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text."
---

# Rudder Spec Bootstarp

Use this skill to create or refresh `.rudder/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm Rudder is initialized and inspect the current `.rudder/spec/` tree.
...
| Writing high-signal Rudder spec files | [references/spec-writing.md](references/spec-writing.md) |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.rudder/spec/`.

## Done Criteria

- `.rudder/spec/` describes the project as it exists now.
