...
Use a single agent as the default execution model. The agent may create Rudder tasks for traceability, but the skill should not require a specific platform, CLI, or parallel worker model.
...
When a Rudder task is useful, write a concise PRD with these sections:

```markdown
# Fill <package-or-layer> Rudder Specs

## Goal
Write project-specific `.rudder/spec/` guidance for <scope>.
...
- `.rudder/spec/.../index.md`
- `.rudder/spec/.../<topic>.md`
