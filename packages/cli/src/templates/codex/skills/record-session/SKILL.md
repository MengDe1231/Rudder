...
```bash
python3 ./.rudder/scripts/get_context.py --mode record
```
...
```bash
python3 ./.rudder/scripts/task.py archive <task-name>
```
...
cat << 'EOF' | python3 ./.rudder/scripts/add_session.py --stdin --title "Title" --commit "hash"
| Feature | Description |
|---------|-------------|
...
- [OK] Auto-commits .rudder/workspace and .rudder/tasks changes
...
| Command | Purpose |
|---------|---------|
| `python3 ./.rudder/scripts/get_context.py --mode record` | Get context for record-session |
| `python3 ./.rudder/scripts/add_session.py --title "..." --commit "..."` | **One-click add session (recommended, branch auto-complete)** |
| `python3 ./.rudder/scripts/task.py archive <name>` | Archive completed task (auto-commits) |
| `python3 ./.rudder/scripts/task.py list` | List active tasks |
