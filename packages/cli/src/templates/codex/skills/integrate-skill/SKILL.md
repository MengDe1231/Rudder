...
description: "Adapts an external skill into project-specific development guidelines in .rudder/spec/. Creates guideline sections, code example templates with .template suffix, and updates spec indexes. Use when integrating an external skill, adding a new skill's patterns to project conventions, or incorporating third-party skill best practices into .rudder/spec/ documentation."
---
...
>
> - Guidelines content -> Write to `.rudder/spec/{target}/doc.md`
> - Code examples -> Place in `.rudder/spec/{target}/examples/skills/<skill-name>/`
...
|----------------|-------------------|
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.rudder/spec/frontend/` |
| Backend/API (`mcp-builder`) | `.rudder/spec/backend/` |
| Documentation (`doc-coauthoring`, `docx`, `pdf`) | `.rudder/` or create dedicated guidelines |
| Testing (`webapp-testing`) | `.rudder/spec/frontend/` (E2E) |
...
- **Integration target**: `.rudder/spec/{target}/`
...
| Type | Path |
|------|------|
| Guidelines doc | `.rudder/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples | `.rudder/spec/{target}/examples/skills/<name>/` |
| Index update | `.rudder/spec/{target}/index.md` |
...
| `doc-coauthoring` | `.rudder/` | N/A (documentation workflow only) |
