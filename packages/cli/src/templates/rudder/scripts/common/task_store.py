...
_SEED_EXAMPLE = (
    ...
    "Run `python3 .rudder/scripts/get_context.py --mode packages` to list available specs. "
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # Agent curates real entries in Phase 1.3 (see .rudder/workflow.md).
    # Agent-less platforms (Kilo / Antigravity / Windsurf) skip this — they
    # load specs via the rudder-before-dev skill instead of JSONL.
    seeded_jsonl = False
...
def _auto_commit_archive(task_name: str, repo_root: Path) -> None:
    """Stage Rudder-owned task paths and commit after archive.

    Only stages specific subpaths (the archive subtree and active task dirs),
    never the whole ``.rudder/`` tree. If ``.gitignore`` blocks the paths,
    we warn + skip — we do NOT retry with ``git add -f``. The warning
    explicitly forbids ``git add -f .rudder/`` (which would fan out to
    caches/backups) and points users at ``session_auto_commit: false``.

    Honors ``session_auto_commit`` in ``.rudder/config.yaml``: when set to
