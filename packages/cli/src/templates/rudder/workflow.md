...
Creates `.rudder/.developer` (gitignored) + `.rudder/workspace/<your-name>/`.

### Spec System

`.rudder/spec/` holds coding guidelines organized by package and layer.

- `.rudder/spec/<package>/<layer>/index.md` — entry point with **Pre-Development Checklist** + **Quality Check**. Actual guidelines live in the `.md` files it points to.
- `.rudder/spec/guides/index.md` — cross-package thinking guides.

```bash
python3 ./.rudder/scripts/get_context.py --mode packages   # list packages / layers
```
...
Every task has its own directory under `.rudder/tasks/{MM-DD-name}/` holding `prd.md`, `implement.jsonl`, `check.jsonl`, `task.json`, optional `research/`, `info.md`.

```bash
# Task lifecycle
python3 ./.rudder/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
python3 ./.rudder/scripts/task.py start <name>          # set active task (session-scoped when available)
python3 ./.rudder/scripts/task.py current --source      # show active task and source
python3 ./.rudder/scripts/task.py finish                # clear active task (triggers after_finish hooks)
python3 ./.rudder/scripts/task.py archive <name>        # move to archive/{year-month}/
...
python3 ./.rudder/scripts/task.py set-base-branch <name> <branch>    # PR target
...
Records every AI session for cross-session tracking under `.rudder/workspace/<developer>/`.
...
```bash
python3 ./.rudder/scripts/get_context.py                            # full session runtime
python3 ./.rudder/scripts/get_context.py --mode packages            # available packages + spec layers
python3 ./.rudder/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
<!--
  ...
  Editing checklist:
    ...
    - Run `rudder update` after editing to push the new bodies to
...
<!-- Per-turn breadcrumb: shown throughout Phase 1 when codex.dispatch_mode=inline.
     ...
     the inline workflow loads `rudder-before-dev` instead of injecting JSONL
     into a sub-agent. -->
...
Code committed via Phase 3.4; run `/rudder:finish-work` to wrap up (archive the task + record session).
...
**Why `rudder-before-dev` is NOT in this table:** you are not the one writing code — the `rudder-implement` sub-agent is. Sub-agent platforms get spec context via `implement.jsonl` injection / prelude, not via the main thread loading `rudder-before-dev`.
...
# e.g. python3 ./.rudder/scripts/get_context.py --mode phase --step 1.1
```
...
Load the `rudder-brainstorm` skill and explore requirements interactively with the user per the skill's guidance.
...
Do the research in the main session directly and write findings into `{TASK_DIR}/research/`. (For `codex-inline` this avoids the `fork_turns="none"` isolation that prevents `rudder-research` sub-agents from resolving the active task path.)
...
**What to put in**:
- **Spec files** — `.rudder/spec/<package>/<layer>/index.md` and any specific guideline files (`error-handling.md`, `conventions.md`, etc.) relevant to this task
...
Skip this step. Context is loaded directly by the `rudder-before-dev` skill in Phase 2.
...
If `task.py start` errors with a session-identity message (no context key from hook input, `RUDDER_CONTEXT_ID`, or platform-native session env), follow the hint in the error to set up session identity, then retry.
...
This section is for developers who want to modify the Rudder workflow itself. All customization is done by editing this file; the scripts are parsers only.
...
Directly edit the body of the corresponding `[workflow-state:STATUS]` block. After editing, run `rudder update` (if you're a template maintainer) or restart your AI session (if you're customizing your own project) — no script changes required.
...
- `.rudder/spec/cli/backend/workflow-state-contract.md` — runtime contract + writer table + test invariants
- `.rudder/scripts/inject-workflow-state.py` — actual parser (reads workflow.md only, no embedded text)
