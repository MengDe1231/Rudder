---
description: "Rudder Copilot prompt: Start Session"
---
...
```bash
cat .rudder/spec/<package>/<layer>/index.md   # Package-specific guidelines
cat .rudder/spec/guides/index.md              # Thinking guides (always read)
```
...
```bash
TASK_DIR=$(python3 ./.rudder/scripts/task.py create "<title>" --slug <name>)
```
...
Task(
  ...
  Please find:
  1. Relevant code-spec files in .rudder/spec/
...
- Put **spec files** (`.rudder/spec/<package>/<layer>/*.md`) and **research files** (`{TASK_DIR}/research/*.md`) only.
...
```bash
python3 ./.rudder/scripts/task.py start "$TASK_DIR"
```

This sets the active task through Rudder' session resolver so hooks can inject context for this AI session. If the command fails because no session identity is available, rerun it from an IDE/session that exposes session identity or set `RUDDER_CONTEXT_ID`.
...
| Script | Purpose |
|--------|---------|
| `python3 ./.rudder/scripts/get_context.py` | Get session context |
| `python3 ./.rudder/scripts/task.py create` | Create task directory (seeds jsonl on sub-agent platforms) |
| `python3 ./.rudder/scripts/task.py add-context` | Append code-spec/research entry to jsonl |
| `python3 ./.rudder/scripts/task.py start` | Set current task |
| `python3 ./.rudder/scripts/task.py finish` | Clear current task |
| `python3 ./.rudder/scripts/task.py archive` | Archive completed task |
