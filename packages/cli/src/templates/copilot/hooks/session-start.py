...
# -*- coding: utf-8 -*-
"""
Copilot Session Start Hook - Emit Rudder session-start context.
...
and Copilot versions, which is outside Rudder's control. UserPromptSubmit
...
def _resolve_active_task(rudder_dir: Path, hook_input: dict):
    ...
    return resolve_active_task(rudder_dir.parent, hook_input, platform="copilot")
...
def _build_workflow_toc(workflow_path: Path) -> str:
    ...
    phases = _extract_range(content, "Phase Index", "Customizing Rudder (for forks)")
    if phases:
