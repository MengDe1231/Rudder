...
// Matches `rudder-implement`, `rudder-check`, `rudder-research` exactly.
// Used by chat.message plugins to skip injection inside Rudder sub-agent turns.
const RUDDER_SUBAGENT_RE = /^rudder-(implement|check|research)$/

/**
 * Return true when the OpenCode `chat.message` input represents a Rudder
 ...
 */
export function isRudderSubagent(input) {
  if (!input || typeof input !== "object") return false
  const agent = typeof input.agent === "string" ? input.agent.trim() : ""
  return RUDDER_SUBAGENT_RE.test(agent)
}

/**
 * Rudder Context Manager
 */
export class RudderContext {
  constructor(directory) {
    this.directory = directory
    debugLog("context", "RudderContext initialized", { directory })
  }

  // ============================================================
  // Rudder Project Detection
  // ============================================================

  isRudderProject() {
    return existsSync(join(this.directory, ".rudder"))
  }

  getContextKey(platformInput = null) {
    const override = stringValue(process.env.RUDDER_CONTEXT_ID)
    if (override) {
  ...
  readContext(contextKey) {
    try {
      const contextPath = join(this.directory, ".rudder", ".runtime", "sessions", `${contextKey}.json`)
  ...
  _resolveSingleSessionFallback() {
    const sessionsDir = join(this.directory, ".rudder", ".runtime", "sessions")
  ...
  resolveTaskDir(taskRef) {
    ...
    return join(this.directory, ".rudder", "tasks", normalized)
  }
