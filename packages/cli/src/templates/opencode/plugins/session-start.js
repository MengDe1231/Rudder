...
import { RudderContext, contextCollector, debugLog, isRudderSubagent } from "../lib/rudder-context.js"
import {
  buildSessionContext,
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      try {
        ...
        // Skip Rudder sub-agent turns — sub-agent context is injected by
        ...
        if (isRudderSubagent(input)) {
          debugLog("session", "Skipping rudder subagent turn:", agent)
          return
        }

        if (process.env.RUDDER_HOOKS === "0" || process.env.RUDDER_DISABLE_HOOKS === "1") {
          debugLog("session", "Skipping - RUDDER_HOOKS disabled")
          return
        }
        ...
        if (await hasPersistedInjectedContext(client, ctx.directory, sessionID)) {
          ...
          debugLog("session", "Skipping - session already contains persisted Rudder context")
          return
        }
