/* global process */
/**
 * Rudder Context Injection Plugin
 *
...
import { RudderContext, debugLog } from "../lib/rudder-context.js"
 ...
 * (`.rudder/tasks/foo`) or absolute; both are resolved via
 * `ctx.resolveTaskDir`.
 */
...
- Spec files: \`.rudder/spec/**/*.md\`
- Known issues: \`.rudder/big-question/\`
...
function buildPrompt(agentType, originalPrompt, context, isFinish = false) {
  const templates = {
    implement: `<!-- rudder-hook-injected -->
...
- Report list of modified/created files when done`,
    ...
    check: isFinish ? `<!-- rudder-hook-injected -->
# Finish Agent Task
...
function buildRudderContextPrefix(contextKey, hostPlatform = process.platform, env = process.env) {
  if (hostPlatform === "win32" && !isWindowsPosixShell(env)) {
    return `$env:RUDDER_CONTEXT_ID = ${powershellQuote(contextKey)}; `
  }
...
function commandStartsWithRudderContext(command) {
  const firstCommand = command.trimStart().split(/[;&|]/, 1)[0].trimStart()
  return (
    /^RUDDER_CONTEXT_ID\s*=/.test(firstCommand) ||
    /^export\s+RUDDER_CONTEXT_ID\s*=/.test(firstCommand) ||
    /^env\s+(?:(?:-\S+|[A-Za-z_][A-Za-z0-9_]*=\S*)\s+)*RUDDER_CONTEXT_ID\s*=/.test(firstCommand) ||
    /^\$env:RUDDER_CONTEXT_ID\s*=/i.test(firstCommand)
  )
}
 ...
 */
function injectRudderContextIntoBash(ctx, input, output, hostPlatform, env) {
  ...
  if (commandStartsWithRudderContext(command)) return false

  const contextKey = ctx.getContextKey(input)
  if (!contextKey) return false

  args[commandKey] = `${buildRudderContextPrefix(contextKey, hostPlatform, env)}${command}`
  return true
}
...
export default async ({ directory, platform: hostPlatform = process.platform, env = process.env }) => {
  ...
  return {
      "tool.execute.before": async (input, output) => {
        try {
          if (process.env.RUDDER_HOOKS === "0" || process.env.RUDDER_DISABLE_HOOKS === "1") {
            return
          }
          ...
          if (toolName === "bash") {
            if (injectRudderContextIntoBash(ctx, input, output, hostPlatform, env)) {
              debugLog("inject", "Injected RUDDER_CONTEXT_ID into Bash command")
            }
            return
          }
          ...
          // Strip "rudder-" prefix added by v0.5.0-beta.5 agent rename migration
          const subagentType = (rawSubagentType || "").replace(/^rudder-/, "")
          ...
          if (AGENTS_REQUIRE_TASK.includes(subagentType)) {
            // subagentType is already stripped of "rudder-" prefix above
            if (!taskDir) {
