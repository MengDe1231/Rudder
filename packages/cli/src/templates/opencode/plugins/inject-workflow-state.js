/* global process */
/**
 * Rudder Workflow State Injection Plugin
 *
 * Per-turn UserPromptSubmit equivalent for OpenCode.
 *
 * On every chat.message, if a Rudder task is active, inject a short
...
import { RudderContext, debugLog, isRudderSubagent } from "../lib/rudder-context.js"
...
function loadBreadcrumbs(directory) {
  const workflowPath = join(directory, ".rudder", "workflow.md")
...
export default async ({ directory }) => {
  ...
  return {
      ...
      "chat.message": async (input, output) => {
        try {
          // Skip Rudder sub-agent turns — the per-turn breadcrumb is for the
          // main session only; sub-agent context comes from the parent's
          // tool.execute.before injection.
          if (isRudderSubagent(input)) {
            debugLog("workflow-state", "Skipping rudder subagent turn:", input?.agent)
            return
          }
