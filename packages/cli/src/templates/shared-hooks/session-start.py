...
def should_skip_injection() -> bool:
    """Check if any platform's non-interactive flag is set, or if Rudder
    hooks are explicitly disabled via RUDDER_HOOKS=0 / RUDDER_DISABLE_HOOKS=1.
    """
    if os.environ.get("RUDDER_HOOKS") == "0":
        return True
    if os.environ.get("RUDDER_DISABLE_HOOKS") == "1":
        return True
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    """Expose Rudder session identity to later Claude Code Bash commands.
...
def run_script(script_path: Path, context_key: str | None = None) -> str:
    try:
        if script_path.suffix == ".py":
            ...
            if context_key:
                env["RUDDER_CONTEXT_ID"] = context_key
...
def _build_workflow_overview(workflow_path: Path) -> str:
    ...
    The meta sections (Core Principles / Rudder System / Customizing
    Rudder) are NOT injected — Core Principles is short prose the AI can
    Read on demand; Rudder System lists reference commands duplicated in
    step bodies; Customizing Rudder is for forks. Workflow-state breadcrumb
    ...
    # Rudder" — the docs-for-forks footer added in v0.5.0-rc.0). Since
    # sections appear in order Phase Index → Phase 1 → Phase 2 → Phase 3 →
    # Customizing Rudder, a single range grab captures all four. The
    ...
    phases = _extract_range(
        content, "Phase Index", "Customizing Rudder (for forks)"
    )
    if phases:
