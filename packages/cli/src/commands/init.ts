...
export function requireSupportedPython(command: string): string {
  ...
  if (process.env.RUDDER_SKIP_PYTHON_CHECK === "1") {
    return `version check skipped (RUDDER_SKIP_PYTHON_CHECK=1)`;
  }
  ...
  if (!versionOutput) {
    throw new Error(
      `Python command "${command}" not found. Rudder init requires Python ≥ 3.9.`,
    );
  }

  if (!isSupportedPythonVersion(versionOutput)) {
    throw new Error(
      `${versionOutput} detected via "${command}", but Rudder init requires Python ≥ 3.9.`,
    );
  }
 ...
 * `python`) had `rudder init` fail outright.
 *
 ...
 *
 * Honors `RUDDER_PYTHON_CMD` (explicit override, no probe) and
 * `RUDDER_SKIP_PYTHON_CHECK=1` (skip probe, trust platform default).
 *
...
export function resolveSupportedPython(): {
  ...
  const override = process.env.RUDDER_PYTHON_CMD?.trim();
  if (override) {
    setResolvedPythonCommand(override);
    return { command: override, version: "set via RUDDER_PYTHON_CMD" };
  }
  ...
  if (process.env.RUDDER_SKIP_PYTHON_CHECK === "1") {
    ...
    return {
      command: fallback,
      version: "version check skipped (RUDDER_SKIP_PYTHON_CHECK=1)",
    };
  }
  ...
  const installHint = isWindows
    ? `Install Python ≥ 3.9 from https://www.python.org/downloads/windows/ — make sure ` +
      ...
      `installed under a different name, set RUDDER_PYTHON_CMD=<your-cmd> ` +
      `before re-running init (e.g. \`set RUDDER_PYTHON_CMD=py -3\`).`
    : `Install Python ≥ 3.9 from https://www.python.org/downloads/ or via your ` +
      `package manager. Or set RUDDER_PYTHON_CMD=<your-cmd> before re-running.`;

  throw new Error(
    `No supported Python command found. Tried: ${candidates.join(", ")}.\n` +
      `Probe results:\n  ${probeFailures.join("\n  ")}\n\n` +
      `Rudder init requires Python ≥ 3.9. ${installHint}\n` +
      `Last-resort escape hatch: set RUDDER_SKIP_PYTHON_CHECK=1 to skip the probe entirely.`,
  );
}
...
function logPythonAdaptationNotice(command: string): void {
  ...
  console.log(
    chalk.blue(
      `📌 ${osName} detected: Rudder rendered Python commands as "${command}" in generated hooks, settings, and help text`,
    ),
  );
}
...
function getBootstrapRelatedFiles(
  ...
  if (packages && packages.length > 0) {
    return packages.map((pkg) => `.rudder/spec/${sanitizePkgName(pkg.name)}/`);
  }
  if (projectType === "frontend") {
    return [".rudder/spec/frontend/"];
  }
  if (projectType === "backend") {
    return [".rudder/spec/backend/"];
  }
  return [".rudder/spec/backend/", ".rudder/spec/frontend/"];
}
...
The developer just ran \`rudder init\` on this project for the first time.
\`.rudder/\` now exists with empty spec scaffolding, and this bootstrap task
exists under \`.rudder/tasks/\`. When they want to work on it, they should start
this task from a session that provides Rudder session identity.

**Your job**: help them populate \`.rudder/spec/\` with the team's real
coding conventions. Every future AI session — this project's
\`rudder-implement\` and \`rudder-check\` sub-agents — auto-loads spec files
...
|------|------------------|
| \`.rudder/spec/backend/directory-structure.md\` | Where different file types go (routes, services, utils) |
| \`.rudder/spec/backend/database-guidelines.md\` | ORM, migrations, query patterns, naming conventions |
| \`.rudder/spec/backend/error-handling.md\` | How errors are caught, logged, and returned |
| \`.rudder/spec/backend/logging-guidelines.md\` | Log levels, format, what to log |
| \`.rudder/spec/backend/quality-guidelines.md\` | Code review standards, testing requirements |
`;
...
| File | What to document |
|------|------------------|
| \`.rudder/spec/frontend/directory-structure.md\` | Component/page/hook organization |
| \`.rudder/spec/frontend/component-guidelines.md\` | Component patterns, props conventions |
| \`.rudder/spec/frontend/hook-guidelines.md\` | Custom hook naming, patterns |
| \`.rudder/spec/frontend/state-management.md\` | State library, patterns, what goes where |
| \`.rudder/spec/frontend/type-safety.md\` | TypeScript conventions, type organization |
| \`.rudder/spec/frontend/quality-guidelines.md\` | Linting, testing, accessibility |
`;
...
\`.rudder/spec/guides/\` contains general thinking guides pre-filled with
...
extract the relevant rules into the matching \`.rudder/spec/\` files —
...
- Every AI coding task spawns two sub-agents: \`rudder-implement\` (writes
  code) and \`rudder-check\` (verifies quality).
- Each task has \`implement.jsonl\` / \`check.jsonl\` manifests listing which
  spec files to load.
- The platform hook auto-injects those spec files + the task's \`prd.md\`
  into every sub-agent prompt, so the sub-agent codes/reviews per team
  conventions without anyone pasting them manually.
- Source of truth: \`.rudder/spec/\`. That's why filling it well now pays
  off forever.
...
\`\`\`bash
${pythonCmd} ./.rudder/scripts/task.py finish
${pythonCmd} ./.rudder/scripts/task.py archive 00-bootstrap-guidelines
\`\`\`
...
"Welcome to Rudder! Your init just set me up to help you fill the project
...
`;
  ...
  if (packages && packages.length > 0) {
    ...
    for (const pkg of packages) {
      ...
      if (pkgType !== "frontend") {
        content += `\n- Backend guidelines: \`.rudder/spec/${specName}/backend/\`\n`;
      }
      if (pkgType !== "backend") {
        content += `\n- Frontend guidelines: \`.rudder/spec/${specName}/frontend/\`\n`;
      }
    }
...
function getBootstrapTaskJson(
  ...
  return emptyTaskJson({
    ...
    createdAt: today,
    relatedFiles,
    notes: `First-time setup task created by rudder init (${projectType} project)`,
  });
}
...
function getJoinerTaskJson(developer: string, taskName: string): TaskJson {
  ...
  return emptyTaskJson({
    id: taskName,
    name: taskName,
    title: `Joining: Onboard to this Rudder project (${developer})`,
    description:
      "Onboard a new developer to an existing Rudder project: learn the workflow, conventions, and find assigned work",
    status: "in_progress",
    dev_type: "docs",
    priority: "P1",
    creator: developer,
    ...
    createdAt: today,
    notes:
      "Generated by rudder init for a new developer joining an existing Rudder project",
  });
}
...
\`${developer}\` just ran \`rudder init\` on a fresh clone, saw "Developer
...
- \`.rudder/workspace/${developer}/journal-*.md\` — their session log,
...
- Check if \`.rudder/workspace/${developer}/\` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run \`${pythonCmd} ./.rudder/scripts/task.py list --assignee ${developer}\` to
...
\`\`\`bash
${pythonCmd} ./.rudder/scripts/task.py finish
${pythonCmd} ./.rudder/scripts/task.py archive 00-join-${slug}
\`\`\`
...
/**
 * Create joiner onboarding task for a new developer on an existing Rudder
...
/**
 * Handle re-init when .rudder/ already exists.
...
async function handleReinit(
    ...
    const { action } = await inquirer.prompt<{ action: string }>([
      {
        type: "list",
        name: "action",
        message: "Rudder is already initialized. What would you like to do?",
        choices: [
  ...
  if (doAddDeveloper) {
    ...
    // joiner onboarding onto an existing Rudder project.
    ...
    try {
      ...
      console.log(
        chalk.gray(
          `  ${pythonCmd} .rudder/scripts/init_developer.py ${devName}`,
        ),
      );
    }
...
export async function init(options: InitOptions): Promise<void> {
  ...
  // (.rudder/ exists, .developer missing) apart from "creator first init".
  const hadDeveloperFileAtStart = fs.existsSync(
    path.join(cwd, DIR_NAMES.WORKFLOW, FILE_NAMES.DEVELOPER),
  );

  // Generate ASCII art banner dynamically using FIGlet "Rebel" font
  const banner = figlet.textSync("Rudder", { font: "Rebel" });
  ...
  // ==========================================================================
  // Re-init fast path: skip full flow when .rudder/ already exists
  // ==========================================================================

  // Aborted-init recovery (issue #204): if .rudder/ exists but tasks/ is
  ...
  if (!developerName && !options.yes) {
    ...
    console.log(
      chalk.gray(
        "\nRudder supports team collaboration - each developer has their own\n" +
  ...
  if (options.monorepo !== false) {
    ...
    if (options.monorepo === true && !detected) {
      ...
      console.log("");
      console.log("To configure manually, add to .rudder/config.yaml:");
      console.log("");
          ...
          for (const pkg of detected) {
            const { specSource } = await inquirer.prompt<{
            ...
            }>([
              {
                ...
                choices: [
                  { name: "From scratch (Rudder default)", value: "blank" },
  ...
  if (selectedTemplate) {
    ...
    if (result.success) {
      ...
      const retryCmd = registry
        ? `rudder init --registry ${registry.gigetSource} --template ${selectedTemplate}`
        : `rudder init --template ${selectedTemplate}`;
    ...
    if (result.success) {
      ...
      console.log(
        chalk.gray(
          `   You can retry later: rudder init --registry ${registry.gigetSource}`,
        ),
      );
    }
  }
  ...
  if (developerName) {
    ...
    //
    // Tasks-empty fallback (issue #204): if .rudder/ exists but tasks dir is
