...
const RUDDER_BLOCK_START = "<!-- RUDDER:START -->";
const RUDDER_BLOCK_END = "<!-- RUDDER:END -->";
const LEGACY_UNTRACKED_AGENTS_MD_BLOCK_HASHES = new Set<string>([
  ...
  // This hash is the pristine Rudder-managed block before the Subagents
...
function getRudderManagedBlock(content: string): string | null {
  const start = content.indexOf(RUDDER_BLOCK_START);
  if (start === -1) {
    return null;
  }

  const end = content.indexOf(RUDDER_BLOCK_END, start);
  if (end === -1) {
    return null;
  }

  return content.slice(start, end + RUDDER_BLOCK_END.length);
}

function replaceRudderManagedBlock(
  existingContent: string,
  templateContent: string,
): string | null {
  const existingStart = existingContent.indexOf(RUDDER_BLOCK_START);
  if (existingStart === -1) {
    return null;
  }

  const existingEnd = existingContent.indexOf(RUDDER_BLOCK_END, existingStart);
  if (existingEnd === -1) {
    return null;
  }

  const templateBlock = getRudderManagedBlock(templateContent);
  if (!templateBlock) {
    return null;
  }
  ...
  return (
    ...
    templateBlock +
    existingContent.slice(existingEnd + RUDDER_BLOCK_END.length)
  );
}
...
function buildAgentsMdTemplate(cwd: string): string {
  ...
  // Existing file already has RUDDER:START/END markers — replace just the
  // managed block, preserving everything outside it.
  const replaced = replaceRudderManagedBlock(existingContent, agentsMdContent);
  if (replaced !== null) {
    return replaced;
  }

  // Existing file has no managed-block markers (pre-0.5.0-beta.18 project, or
  // user hand-wrote AGENTS.md without ever running through Rudder). Append
  // the template's managed block at the end so user content is preserved
  // instead of clobbered.
  const templateBlock = getRudderManagedBlock(agentsMdContent);
  if (!templateBlock) {
    return agentsMdContent;
  }
...
function isKnownUntrackedTemplate(
  ...
  const managedBlock = getRudderManagedBlock(existingContent);
  if (!managedBlock) {
    return false;
  }
...
/**
 * Load update.skip paths from .rudder/config.yaml
 *
 ...
 *
 * Old Rudder versions used `.agents/skills/` as codex's configDir.
 * New versions use `.codex/` for Codex-specific config and `.agents/skills/`
 * as a shared layer.
 *
 * Detection: Rudder-tracked hashes contain `.agents/skills/` entries
...
function needsCodexUpgrade(cwd: string): boolean {
  ...
  // command-as-skill files `rudder-continue/SKILL.md` and
  // `rudder-finish-work/SKILL.md` under `.agents/skills/`. Other platforms
  ...
  return (
    keys.some((key) => key === ".agents/skills/rudder-continue/SKILL.md") ||
    keys.some((key) => key === ".agents/skills/rudder-finish-work/SKILL.md")
  );
}
...
function classifyMigrations(
  ...
  for (const item of migrations) {
    ...
    // rename/rename-dir are allowed to target protected paths (e.g., 0.2.0 renames into .rudder/workspace)
    if (
      item.to &&
...
async function promptMigrationAction(
  ...
  const whyFlagged = item.reason
    ...
    : chalk.gray(
        `  Why prompted: file content doesn't match the Rudder template hash\n` +
 ...
 * Recursively removes empty parent directories up to .rudder root
 */
/** @internal Exported for testing only */
export function cleanupEmptyDirs(cwd: string, dirPath: string): void {
  ...
  // Safety: never delete managed root directories themselves (e.g., .claude, .rudder)
...
async function executeMigrations(
  ...
  for (const item of classified.confirm) {
    ...
    // This is in addition to the full project snapshot at .rudder/.backup-*/;
    ...
    if (item.type === "rename" && item.to) {
      ...
      if (action === "backup-rename") {
        ...
        // inline without digging through .rudder/.backup-*/.
...
export async function update(options: UpdateOptions): Promise<void> {
  ...
  // Check if Rudder is initialized
  if (!fs.existsSync(path.join(cwd, DIR_NAMES.WORKFLOW))) {
    console.log(chalk.red("Error: Rudder not initialized in this directory."));
    console.log(chalk.gray("Run 'rudder init' first."));
    return;
  }
  ...
  if (cliVsProject < 0) {
    ...
    if (!options.allowDowngrade) {
      ...
      console.log(
        chalk.gray(`  2. Force downgrade: rudder update --allow-downgrade\n`),
      );
      return;
    }
  ...
  if (isUnknownVersion) {
    console.log(
      chalk.yellow(
        "⚠️  No version file found. Skipping migrations — run rudder init to fix.",
      ),
    );
  ...
  // Detect legacy Codex (has .agents/skills/ tracked by Rudder but no .codex/)
  ...
  if (hasMigrations) {
    ...
    if (
      ...
      if (gateMetadata.breaking && gateMetadata.recommendMigrate) {
        ...
        console.log("");
        console.log(chalk.yellow(`  Run: rudder update --migrate`));
        console.log("");
        console.log(
          chalk.gray(
    ...
    if (preConfirmMetadata.breaking) {
      ...
      if (breakingBypass && skipPaths.length > 0) {
        ...
        if (willBypass.length > 0) {
          ...
          console.log(
            chalk.gray(
              "  Hash-verified: only files matching known Rudder templates are deleted. Your local customizations (hash mismatch) are still preserved.",
            ),
          );
        }
      }
  ...
  if (options.migrate && classifiedMigrations) {
    ...
    // traces-*.md files are in .rudder/workspace/{developer}/ with variable developer names
