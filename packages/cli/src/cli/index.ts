...
/**
 * Check if a Rudder update is available (compare project version with CLI version)
 */
function checkForUpdates(cwd: string): void {
  ...
  if (comparison > 0) {
    ...
    console.log(
      chalk.yellow(
        `\n⚠️  Rudder update available: ${projectVersion} → ${cliVersion}`,
      ),
    );
...
// Check for updates at CLI startup (only if .rudder exists)
...
program
  .command("init")
  .description("Initialize rudder in the current project")
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      ...
      if (process.env.DEBUG || process.env.RUDDER_DEBUG) {
...
program
  .command("update")
  .description("Update rudder configuration and commands to latest version")
...
program
  .command("uninstall")
  .description(
    "Remove all rudder files (managed platform files + .rudder/) from this project",
  )
