...
Triggered from `start` (Rudder command) when the user describes a development task, especially when:
...
```bash
TASK_DIR=$(python3 ./.rudder/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
### Delegate to `rudder-research` sub-agent (don't research inline)

For each research topic, **spawn a `rudder-research` sub-agent via the Task tool** — don't do WebFetch / WebSearch / `gh api` inline in the main conversation.
...
✅ Good:
```
Main agent: Task(subagent_type="rudder-research",
                 prompt="Research topic A; persist to research/topic-a.md")
          + Task(subagent_type="rudder-research",
...
Each `rudder-research` sub-agent should:
...
```bash
# Create child tasks
CHILD1=$(python3 ./.rudder/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 ./.rudder/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 ./.rudder/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
| Command | When to Use |
|---------|-------------|
| ``start` (Rudder command)` | Entry point that triggers brainstorm |
| ``finish-work` (Rudder command)` | After implementation is complete |
| ``update-spec` (Rudder command)` | If new patterns emerge during work |
