# Rudder File Reference

Complete reference of all files in the `.rudder/` directory.
...
├── .version                # Installed Rudder version
...
```json
{
  "current_task": ".rudder/tasks/01-31-add-login-taosu",
  "current_run": null,
  "platform": "claude",
...
**Purpose**: Legacy ignored pointer from older Rudder versions.

**Active-task behavior**: Not read or written as a fallback. Current Rudder
...
**Purpose**: Track template file versions for `rudder update`.

**Created by**: `rudder init` or `rudder update`

**Format**: JSON object mapping file paths to SHA-256 hashes.

```json
{
  ".rudder/workflow.md": "028891d1fe839a266...",
...
**Used by**:
- `rudder update` - Detect which files have been modified
...
**Purpose**: Track installed Rudder CLI version.
...
**Used by**:
- `rudder update` - Determine if update is needed
...
**Location**: `.rudder/workspace/{developer}/.agents/`
...
These files are managed by `rudder update`:

| File | Purpose |
|------|---------|
| `.rudder/workflow.md` | Workflow documentation |
| `.rudder/worktree.yaml` | Multi-session config |
| `.rudder/.gitignore` | Git ignore rules |
...
.rudder/tasks/{task}/ → .rudder/tasks/archive/YYYY-MM/
