...
Common Rudder customization scenarios and what files need to be modified.
...
|------|-----------------|----------|
| [Add slash command](#add-slash-command) | commands/, rudder-local | All |
| [Add agent](#add-agent) | agents/, hook, jsonl, rudder-local | CC |
| [Modify hook](#modify-hook) | hooks/, settings.json, rudder-local | CC |
| [Add spec category](#add-spec-category) | spec/, jsonl, rudder-local | All |
| [Change verify commands](#change-verify-commands) | worktree.yaml | CC |
| [Add workflow phase](#add-workflow-phase) | task.json, dispatch, rudder-local | CC |
| [Add post_create step](#add-post_create-step) | worktree.yaml | CC |
| [Modify session start](#modify-session-start) | session-start.py, rudder-local | CC |
| [Add core script](#add-core-script) | scripts/, rudder-local | All |
...
**Scenario**: Add a new `/rudder:my-command` command.

**Files to modify**:

```
.claude/commands/rudder/my-command.md    # Create: Command prompt
.cursor/commands/my-command.md            # Create: Mirror for Cursor (optional)
.rudder-local/SKILL.md                   # Update: Document the change
```

**Steps**:
1. Create command file with YAML frontmatter
2. Mirror to Cursor if needed
3. Document in rudder-local
...
.rudder/tasks/{template}/my-agent.jsonl            # Create: Context template
.rudder-local/SKILL.md                             # Update: Document the change
```
...
```
.rudder/spec/mobile/index.md             # Create: Category index
.rudder/spec/mobile/*.md                 # Create: Spec files
.rudder/tasks/{template}/*.jsonl         # Update: Reference new specs
.rudder-local/SKILL.md                   # Update: Document the change
```
...
```
.rudder/worktree.yaml                    # Modify: verify section
```
...
```
.rudder/worktree.yaml                    # Modify: post_create section
```
...
```
.rudder/scripts/my-script.py             # Create: Script
.rudder/scripts/common/*.py              # Create/Modify: If shared utilities
.rudder-local/SKILL.md                   # Update: Document the change
```
...
```
.rudder/scripts/task.py                  # Modify: init-context logic
.rudder/tasks/{template}/*.jsonl         # Create: New JSONL templates
.rudder-local/SKILL.md                   # Update: Document the change
```
