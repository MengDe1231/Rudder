...
Complete guide to `.rudder/worktree.yaml` configuration.
...
**Note**: Rudder uses a custom YAML parser (not PyYAML). Supports basic key-value pairs and arrays; complex nested structures may not work.
...
Rudder uses a custom YAML parser (not PyYAML) with these limitations:
