...
# Show src/ changes only (skip .rudder/, docs, chore)
...
**Voice**: technical reference doc. Short, clear, plain. Not a story, not a sales pitch. Style guide: `.rudder/spec/docs-site/docs/style-guide.md` → "Changelog / Release Notes Voice".
...
**Manifest `changelog` field** (terminal display during `rudder update`): same rules, single string with `\n` separators, group with `**Enhancements:**` / `**Bug Fixes:**` / `**Internal:**` bold prefixes.
...
| `notes` | Brief guidance for users (e.g., "run `rudder update --migrate` to sync"). Shown inline in terminal during update. |
...
| `reason` | **Why** the user might see this entry flagged as modified. Version-specific context (e.g. "Rudder 0.4.0 skipped hashing this path — pristine copies show as modified. [r] is safe."). Keeps version-specific hints out of `update.ts`. | optional |
...
- `rename` uses the **project-local** `.rudder/.template-hashes.json` (auto-maintained by Rudder), NOT the manifest's `allowed_hashes` field.
  ...
  "migrations": [
    {
      ...
      "to": ".claude/skills/rudder-new-path/SKILL.md",
...
Full reference: `.rudder/spec/docs-site/docs/release-lifecycle.md`.
...
- **`allowed_hashes`** — required ONLY for `safe-file-delete`. `rename` classification uses project-local `.rudder/.template-hashes.json`; you do NOT need to collect historical hashes for rename entries.
...
npx -y @mengde1231/rudder@<last-ga> init -y -u test --claude --cursor --<all-platforms-you-care-about>
...
Watch for:
- **Orphan files** — stale paths written by the old version that don't match any rename/safe-file-delete. Grep `find . -path "*/skills/*" -not -path "*/rudder-*"` to catch plain-name skill dirs.
...
- **Backup bloat** — confirm `.rudder/.backup-*/` doesn't contain `/worktrees/` or `/workspace/` paths.
