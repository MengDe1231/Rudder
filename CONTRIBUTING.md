...
Thanks for your interest in contributing to Rudder! This document provides guidelines for contributing to the project.

## Ways to Contribute

### Reporting Bugs

Before creating a bug report, please check [existing issues](https://github.com/MengDe1231/Rudder/issues) to avoid duplicates.
...
2. **Clone your fork**
   ```bash
   git clone https://github.com/YOUR_USERNAME/Rudder.git
   cd Rudder
   ```
...
├── .rudder/               # Rudder workflow (project's own) ←
│   ├── scripts/            # Bash scripts
│   └── spec/               # Spec file templates
└── docs/                   # Documentation
```

> **Important:** When modifying `.claude/`, `.rudder/`, or `.cursor/`, check if the same changes need to be applied to `src/templates/`. The project uses its own config files, but templates are what gets installed to user projects.
...
Every contribution helps make Rudder better. We appreciate your time and effort!
